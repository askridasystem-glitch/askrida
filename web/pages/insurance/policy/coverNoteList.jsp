<%@ page import="com.webfin.insurance.model.CoverNoteView,
                 com.crux.util.JSPUtil,
                 com.crux.lov.LOVManager,
                 com.crux.util.LOV,
                 java.util.Iterator,
                 com.webfin.insurance.form.CoverNoteListForm"%>
<%@ taglib prefix="c" uri="crux" %><c:frame >
<%
   final CoverNoteListForm form = (CoverNoteListForm) frame.getForm();
...
<table cellpadding=2 cellspacing=1>
            ...
            <tr>
               <td>
                  <table cellpadding=2 cellspacing=1>
                     ...
                     <c:field caption="{L-ENGShow active &amp; Pending only-L}{L-INAHanya tampilkan polis aktif &amp; Pending-L} " name="stOutstandingFlag" type="check" presentation="standard" changeaction="refresh" />
                  </table>
               </td>
               <td>
   ...
   <tr>
      <td>
         ...
         <%
            final CoverNoteView pol = (CoverNoteView) current;
         %>
   ...
   <tr>
      <td>                                                                 
         <c:button show="<%=form.isEnableCreateProposal()%>"  text="{L-ENGCreate-L}{L-INABuat-L} {L-ENGQuotation-L}{L-INAPenawaran-L}" event="clickCreate" />
         ...
         <c:button show="<%=form.isEnableCreatePolis()%>"  text="{L-ENGCreate-L}{L-INABuat-L} Polis" event="clickCreate"/>
         <c:button show="<%=form.isEnableEndorsement()%>"  text="{L-ENGCreate-L}{L-INABuat-L} Endorsement" event="clickCreateEndorse"/>
         ...
         <c:button show="<%=form.isCanApprove()%>"  text="{L-ENGApproval-L}{L-INAMenyetujui-L}" event="clickApproval" />
         <c:button show="<%=reas%>"  text="{L-ENGApproval Reins-L}{L-INAMenyetujui Reas-L}" event="clickApprovalReins" />
...
<% } %>--%>
   ...
   <c:evaluate when="<%=form.isEnablePrint()%>" >
      <tr>
         <td>
            Print <c:field name="stPrintForm" width="250" type="string" lov="LOV_POL_PRINTING" ><c:param name="vs" value="<%=form.getPrintingLOV()%>" /></c:field> in <c:field name="stLang" type="string" lov="LOV_LANG" />
