    ...
    <fo:page-sequence master-reference="only" initial-page-number="1">
        ...
        <fo:flow flow-name="xsl-region-body">
            ...
            <fo:block font-size="16pt" font-family="TAHOMA"
                      ...
                      text-align="center"
                      padding-top="10pt">
                {L-INA IKHTISAR PERTANGGUNGAN B -L}{L-ENG POLICY SCHEDULE B -L}
            </fo:block>
 ...
 %>
  ...
  <fo:block text-align="end"
    ...
    Print Date : <%=JSPUtil.print(DateUtil.getDateStr(new Date(),"dd/MM/yyyy HH:mm:ss"))%> PrintStamp:<%=pol.getStPrintCode()%> PrintCode:<%=pol.getStPrintStamp()%> Page:<fo:page-number/> Terlampir : [<%=JSPUtil.print(request.getAttribute("DOCUMENT_NAME"))%>]
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        <fo:table-row>
                            <fo:table-cell ><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell >
                                <fo:block text-align="center">{L-ENG Signed at-L}{L-INA Dibuat di-L} <%= JSPUtil.printX(pol.getCostCenter(pol.getCostCenter(pol.getStCostCenterCode()).getStSubCostCenterCode()).getStDescription())%><%if (tanpaTanggal) {%>, {L-ENG On-L}{L-INA pada tanggal-L} <%=DateUtil.getDateStr(pol.getDtPolicyDate(), "d ^^ yyyy")%><%}%></fo:block>
            ...
            <%--
                  <fo:block space-before.optimum="55pt" font-size="<%=fontsize%>">
                   <fo:table table-layout="fixed">
                    <fo:table-column column-width="80mm"/>
                    <fo:table-column column-width="100mm"/>
                    <fo:table-column column-width="20mm"/>
                    ...
                    <fo:table-body>
                       <fo:table-row>
                        <fo:table-cell ><fo:block></fo:block></fo:table-cell>
                        <fo:table-cell >
                          <fo:block text-align="center"><%if(Parameter.readString("BRANCH_SIGN_"+pol.getStCostCenterCode())==null){%><%}else{%>(<%=Parameter.readString("BRANCH_SIGN_"+pol.getStCostCenterCode())%>)<%}%></fo:block>
                        </fo:table-cell>
                      </fo:table-row>
                   </fo:table-body>
                  </fo:table>
