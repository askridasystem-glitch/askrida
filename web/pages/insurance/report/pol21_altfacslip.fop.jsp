...
<% if (!effective) {%>
      <fo:block font-size="16pt" font-family="SERIF" line-height="16pt" color="red" text-align="center">
       {L-ENG FACULTATIVE REINSURANCE OFFER-L}{L-INA PENAWARAN REASURANSI FAKULTATIF-L}
      </fo:block>
<% }else {%>
      <fo:block font-size="16pt" font-family="SERIF" line-height="16pt" color="black" text-align="center">
       {L-ENG FACULTATIVE REINSURANCE SLIP-L}{L-INA SLIP REASURANSI FAKULTATIF-L}
      </fo:block>
<% }%>

      <fo:block font-size="<%=fontsize%>" font-family="SERIF" line-height="16pt" space-after.optimum="10pt" color="black" text-align="center"> NO. <%=JSPUtil.printX(ri.getStRISlipNo())%>
      </fo:block>
...
%>--%>
                        <fo:table-row>
                           <fo:table-cell ><fo:block><%=JSPUtil.printX(reasuradur.getStEntityName())%></fo:block></fo:table-cell>
                           <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
                           <fo:table-cell ><fo:block><%=JSPUtil.printX(ri.getDbPremiRate(),3)%> %</fo:block></fo:table-cell>
                        </fo:table-row>
<%-- <% } %> --%>
                     </fo:table-body>
                  </fo:table>
...
<% if(Tools.compare(ri.getDbRICommRate(),new BigDecimal(0))>0){ %>
           ...
           <fo:table-row>
             <fo:table-cell ><fo:block space-after.optimum="10pt">{L-ENG R/I Commission -L}{L-INA Komisi Reasuransi -L}</fo:block></fo:table-cell>
             <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
             <fo:table-cell ><fo:block><%=JSPUtil.print(ri.getDbRICommRate(),2)%> %</fo:block></fo:table-cell>
           </fo:table-row>
