    ...
    <fo:page-sequence master-reference="only" initial-page-number="1">
        ...
        <fo:flow flow-name="xsl-region-body">
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Dengan ini dinyatakan bahwa kami : -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference1())%></fo:inline> {L-ENG XXX -L}{L-INA yang beralamat di -L} <%=JSPUtil.printX(obj.getStReference3())%>  {L-ENG XXX -L}{L-INA sebagai Penyedia, selanjutnya disebut TERJAMIN, dan -L} <fo:inline font-weight="bold">PT. Asuransi Bangun Askrida</fo:inline> {L-ENG XXX -L}{L-INA yang beralamat di -L}<%=Parameter.readString("ASKRIDA_ADDRESS")%>{L-ENG XXX -L}{L-INA sebagai Penjamin, selanjutnya disebut sebagai PENJAMIN, bertanggung jawab dan dengan tegas terikat pada -L} <fo:inline font-weight="bold"><%=pol.getStCustomerName()%></fo:inline> {L-ENG XXX -L}{L-INA yang beralamat di -L} <%=pol.getStCustomerAddress()%>{L-ENG XXX -L}{L-INA sebagai Pelaksana Pelelangan Umum, selanjutnya disebut sebagai PENERIMA JAMINAN atas uang sejumlah -L}<%=JSPUtil.print(pol.getStCurrencyCode())%> <%=JSPUtil.printX(pol.getDbInsuredAmount(), 2)%> {L-ENG XXX -L}{L-INA Terbilang ( -L}<%=NumberSpell.readNumber(JSPUtil.printX(pol.getDbInsuredAmount(), 2))%> <%=JSPUtil.printX(pol.getCurrency().getStCcyDescription())%> )</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Maka kami TERJAMIN dan PENJAMIN dengan ini mengikatkan diri untuk melakukan pembayaran jumlah tersebut diatas dengan baik dan benar bilamana TERJAMIN tidak memenuhi kewajiban sebagaimana ditetapkan dalam Dokumen Pengadaan untuk pelaksanaan pelelangan pekerjaan -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference11())%></fo:inline> {L-ENG XXX -L}{L-INA yang diselenggarakan oleh PENERIMA JAMINAN. -L}. </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Jaminan ini berlaku apabila : -L}</fo:block></fo:table-cell>
                        </fo:table-row>

                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA TERJAMIN tidak memenuhi kewajibannya melakukan pemeliharaan sebagaimana ditentukan dalam Dokumen kontrak. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Surat Jaminan ini tidak berlaku apabila terbukti adanya : -L}</fo:block></fo:table-cell>
                        </fo:table-row>

                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA a. praktek Korupsi, Kolusi dan Nepotisme -L}</fo:block></fo:table-cell>
                        </fo:table-row>

                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA b. Penipuan / pemalsuan atas informasi yang disampaikan dalam dokumen Kontrak -L}</fo:block></fo:table-cell>
                        </fo:table-row>

                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA c. Tindakan yang diindikasikan disebabkan oleh hal - hal sebagaimana disebutkan dalam huruf (a) dan (b) diatas, yang dilakukan oleh TERJAMIN maupun PENERIMA JAMINAN. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA PENJAMIN akan membayar kepada PENERIMA JAMINAN sejumlah nilai jaminan tersebut di atas dalam waktu paling lambat 14 (empat belas) hari kerja tanpa syarat (UNCONDITIONAL) setelah menerima tuntutan pencairan secara tertulis dari PENERIMA JAMINAN berdasar Keputusan PENERIMA JAMINAN mengenai pengenaan sanksi akibat TERJAMIN cidera janji. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                            ...
                            <fo:table-cell>
                                <fo:block text-align = "center">
                                    <%--<fo:external-graphic
                                        content-height="60%"
                                        width="60%"
                                        scaling="uniform" src="url(D:\jboss-3.2.5\server\default\deploy\fin.ear\fin.war\pages\main\img\stepanus.jpg)"/>

                                    <fo:external-graphic
                                        content-height="60%"
                                        width="60%"
                                        scaling="uniform" src="url(D:\fin-repository\00\20130221\1361441261089)"/>
                                    --%>
                                    <fo:external-graphic
                                        content-height="60%"
                                        width="60%"
                            ...
                            <fo:table-cell>
                                <fo:block text-align = "center">
                                    ...
                                    <fo:external-graphic
                                        content-height="60%"
                                        width="60%"
                                        scaling="uniform" src="url(D:\fin-repository\00\20130221\1361439311926.jpg)"/>--%>
                                </fo:block>
                            </fo:table-cell>
                        </fo:table-row> 
                        <% }%>  
            ...
            <fo:block font-size="6pt"
                ...
                <% if (effective) {%>
                PrintCode:<%=pol.getStPrintCode()%> PrintStamp:<%=pol.getStPrintStamp()%> Page:<fo:page-number/> Terlampir : [<%=JSPUtil.print(request.getAttribute("DOCUMENT_NAME"))%>] <%if (usingDigitalSign) {%>Sign Code : <%=pol.getStSignCode()%> <%}%>
                <% }%>   
            </fo:block>
