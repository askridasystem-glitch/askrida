...
ref-id="end-of-document"/>
        ...
        <fo:flow flow-name="xsl-region-body">
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>																																																																																																																																																																																
                            ...
                            <fo:table-cell><fo:block>{L-ENG We, the PRINCIPAL and the SURETY bind ourselves for the payment of which sum, well and truly to be made, firmly by this presents that if the PRINCIPAL fails to fulfill his obligation the performance of the contract which has been awarded to him under the OBLIGEE?S notice of award Contract -L}{L-INA Maka kami PRINCIPAL dan SURETY dengan ini mengikatkan diri untuk melakukan pembayaran jumlah tersebut diatas dengan baik dan benar bilamana PRINCIPAL tidak memenuhi kewajibannya dalam melaksanakan Pekerjaan yang telah dipercayakan kepadanya atas dasar Surat Penunjukan Pemenang Lelang dari OBLIGEE No.  -L} <%if(obj.getStReference10()!=null){%><%=JSPUtil.printX(obj.getStReference10())%><%}else{%>.......................<%}%>{L-ENG dated -L}{L-INA tanggal -L} <%if(obj.getDtReference1()!=null){%><%=DateUtil.getDateStr(obj.getDtReference1(),"d ^^ yyyy")%><%}else{%>................<%}%>  {L-ENG and which is confirmed by the contract -L}{L-INA yang selanjutnya dikukuhkan dalam Kontrak No. -L}<%if(obj.getStReference11()!=null){%><%=JSPUtil.printX(obj.getStReference11())%><% }else{ %>...............<% } %> 
                            {L-ENG dated -L}{L-INA tanggal -L} <% if(obj.getDtReference2()!=null) {%><%=DateUtil.getDateStr(obj.getDtReference2(),"d ^^ yyyy")%><% }else{ %>................<% } %> {L-ENG the works of -L}{L-INA atas pekerjaan -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference12())%></fo:inline> {L-ENG between the OBLIGEE and the PRINCIPAL which contract is inherent within this bond. -L}{L-INA antara pihak PRINCIPAL dan OBLIGEE, dan kontrak tersebut merupakan bagian yang tidak terpisahkan dari jaminan ini. -L} </fo:block></fo:table-cell>
                        </fo:table-row>
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG If the PRINCIPAL promptly and faithfully perform the said Contract then this bond shall be null and void: -L}{L-INA jika PRINCIPAL menyelesaikan pekerjaan tersebut pada waktunya dengan baik dan benar sesuai dengan ketentuan dalam Kontrak; atau -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        
                        <fo:table-row>
                            <fo:table-cell><fo:block> </fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-INA b. -L}</fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-INA jika PRINCIPAL membayar, memperbaiki, dan mengganti pada OBLIGEE semua kerugian dan kerusakan yang sesungguhnya diderita OBLIGEE oleh sebab kegagalan atau kelalaian dari pihak PRINCIPAL dalam melaksanakan Kontrak; -L}</fo:block></fo:table-cell>
                        </fo:table-row>
            ...
            <fo:block font-size="<%=fontsize%>" text-align = "justify">
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block> </fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG otherwise to remain in full force and effect as from -L}{L-INA maka jaminan ini menjadi batal dan tidak berlaku; sebaliknya jika tidak maka Jaminan ini tetap berlaku dan efektif mulai tanggal -L} <fo:inline font-weight="bold"><%=DateUtil.getDateStr(pol.getDtPeriodStart(),"d ^^ yyyy")%></fo:inline> {L-ENG up to -L}{L-INA sampai dengan tanggal -L} <fo:inline font-weight="bold"><%=DateUtil.getDateStr(pol.getDtPeriodEnd(),"d ^^ yyyy")%></fo:inline> {L-ENG and can be extended by the application of the PRINCIPAL until 14 (fourteen) days after the bond expires. -L}{L-INA dan dapat dimintakan perpanjangan oleh PRINCIPAL sampai 14 (empat belas) hari setelah masa Jaminan berakhir. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG Any claim on this bond shall be made in written application by the OBLIIGEE to the SURETY promptly after such default arises that the PRINCIPAL fails to fulfill his obligation as specified in the above contract and is not caused by the owner?s risks. -L}{L-INA Tuntutan penagihan (klaim) atas Surat Jaminan ini dilaksanakan oleh OBLIGEE secara tertulis kepada SURETY segera setelah timbul cidera janji (Wanprestasi/default) oleh pihak PRINCIPAL dalam melaksanakan Kontrak, bukan karena risiko-risiko pemilik -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block> </fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG The SURETY shall pay to the OBLIGEE the amount of bond value as mentioned above not later than 30 (thirty) calendar days after having received a written claim from the OBLIGEE based on OBLIGEE?s Decree concerning the sanction as a result of the default of the PRINCIPAL. -L}{L-INA SURETY akan membayar kepada OBLIGEE sejumlah kerugian yang sesungguhnya diderita olehnya maksimum sebesar nilai jaminan tersebut diatas selambat-lambatnya 30 (tiga puluh) hari kalender setelah menerima tuntutan penagihan dari pihak OBLIGEE berdasarkan Keputusan OBLIGEE yang disetujui PRINCIPAL. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell>
                                ...
                                <%= JSPUtil.printX(pol.getStCostCenterDesc())%><%if(tanpaTanggal){%>, {L-ENG dated on -L}{L-INA pada tanggal -L} <%=DateUtil.getDateStr(pol.getDtPolicyDate(),"d ^^ yyyy")%><%}%>.</fo:block>
                                <% } else { %>  
                                <fo:block >{L-ENG Signed, sealed, and stamped in -L}{L-INA Ditanda tangani serta dibubuhkan materai di -L}
                                Jakarta<%if(tanpaTanggal){%>, {L-ENG dated on -L}{L-INA pada tanggal -L} <%=DateUtil.getDateStr(pol.getDtPolicyDate(),"d ^^ yyyy")%><%}%>.</fo:block>
                                <% } %>
                            </fo:table-cell>
                        </fo:table-row>
            ...
            <fo:block font-size="6pt"
                      ...
                      text-align="justify" >
                Dokumen ini diterbitkan tidak memerlukan tanda tangan basah/asli, dicetak secara sistem dan dinyatakan sah.
            </fo:block>
            <% } %>
