                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Maka kami PRINCIPAL dan SURETY dengan ini mengikatkan diri untuk melakukan pembayaran bilamana pihak PRINCIPAL tidak memenuhi kewajibannya melaksanakan pekerjaan pemeliharaan sebagaimana telah ditetapkan dalam Kontrak No.  -L} <%=JSPUtil.printX(obj.getStReference10())%>{L-ENG 1. -L}{L-INA tanggal -L} <%=DateUtil.getDateStr(obj.getDtReference1(),"d ^^ yyyy")%>  {L-ENG 1. -L}{L-INA atas pekerjaan -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference11())%></fo:inline> {L-ENG 1. -L}{L-INA antara pihak PRINCIPAL dan pihak OBLIGEE, dan kontrak tersebut merupakan bagian yang tidak terpisahkan dari jaminan ini. -L} </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Adapun ketentuan dari Surat Jaminan ini adalah jika : PRINCIPAL telah melaksanakan kewajibannya melakukan pemeliharaan sebagaimana ditetapkan didalam Kontrak tersebut diatas, maka Jaminan ini menjadi tidak berlaku; sebaliknya jika tidak, maka Jaminan ini tetap berlaku untuk jangka waktu dari saat penyerahan pertama tanggal -L} <fo:inline font-weight="bold"><%=DateUtil.getDateStr(pol.getDtPeriodStart(),"d ^^ yyyy")%></fo:inline> {L-ENG XXX -L}{L-INA sampai dengan saat penyerahan kedua tanggal -L} <fo:inline font-weight="bold"><%=DateUtil.getDateStr(pol.getDtPeriodEnd(),"d ^^ yyyy")%></fo:inline>{L-ENG XXX -L}{L-INA dengan syarat-syarat berikut ini. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
