...
ref-id="end-of-document"/>
        ...
        <fo:flow flow-name="xsl-region-body">
            ...
            <fo:block font-size="12pt" font-family="TAHOMA" line-height="0pt" space-after.optimum="10pt"
                      ...
                      text-align="center"
                      padding-top="10pt">
                {L-ENG -L}{L-INA Maintenance Bond -L}
            </fo:block>
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG We, the PRINCIPAL and the SURETY  bind ourselves to pay the said amount above, if the PRINCIPAL fails to fulfil his obligation to do the maintenance works specified in the -L}{L-INA Maka kami PRINCIPAL dan SURETY dengan ini mengikatkan diri untuk melakukan pembayaran bilamana pihak PRINCIPAL tidak memenuhi kewajibannya melaksanakan pekerjaan pemeliharaan sebagaimana telah ditetapkan dalam Kontrak No. -L} <%=JSPUtil.printX(obj.getStReference10())%>{L-ENG dated -L}{L-INA tanggal -L} <%=DateUtil.getDateStr(obj.getDtReference1(),"d ^^ yyyy")%>  {L-ENG the works of -L}{L-INA atas pekerjaan -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference11())%></fo:inline> {L-ENG between the OBLIGEE and the PRINCIPAL which contract is inherent within this bond. -L}{L-INA antara pihak PRINCIPAL dan pihak OBLIGEE, dan kontrak tersebut merupakan bagian yang tidak terpisahkan dari jaminan ini. -L} </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG NOW THEREFORE, the condition of this bond is such that, if the PRINCIPAL has already done the maintenance works as specified in the contract, then this bond shall be null and void: otherwise to remains in full force and effect as from the first delivery dated -L}{L-INA Adapun ketentuan dari Surat Jaminan ini adalah jika : PRINCIPAL telah melaksanakan kewajibannya melakukan pemeliharaan sebagaimana ditetapkan didalam Kontrak tersebut diatas, maka Jaminan ini menjadi tidak berlaku; sebaliknya jika tidak, maka Jaminan ini tetap berlaku untuk jangka waktu dari saat penyerahan pertama tanggal -L} <fo:inline font-weight="bold"><%=DateUtil.getDateStr(pol.getDtPeriodStart(),"d ^^ yyyy")%></fo:inline> {L-ENG up to the second delivery dated -L}{L-INA sampai dengan saat penyerahan kedua tanggal -L} <fo:inline font-weight="bold"><%=DateUtil.getDateStr(pol.getDtPeriodEnd(),"d ^^ yyyy")%></fo:inline>{L-ENG with the conditions as follows. -L}{L-INA dengan syarat-syarat berikut ini. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG Any claims on this bond shall be made in written application by the OBLIGEE to the SURETY promptly after the default arises by the PRINCIPAL in doing the maintenance as specified in the said contract and is not caused by a force majeure with a written confirmation by the OBLIGEE. -L}{L-INA Tuntutan penagihan (klaim) atas surat jaminan ini dilaksanakan oleh OBLIGEE secara tertulis kepada SURETY segera setelah timbul cidera janji (Wanprestasi/default) oleh pihak PRINCIPAL dalam melaksanakan pemeliharaan sesuai dengan ketentuan-ketentuan dalam Kontrak tersebut diatas dan bukan karena force majeure serta dinyatakan secara tertulis oleh OBLIGEE -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block>{L-ENG -L}{L-INA -L}</fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG SURETY shall pay to the OBLIGGE maximum the amount of bond value mentioned above at the latest 30 (thirty) calendar days after having received a claim letter from the OBLIGEE, a certain sum or sums for the repairing east raised in the maintenance period. -L}{L-INA SURETY akan membayar kepada OBLIGEE maksimum sejumlah nilai Jaminan tersebut diatas selambat-lambatnya 30 (Tiga Puluh) hari kalender setelah menerima tuntutan penagihan (klaim). Besarnya klaim yang akan dibayar oleh SURETY adalah sesuai dengan jumlah biaya yang diperlukan untuk memperbaiki kerusakan yang terjadi selama masa pemeliharaan dengan maksimum pembayaran sebesar nilai jaminan. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
