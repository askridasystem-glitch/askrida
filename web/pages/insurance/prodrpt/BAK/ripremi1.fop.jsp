...
ref-id="end-of-document"/>
    ...
    <fo:flow flow-name="xsl-region-body">
      <fo:block font-size="12pt" line-height="12pt" text-align="center">{L-ENG REINSURANCE PREMIUM-L}{L-INA PREMI REASURANSI-L}</fo:block>

      <fo:block font-size="12pt" text-align="center">
       <% if (form.getStFltTreatyType()!=null) {%> 
         Treaty : <%=JSPUtil.printX(form.getStFltTreatyType())%>  
       <% } %>
      </fo:block>  

      <fo:block font-size="12pt" text-align="center">
       <% if (form.getStEntityName()!=null) {%> 
         For : <%=JSPUtil.printX(form.getStEntityName())%> 
       <% } %>
      </fo:block>  
...
<%
   ...
   sqa.addSelect(
           "member_ent_id, coalesce(d.short_name, d.ent_name) as ent_name, a.premi_amount, c.description, a.ricomm_amt as comission, a.premi_amount-a.ricomm_amt as netto"); 
   sqa.addQuery( 
           "   from ins_pol_ri a\n" + 
           ...
           "      inner join ins_policy f on f.pol_id=g.pol_id and (f.status='POLICY' or f.status='ENDORSE') and f.posted_flag='Y'" + 
           "      inner join ins_policy_types c on c.pol_type_id=b.policy_type_id\n" + 
           "      inner join ent_master d on d.ent_id = a.member_ent_id"); 
   ...
   String q =
           "select member_ent_id, ent_name, sum(premi_amount) as premi_amount, description, sum(comission) as comission, sum(netto) as netto from (" + 
           sqa.getSQL()+
           ") a group by member_ent_id, ent_name, description";
   ...
   final FOPTableSource fopTableSource = new FOPTableSource(
      ...
      public Object getColumnValue(int rowNo, int columnNo) {
         ...
         switch(columnNo) {
            ...
            case 1: return h.getFieldValueByFieldNameST("ent_name");
            case 2: return h.getFieldValueByFieldNameST("description");
            case 3: return h.getFieldValueByFieldNameBD("premi_amount");
