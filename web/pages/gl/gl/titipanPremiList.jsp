...
<%@ taglib prefix="c" uri="crux" %><c:frame title="TITIPAN PREMI">
    ...
    <table cellpadding=2 cellspacing=1>
                    ...
                    <tr>
                        <td>
                            <table cellpadding=2 cellspacing=1>
                                ...
                                <c:field name="policyNo" type="string" caption="No Polis" presentation="standard" width="200" />
                        ...
                        <td>
                            <table cellpadding=2 cellspacing=1>
                                ...
                                <tr>
                                    ...
                                    <c:field name="stReferenceNo" type="string" caption="No Referensi" presentation="standard" width="200" />
                                </tr>
                            </table> 
                        </td>
                        <td><c:button text="Refresh" event="btnSearchTitipan" /></td>
                    </tr>
                </table>
            </td>
        </tr>
        ...
        <tr>
            <td>
                <c:button text="History Titipan - Realisasi" event="btnClickPrint" /> <c:button text="History Titipan - Realisasi (Excel)" event="printRealisasiTitipanExcel" />
            </td>
        </tr>
        <tr>
            <td>
                <%--<c:button text="Rincian Titipan Per Tanggal" event="btnClickPrintOS" />--%>
                <c:button text="Rincian Titipan Per Tanggal Neraca (Excel)" event="printRincianTitipanExcel" />
            </td>
        </tr>
        <tr>
            <td>
                <c:field name="printType" type="string" lov="LOV_TITIPAN" /> in <c:field name="printLang" lov="LOV_LANG" type="string" /><c:button text="Rincian Titipan Per Hari Ini" event="printTitipan" /> <c:button text="Rincian Titipan Per Hari Ini (Excel)" event="printTitipanExcel" />
            </td>
        </tr>
        <tr>
            <td>
                ...
                <c:listbox name="titipanPremiList" paging="true" view="com.webfin.gl.model.TitipanPremiView">
                    <%
                    ...
                    <c:evaluate when="<%=jv == null%>" >
                        ...
                        <c:listcol title="No Referensi" ></c:listcol>
                    ...
                    <c:evaluate when="<%=jv != null%>" >
                        ...
                        <c:listcol title="POL NO" ><div style="width:120"><%=jspUtil.print(jv.getStPolicyNo())%></div></c:listcol>
                        <c:listcol title="REF NO" ><div style="width:120"><%=jspUtil.print(jv.getStReferenceNo())%></div></c:listcol>
        ...
        <tr>
            <td>
                <%=jspUtil.getButtonNormal("crt", "Buat", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_ADD', 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("edt", "Ubah", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_EDIT&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("edt", "Hapus", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_DELETE&trxhdrid='+f.trxhdrid.value, 900,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("v", "Lihat", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_VIEW&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("app", "Setujui", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_APPROVE&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("rev", "Reverse", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_REVERSE&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%-- <%=jspUtil.getButtonNormal("prt", "Print", "openDialog('so.ctl?EVENT=TP_GL_ENTRY_REVERSE&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                --%>
            </td>
        </tr>
    </table>
