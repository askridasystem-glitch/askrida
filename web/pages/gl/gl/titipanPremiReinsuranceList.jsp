...
<%@ taglib prefix="c" uri="crux" %><c:frame title="TITIPAN PREMI REASURANSI">
    <%
    ...
    <table cellpadding=2 cellspacing=1>
        ...
        <tr>
            <td>
                <c:button text="History Titipan - Realisasi" event="btnClickPrintReinsurance" /> <c:button text="History Titipan - Realisasi (Excel)" event="printRealisasiTitipanReinsuranceExcel" />
            </td>
        </tr>
        <tr>
            <td>
                <%--<c:button text="Rincian Titipan Per Tanggal" event="btnClickPrintOS" />--%>
                <c:button text="Rincian Titipan Per Tanggal Neraca (Excel)" event="printRincianTitipanReinsuranceExcel" />
            </td>
        </tr>
        <tr>
            <td>
                <c:field name="printType" type="string" lov="LOV_TITIPAN" /> in <c:field name="printLang" lov="LOV_LANG" type="string" /><c:button text="Rincian Titipan Per Hari Ini" event="printTitipanReinsurance" /> <c:button text="Rincian Titipan Per Hari Ini (Excel)" event="printTitipanReinsuranceExcel" />
            </td>
        </tr>
        <tr>
            <td>
                <%
                            TitipanPremiReinsuranceView lastjv = new TitipanPremiReinsuranceView();
                %>
                <c:field name="trxhdrid" type="string"  hidden="true" />
                <c:listbox name="titipanPremiReinsuranceList" paging="true" view="com.webfin.gl.model.TitipanPremiReinsuranceView">
                    <%
                                final TitipanPremiReinsuranceView jv = (TitipanPremiReinsuranceView) current;
        ...
        <tr>
            <td>
                <%=jspUtil.getButtonNormal("crt", "Buat", "openDialog('so.ctl?EVENT=TP_REINS_GL_ENTRY_ADD', 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("edt", "Ubah", "openDialog('so.ctl?EVENT=TP_REINS_GL_ENTRY_EDIT&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("edt", "Hapus", "openDialog('so.ctl?EVENT=TP_REINS_GL_ENTRY_DELETE&trxhdrid='+f.trxhdrid.value, 900,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("v", "Lihat", "openDialog('so.ctl?EVENT=TP_REINS_GL_ENTRY_VIEW&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("app", "Setujui", "openDialog('so.ctl?EVENT=TP_REINS_GL_ENTRY_APPROVE&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
                <%=jspUtil.getButtonNormal("rev", "Reverse", "openDialog('so.ctl?EVENT=TP_REINS_GL_ENTRY_REVERSE&trxhdrid='+f.trxhdrid.value, 980,550,refreshcb);")%>
