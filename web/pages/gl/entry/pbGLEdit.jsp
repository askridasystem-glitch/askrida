...
%>
                     <tr class=row<%=i%2%>>
                        ...
                        <td width=25><%=!allowDelete?"":jspUtil.getButtonNormal("bx","-","document.all.rn.value="+i+";submitEvent('PB_GL_EDIT_DEL_LINE')")%></td>
                     </tr>
<% } %>
                     <tr>
                        <%--<td></td>--%>
...
<% } %>
            ...
            <tr>
               <td align=right>
                  <%=ro?"":jspUtil.getButtonNormal("crt","+Line","submitEvent('PB_GL_EDIT_ADD_LINE')")%>
                  <br>
                  <br>
               </td>
            </tr>
<% if (!ro) {%>
            <tr>
               <td align=center>
                  <%=jspUtil.getButtonSubmit("bsave","Save","dialogReturn('yay');f.EVENT.value='PB_GL_EDIT_SAVE'")%>
...
<script>
   ...
   function selectAccountByBranch(o){
   	...
   	openDialog('so.ctl?EVENT=PB_GL_ACCOUNTS_SELECT&acccode='+acccode+'&costcenter='+cabang+'', 400,400,selectAccount);
   }
