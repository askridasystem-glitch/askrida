...
<%
   ...
   if (form.isClDate()) cmap.add(new Integer(12));
...
<% if (form.isClDate()) {%><fo:table-cell border-width="<%=bw%>" border-bottom-style="solid" padding="2pt"><fo:block line-height="10mm"  background-color="#C0C0C0"  text-align="center" font-size="10pt" font-weight="bold">TANGGAL</fo:block></fo:table-cell><% }%>
...
<%
 ...
 //     boolean isNM = "NM".equalsIgnoreCase(opCode);
  ...
  /*    
      if (isNM) {
      	...
      	DTOList glInfos = glr.getGLSub(acctFrom,acctTo,lPeriodFrom,lPeriodTo,lYearFrom,lYearTo);
      	for(int t=0; t<glInfos.size();t++){
      		GLInfoView gli = (GLInfoView) glInfos.get(t);
      		
      		%>
      		<fo:table-row>
      		<fo:table-cell border-width="<%=bw%>" padding="2pt" border-left-style="solid"><fo:block><%=JSPUtil.printX(gli.getStName())%></fo:block></fo:table-cell>
			</fo:table-row>
			<%
      	}
      }
*/
      if (isACTD) {
        ...
        <fo:table-row>
          ...
          <% if (form.isClDate()) {%><fo:table-cell border-width="<%=bw%>" padding="2pt" border-right-style="solid"><fo:block text-align="end" <%=style1%>><%=JSPUtil.printX(gli.getDtTrxDate())%></fo:block></fo:table-cell><%}%>
      ...
      %>
        <fo:table-row>
          <% if (form.isClShowNo()) {%><fo:table-cell border-width="<%=bw%>" padding="2pt" border-right-style="solid"><fo:block text-align="end" <%=style1%>><%=JSPUtil.printX(form.getLineNo())%></fo:block></fo:table-cell><%}%>
          <% if (form.isClDate()) {%><fo:table-cell border-width="<%=bw%>" padding="2pt" border-right-style="solid"><fo:block text-align="end" <%=style1%>><%=JSPUtil.printX("")%></fo:block></fo:table-cell><%}%>
          <% if (form.isClTrxNo()) {%><fo:table-cell border-width="<%=bw%>" padding="2pt" border-right-style="solid"><fo:block text-align="end" <%=style1%>><%=JSPUtil.printX("")%></fo:block></fo:table-cell><%}%>
