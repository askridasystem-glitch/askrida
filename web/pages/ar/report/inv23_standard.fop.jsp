    ...
    <fo:page-sequence master-reference="only" initial-page-number="1">
        ...
        <fo:flow flow-name="xsl-region-body">                 
            ...
            <fo:block font-size="16pt" line-height="16pt" color="black" text-align="center" space-after.optimum="5pt">
                <%if (pol.isDLA()) {%>{L-ENG DEFINITE CLAIM INWARD CASH LOSS -L}{L-INA KLAIM PASTI INWARD CASH LOSS-L} <%} else {%> {L-ENG PRELIMINARY CLAIM INWARD CASH LOSS -L}{L-INA KLAIM SEMENTARA INWARD CASH LOSS-L} <%}%>
            </fo:block>
            ...
            <fo:block font-size="10pt">
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell ><fo:block space-after.optimum="10pt">{L-ENG Type of Treaty-L}{L-INA Jenis Treaty-L}</fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block><%=JSPUtil.printX(pol.getTreatyType(pol.getStRefID0()).getStInsuranceTreatyTypeName())%></fo:block></fo:table-cell>
                        </fo:table-row>

                        <fo:table-row>
                            <fo:table-cell ><fo:block space-after.optimum="10pt">{L-ENG Year of Treaty-L}{L-INA Tahun Treaty-L}</fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block><%=JSPUtil.xmlEscape(pol.getEntity().getStEntityName().toUpperCase())%></fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            <fo:table-cell ><fo:block space-after.optimum="10pt">{L-ENG 100% Claim of <%=JSPUtil.printX(pol.getTreatyType(pol.getStRefID0()).getStInsuranceTreatyTypeName())%>-L}{L-INA Klaim <%=JSPUtil.printX(pol.getTreatyType(pol.getStRefID0()).getStInsuranceTreatyTypeName())%> 100%-L}</fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
                        ...
                        <%
                                    ...
                                    for (int i = 0; i < details.size(); i++) {
                                        ...
                                        amount = io.getDbEnteredAmount();
                        %>

                        <fo:table-row>
                            <fo:table-cell ><fo:block space-after.optimum="10pt"><%=JSPUtil.printX(io.getStDescription())%></fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
