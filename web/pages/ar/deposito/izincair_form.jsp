        ...
        <c:evaluate when="<%=phase >= 2%>">
            <tr>
                <td colspan=2>
                    ...
                    <c:field name="notesrealisasiindex" hidden="true" type="string"/>
                    <c:listbox name="pencairandet">
                        ...
                        <c:evaluate when="<%=pencairandet != null%>" >

                    </tr>
                    <tr>
                        <td class=row1 colspan="5">
                            <c:button text="Tujuan Pencairan" event="addRealisasi" clientEvent="f.notesindex.value='$index$';" validate="false" enabled="<%=!ro%>"/>
                        </td>
                    </tr>
                    <c:evaluate when="<%=pencairandet != null && pencairandet.getListRealisasi().size() > 0%>">
                        <tr class="row1">
                            <td colspan="7">
                                <%
                                            ARInvestmentIzinPencairanDetView rclTitipReal = (ARInvestmentIzinPencairanDetView) pencairandet.getListRealisasi().get(0);
                                %>
                            </td>
                        ...
                        <tr class="row1">
                            <td colspan="10">
                                <table>
                                    ...
                                    <%
                                                final DTOList titipanDet = pencairandet.getListRealisasi();

                                                for (int i = 0; i < titipanDet.size(); i++) {
                                                    ARInvestmentIzinPencairanDetView rclTitip = (ARInvestmentIzinPencairanDetView) titipanDet.get(i);
                                                    System.out.print("@@@@@@@@@@@@@@@@@@@2 " + rclTitip.getStJenisCair());

                                                    boolean expanded = false;
                                                    if (rclTitip.getStJenisCair().equalsIgnoreCase("1")) {
                                                        expanded = true;
                                                    }
                                    %>
                                    <tr class="row1">
                                        <td class=row1><c:button text="-" event="onDeleteInvoiceTitipanItem" clientEvent="<%="f.notesindex.value='$index$';f.notesrealisasiindex.value='" + i + "';"%>" validate="false" enabled="<%=!ro%>" /></td>
                                        <td class=row1 >
                                            <c:field name="<%="pencairandet.[$index$].listRealisasi.[" + i + "].stJenisCair"%>" width="100" caption="Realisasi" type="string" lov="LOV_IZIN_PENCAIRAN" changeaction="refreshIzin"/>
                                        </td>
                                        <td class=row1 >
                                            <c:evaluate when="<%=expanded%>">
                                                <c:field  width="160" clientchangeaction="<%="selectRealisasi($index$," + i + ")"%>"
                                                name="<%="pencairandet.[$index$].listRealisasi.[" + i + "].stPolicyID"%>" descfield="<%="pencairandet.[$index$].listRealisasi.[" + i + "].stDLANo"%>"
                                                          readonly="false" popuplov="true" lov="LOV_CLAIM2" caption="Titipan" type="string" >
                                                    <c:param name="param" value="<%=izinpencairan.getStCostCenterCode()%>" />
                                                </c:field>
                                            </c:evaluate>
                                        </td>
                                        <td class=row1 >
                                            <c:field name="<%="pencairandet.[$index$].listRealisasi.[" + i + "].stPencairanKet"%>" width="200" mandatory="true" caption="Keterangan" type="string"/>
                                        </td>
                                        <td align="right" class=row1 >
                                            <c:field name="<%="pencairandet.[$index$].listRealisasi.[" + i + "].dbNilai"%>" width="100" mandatory="true" caption="Keterangan" type="money16.2"/>
                                        </td>
                                    </tr>
...
<script>
    ...
    function selectRealisasi(i,j) {
        var o = window.lovPopResult;

        document.getElementById('pencairandet.[' + i + '].listRealisasi.[' + j + '].stPencairanKet').value = o.dla_no;
        document.getElementById('pencairandet.[' + i + '].listRealisasi.[' + j + '].dbNilai').value = o.jumlah;

        f.artitipanid.value = docEl('pencairandet.[' + i + '].listRealisasi.[' + j + '].stPolicyID').value;

        f.invoicesindex.value = i;
        f.notesrealisasiindex.value = j;
        //f.action_event.value='validateKlaimAlreadyIn';
        f.submit();
    }
</script>