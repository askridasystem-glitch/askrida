...
%>
   <body>
      <form name=f method=POST action="ctl.ctl">
         <input type=hidden name=EVENT value="AR_RECEIPT_SEARCH_NO">
...
%>
                     <tr class=row<%=i%2%>>
                        ...
                        <td id=COINSID><%=jspUtil.print(iv.getStClaimCoinsID())%></td>
                        <td id=POLID><%=jspUtil.print(iv.getStAttrPolicyID())%></td>
                        <td id=COINSNAME><%=jspUtil.print(iv.getStClaimCoinsName())%></td>
                        <td id=COINSADDRESS><%=jspUtil.print(iv.getStClaimCoinsAddress())%></td>
                     </tr>
<% } %>
                  </table>
               </td>
      ...
      dialogReturn(
         {
          ...
          COINSID:document.getElementById('COINSID').innerText,
          COINSNAME:document.getElementById('COINSNAME').innerText,
          COINSADDRESS:document.getElementById('COINSADDRESS').innerText
         }
      );
      window.close();
   }
