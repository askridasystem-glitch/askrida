...
<%@ taglib prefix="c" uri="crux" %><c:frame title="SETTLEMENT" >
    <%
                ...
                if (arrequest.getStYears() != null) {
                    phase = 2;
                }

                System.out.print("$$$$$$$$$$$$$$$$$$$ " + arrequest.isProgramKerja());
                System.out.print("$$$$$$$$$$$$$$$$$$$ " + arrequest.isBiayaRutin());
                    ...
                    <tr>
                        <td>
                            <table cellpadding=2 cellspacing=1>
                                ...
                                <%--
                                <c:field show="<%=arrequest.getStCostCenterCode() != null%>" lov="LOV_HCIS_UnitKerja" mandatory="false" readonly="<%=arrequest.getStRegionID() != null%>" changeaction="refresh"
                                         width="300" name="arrequest.stRegionID" caption="Unit Kerja" type="string" presentation="standard">
                                    <c:lovLink name="ccCOde" link="arrequest.stCostCenterCode" clientLink="false"/>
                                </c:field>
                                --%>
                                ...
                                <c:evaluate when="<%=phase >= 1%>">
                                    <c:field name="arrequest.stMonths" type="string" caption="{L-ENGMonth-L}{L-INABulan Transaksi-L}" readonly="<%=arrequest.getStMonths() != null%>" lov="LOV_MONTH_Period" mandatory="true" presentation="standard" changeaction="refresh"/>
                                    <c:field show="<%=arrequest.getStMonths() != null%>" name="arrequest.stYears" type="string" caption="{L-ENGBulan-L}{L-INATahun Transaksi-L}" lov="LOV_GL_Years3"  readonly="<%=arrequest.getStYears() != null%>" presentation="standard" changeaction="setDate"/>
                                </c:evaluate>
                        ...
                        <td>
                            <table cellpadding=2 cellspacing=1>
                                <c:evaluate when="<%=phase >= 2%>">
                                    ...
                                    <c:field caption="Tanggal Realisasi" name="arrequest.dtTglRequest" type="date" mandatory="true" presentation="standard"/>
                                    <c:field caption="Keterangan Biaya" width="300" rows="4" name="arrequest.stDescription" type="string" presentation="standard"/>
                        ...
                        <c:tabpage name="TAB_DETIL">
                            <table cellpadding=2 cellspacing=1>
                                ...
                                <c:listbox name="reqObject">
                                    <c:evaluate when="<%=arrequest.isBiayaRutin()%>">
                                    ...
                                    <c:listcol title="Account" >
                                        ...
                                        <c:field width="100" caption="Akun" name="reqObject.[$index$].stAccountNo" type="string" mandatory="true" readonly="<%=arrequest.isProgramKerja()%>"/>
                                        <c:evaluate when="<%=arrequest.isBiayaRutin()%>">
                                            <c:button text="..." clientEvent="selectKertasKerja($index$)" validate="false" enabled="true"/>
                                        </c:evaluate>
                                    </c:listcol>
                                    <c:listcol title="Kertas Kerja" >
                                        <c:field name="reqObject.[$index$].stAccountDesc" width="200" rows="2" readonly="<%=arrequest.isProgramKerja()%>" mandatory="true" caption="Deskripsi" type="string"/>
                                    </c:listcol>
                                    <c:listcol title="Anggaran" >
                                        <c:field name="reqObject.[$index$].dbNominal" width="100" readonly="<%=arrequest.isProgramKerja()%>" caption="Deskripsi" type="money16.2"/>
                                    </c:listcol>
                                    <c:listcol title="Keterangan" >
                                        <c:field name="reqObject.[$index$].stDescription" width="200" readonly="<%=arrequest.isProgramKerja()%>" rows="2" mandatory="true" caption="Deskripsi" type="string"/>
                                    </c:listcol>
                                    <c:listcol title="Qty" >
                                        <c:field name="reqObject.[$index$].stQuantity" width="30" readonly="<%=arrequest.isProgramKerja()%>" caption="Deskripsi" type="string"/>
                                    </c:listcol>
                                    <c:listcol title="" >
                                        <c:field name="reqObject.[$index$].stSatuanID" lov="LOV_QUANTITY" width="50" readonly="<%=arrequest.isProgramKerja()%>" caption="Deskripsi" type="string"/>
                                    </c:listcol>
                                    <c:listcol title="Tanggal<br>Pengajuan" >
                                        <c:field name="reqObject.[$index$].dtTglCashback" readonly="true" caption="Deskripsi" type="date"/>
                                    </c:listcol>
                                    <c:evaluate when="<%=arrequest.isProgramKerja()%>" >
                                        <c:listcol title="Pengajuan" >
                                            <c:field name="reqObject.[$index$].dbHargaSatuan" width="80" readonly="<%=arrequest.isProgramKerja()%>" caption="Deskripsi" type="money16.2"/>
                                        </c:listcol>
                                    </c:evaluate>
                                    <c:listcol title="Realisasi" >
                                        <c:field name="reqObject.[$index$].dbNominalRealisasi" width="80" mandatory="false" caption="Deskripsi" type="money16.2"/>
                                    </c:listcol>
                                    <c:listcol title="Total" >
                                        <c:field name="reqObject.[$index$].dbTotalNilai" width="100" readonly="true" caption="Deskripsi" type="money16.2"/>
                                    </c:listcol>
                                    <c:listcol title="Tanggal<br>Invoice" >
                                        <c:field name="reqObject.[$index$].dtTglInvoice" mandatory="false" caption="Deskripsi" type="date"/>
                                    </c:listcol>
                                    <c:listcol title="Upload Bukti<br>(Invoice)" >
                                        <c:field name="reqObject.[$index$].stKwitansi" width="50" caption="Lampiran Surat" type="file" thumbnail="true"/>
                                    </c:listcol>
                                    <c:evaluate when="<%=!ro%>" >
