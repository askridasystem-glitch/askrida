...
<% if (Tools.compare(premi, new BigDecimal(0))>0 && Tools.compare(tsi, new BigDecimal(0))>0) {  %>           
...
<% } %>
       ...
       <fo:block font-size="<%=fontsize%>"
                ...
                text-align="center" >
                {L-ENG SAY :-L}{L-INA TERBILANG :-L} <%=NumberSpell.readNumber(JSPUtil.printX(BDUtil.add(pol.getDbTotalDue(), pol.getDbTotalDisc()),2),pol.getStCurrencyCode())%> RUPIAH
      </fo:block>
...
<% if (Tools.isEqual(premi, new BigDecimal(0)) && Tools.isEqual(tsi, new BigDecimal(0))) { %>
