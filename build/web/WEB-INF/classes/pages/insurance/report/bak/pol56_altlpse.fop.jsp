...
<% DTOList cover = pol.getCoverage2();
	for (int m = 0; m < cover.size(); m++) {
		InsurancePolicyCoverView cov = (InsurancePolicyCoverView) cover.get(m); %>           

            <fo:table-row>
             <fo:table-cell><fo:block>{L-ENG 2. -L}{L-INA 2. -L}</fo:block></fo:table-cell>
             <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Dan oleh karena itu <fo:inline font-weight="bold">PENYEDIA JASA</fo:inline> terikat oleh kontrak yang mewajibkan <fo:inline font-weight="bold">PENYEDIA JASA</fo:inline> memberikan jaminan pelaksanaan kepada <fo:inline font-weight="bold">PEJABAT PEMBUAT KOMITMEN</fo:inline> sebesar <%= JSPUtil.printX(cov.getDbRate(),0) %>% persen -L}</fo:block></fo:table-cell>
           </fo:table-row>
