    ...
    <fo:page-sequence master-reference="only" initial-page-number="1">
        ...
        <fo:flow flow-name="xsl-region-body">
            ...
            <fo:block font-size="<%=fontsize%>" font-family="TAHOMA" line-height="0pt" 
                      ...
                      text-align="center"
                      padding-top="10pt">
                Nomor : <%= pol.getStPolicyNo()%> <%--Tanggal : <%=DateUtil.getDateStr(pol.getDtPolicyDate(),"d ^^ yyyy")%>--%>
            </fo:block>
            ...
            <fo:block font-size="<%=fontsize%>" text-align = "justify">
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block space-before.optimum="8pt" space-after.optimum="5pt"><fo:inline font-weight="bold">PT. Asuransi Bangun Askrida</fo:inline> berkedudukan di <%=Parameter.readString("ASKRIDA_ADDRESS")%> yang selanjutnya disebut <fo:inline font-weight="bold">PENJAMIN</fo:inline></fo:block></fo:table-cell>
                        </fo:table-row>
            ...
            <fo:block font-size="<%=fontsize%>" text-align = "center">
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block space-after.optimum="5pt"><fo:inline font-weight="bold">MENJAMIN</fo:inline></fo:block> </fo:table-cell>
                        </fo:table-row>
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA berkedudukan di -L}<%=JSPUtil.xmlEscape(pol.getStCustomerAddress())%> </fo:block></fo:table-cell>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA yang selanjutnya disebut <fo:inline font-weight="bold">PENERIMA JAMINAN</fo:inline> -L} </fo:block></fo:table-cell>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                        </fo:table-row>
            ...
            <fo:block font-size="<%=fontsize%>" text-align = "justify">
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block space-before.optimum="8pt" space-after.optimum="5pt">
                                    {L-ENG XXX -L}{L-INA Sejumlah uang sebesar <%=JSPUtil.printX(pol.getStCurrencyCode())%> <%=JSPUtil.printX(pol.getDbInsuredAmount(),2)%> {L-ENG XXX -L}{L-INA (Terbilang : -L}<%=NumberSpell.readNumber(JSPUtil.printX(pol.getDbInsuredAmount(),2),"",NumberSpell.INDONESIA)%> RUPIAH) dalam hal terjadi pencairan Bank Garansi 
                                    <% if (obj.getStReference12()!=null) { %>No <%=JSPUtil.printX(obj.getStReference12())%> <% } %> <% if (obj.getDtReference4()!=null) { %> Tanggal <%=JSPUtil.printX(obj.getDtReference4())%> <% } %>
                                    yang diterbitkan oleh PENERIMA JAMINAN.  -L}
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block></fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Nama Perusahaan -L}</fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>:</fo:block></fo:table-cell>
            ...
            <fo:block font-size="<%=fontsize%>" text-align = "justify">
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            <fo:table-cell><fo:block space-before.optimum="5pt" space-after.optimum="5pt">{L-ENG XXX -L}{L-INA yang selanjutnya disebut TERJAMIN, untuk keperluan pekerjaan <%=JSPUtil.printX(obj.getStReference6())%> yang diberikan oleh <%=JSPUtil.printX(obj.getStReference5())%>, dimana pencairan Bank Garansi tersebut terjadi terhitung sejak tanggal <%=DateUtil.getDateStr(pol.getDtPeriodStart(),"d ^^ yyyy")%> sampai dengan tanggal <%=DateUtil.getDateStr(pol.getDtPeriodEnd(),"d ^^ yyyy")%> ( <%=String.valueOf(day+1)%> hari).  -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        
                        <fo:table-row>
                            <fo:table-cell><fo:block space-after.optimum="5pt">{L-ENG XXX -L}{L-INA Pembayaran sejumlah uang tersebut diatas dilaksanakan atas perintah tertulis yang pertama, dengan tidak ada pengecualian, yang disampaikan PENERIMA JAMINAN dan selambat-lambatnya 14 (empat belas) hari kerja sejak tanggal diterimanya Klaim Jaminan Kontra Bank Garansi dengan bukti-bukti pencairan Bank Garansi tersebut diatas.  -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        
                        <fo:table-row>
                            <fo:table-cell><fo:block space-after.optimum="5pt">{L-ENG XXX -L}{L-INA Sertifikat Jaminan Kontra Bank Garansi ini merupakan bagian tidak terpisahkan dari Persetujuan Prinsip Jaminan Kontra Bank Garansi PT. ASURANSI BANGUN ASKRIDA pada <%=pol.getStCustomerName()%> No. <%=JSPUtil.printX(pol.getStReference1())%> Tentang Penjaminan Bank Garansi No <%=JSPUtil.printX(obj.getStReference12())%> tanggal <%=JSPUtil.printX(obj.getDtReference4())%> dan tidak dapat dipindah-tangankan atau dijadikan jaminan kepada pihak lain serta untuk keperluan pemberian jaminan ini beserta segenap akibat yang timbul padanya.  -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        
                        <fo:table-row>
                            <fo:table-cell><fo:block space-after.optimum="5pt">{L-ENG XXX -L}{L-INA Menunjuk Pasal 1832 Kitab Undang-Undang Hukum Perdata, dengan ini ditegaskan kembali bahwa PENJAMIN melepaskan hak-hak istimewanya untuk menuntut supaya harta benda pihak yang dijamin lebih dahulu disita dan dijual guna melunasi hutangnya sebagaimana dimaksud dalam Pasal 1831 Kitab Undang-Undang Hukum Perdata.  -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                        
                        <fo:table-row>
                            <fo:table-cell><fo:block space-after.optimum="5pt">{L-ENG XXX -L}{L-INA Ditandatangani serta dibubuhi materai di <%= JSPUtil.printX(pol.getStCostCenterDesc())%> pada tanggal <%=DateUtil.getDateStr(pol.getDtPolicyDate(),"d ^^ yyyy")%>.  -L}</fo:block></fo:table-cell>
                        </fo:table-row>
            ...
            <fo:block font-size="10pt"
                      ...
                      space-after.optimum="10pt"
                      text-align="left" >
                Kode Pemasar : <%=JSPUtil.print(pol.getStProducerName())%>
            </fo:block>
