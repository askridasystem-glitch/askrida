   ...
   <!-- GARIS  -->
     <fo:block border-width="0.75pt" border-style="solid" border-before-precedence="0" line-height="0.75pt" space-before.optimum="35pt"></fo:block>
      ...
      <fo:block font-size="16pt" font-family="TAHOMA" line-height="16pt" space-after.optimum="10pt"
            ...
            text-align="center"
            padding-top="10pt">
       {L-INA Polis Asuransi -L}<%=pol.getStPolicyTypeDesc2()%>{L-ENG Insurance Policy -L} 
      </fo:block>
...
<!-- CLAUSE START -->
       ...
       <% if (Tools.isYes(pol.getStPFXClauses())) {%>
...
<!-- OBJECT START -->
 ...
 --%>
       ...
       <fo:block font-size="<%=fontsize%>" space-before.optimum="5pt">{L-ENG OBJECT DETAILS-L}{L-INA KETERANGAN OBJEK-L} 
           <% if (Tools.isYes(pol.getStPFXClauses())) {%> : {L-ENG See Attached-L}{L-INA Lihat Lampiran -L}
         <%}%>
      </fo:block>
...
<% if (!Tools.isYes(pol.getStPFXCoverage())) {%>
   <%
   ...
   %>
            ...
            <fo:table-row>
             ...
             <fo:table-cell ><fo:block>:</fo:block></fo:table-cell>
             <fo:table-cell ><fo:block text-align="left"><%=hasRate?(JSPUtil.printPct(cover.getDbRatePct(),3)+" %"):(pol.getStCurrencyCode()+" "+JSPUtil.print(cover.getDbInsuredAmount(),2))%></fo:block></fo:table-cell>
             <%--<fo:table-cell ><fo:block text-align="center">x</fo:block></fo:table-cell>
             <fo:table-cell ><fo:block text-align="end"><%=JSPUtil.print(cover.getDbPremi(),3)%></fo:block></fo:table-cell>--%>
           </fo:table-row>
      <%
...
<% } else {%>
            <fo:table-row>
             <fo:table-cell number-columns-spanned="5"><fo:block>SEE ATTACHED</fo:block></fo:table-cell>
           </fo:table-row>
<% }%>
               </fo:table-body>
                </fo:table>
             ...
             <fo:table-cell ><fo:block>
                  <fo:block font-size="<%=fontsize%>">
                    <fo:table table-layout="fixed">
                     ...
                     <fo:table-column />
                     <fo:table-body>
<% if (!Tools.isYes(pol.getStPFXDeductible())) {%>
      <%
