                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>         
                        ...
                        <%
                                    ...
                                    InsurancePolicyObjDefaultView obj3 = (InsurancePolicyObjDefaultView) objects2.get(0);

                                    DTOList cover = obj3.getCoverage();
                                    String PHK = "";
                                    String PAW = "";
                                    for (int k = 0; k < cover.size(); k++) {
                                        InsurancePolicyCoverView cov = (InsurancePolicyCoverView) cover.get(k);

                                        if (cov.getStInsuranceCoverID().equalsIgnoreCase("124")) {
                                            PHK = cov.getStInsuranceCoverDesc2();
                                        }

                                        if (cov.getStInsuranceCoverID().equalsIgnoreCase("125")) {
                                            PAW = cov.getStInsuranceCoverDesc2();
                                        }
                                    }

                        %>
                        <%if (!PHK.equalsIgnoreCase("") || !PAW.equalsIgnoreCase("")) {%>
                        <fo:table-row>
                            <fo:table-cell ><fo:block> {L-ENG Coverage-L}{L-INA Jaminan-L}</fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block> : </fo:block></fo:table-cell>
                            <fo:table-cell ><fo:block> <%=JSPUtil.print(PHK)%> <%if (!PAW.equalsIgnoreCase("")) {%><%=JSPUtil.print(PAW)%><%}%></fo:block></fo:table-cell>
                        </fo:table-row>
                        <%}%>
                    </fo:table-body>
                </fo:table>
...
<% }  else { %>            
             <fo:table-cell  border-width="<%=bw%>" border-left-style="solid"><fo:block text-align="center" line-height="5mm" ><%=JSPUtil.printX(new BigDecimal(Integer.parseInt(obj.getStReference4())),new BigDecimal(12)))%></fo:block></fo:table-cell>
<% } %>
