                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Maka kami TERJAMIN dan PENJAMIN dengan ini mengikatkan diri untuk melakukan pembayaran jumlah tersebut diatas dengan baik dan benar bilamana TERJAMIN tidak memenuhi kewajiban sebagaimana ditetapkan dalam <% if (obj.getStReference10() != null) {%> Surat Penunjukkan Penyediaan Barang/Jasa No. <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference10())%></fo:inline> tanggal <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getDtReference1())%></fo:inline> <% } else {%> Surat Kontrak No. <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference11())%></fo:inline> tanggal <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getDtReference2())%></fo:inline><% }%> untuk pelaksanaan pekerjaan -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference12())%></fo:inline> {L-ENG XXX -L}{L-INA yang diselenggarakan oleh PENERIMA JAMINAN -L}. </fo:block></fo:table-cell>
                        </fo:table-row>
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA TERJAMIN tidak menyelesaikan pekerjaan tersebut pada waktunya dengan baik dan benar sesuai dengan ketentuan dalam Kontrak -L}.</fo:block></fo:table-cell>
                        </fo:table-row>

                        <fo:table-row>
                            <fo:table-cell><fo:block> </fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG b. -L}{L-INA b. -L}</fo:block></fo:table-cell>
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Pemutusan kontrak akibat kesalahan TERJAMIN. -L}</fo:block></fo:table-cell>
                        </fo:table-row>
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>Is to be paid to the said Obligee, for which the payment THE PRINCIPAL and THE SURETY to bind themselves, firmly by this presents that if THE PRINCIPAL fails to fulfill his Obligation in the <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference10())%></fo:inline> entered into a certain contract with THE OBLIGEE for <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference12())%></fo:inline> at an agreed contract price of <%=JSPUtil.print(pol.getStCurrencyCode())%> <%=JSPUtil.printX(pol.getDbInsuredAmount(), 2)%> in words: <%=NumberSpell.readNumber(JSPUtil.printX(pol.getDbInsuredAmount(), 2), pol.getStCurrencyCode())%> <%=JSPUtil.printX(pol.getCurrency().getStCcyDescription())%> which agreement is inherent within this bond. </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>Any claim on this bond shall be made in written application by THE OBLIGEE to THE SURETY promptly  after Principal fails to fulfill his obligation as specified in the above contract and is not caused by a force majeure. </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>Surety shall pay to THE OBLIGEE maximum in the same amount of this bond unconditionally within 30 (thirty) calendar days after having received a written claim based on THE OBLIGEE's Decree concerning the penalty due to the result of such default by THE PRINCIPAL. </fo:block></fo:table-cell>
                        </fo:table-row>
...
<%--
            ...
            <fo:block font-size="6pt" space-after.optimum="115pt">
                <%if (!tanpaTanggal) {%>
