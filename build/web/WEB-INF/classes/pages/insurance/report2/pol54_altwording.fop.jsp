                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>{L-ENG XXX -L}{L-INA Maka kami TERJAMIN dan PENJAMIN dengan ini mengikatkan diri untuk melakukan pembayaran jumlah tersebut diatas dengan baik dan benar bilamana TERJAMIN tidak memenuhi kewajiban sebagaimana ditetapkan dalam Berita Acara Serah Terima Pekerjaan Pertama (BAST I) atau PHO/BAST No. -L} <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference10())%></fo:inline> tanggal <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getDtReference1())%></fo:inline> untuk pekerjaan <fo:inline font-weight="bold"><%=JSPUtil.xmlEscape(obj.getStReference11())%></fo:inline> {L-ENG XXX -L}{L-INA yang diselenggarakan oleh PENERIMA JAMINAN. -L}. </fo:block></fo:table-cell>
                        </fo:table-row>
                ...
                <fo:table table-layout="fixed">
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>We, THE PRINCIPAL and THE SURETY bind ourselves to pay the said amount above, if THE PRINCIPAL fails to fulfill his obligation to do the maintenance works of <fo:inline font-weight="bold"><%=JSPUtil.printX(obj.getStReference10())%></fo:inline> entered into a certain contract with THE OBLIGEE for <fo:inline font-weight="bold"><%=JSPUtil.xmlEscape(obj.getStReference11())%></fo:inline> at an agreed contract price of <%=JSPUtil.print(pol.getStCurrencyCode())%> <%=JSPUtil.printX(pol.getDbInsuredAmount(), 2)%> in words: <%=NumberSpell.readNumber(JSPUtil.printX(pol.getDbInsuredAmount(), 2), pol.getStCurrencyCode())%> <%=JSPUtil.printX(pol.getCurrency().getStCcyDescription())%> which agreement is inherent within this bond. </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>NOW THEREFORE, the condition of this bond is such that, if THE PRINCIPAL has already done the maintenance works as specified in the contract, then the bond shall be null and void; otherwise remains in full force and effect as from <%=DateUtil.getDateStr(pol.getDtPeriodStart(), "d ^^ yyyy")%> up to <%=DateUtil.getDateStr(pol.getDtPeriodEnd(), "d ^^ yyyy")%> with the condition as follows. </fo:block></fo:table-cell>
                        </fo:table-row>
                        ...
                        <fo:table-row>
                            ...
                            <fo:table-cell><fo:block>SURETY shall pay to THE OBLIGEE maximum the amount of bond value mentioned above unconditionally at the least 30 (thirty) calendar days after having received a claim letter from THE OBLIGEE, a certain sum or sums for the repairing cost arise in the maintenance period.</fo:block></fo:table-cell>
                        </fo:table-row>
