...
%>
   <body>
      <form name=f method=POST action="ctl.ctl" onsubmit="return dovalidate();">
         <input type=hidden name=EVENT value="MEM_GL_ENTRY_EDIT_REFRESH">
         <input type=hidden name=rn value="">
         <table cellpadding=2 cellspacing=1>
            <tr>
               <td>
                  <%=jspUtil.getHeader("JURNAL MEMORIAL")%>
               </td>
            </tr>
            <tr>
               <td><table cellpadding=2 cellspacing=1>
                  ...
                  <%--<<tr><td>{L-ENGJOURNAL TYPE-L}{L-INAJENIS JURNAL-L}</td><td>:</td><td><%=jspUtil.getInputSelect("jt|Journal Type|string",null,cbJournalType.setLOValue(jh.getStJournalCode()), 100, JSPUtil.MANDATORY|cf|JSPUtil.NOTEXTMODE)%></td></tr>
               	--%>
                <tr><td>{L-ENGBRANCH-L}{L-INACABANG-L}</td><td>:</td><td><%=jspUtil.getInputSelect("costcenter|Branch|string||onChange:MEM_GL_ENTRY_CHG_YEAR",null,CC.setLOValue(jh.getStCostCenter()),200,JSPUtil.MANDATORY|cf)%></td></tr>
...
%>
                     <tr class=row<%=i%2%>>
                        ...
                        <td><%=jspUtil.getInputText("polno"+i,new FieldValidator("accountno","Description","string",128),jv.getStPolicyNo(), 150, cf)%></td>
                        ...
                        <td width=25><%=!allowDelete?"":jspUtil.getButtonNormal("bx","-","document.all.rn.value="+i+";submitEvent('MEM_GL_ENTRY_DEL_LINE')")%></td>
                     </tr>
<% } %>
...
<% } %>
            ...
            <tr>
               <td align=right>
                  <%=ro?"":jspUtil.getButtonNormal("crt","+Line","submitEvent('MEM_GL_ENTRY_ADD_LINE')")%>
                  <br>
                  <br>
               </td>
            </tr>
<% if (!ro) {%>
            <tr>
               <td align=center>
                    <%if(isReverse){%>
                       <%=jspUtil.getButtonSubmit("breverse","Reverse","dialogReturn('yay');f.EVENT.value='MEM_GL_ENTRY_REVERSE'")%>
                  <%}else{%>
                        <%=jspUtil.getButtonSubmit("bsave","Simpan","dialogReturn('yay');f.EVENT.value='MEM_GL_ENTRY_SAVE'")%>
                  <%}%>
...
<script>
   ...
   function selectAccountByBranch(o){
   	...
   	//openDialog('so.ctl?EVENT=GL_ACCOUNTS_SELECT&acccode='+acccode+'&costcenter='+cabang+'', 600,500,selectAccount);
        openDialog('so.ctl?EVENT=GL_ACCOUNTS_SELECT&acccode='+acccode+'', 600,500,selectAccount);
   }
