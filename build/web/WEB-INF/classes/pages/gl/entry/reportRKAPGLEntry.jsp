...
com.webfin.gl.ejb.CurrencyManager"%><html><% final JSPUtil jspUtil = new JSPUtil(request, response);%>
    ...
    <%
                ...
                luyears.setLOValue(String.valueOf(jh.getStYears()));
    ...
    <body>
        <form name=f method=POST action="ctl.ctl" onsubmit="return dovalidate();">
            <input type=hidden name=EVENT value="REPORT_REPORT_GL_ENTRY_EDIT_REFRESH">
            <input type=hidden name=rn value="">
            <table cellpadding=2 cellspacing=1>
                <tr>
                    <td>
                        <%=jspUtil.getHeader("LAPORAN RKAP")%>
                    </td>
                </tr>
                <tr>
                    <td><table cellpadding=2 cellspacing=1>
                            <tr><td>TAHUN</td><td>:</td><td><%=jspUtil.getInputSelect("fiscal", new FieldValidator("", "Years", "integer", 5), luyears, 100, JSPUtil.MANDATORY | cf)%></td></tr>
                        </table>
                    </td>
                </tr>
                <tr>
                    <td>
                        <table cellpadding=2 cellspacing=1>
                            <tr class=header>
                                <%--<td>JT</td>--%>
                                <td>NO.</td>
                                <td>{L-ENGGROUP ID-L}{L-INAGROUP ID-L} </td>
                                <td>{L-ENGDESCRIPTION-L}{L-INADESKRIPSI-L} </td>
                                <td>RKAP KONVENSIONAL</td>
                                <td>RKAP SYARIAH</td>
                                <td></td>
                            </tr>
                            <%
                            ...
                            <tr class=row<%=i % 2%>>
                                <td><%=(i + 1)%></td>
                                <td><%=jspUtil.getInputText("acno" + i, new FieldValidator("accountno", "Description", "string", 128), jv.getStRKAPGroupID(), 350, JSPUtil.READONLY)%></td>
                                <td><%=jspUtil.getInputText("desc" + i, new FieldValidator("description", "Description", "string", 128), jv.getStRKAPDescription(), 350, JSPUtil.READONLY)%></td>
                                <td>
                                    <%=jspUtil.getInputText("konvensional" + i, new FieldValidator("konvensional", "Debit Value", "money16.2", -1), jv.getDbKonvensional(), 140, JSPUtil.MANDATORY | cf | JSPUtil.NOTEXTMODE)%>
                                </td>
                                <td>
                                    <%=jspUtil.getInputText("syariah" + i, new FieldValidator("syariah", "Credit Value", "money16.2", -1), jv.getDbSyariah(), 140, JSPUtil.MANDATORY | cf | JSPUtil.NOTEXTMODE)%>
                                </td>
                ...
                <tr>
                    <td align=center>
                        <%=jspUtil.getButtonSubmit("bsave", "Simpan", "dialogReturn('yay');f.EVENT.value='REPORT_GL_ENTRY_SAVE'")%>
