<%@ page import="com.webfin.gl.model.TitipanPremiExtracomptableView,
                 com.webfin.gl.model.TitipanPremiExtracomptableHeaderView,
...
<%
   ...
   final TitipanPremiExtracomptableHeaderView jh = (TitipanPremiExtracomptableHeaderView)request.getAttribute("JH");
...
%>
   <body>
      <form name=f method=POST action="ctl.ctl" onsubmit="return dovalidate();">
         <input type=hidden name=EVENT value="TP_GL_EXTRACOMPTABLE_EDIT_REFRESH">
         <input type=hidden name=rn value="">
         <table cellpadding=2 cellspacing=1>
            <tr>
               <td>
                  <%=jspUtil.getHeaderOnly("TITIPAN PREMI POLIS KHUSUS")%>
               </td>
            </tr>
            <tr>
               <td><table cellpadding=2 cellspacing=1>
                  ...
                  <tr><td>BULAN</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("month|Month|string",null,MONTH.setLOValue(jh.getStMonths()), 100, JSPUtil.READONLY|cf)%></td></tr>
                  <tr><td>TAHUN</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("year|Year|string",null,YEAR.setLOValue(jh.getStYears()), 100, JSPUtil.READONLY|cf)%></td></tr>
                  ...
                  <tr><td>KURS</td><td>:</td><td colspan="2"><%=jspUtil.getInputText("rate|Rate|money16.2",null,jh.getDbCurrencyRate(), 80, JSPUtil.MANDATORY|cf|rateflags|JSPUtil.NOTEXTMODE)%></td></tr>
                   ...
                   <tr><td>JENIS JURNAL</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("jt|Journal Type|string",null,cbJournalType.setLOValue("AOG"), 100, JSPUtil.MANDATORY|cf|JSPUtil.NOTEXTMODE)%></td></tr>
              	  --%>
                  <tr><td>CABANG</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("costcenter|Branch|string",null,CC.setLOValue(jh.getStCostCenter()),170,JSPUtil.READONLY|cf)%></td></tr>
              	  <tr><td>METODE</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("method2|Method|string||onChange:TP_GL_ENTRY_CHG_YEAR",null,METHOD2.setLOValue(jh.getStMethodCode()), 220, JSPUtil.READONLY|cf)%></td></tr>
                  <tr><td>REKENING</td><td>:</td><td colspan="2"><input type=hidden name=acidmaster value=<%=jspUtil.print(jh.getStAccountIDMaster())%>><%=jspUtil.getInputText("acnomaster",new FieldValidator("accountno","Account Number","string",32), jh.getStAccountNoMaster(), 150, JSPUtil.READONLY|JSPUtil.NOTEXTMODE|JSPUtil.READONLY)%>
                        <%=ro?"":jspUtil.getButtonNormal("bx","...","selectAccountByBranch();")%></td>
                  </tr>
                  <tr><td>KETERANGAN</td><td>:</td><td colspan="2"><%=jspUtil.getInputText("descmaster",new FieldValidator("accountno","Description","string",128),jh.getStDescriptionMaster(), 350, JSPUtil.READONLY)%></td>
...
%>
                     <tr class=row<%=i%2%>>
                        ...
                        <td><%=jspUtil.getInputCheck("activeflag",jv.getStActiveFlag(),"",JSPUtil.MANDATORY|cf )%></td>
                        <td><input type=hidden name=acid<%=i%> value=<%=jspUtil.print(jv.getStAccountID())%>><%=jspUtil.getInputText("acno"+i,new FieldValidator("accountno","Account Number","string",32),jv.getStAccountNo(), 110, JSPUtil.READONLY)%></td>
                        <td width=25><%if(i==0){%><%=ro?"":jspUtil.getButtonNormal("bx","...","rnc="+i+";selectAccountByBranch2();")%><%}%></td>
                        <td><%=jspUtil.getInputText("desc"+i,new FieldValidator("accountno","Description","string",128),jv.getStDescription(), 220, JSPUtil.READONLY)%></td>
                        <td><%=jspUtil.getInputSelect("cause"+i,new FieldValidator("cause","Penyebab","string",128),CAUSE.setLOValue(jv.getStCause()), 140, JSPUtil.READONLY)%></td>
                        <%--<td><%=jspUtil.getInputText("polno"+i,new FieldValidator("polno","Nomor Polis","string",18),jv.getStPolicyNo(), 140, cf)%></td>
                        --%>

                        <td><input type=hidden name=polid<%=i%> value=<%=jspUtil.print(jv.getStPolicyID())%>><%=jspUtil.getInputText("polno"+i,new FieldValidator("polno","Nomor Polis","string",32),jv.getStPolicyNo(), 140, JSPUtil.READONLY)%></td>
                        <td width=25><%=ro?"":jspUtil.getButtonNormal("bx","...","rnc="+i+";selectPolicyByBranch2();")%></td>
                        <td><%=jspUtil.getInputText("trxdate"+i,new FieldValidator("trxdate","Tanggal","date",-1),jv.getDtApplyDate(), 150, JSPUtil.READONLY)%></td>
                        <td><%=jspUtil.getInputText("amount"+i,new FieldValidator("amount","Amount","money16.2",-1),jv.getDbAmount(), 110, JSPUtil.READONLY)%>

                        </td>
                        <td>
                            <%=jspUtil.getInputText("excess"+i,new FieldValidator("excess","Excess","money16.2",-1),jv.getDbExcessAmount(), 110, JSPUtil.READONLY)%>
                        </td>
                        <td><%=jspUtil.getInputText("balance"+i,new FieldValidator("balance","Balance","money16.2",-1),jv.getDbBalance(), 110, JSPUtil.READONLY)%>
...
<% if (!ro) {%>
            <tr>
               <td align=center>
                  ...
                  <%}else{%>
                        <%=jspUtil.getButtonSubmit("bsave","Simpan","dialogReturn('yay');f.EVENT.value='TP_GL_EXTRACOMPTABLE_SAVE'")%>
                  <%}%>
