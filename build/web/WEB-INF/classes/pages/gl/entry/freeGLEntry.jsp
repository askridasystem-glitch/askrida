   ...
   <body>
      <form name=f method=POST action="ctl.ctl" onsubmit="return dovalidate();">
         ...
         <table cellpadding=2 cellspacing=1>
            <tr>
               <td>
                  <%=jspUtil.getHeaderOnly("JURNAL UANG MUKA KLAIM")%>
               </td>
            </tr>
            <tr>
               <td><table cellpadding=2 cellspacing=1>
                  <%if(jh.getStTransactionNo()!=null){%><tr><td>NO. BUKTI</td><td>:</td><td><%=jspUtil.getInputText("trxno",new FieldValidator("","Transaction Number","string",64),jh.getStTransactionNo(), 100, JSPUtil.READONLY|cf)%></td></tr><%}%>
      	...
      	<td>
            ...
            <tr>
               <td>
                  <table cellpadding=2 cellspacing=1>
                     <tr class=header>
                        <td colspan=2>NO LKP/LKS</td>
                        <td>TANGGAL</td>
                        <td>DEBIT</td>
                        <td>CREDIT</td>
                        <td><%=ro?"":jspUtil.getButtonNormal("crt","+","submitEvent('FREE_GL_ENTRY_ADD_LINE')")%></td>
                     </tr>
<%
...
%>
                     <tr class=row<%=i%2%>>
                        <td><input type=hidden name=polid<%=i%> value=<%=jspUtil.print(jv.getStPolicyID())%>><%=jspUtil.getInputText("ref1"+i,new FieldValidator("reference1","No LKS/LKP","string",32),jv.getStReference1(), 150, JSPUtil.MANDATORY|cf|JSPUtil.NOTEXTMODE|JSPUtil.READONLY)%></td>
                        ...
                        <td><%=!allowDelete?"":jspUtil.getButtonNormal("bx","-","document.all.rn.value="+i+";submitEvent('FREE_ENTRY_DEL_LINE')")%></td>
                     </tr>
...
<% if (!ro) {%>
            <tr>
               <td align=center>
                  <%=jspUtil.getButtonSubmit("bsave","Save","dialogReturn('yay');f.EVENT.value='FREE_GL_ENTRY_SAVE'")%>
...
<script>
   ...
   function selectPolicy(o){
   	var cabang = document.getElementById('costcenter').value;
   	openDialog('so.ctl?EVENT=INS_CLAIM_SEARCH&costcenter='+cabang+'', 500,450,selectPolicy2);
   }
   ...
   function selectPolicy2(o) {
      ...
      document.getElementById('ref1'+pol).value=o.dlano;
      document.getElementById('debit'+pol).value=o.amount;
   }
