...
%>
   <body>
      <form name=f method=POST action="ctl.ctl" onsubmit="return dovalidate();">
         <input type=hidden name=EVENT value="TP_REINS_GL_ENTRY_EDIT_REFRESH">
         <input type=hidden name=rn value="">
         <table cellpadding=2 cellspacing=1>
            <tr>
               <td>
                  <%=jspUtil.getHeaderOnly("TITIPAN PREMI REASURANSI")%>
               </td>
            </tr>
            <tr>
               <td><table cellpadding=2 cellspacing=1>
                  ...
                  <tr><td>CABANG</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("costcenter|Branch|string",null,CC.setLOValue(jh.getStCostCenter()),170,JSPUtil.MANDATORY|cf)%></td></tr>
              	  <tr><td>METODE</td><td>:</td><td colspan="2"><%=jspUtil.getInputSelect("method2|Method|string||onChange:TP_REINS_GL_ENTRY_CHG_YEAR",null,METHOD2.setLOValue(jh.getStMethodCode()), 220, JSPUtil.MANDATORY|cf)%></td></tr>
      	...
      	<tr>
      	<td>
            <strong>DAFTAR TITIPAN PREMI REASURANSI </strong>
      	</td>
      	</tr>
...
%>
                     <tr class=row<%=i%2%>>
                        <td width=25> <%=!allowDelete?"":jspUtil.getButtonNormal("bx","-","document.all.rn.value="+i+";submitEvent('TP_REINS_GL_EDIT_DEL_LINE')")%></td>
...
<% } %> 
                     ...
                     <tr>
                        ...
                        <td></td>
                        <td align=right>
                          <%=ro?"":jspUtil.getButtonNormal("crt","+ Titipan","submitEvent('TP_REINS_GL_ENTRY_ADD_LINE')")%>
                        </td>
                        <td></td>
                        <td align=right>
                          Total :
...
<% if (!ro) {%>
            <tr>
               <td align=center>
                  <%if(isReverse){%>
                       <%=jspUtil.getButtonSubmit("breverse","Reverse","dialogReturn('yay');f.EVENT.value='TP_REINS_GL_REVERSE'")%>
                  <%}else{%>
                        <%=jspUtil.getButtonSubmit("bsave","Simpan","dialogReturn('yay');f.EVENT.value='TP_REINS_GL_ENTRY_SAVE'")%>
                  <%}%>
...
<% } else {%>
            <tr>
               <td align=center>
                  <%if(isApproved){%> 
                    <%=jspUtil.getButtonSubmit("bsave","Setujui","dialogReturn('yay');f.EVENT.value='TP_REINS_GL_ENTRY_SAVE'")%>
                  <%}%>
...
<script>
  ...
  function sendCcCode(o){
   ...
   function selectAccountByBranch2(o){
        ...
        if(receiptclass=='A') acccode = '48940';
        else if(receiptclass=='B') acccode = '48940';
        else if(receiptclass=='C') acccode = '48940';
        else if(receiptclass=='D') acccode = '48940';
