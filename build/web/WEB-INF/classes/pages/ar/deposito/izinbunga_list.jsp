...
<%@ taglib prefix="c" uri="crux" %><c:frame title="IZIN BUNGA" >
    ...
    <table cellpadding=2 cellspacing=1>
        ...
        <tr>
            <td>
                <c:listbox name="izinbungalist" autofilter="true" paging="true" selectable="true" view="com.webfin.ar.model.ARInvestmentIzinBungaView" >
                    <c:listcol name="stARIzinBngID" title="" selectid="arizinbngid" />
        ...
        <tr>
            <td>
                <c:button text="Pengajuan Bunga" event="clickCreateIzinBng" />
                <c:button text="Ubah" event="clickEditIzinBng" />
                <c:button text="Lihat" event="clickViewIzinBng" />
            </td>
        </tr>
        <tr>
            <td>
                <c:button show="<%=form.isIzinApprovedCab()%>" text="Setujui Cabang" event="clickAppIzinBngCab" />
                <c:button show="<%=form.isIzinApprovedPus()%>" text="Setujui Pusat" event="clickAppIzinBngPus" />
            </td>
        </tr>
        <tr>
            <td>
                Print <c:field name="stPrintForm" width="250" type="string" lov="LOV_DEPO_PRINTING" ><c:param name="vs" value="IZINBUNGA" /></c:field>
...
<script>
    ...
    function dynPrintClick() {
        ...
        if (f.arizinbngid.value=='') {
            alert('Pilih Data');
            return;
        }

        if (true) {
            frmx.src='x.fpc?EVENT=CAIR_PRINT&arizinbngid='+f.arizinbngid.value+'&alter='+f.stPrintForm.value;
            return;
        }
    }
</script>