                ...
                <tr>
                    <td>
                        <table cellpadding=2 cellspacing=1>
                            ...
                            <c:field lov="LOV_CostCenter" width="200" name="receipt.stCostCenterCode" mandatory="true" caption="Branch" type="string" presentation="standard" readonly="<%=receipt.getStCostCenterCode()!=null && !canNavigateBranch%>" changeaction="changeCostCenter" />
                            ...
                            <c:evaluate when="<%=phase>=3%>" >
                                <c:evaluate when="<%=!isNote%>">
                                    ...
                                    <c:evaluate when="<%=isUsingEntityID%>"> 
                                        <c:field caption="Payment Entity" name="receipt.stAccountEntityID" popuplov="true" lov="LOV_ENTITY" type="string" width="200" presentation="standard"/>
                                    </c:evaluate>
                                </c:evaluate>
                            </c:evaluate>
                            <c:evaluate when="<%=phase>=3%>">
        ...
        <c:evaluate when="<%=showInvoices%>">
            ...
            <tr>
                  <td>
                        ...
                        <c:listcol title="ID" ><%=jspUtil.print(currentLine.getStInvoiceID())%></c:listcol>
                    ...
                    <td class=row1></td> 
                        ...
                        <c:evaluate when="<%=useCheckFlag%>" >
                            <td class=row1><c:field name="<%="listInvoices.[$index$].details.["+i+"].stCheck"%>" readonly="<%=isLock%>" changeaction="checkBayarFlag" caption="Bayar" type="check" /></td>
                        </c:evaluate>
                        <%
                        }
                        %>
    ...
    <td align=center>
        <c:evaluate when="<%=!readOnly%>" >
            <c:evaluate when="<%=phase>=3%>">
                ...
                <c:button text="Save" event="doSavePembayaranLKS" validate="true" confirm="Do you want to save ?" />
            </c:evaluate>
        ...
        <c:evaluate when="<%=form.isApproveMode()%>">
            <c:button text="Approve"  event="doApprovePembayaranLKS" confirm="Do you want to approve ?" />
        </c:evaluate>
    </td>
</tr>
</table>
...
<script>
   function addnewinvoice() {
      openDialog('c.ctl?EVENT=AR_RECEIPT_SEARCH_LKS'
         ...
         function (o) {
            if (o!=null) {
               f.parinvoiceid.value=o.POLICY_ID;
               f.action_event.value='onNewLKS';
               f.submit();
            }
         }
      );
