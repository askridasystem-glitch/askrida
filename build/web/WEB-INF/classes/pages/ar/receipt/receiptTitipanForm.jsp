...
<table cellpadding=2 cellspacing=1>
               ...
               <table cellpadding=2 cellspacing=1>
                  ...
                  <c:evaluate when="<%=phase>=3%>" >
                     <c:evaluate when="<%=!isNote%>">
                        ...
                        <c:evaluate when="<%=isUsingEntityID%>">
                        <c:field name="receipt.stAccountEntityID" type="string" width="200" popuplov="true"  lov="LOV_Entity" mandatory="true" caption="Payment Entity" presentation="standard" />
                     	</c:evaluate>
                     </c:evaluate>
                  </c:evaluate>
                  <c:evaluate when="<%=phase>=3%>">
   ...
   <c:evaluate when="<%=phase>=3%>">
                     ...
                     <td>
                        ...
                        %> 
                           ...
                           <c:listcol title="" columnClass="detail" >
                              ...
                              <c:button text="Object" event="drillToggle" clientEvent="<%="f.invoicesindex.value="+index%>" />
                           </c:listcol>
                           ...
                           <c:evaluate when="<%=currentLine!=null && currentLine.getDetails().size()>0%>" >
                                    ...
                                    <%
                                       ...
                                       for (int i = 0; i < commissions.size(); i++) {
                                          ...
                                          <td class=row1><c:field readonly="<%=isLock%>" name="<%="listInvoices.[$index$].details.["+i+"].dbEnteredAmount"%>" caption="Amount" type="money16.2" /></td>
      ...
      <c:evaluate when="<%=form.isDrillMode()%>" >
          <tr>
             <td>
               <c:button text="Back to Invoice" event="drillToggle" />
               <c:field name="objectindex" type="string" hidden="true" />
               <c:listbox name="objects" paging="true" >
                  <c:listcol title="Object" ><c:field width="200" readonly="true" name="objects.[$index$].stReference1" type="string" caption="Ref1" /></c:listcol> 
               	  <c:listcol title="Premi Outstanding" ><c:field width="100" readonly="true" name="objects.[$index$].dbObjectPremiTotalAmount" type="money16.2" caption="Premi" /></c:listcol> 
               	  <c:field hidden="true" width="100" name="objects.[$index$].dbObjectPremiSettled" type="money16.2" caption="Premi" />
               	  <c:listcol title="Paid" ><c:field width="100"  name="objects.[$index$].stPremiSettledFlag" clientchangeaction="switchRates($index$)" type="check" caption="Paid" /></c:listcol> 
               </c:listbox>
             </td>
         </tr>
      </c:evaluate>
      ...
      <td align=center>
         <c:evaluate when="<%=!readOnly%>" >
            <c:evaluate when="<%=phase>=3%>">
               ...
               <c:button text="Save" event="doSaveTitipan" validate="true" confirm="Do you want to save ?" />
            </c:evaluate>
...
<script>
      ...
      openDialog('c.ctl?EVENT=AR_RECEIPT_SEARCH_INVOICE'
         ...
         function (o) {
            if (o!=null) {
               ...
               f.action_event.value='onNewInvoiceTitipan';
               f.submit();
            }
         }
      );
   ...
   function switchRates(i) {
      try {
         var checked = docEl('objects.['+i+'].stPremiSettledFlag').checked;
         
         if(checked==true){
         	document.getElementById('objects.['+i+'].dbObjectPremiSettled').value=document.getElementById('objects.['+i+'].dbObjectPremiTotalAmount').value;
         }else{
         	document.getElementById('objects.['+i+'].dbObjectPremiSettled').value=0.00;
         }
        } catch(e) {
      }
   }
