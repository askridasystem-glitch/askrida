...
final boolean isPPH21Payment = receipt.isPPH21Payment();
final boolean isPPH23Payment = receipt.isPPH23Payment();
                    ...
                    <td>
                        <table cellpadding=2 cellspacing=1>
                            ...
                            <c:evaluate when="<%=phase>=3%>">
                                ...
                                <c:field lov="VS_TAX_CODE" name="receipt.stTaxCode" readonly="<%=receipt.getStTaxCode()!=null%>" caption="Jenis Pajak" type="string" mandatory="true" presentation="standard" changeaction="changeTaxMode"/>
        ...
        <c:evaluate when="<%=showInvoices%>">
            ...
            <tr>
                <td>
                    ...
                    <c:evaluate when="<%=isPPH21Payment%>">
                        <c:button text="Surat Hutang Pph21" clientEvent="addnewsurathutangpph21()" validate="false" enabled="<%=!readOnly%>"/>
                    </c:evaluate>
                    <c:evaluate when="<%=isPPH23Payment%>">
                        <c:button text="Surat Hutang Pph23" clientEvent="addnewsurathutangpph23()" validate="false" enabled="<%=!readOnly%>"/>
                    </c:evaluate>
            ...
            <tr>
                ...
                <c:listcol title="" columnClass="detail" >
                    <c:button text="-" event="onDeleteInvoiceItemPajak" clientEvent="f.invoicesindex.value='$index$';" validate="false" enabled="<%=!readOnly%>" />
                ...
                <td class=row1><c:field name="<%="listInvoices.[$index$].details.["+i+"].stCheck"%>" readonly="true" changeaction="checkBayarFlag" caption="Bayar" type="check" /></td>
                <%
                }
                %>
            </tr>
    ...
    <td align=center>
        <c:evaluate when="<%=!readOnly%>" >
            <c:evaluate when="<%=phase>=3%>">
                ...
                <c:button text="Simpan" event="doSavePembayaranPajak" validate="true" confirm="Do you want to save ?" />
            </c:evaluate>
        ...
        <c:evaluate when="<%=form.isApproveMode()%>">
            <c:button text="Setujui"  event="doApprovePembayaranPajak" confirm="Do you want to approve ?" />
        </c:evaluate>
    </td>
</tr>
</table>
...
<script>
      ...
      openDialog('c.ctl?EVENT=AR_RECEIPT_SEARCH_INVOICE'
         ...
         function (o) {
            if (o!=null) {
               ...
               f.action_event.value='onNewInvoicePembayaranPajak';
               f.submit();
            }
         }
      );
   ...
   function addnewsurathutangpph21() {
      openDialog('c.ctl?EVENT=AR_RECEIPT_SEARCH_SURAT_HUTANG'
                  +'&ccy='+docEl('receipt.stCurrencyCode').value
                  +'&arsid='+docEl('receipt.stARSettlementID').value
                  +'&cc_code='+docEl('receipt.stCostCenterCode').value
         ...
         function (o) {
            if (o!=null) {
               f.nosurathutang.value=o.NO_SURAT_HUTANG;
               //f.action_event.value='onNewSuratHutangPajak21';
               f.action_event.value='onNewSuratHutangPph21New';
               f.submit();
            }
         }
      );
      ...
      function addnewsurathutangpph23() {
      openDialog('c.ctl?EVENT=AR_RECEIPT_SEARCH_SURAT_HUTANG'
                  +'&ccy='+docEl('receipt.stCurrencyCode').value
                  +'&arsid='+docEl('receipt.stARSettlementID').value
                  +'&cc_code='+docEl('receipt.stCostCenterCode').value
         ...
         function (o) {
            if (o!=null) {
               f.nosurathutang.value=o.NO_SURAT_HUTANG;
               //f.action_event.value='onNewSuratHutangPajak23';
               f.action_event.value='onNewSuratHutangPph23New';
               f.submit();
            }
         }
      );
