...
function openDialog(url, width, height, callback,vz) {
   ...
   if(ie)
        myWin = window.showModalDialog('frame.ctl?EVENT=FRAME_GEN&url='+escape(url),vz,'dialogHeight:'+height+'px;dialogWidth:'+width+'px;center:yes;resizable:no;scroll:yes;status:no;');
   else{

       myWin  = window.open('frame.ctl?EVENT=FRAME_GEN&url='+escape(url),vz,'dialogHeight:'+height+'px;dialogWidth:'+width+'px;center:yes;resizable:no;scroll:yes;status:no;');
   }
...
function openModalDialog(url, width, height, callback,vz) {
   if (url.indexOf('?')<0) url+='?'; else url+='&';
   url+='t='+(new Date().getTime());
   url+='&pop=y';
...
function dialogReturn(x) {
    if (window.opener) {
      window.opener.top.returnValue = x;
    }
...
function lovPop(fldName, fldName2, caption, lovname, params) {
   openDialog('dlg.ctl?EVENT=LOVPOP&code='+ fldName +'&desc='+ fldName2 +'&cap='+caption+'&lov='+lovname+'&'+params,700,400,
      function (a) {
         if (a!=null) {
            if(ie){
                var f=docEl(fldName);
                ...
                /*
                var f=docElParentNonIE(fldName);
                var f2=docElParentNonIE(fldName2);
                f.value=a.code;
                f2.value=a.desc;
                window.lovPopResult=a;*/
                window.lovPopResult=a;
...
function uploadDoc2(par, f) {
   var p="";
   for (x in par) {
      p+='&'+x+'='+par[x];
   }

   openModalDialog('w.ctl?EVENT=FILE_UPLOAD'+p,600,300,f);
}
...
function ireload2(x,n,id, image, text) {
   var p =docEl(x+'descdiv');
   if (p==null) return;

   p.innerHTML='<img src="thumb.ctl?EVENT=FILE&fileid='+id+'&thumb=64">';

   if (n!=null)
      if (image=='Y') {
         p.innerHTML='<img src="thumb.ctl?EVENT=FILE&fileid='+id+'&thumb=64">';
      } else {
         p.innerHTML='<div style="width:100%; border:solid 1px black">'+text+'</div>';
      }
...
function docElParentNonIE(x) {
   return parent.window.opener.document.getElementById(x);
}