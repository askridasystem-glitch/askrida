...
public class WarningPiutangPremi extends Helper implements StatefulJob {

    private final static transient LogManager logger = LogManager.getInstance(WarningPiutangPremi.class);
...
////                execute00All();
    ...
    public void validasicabang() throws Exception {
        ...
        if (hourAuto == hour) {
            ...
            String query = "select a.cc_code,a.attr_pol_no as pol_no,a.mutation_date,date_part('day','" + DateUtil.getDateStr(new Date(), "yyyy-MM-dd 00:00:00") + "' -a.mutation_date) as hari,"
                    ...
                    + "and date_part('day','" + DateUtil.getDateStr(new Date(), "yyyy-MM-dd 00:00:00") + "' - a.mutation_date) = '61' "
            ...
            for (int i = 0; i < cabang.size(); i++) {
                ...
                executeCab(cab.getStCostCenterCode());

            }
        }
    }

    public void executeCab(String koda) throws Exception {
...
//            String counterKey = DateUtil.getYear(new Date());
            String rn = String.valueOf(IDFactory.createNumericID("REQOSPREMI" + counterKey, 1));
            rn = StringTools.leftPad(rn, '0', 3);

            requestNo = rn + "/KEU-OS/" + Parameter.readString("BRANCH_SHORT_" + koda) + "/" + DateUtil.getMonth2Digit(new Date()) + "/" + DateUtil.getYear(new Date());
            ...
            //insert judul
            PdfPCell Title = new PdfPCell(new Phrase("Hal : Konfirmasi Outstanding Premi Umur 61 Hari",
                    smallb));
            ...
            String created2 = "Diberitahukan kepada Saudara/i bahwa per tanggal " + hariIni + ", polis yang tercatat sebagai "
                    + "Outstanding premi umur 61 hari adalah terlampir dengan total premi netto Rp. " + JSPUtil.printX(premiNet, 2) + ".\n\n"
...
//                    smallc = new Font(Font.FontFamily.TIMES_ROMAN, 11);
            ...
            String subject = " (NO-REPLY) Laporan OS Premi (61 Hari) Per Hari Ini " + hariIni;
            String text = "Dengan hormat,\n\n"
                    + "Bersama ini kami lampirkan “Laporan OS Premi (61 Hari)” yang dikirim otomatis oleh sistem.\n"
...
//            mail.sendEmailMultiFileNoReceiver(pathTemp, fileName, pathTemp2, fileName2, null, null, null, null, null, null, subject, text);
            ...
            retrievePiutang(queryPiutangPremi, requestNo);
    ...
    public void execute00() throws Exception {
        final Connection conn = ConnectionCache.getInstance().getConnection();
        Calendar now = Calendar.getInstance();
        int hour = now.get(Calendar.HOUR_OF_DAY);
        int hourAuto = 5;
        ...
        if (hourAuto == hour) {
            ...
            try {
                ...
                String queryPiutangPremi = " select row_number() over(order by a.cc_code,a.pol_no) as no,a.cc_code,a.pol_id::text,a.pol_no,a.tertanggung,a.mutation_date as policy_date,a.hari,a.premibru as amount,"
                        ...
                        + "inner join ent_master b on b.ent_id = a.ent_id "
                        + "where a.posted_flag = 'Y' and coalesce(a.cancel_flag,'') <> 'Y' "
                        + "and " + Parameter.readString("WARNING_PIUTANG_H2H") + " "
                        + "and a.invoice_type in ('AR','AP') and a.ar_trx_type_id in (5,6,7) "
                        + "and substr(a.attr_pol_no,1,16) in ( select pol_no from ( "
                        + "select substr(a.attr_pol_no,1,16) as pol_no,"
                        + "sum(getpremi(a.ar_trx_type_id = 10,getpremi2(d.item_class in ('PREMIG'), c.amount)*-1,getpremi2(d.item_class in ('PREMIG'), c.amount))) as premi_total,"
                ...
                String fileName = "konfirmasiOSpremi_KP" + DateUtil.getDateStr(new Date(), "yyyyMMdd");
                File fo = new File("C:/");
...
//            String counterKey = DateUtil.getYear(new Date());
                ...
                requestNo = rn + "/KEU-OS/KP/" + DateUtil.getMonth2Digit(new Date()) + "/" + DateUtil.getYear(new Date());
                ...
                String created2 = "Diberitahukan kepada Saudara/i bahwa per tanggal " + hariIni + ", polis yang masih tercatat sebagai "
                        + "Outstanding premi umur 61 hari adalah terlampir dengan total premi netto Rp. " + JSPUtil.printX(premiNet, 2) + ".\n\n"
                        + "Menunjuk pada SK Direksi No: SK.013/DIR/2016 Bab XI pasal 20 maka Outstanding premi tersebut agar dapat ditindaklanjuti.\n\n"
                        + "Surat pemberitahuan ini adalah secara otomatis dikonfirmasi oleh sistem dan tidak perlu membalas email ini.\n\n\n"
                        + "Konfirmasi via Sistem";
                ...
                //Create QR Code by using BarcodeQRCode Class
                BarcodeQRCode my_code = new BarcodeQRCode(DateUtil.getDateStr(new Date(), "dd-MMM-yyyy HH:mm:ss") + "_KP", 3, 3, null);
                //Get Image corresponding to the input string
                Image qr_image = my_code.getImage();
                ...
                //batas lampiran OS polis
                ResultSet query_set = S.executeQuery(queryPiutangPremi);
                String fileName2 = "lampiranpolis_KP" + DateUtil.getDateStr(new Date(), "yyyyMMdd");
...
//                    smallc = new Font(Font.FontFamily.TIMES_ROMAN, 11);
                ...
                String receiver = getCostCenter("00").getStEmail();
                String Cc = "internal.audit@askrida.co.id";
                String subject = " (NO-REPLY) Laporan OS Premi (61 Hari) Per Hari Ini " + hariIni;
                String text = "Dengan hormat,\n\n"
                        + "Bersama ini kami lampirkan “Laporan OS Premi (61 Hari)” yang dikirim otomatis oleh sistem.\n"
...
//                mail.sendEmailMultiFileNoReceiver(pathTemp, fileName, pathTemp2, fileName2, null, null, null, null, null, null, subject, text);
                ...
                retrievePiutang00(queryPiutangPremi, requestNo);
    ...
    public void retrievePiutang(String queryPiutangPremi, String requestNo) throws Exception {
        headerPiutang = new UploadHeaderPiutangPremiView();
        headerPiutang.markNew();
        ...
        for (int i = 0; i < l.size(); i++) {
            ...
            uploadPiutangPremiView piutang = new uploadPiutangPremiView();
            piutang.markNew();
        ...
        headerPiutang.setStNoSuratHutang(requestNo);

        saveWarningPiutang(headerPiutang, headerPiutang.getDetails());
    }

    public void retrievePiutang00(String queryPiutangPremi, String requestNo) throws Exception {
        headerPiutang = new UploadHeaderPiutangPremiView();
        headerPiutang.markNew();
        ...
        for (int i = 0; i < l.size(); i++) {
            ...
            piutang.setStTertanggung(h.getFieldValueByFieldNameST("tertanggung"));
            piutang.setDtPolicyDate(h.getFieldValueByFieldNameDT("policy_date"));
            piutang.setStCostCenterCode("00");
            piutang.setStPolicyID(h.getFieldValueByFieldNameST("pol_id"));
            piutang.setDbAmount(new BigDecimal(h.getFieldValueByFieldNameBD("amount").doubleValue()));
            piutang.setDbPremiNetto(new BigDecimal(h.getFieldValueByFieldNameBD("preminett").doubleValue()));
    ...
    public void saveWarningPiutang(UploadHeaderPiutangPremiView header, DTOList l) throws Exception {
        ...
        if (header.isNew() && header.getStInsuranceUploadID() == null) {
            header.setStInsuranceUploadID(String.valueOf(IDFactory.createNumericID("INSPIUTANGPREMIID")));
        }

        try {
            for (int i = 0; i < l.size(); i++) {
                uploadPiutangPremiView j = (uploadPiutangPremiView) l.get(i);
                ...
                if (j.isNew()) {
                    j.setStInsuranceUploadDetailID(String.valueOf(IDFactory.createNumericID("INSPIUTANGPREMIDTLID")));
                }
    ...
    public void execute00All() throws Exception {
        final Connection conn = ConnectionCache.getInstance().getConnection();
        Calendar now = Calendar.getInstance();
        int hour = now.get(Calendar.HOUR_OF_DAY);
        int hourAuto = 5;
        ...
        if (hourAuto == hour) {
            ...
            try {
                ...
                String fileName = "konfirmasiOSpremi_KPPWS" + DateUtil.getDateStr(new Date(), "yyyyMMdd");
                File fo = new File("C:/");
...
//            String counterKey = DateUtil.getYear(new Date());
                ...
                requestNo = rn + "/KEU-OS/KP-PWS/" + DateUtil.getMonth2Digit(new Date()) + "/" + DateUtil.getYear(new Date());
                ...
                //batas lampiran OS polis
                ResultSet query_set = S.executeQuery(queryPiutangPremi);
                String fileName2 = "lampiranpolis_KPPWS" + DateUtil.getDateStr(new Date(), "yyyyMMdd");
...
//                    smallc = new Font(Font.FontFamily.TIMES_ROMAN, 11);
                ...
                String receiver = getCostCenter("00").getStEmail2();
                String Cc = "internal.audit@askrida.co.id";
                String subject = " (NO-REPLY) Laporan OS Premi (61 Hari) Per Hari Ini " + hariIni;
                String text = "Dengan hormat,\n\n"
                        + "Bersama ini kami lampirkan “Laporan OS Premi (61 Hari)” yang dikirim otomatis oleh sistem.\n"
