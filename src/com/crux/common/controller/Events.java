/***********************************************************************
 * Module:  com.crux.common.controller.Events
...
public class Events {
   ...
   public ReportMeta getReport(String stEventName) {
      return (ReportMeta) reports.get(stEventName);
   }
   ...
   public Events(ServletContext context) throws Exception {
      ...
      for (int i = 0; i < nodes.getLength(); i++) {
         ...
         if (rsrc == null) {
            //throw new IllegalArgumentException("Cannot find control file : "+ctlFile);
            logger.logError("Cannot find control file : "+ctlFile);
         } else
   ...
   public void loadFile(String stFileName) throws Exception {
      ...
      for (int i=0;i<nodes.getLength();i++) {
         ...
         if (node.getNodeName().equalsIgnoreCase("report")) {
            ReportMeta rpt = new ReportMeta();
            rpt.setStName(getNodeValue(node,"name").toLowerCase());
            reports.put(rpt.getStName(), rpt);
            ...
            for (int j=0;j<eventchild.getLength();j++) {
               ...
               if ("datasource".equalsIgnoreCase(nname)) {
                  rpt.setStDataSourceFullClassName(getNodeValue(n1,"class"));
                  rpt.setStDataSourceName(getNodeValue(n1,"ds"));
               }
               else if ("helper".equalsIgnoreCase(nname)) {
                  rpt.setStHelperFullClassName(getNodeValue(n1,"class"));
               }
               else if ("jasper".equalsIgnoreCase(nname)) {
                  rpt.setStJasperDesignFile(n1.getAttributes().getNamedItem("file").getNodeValue());
               }
               else if ("subreport".equalsIgnoreCase(nname)) {
                  ReportMeta sub = new ReportMeta();
                  sub.setStName(n1.getAttributes().getNamedItem("name").getNodeValue());
                  sub.setStJasperDesignFile(n1.getAttributes().getNamedItem("jasper").getNodeValue());
                  rpt.getSubReports().add(sub);
               }
            }
         }
         ...
         if (node.getNodeName().equalsIgnoreCase("event")) {
            ...
            event.setStFlags(getNodeValue(node,"flags"));
