...
import com.webfin.gl.model.TitipanPremiExtracomptableHeaderView;
...
public class TitipanPremiEntryHelper extends Helper {
   ...
   public void add(HttpServletRequest rq)  throws Exception {
      final TitipanPremiHeaderView  jh = new TitipanPremiHeaderView ();
   ...
   private void populate(HttpServletRequest rq, TitipanPremiHeaderView  jh) throws Exception {
      if (jh==null) jh=(TitipanPremiHeaderView ) get(rq,"JH");
      put(rq,"JH",jh);
   ...
   public void edit(HttpServletRequest rq)  throws Exception {
      ...
      final TitipanPremiHeaderView  jh = view(rq);
   ...
   public TitipanPremiHeaderView  view(HttpServletRequest rq)  throws Exception {
      final String trxhdrid = getString(rq.getParameter("trxhdrid"));

      final DTOList details = getRemoteGeneralLedger().getTitipanPremi(trxhdrid);
      

      if (details.size()>0) {
         final TitipanPremiHeaderView  jh = new TitipanPremiHeaderView ();

         final TitipanPremiView  jv = (TitipanPremiView )details.get(0);
	 ...
	 jh.setReadOnly(true);
         ...
         for (int i = 0; i < details2.size(); i++) {
             TitipanPremiView  jv2 = (TitipanPremiView ) details2.get(i);
   ...
   public void saveTP(HttpServletRequest rq)  throws Exception {
      final TitipanPremiHeaderView  jh = retrieveCashBank(rq);

      final DTOList details = jh.getDetails();
      
      validate(jh, details);

      getRemoteGeneralLedger().saveTitipanPremi(jh,details);
   }
   ...
   public void addline(HttpServletRequest rq)  throws Exception {
      final TitipanPremiHeaderView  jh = retrieve(rq);
   ...
   private TitipanPremiHeaderView  retrieve(HttpServletRequest rq) throws Exception {
      final TitipanPremiHeaderView  jh = (TitipanPremiHeaderView )get(rq,"JH");
   ...
   public void changeMethod(HttpServletRequest rq)  throws Exception {
    ...
    private void populateCashBank(HttpServletRequest rq, TitipanPremiHeaderView  jh) throws Exception {
      if (jh==null) jh=(TitipanPremiHeaderView ) get(rq,"JH");
      put(rq,"JH",jh);
    ...
    private TitipanPremiHeaderView  retrieveCashBank(HttpServletRequest rq) throws Exception {
      final TitipanPremiHeaderView  jh = (TitipanPremiHeaderView )get(rq,"JH");
      ...
      for (int i = 0; i < details.size(); i++) {
         ...
         jv.setStReferenceNo(getString(rq.getParameter("referenceno"+i)));
    ...
    public void addlineCashBank(HttpServletRequest rq)  throws Exception {
      ...
      if(details.size()>0){
          TitipanPremiView titipanAwal = (TitipanPremiView) details.get(0);
...
/*
    ...
    public void save(HttpServletRequest rq)  throws Exception {
      ...
      getRemoteGeneralLedger().saveEditTitipanPremi(jh,details);
    ...
    private void validate(TitipanPremiHeaderView jh, DTOList details) throws Exception{
        ...
        String glCode = jh.getStAccountNoMaster().substring(5,10);
          ...
          for (int i = 0; i < details.size(); i++) {
             ...
             if(jv.getStPolicyNo()!=null)
                 if(jv.getStPolicyNo().length()<18 || jv.getStPolicyNo().length()>19)
                     throw new RuntimeException("Nomor Polis harus 18 atau 19 digit");
   ...
   private void cekRealisasiTitipanPremi(String trxNo) throws Exception{
       ...
       try{
            String cekTitipan = " select a.receipt_no "+
                            ...
                            " inner join ar_titipan_premi c on b.titipan_premi_id = c.trx_id "+
   ...
   public void activate(HttpServletRequest rq)  throws Exception {

      setEdit(true);

      final TitipanPremiExtracomptableHeaderView  jh = viewExtracomptable(rq);
      ...
      for (int i = 0; i < details.size(); i++) {
         TitipanPremiExtracomptableView  jv = (TitipanPremiExtracomptableView ) details.get(i);
   ...
   public TitipanPremiExtracomptableHeaderView  viewExtracomptable(HttpServletRequest rq)  throws Exception {
      final String trxhdrid = getString(rq.getParameter("trxhdrid"));

      final DTOList details = getRemoteGeneralLedger().getTitipanPremiExtracomptable(trxhdrid);


      if (details.size()>0) {
         final TitipanPremiExtracomptableHeaderView  jh = new TitipanPremiExtracomptableHeaderView ();

         final TitipanPremiExtracomptableView  jv = (TitipanPremiExtracomptableView )details.get(0);

         if(edit)
         	if(jv.getStActiveFlag()!=null)
         		if(jv.getStActiveFlag().equalsIgnoreCase("Y"))
         			throw new RuntimeException("Data sudah di aktivasi");
	 ...
	 jh.setReadOnly(true);
         ...
         for (int i = 0; i < details2.size(); i++) {
             TitipanPremiExtracomptableView  jv2 = (TitipanPremiExtracomptableView ) details2.get(i);
   ...
   private void populateExtracomptable(HttpServletRequest rq, TitipanPremiExtracomptableHeaderView  jh) throws Exception {
      if (jh==null) jh=(TitipanPremiExtracomptableHeaderView ) get(rq,"JH");
   ...
   private TitipanPremiExtracomptableHeaderView retrieveExtracomptable(HttpServletRequest rq) throws Exception {
      final TitipanPremiExtracomptableHeaderView  jh = (TitipanPremiExtracomptableHeaderView )get(rq,"JH");
   ...
   public void saveExtracomptable(HttpServletRequest rq)  throws Exception {
      final TitipanPremiExtracomptableHeaderView  jh = retrieveExtracomptable(rq);

      final DTOList details = jh.getDetails();

      getRemoteGeneralLedger().saveTitipanPremiExtracomptable(jh,details);
   }

   public void editrefreshExtracomptable(HttpServletRequest rq)  throws Exception {
      populateExtracomptable(rq,null);
   }
