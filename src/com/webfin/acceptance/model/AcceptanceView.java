/***********************************************************************
 * Module:  com.webfin.insurance.model.AcceptanceView
...
import com.webfin.insurance.model.InsuranceKoasurView;
import com.webfin.insurance.model.InsuranceKreasiView;
import com.webfin.insurance.model.InsuranceLunasView;
import com.webfin.insurance.model.InsurancePeriodView;
import com.webfin.insurance.model.InsurancePolicyClausulesView;
import com.webfin.insurance.model.InsurancePolicyCoinsView;
import com.webfin.insurance.model.InsurancePolicyCoverReinsView;
...
import com.webfin.insurance.model.InsurancePolicySubTypeView;
...
public class AcceptanceView extends DTO implements RecordAudit, RecordBackup {
    /*
 ...
 ALTER TABLE ins_policy ADD COLUMN period_rate_before numeric;
     ...
     */
    private final static transient LogManager logger = LogManager.getInstance(AcceptanceView.class);
    
    public static String tableName = "ins_pol_acceptance";

    public static String comboFields[] = {"pol_id", "pol_no", "cust_name","pla_no"};
    ...
    public static String fieldMap[][] = {
        ...
        {"stUnderwritingSheetNo", "underwriting_sheet_no"},
        {"stPICName", "pic_name*n"},
    ...
    private AcceptanceObjectView claimObject;
...
//    private DTOList list1;
    ...
    private String stUnderwritingSheetNo;
    private String stDataStatus;
    private String stPICName;
    ...
    public Class getClObjectClass() {
        ...
        if (clObjectClass == null) {
            ...
            if (pt != null){
                  ...
                  pt.getStPolicyTypeCode() != null &&
                  pt.getStPolicyTypeCode().indexOf("OM_") == 0;

                  if (isObjectMap)
                     return AcceptanceObjDefaultView.class;

                  clObjectClass = (Class) FinCodec.PolicyTypeCodeMap2.getLookUp().getValue(pt.getStPolicyTypeCode());
            }
...
//                    invokeCustomHandlersClaimObject(true);
    ...
    public void recalculateClaim() throws Exception {
        ...
        if (isStatusClaim()||isStatusClaimEndorse()){
            ...
            if (!getStClaimCurrency().equalsIgnoreCase("IDR")) scale = 2;
            else scale = 0;
                ...
                for (int i = 0; i < treatyDetails.size(); i++) {
                    ...
                    for (int j = 0; j < shares.size(); j++) {
                        ...
                        if(BDUtil.isZeroOrNull(ri.getDbSharePct())){
                            ...
                            ri.setDbClaimAmount(BDUtil.mulRound(proRate, trd.getDbClaimAmount(),0));
                        }
                    }
                }
    ...
    public void recalculateTreatyEndorse() throws Exception {
        ...
        for (int i = 0; i < parentObj.size(); i++) {
            AcceptanceObjectView parentObjView = (AcceptanceObjectView) parentObj.get(i);
            ...
            for (int k = 0; k < obj.size(); k++) {
                AcceptanceObjectView ObjView = (AcceptanceObjectView) obj.get(i);
    ...
    public void recalculateBasic() throws Exception {
        ...
        if ((objects != null) && (objects.size() > 0)) {
            ...
            for (int i = 0; i < objects.size(); i++) {
                ...
                AcceptanceObjDefaultView objc = (AcceptanceObjDefaultView) obj;
        ...
        for (int j = 0; j < objects.size(); j++) {
            AcceptanceObjectView obj = (AcceptanceObjectView) objects.get(j);
...
//                setCustomStringToPolicy(objc);
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if(item.isPPN() || item.isPPNFeeBase()) continue;
  ...
  125.000.000 x 35% = 43.750.000
    ...
    public AcceptanceView getParentPolicy() {
        return (AcceptanceView) DTOPool.getInstance().getDTO(AcceptanceView.class, stParentID);
    }
        ...
        try {
            ...
            if (objects == null) {
                objects = ListUtil.getDTOListFromQuery(
                        "select * from ins_pol_acceptance_obj where pol_id  = ? order by ins_pol_obj_id asc",
                        new Object[]{stPolicyID},
                        AcceptanceObjDefaultView.class
                        );
    ...
    public AcceptanceObjectView createObject() {
        try {
            getClObjectClass();
            
            final AcceptanceObjectView obj = (AcceptanceObjectView) clObjectClass.newInstance();
    ...
    public String getStSPPANoTrace() throws Exception {
        final AcceptanceView sppa = getLink(FinCodec.PolicyStatus.SPPA);
    ...
    private AcceptanceView getLink(String status) throws Exception {
    ...
    public String getStProposalNo() throws Exception {
        final AcceptanceView draft = getLink(FinCodec.PolicyStatus.DRAFT);
    ...
    public Object getStObjectDescription(int i) {
        ...
        final AcceptanceObjectView o = (AcceptanceObjectView) objects.get(i);
    ...
    public BigDecimal getDbObjectPremiRate(int n, int i) throws Exception {
        ...
        final AcceptanceObjectView o = (AcceptanceObjectView) objects.get(n);
            ...
            if (isStatusClaimDLA() || isStatusClaimEndorse()) {
                ...
                if (dtPolicyDate != null) {
                    if(!isStatusClaim() && !isStatusClaimEndorse() && !isStatusSPPA()){
                            ...
                            if (isApproving)
                                if(currentDate.isBefore(policyDate)) throw new RuntimeException("Tanggal Polis / Endorsemen Tidak Valid (Melebihi Tanggal Approval)");
                            ...
                            //test tutup tgl 1
                            DateTime currentDateFirstDay = new DateTime(DateUtil.getDate("30/06/2016"));
                            currentDateFirstDay = currentDateFirstDay.dayOfMonth().withMinimumValue();
                                    ...
                                    if(isAdmin){
                                        if(!isEditReasOnlyMode())
                                         ...
                                         if(!withinCurrentMonth){
                                             ...
                                             //if(policyDate.isBefore(currentDateFirstDay)) throw new RuntimeException("Tanggal Polis / Endorsemen Tidak Valid (Sudah Tutup Produksi)");
                                         }
                                    }
                                }
                            }
            ...
            for (int j = 0; j < treatyDetails.size(); j++) {
                ...
                if(!isStatusEndorse()){
                    if(getStPolicyTypeID().equalsIgnoreCase("59")){
                        ...
                        if(usiaPlusLamaKredit > 75) validasiReasuransi = true;
                    }
                }
...
//                equalPremiTotal = false;
        ...
        if(validasiReasuransi){
            if (raiseErrors) {
                if(stReinsuranceApprovedWho==null)
                        throw new RuntimeException("Endorsemen ini harus melalui persetujuan reasuransi kantor pusat terlebih dahulu");
            } else {
                return;
            }
        }
    ...
    public boolean isPremiPaidInOldSystem() throws Exception {
        boolean canClaim = false;
        /*
        AcceptanceView parent = getParentPolicy();
...
//   }
        ...
        try {
            ...
            if (objectsClaim == null) {
                ...
                for (int i = 0; i < objectsClaim.size(); i++) {
                    AcceptanceObjectView obj = (AcceptanceObjectView) objectsClaim.get(i);
         ...
         */
            ...
            if(stPolicyTypeID.equalsIgnoreCase("59"))
            {
                ...
                for (int i = 0; i < coins.size(); i++) {
                    ...
                    if(!BDUtil.isZeroOrNull(coin.getDbDiscountRate())) coin.setDbDiscountAmount(BDUtil.mul(coin.getDbPremiAmount(), BDUtil.getRateFromPct(coin.getDbDiscountRate()), scale));
                    premiAfterDisc = BDUtil.sub(coin.getDbPremiAmount(), coin.getDbDiscountAmount());
                    //if(!"21".equalsIgnoreCase(getStPolicyTypeID())) coin.setDbCommissionAmount(BDUtil.mul(premiAfterDisc, BDUtil.getRateFromPct(coin.getDbCommissionRate()), scale));
                    if(!BDUtil.isZeroOrNull(coin.getDbCommissionRate())) coin.setDbCommissionAmount(BDUtil.mul(premiAfterDisc, BDUtil.getRateFromPct(coin.getDbCommissionRate()), scale));
                    if(!BDUtil.isZeroOrNull(coin.getDbBrokerageRate())) coin.setDbBrokerageAmount(BDUtil.mul(premiAfterDisc, BDUtil.getRateFromPct(coin.getDbBrokerageRate()), scale));
    ...
    public void validateTSILimit(BigDecimal transactionLimit) throws Exception{
        ...
        for(int i=0;i<object.size();i++){
            ...
            //else if(!BDUtil.isZeroOrNull(transactionLimitPerRiskLevel))
                //enoughLimit = Tools.compare(transactionLimitPerRiskLevel, tsiObject)>=0;
...
//                getRemoteInsurance().saveInputPaymentDate(this, approvalMode);
    ...
    public void checkEndorseNoBefore(String stPolicyNo) throws Exception{
        ...
        for (int i = 0; i < root.size(); i++) {
            AcceptanceView pol = (AcceptanceView) root.get(i);
    ...
    public void checkHistoryPolicyNo() throws Exception{
        ...
        for (int i = 0; i < history.size(); i++) {
            AcceptanceView pol = (AcceptanceView) history.get(i);
        ...
        try {
            ...
            if (objects_year == null) {
                ...
                for (int i = 0; i < objects_year.size(); i++) {
                    AcceptanceObjectView obj = (AcceptanceObjectView) objects_year.get(i);
    ...
    public AcceptanceView getStParentPolicy() {
        return (AcceptanceView) DTOPool.getInstance().getDTO(AcceptanceView.class, stReference2);
    }
    ...
    public void checkPolicyNoBefore(String stPolicyNo) throws Exception{
        ...
        for (int i = 0; i < root2.size(); i++) {
            AcceptanceView pol = (AcceptanceView) root2.get(i);
        ...
        try {
            ...
            if (objects_koas == null) {
                ...
                for (int i = 0; i < objects_koas.size(); i++) {
                    AcceptanceObjectView obj = (AcceptanceObjectView) objects_koas.get(i);
    ...
    public AcceptanceView getTemporaryPolicy() {
        return (AcceptanceView) DTOPool.getInstance().getDTO(AcceptanceView.class, stReference6);
    }
    ...
    public void checkLastPolicyNo(String stPolicyNo) throws Exception{
        ...
        for (int i = 0; i < claim.size(); i++) {
            AcceptanceView pol = (AcceptanceView) claim.get(0);
...
//        invokeCustomHandlers(true);
        ...
        if ((objects != null) && (objects.size() > 0)) {
            ...
            if(getStPolicyTypeID().equalsIgnoreCase("21")){
                if(isStatusClaim() && getClaimObject()!=null){
                    setCoasAJS(false);
                    setCoasJS(false);
                    AcceptanceObjDefaultView obje = (AcceptanceObjDefaultView) getClaimObject();
    ...
    public boolean validateClaimObject(AcceptanceObjectView claimObj) throws Exception{
...
//        return view.getStUserName();
    ...
    public void setCustomStringToPolicy(AcceptanceObjDefaultView objc){
        if (getStPolicyTypeGroupID().equalsIgnoreCase(FinCodec.InsurancePolicyTypeGroup.SURETYBOND) || getStPolicyTypeGroupID().equalsIgnoreCase(FinCodec.InsurancePolicyTypeGroup.BANKGUARANTEE)) {
            if(!getStPolicyTypeID().equalsIgnoreCase("59")){
                if(objc.getStReference1()!=null){
                    setStReference5(objc.getStReference1()); //nama principal
    ...
    public void invokeCustomHandlersPolicy(boolean validate) {
        if(objects.size()>0){
            AcceptanceObjectView obj = (AcceptanceObjectView) objects.get(0);
...
//            CustomHandlerManager.getInstance().onCalculatePolicy(this, obj, validate);
     ...
     */
    public void setClaimObject(AcceptanceObjectView claimObject) {
...
//            CustomHandlerManager.getInstance().onCalculateSpreading(this, obj, validate);
    ...
    /**
     * @return the stUnderwritingSheetNo
     */
    public String getStUnderwritingSheetNo() {
        return stUnderwritingSheetNo;
    }

    /**
     * @param stUnderwritingSheetNo the stUnderwritingSheetNo to set
     */
    public void setStUnderwritingSheetNo(String stUnderwritingSheetNo) {
        this.stUnderwritingSheetNo = stUnderwritingSheetNo;
    }

    /**
     * @return the stDataStatus
     */
    public String getStDataStatus() {
        return stDataStatus;
    }

    /**
     * @param stDataStatus the stDataStatus to set
     */
    public void setStDataStatus(String stDataStatus) {
        this.stDataStatus = stDataStatus;
    }

    /**
     * @return the stPICName
     */
    public String getStPICName() {
        return stPICName;
    }

    /**
     * @param stPICName the stPICName to set
     */
    public void setStPICName(String stPICName) {
        this.stPICName = stPICName;
    }
