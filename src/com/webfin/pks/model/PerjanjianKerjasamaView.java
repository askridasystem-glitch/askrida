...
public class PerjanjianKerjasamaView extends DTO implements RecordAudit, RecordBackup {
    /*
    ...
    public static String tableName = "perjanjian_kerjasama";
    public static String comboFields[] = {"pks_id", "pol_no"};
    public static String fieldMap[][] = {
        {"stPolicyID", "pks_id*pk"},
        {"stSPPANo", "sppa_no"},
        ...
        {"stPksIndukID", "pks_induk_id"},
        {"stNonActFlag", "nonactive_flag"},
        {"stNonActWho", "nonactive_who"},
        {"dtNonActDate", "nonactive_date"},};
    ...
    private String stPksIndukID;
    private String stNonActFlag;
    private String stNonActWho;
    private Date dtNonActDate;
...
//        CustomHandlerManager.getInstance().customCriteria(this, obj2);
        ...
        try {
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                ...
                if (stPolicyTypeGroupID.equalsIgnoreCase(FinCodec.InsurancePolicyTypeGroup.EARTHQUAKE)) {
                    ...
                    //tre.adjustRatioEarthquake(tlrMaipark, tlr, dbCurrencyRateTreaty, getStCurrencyCode());
        ...
        try {
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                ...
                if ((stPolicyTypeGroupID.equalsIgnoreCase(stEarthquakeTypeId))) {
                    //tre.adjustRatioEarthquake(tlrMaipark, tlr, dbCurrencyRateTreaty, currencyCode);
    ...
    public void recalculateBasicBackup() throws Exception {
        ...
        if (isStatusEndorse() || isStatusEndorseIntern()) {
    ...
    private void calculatePPH21And23() throws Exception {
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            final boolean hasNPWP = item.getStNPWP() != null ? true : false;
...
//        stDLANo = "LKP/" + getDigit(policyType2Digit, 2) + "/" +
    ...
    public void validate(boolean isApproving) throws Exception {
        ...
        if (isApproving) {
            ...
            if (isStatusPolicy() || isStatusEndorse() || isStatusRenewal() || isStatusEndorseRI() || isStatusEndorseIntern() || isStatusTemporaryPolicy() || isStatusTemporaryEndorsemen()) {
                validateTreaty(true);
            }
        }
        ...
        if (isModified()) {
            if (isStatusClaim() || isStatusClaimEndorse()) {
                ...
                if (dtPolicyDate != null) {
                    ...
                    DateTime policyDT = new DateTime(DateUtil.truncDate2(getDtPolicyDate()));
                    DateTime sysDate = new DateTime(DateUtil.truncDate2(new Date()));
                    Months m = Months.monthsBetween(sysDate, policyDT);
                    int mon = m.getMonths();

                    if (mon >= 0) {
                        withinCurrentMonth = true;
                    }

                    if (blockValidPolicyDate && (mon < 0) && !isAdmin) {
        ...
        if (isStatusClaim()) {
            if (isStatusClaimPLA()) {
                if (getStPLANo() == null) {
                    throw new RuntimeException("NO LKS Belum Diisi");
                }
...
//                if (getStDLANo() == null) throw new RuntimeException("NO LKP Belum Diisi");
    ...
    public void validateTreaty(boolean raiseErrors) {
        ...
        if (!(isStatusEndorse() || isStatusPolicy() || isStatusRenewal() || isStatusEndorseRI() || isStatusEndorseIntern() || isStatusTemporaryPolicy() || isStatusTemporaryEndorsemen())) {
            return;
        }
...
//                    obj.setPolicy(this);
    ...
    public void validateTaxCode() throws Exception {
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if (!isStatusClaim()) {
                if (items.getInsItem().getStItemCategory().equalsIgnoreCase("PCOST")
                        || items.getInsItem().getStItemCategory().equalsIgnoreCase("SFEE")) {
    ...
    public void validateTSILimit(BigDecimal transactionLimit) throws Exception {
        ...
        for (int i = 0; i < object.size(); i++) {
            ...
            final boolean enoughLimit = Tools.compare(transactionLimit, BDUtil.mul(obj.getDbObjectInsuredAmount(), getDbCurrencyRate())) >= 0;
...
//                    obj.setPolicy(this);
    ...
    public void loadReins() {
        ...
        try {
            ...
            reins = ListUtil.getDTOListFromQuery(
                    "select f.ins_treaty_detail_id,ins_treaty_shares_id,e.member_ent_id, sum(e.tsi_amount) as tsi_amount, sum(e.premi_amount) premi_amount, sum(e.ricomm_amt) ricomm_amt "
                    + " from ins_policy a "
                    ...
                    + " where  a.pol_id = ? "
                    + " group by e.ins_treaty_shares_id,f.ins_treaty_detail_id,e.member_ent_id",
                    new Object[]{stPolicyID},
    ...
    private DTOList rootpks;

    public DTOList getRootPks() {
        loadRootPks();
        return rootpks;
    }

    public void loadRootPks() {
        try {
            if (rootpks == null) {
                rootpks = ListUtil.getDTOListFromQuery(
                        "select parent_id,pol_no "
                        + " from perjanjian_kerjasama "
                        + " where root_id = ? and active_flag='Y' order by pks_id desc limit 1",
                        new Object[]{stRootID},
    ...
    public void checkLastPolicyNo(String stPolicyNo) throws Exception {
        final DTOList pks = getRootPks();

        for (int i = 0; i < pks.size(); i++) {
            PerjanjianKerjasamaView pol = (PerjanjianKerjasamaView) pks.get(i);

            if (pol.getStParentID() != null) {
                if (!pol.getStParentID().equalsIgnoreCase(stPolicyNo)) {
                    throw new RuntimeException("Addendum PKS harus dibuat dari PKS terakhir : " + pol.getStPolicyNo());
                }
            }
        }
    }
...
//        return view.getStUserName();
    ...
    public void checkRealisasiNoBefore(String policyno) throws Exception {
        final DTOList root = getRootRequest();

        for (int i = 0; i < root.size(); i++) {
            PerjanjianKerjasamaView pol = (PerjanjianKerjasamaView) root.get(i);

            if (pol.getStPolicyNo().equalsIgnoreCase(policyno)) {
                throw new RuntimeException("No PKS " + policyno + " Sudah Pernah Dibuat");
            }
        ...
        try {
            if (rootrequest == null) {
                rootrequest = ListUtil.getDTOListFromQuery(
                        "select * "
                        + "from perjanjian_kerjasama "
                        + "where status = 'PKSADD' and active_flag = 'Y' "
                        + "and root_id = ? ",
                        new Object[]{stRootID},
                        PerjanjianKerjasamaView.class);
            }
    ...
    /**
     * @return the stPksIndukID
     */
    public String getStPksIndukID() {
        return stPksIndukID;
    }

    /**
     * @param stPksIndukID the stPksIndukID to set
     */
    public void setStPksIndukID(String stPksIndukID) {
        this.stPksIndukID = stPksIndukID;
    }

    /**
     * @return the stNonActFlag
     */
    public String getStNonActFlag() {
        return stNonActFlag;
    }

    /**
     * @param stNonActFlag the stNonActFlag to set
     */
    public void setStNonActFlag(String stNonActFlag) {
        this.stNonActFlag = stNonActFlag;
    }

    /**
     * @return the stNonActWho
     */
    public String getStNonActWho() {
        return stNonActWho;
    }

    /**
     * @param stNonActWho the stNonActWho to set
     */
    public void setStNonActWho(String stNonActWho) {
        this.stNonActWho = stNonActWho;
    }

    /**
     * @return the dtNonActDate
     */
    public Date getDtNonActDate() {
        return dtNonActDate;
    }

    /**
     * @param dtNonActDate the dtNonActDate to set
     */
    public void setDtNonActDate(Date dtNonActDate) {
        this.dtNonActDate = dtNonActDate;
    }

    public boolean isPKSInduk() {
        return getStReference1().equalsIgnoreCase("1")
                || getStReference1().equalsIgnoreCase("6");
    }

    public boolean isPKSAddendum() {
        return getStReference1().equalsIgnoreCase("2")
                || getStReference1().equalsIgnoreCase("3")
                || getStReference1().equalsIgnoreCase("4")
                || getStReference1().equalsIgnoreCase("5");
    }
}
