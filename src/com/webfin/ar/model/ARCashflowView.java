...
public class ARCashflowView extends DTO implements RecordAudit {
   /*
...
ALTER TABLE ar_receipt ADD COLUMN entity_id int8;
    ...
    */
     private final static transient LogManager logger = LogManager.getInstance(ARCashflowView.class);
    
    public static String tableName = "ar_cashflow";
    
    public static String fieldMap[][] = {
        {"stARCashflowID","ar_cashflow_id*pk"},
    ...
    public String getStARCashflowID() {
        return stARCashflowID;
    }

    public void setStARCashflowID(String stARCashflowID) {
        this.stARCashflowID = stARCashflowID;
    }
    ...
    public void recalculate() throws Exception {
        ...
        if (isNote()) {
            ...
            if (notes!=null)
                for (int i = 0; i < notes.size(); i++) {
                ARCashflowDetailsView arrc = (ARCashflowDetailsView) notes.get(i);
        ...
        for (int i = 0; i < details.size(); i++) {
            ARCashflowDetailsView rl = (ARCashflowDetailsView) details.get(i);
            ...
            for (int j = 0; j < details.size(); j++) {
                ARCashflowDetailsView d = (ARCashflowDetailsView) details.get(j);
            ...
            for (int k = 0; k < titipan.size(); k++) {
                ARCashflowDetailsView titip = (ARCashflowDetailsView) titipan.get(k);
            ...
            if(rl.getListTitipan().size()>0){
                if(rl.getInvoice()==null){
                    ...
                    for (int j = 0; j < detailsRealisasi.size(); j++) {
                        ARCashflowDetailsView d = (ARCashflowDetailsView) detailsRealisasi.get(j);
        ...
        getGLs();
        for (int i = 0; i < gls.size(); i++) {
            ARCashflowDetailsView rl = (ARCashflowDetailsView) gls.get(i);
    ...
    private DTOList loadDetailsGLS(String receiptLineType) throws Exception {
        if (stARCashflowID!=null)
            return ListUtil.getDTOListFromQuery(
                    "select * from  ar_cashflow_details where ar_cashflow_id = ? and line_type = ? and ar_invoice_id = ?",
                    new Object [] {stARCashflowID, receiptLineType, },
    ...
    private DTOList loadDetails(String receiptLineType) throws Exception {
        if (stARCashflowID!=null)
            return ListUtil.getDTOListFromQuery(
                    "select * from  ar_cashflow_details where ar_cashflow_id = ? and line_type = ? order by ar_rcl_id",
                    new Object [] {stARCashflowID, receiptLineType},
                    ARCashflowDetailsView.class
                    );
    ...
    public void generateReceiptNo() throws Exception {
        ...
        final String methodCode = "CF";//Tools.getDigitRightJustified(getReceiptClass().getStMethodCode(),1);
        ...
        String rn = String.valueOf(IDFactory.createNumericID("CFNO" + counterKey + ccCode,1));
    ...
    public ARReceiptLinesView getARReceiptLines() {
        return (ARReceiptLinesView) DTOPool.getInstance().getDTO(ARReceiptLinesView.class, stARCashflowID);
    }
    ...
    */
    ...
    private DTOList loadDetailsTitipan(String receiptLineType) throws Exception {
        if (stARCashflowID!=null)
            return ListUtil.getDTOListFromQuery(
                    "select * from  ar_cashflow_details where ar_cashflow_id = ? and line_type = ?",
                    new Object [] {stARCashflowID, receiptLineType},
