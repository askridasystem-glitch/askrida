...
public class InvoiceInwardTreatyForm extends Form {
   ...
   public void setInvoice(ARInvoiceView invoice) {
   ...
   public void clickSave() throws Exception {
      getRemoteAccountReceivable().saveSaldoAwal(invoice);
      close();
   }
   ...
   public void clickSaveSuratHutang() throws Exception {
   	  ...
   	  //invoice.markUpdate();
      getRemoteAccountReceivable().saveSuratHutang(invoice,list);
      close();
   }

   public void create() throws Exception {
      invoice = new ARInvoiceView();
...
//      sqa.addPar(DateUtil.dateBracketLow(invoice.getDtSuratHutangPeriodTo()));
   ...
   public void edit(String invoiceid) throws Exception {
      ...
      for (int i = 0; i < invoice.getDetails().size(); i++) {
         ARInvoiceDetailView ivd = (ARInvoiceDetailView) invoice.getDetails().get(i);
   ...
   public void viewSuratHutang(String nosurathutang) throws Exception {
      invoice = getRemoteAccountReceivable().getSuratHutang(nosurathutang);
   ...
   public void view(String invoiceid) throws Exception {
      invoice = getRemoteAccountReceivable().getARInvoice(invoiceid);
   ...
   public void approve(String invoiceid) throws Exception {
    ...
    public void recalculateInwardTreaty() throws Exception {
        logger.logDebug("++++++++++++++ MASUK INVOICE INWARD TREATY FORM +++++++++++++=");
      invoice.recalculateInwardTreaty();
   }
    ...
    public void onChangeTRXTypeInwardTreaty() {
      ...
      final ARInvoiceDetailView ivdHeader = new ARInvoiceDetailView();
      
      ivdHeader.markNew();

      invoice.getDetails().add(ivdHeader);
      
      final ARInvoiceDetailView dtl = (ARInvoiceDetailView) invoice.getDetails().get(Integer.parseInt(itemIndex));
    ...
    public void editInwardTreaty(String invoiceid) throws Exception {
      ...
      for (int i = 0; i < invoice.getDetailsInwardTreaty().size(); i++) {
         ARInvoiceDetailView ivd = (ARInvoiceDetailView) invoice.getDetailsInwardTreaty().get(i);
         ...
         for (int j = 0; j < det.size(); j++) {
            ARInvoiceDetailView detail = (ARInvoiceDetailView) det.get(j);
