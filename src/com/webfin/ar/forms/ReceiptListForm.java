...
public class ReceiptListForm extends Form {
   ...
   public void clickCreateReas() throws Exception {
      final ReceiptForm form = (ReceiptForm)newForm("receiptpembayaranreasform", this);
   ...
   public void clickReverseReas() throws Exception {
   ...
   public void clickCreatePembayaranKomisi() throws Exception {
      final ReceiptForm form = (ReceiptForm)newForm("receiptpembayarankomisiform", this);
   ...
   public void clickViewPembayaranKomisi() throws Exception {
    ...
    public void clickCreatePembayaranPajak() throws Exception {
      final ReceiptForm form = (ReceiptForm)newForm("receiptpembayaranpajakform", this);
   ...
   public void clickViewPembayaranPajak() throws Exception {
    ...
    public void clickCreatePembayaranUangMukaKlaim() throws Exception {
      final ReceiptForm form = (ReceiptForm)newForm("claimform", this);
    ...
    public void clickCreatePembayaranDataFoxpro2() throws Exception {
        ...
        //try {
                ...
                listBayar = ListUtil.getDTOListFromQuery(
                       ...
                       "          and amount_settled is null and attr_pol_id in "+
                       "          ( "+
                       "          select pol_id "+
                       "          from data_bayar_aba_2012 "+
                       "          where pol_id is not null and kodeko = '00' "+
                       "          ) "+
                       "  ) "+
                      "   order by nomor;",
                        HashDTO.class);
                ...
                for (int i = 0; i < listBayar.size(); i++) {
                    ...
                    logger.logWarning("++==== NO POLIS : "+ dto.getFieldValueByFieldNameST("nopol"));
...
//                    if(!inv.getStCurrencyCode().equalsIgnoreCase("IDR"))
//                        continue;
    ...
    public void clickCreatePembayaranKomisiFoxpro2() throws Exception {
        ...
        DTOList listBayar = null;
        listBayar = ListUtil.getDTOListFromQuery(
                " select pol_id,nopol,nobukbay,nobukkom"+ komisi +",komisi"+ komisi +",bayarkom"+ komisi +",tglbaykom"+ komisi +",pajak"+ komisi +","+
                " komisi"+ komisi +"-pajak"+ komisi +" as utang_komisi"+ komisi +",* "+
                "      from data_bayar_aba_2012    "+
                 "     where proses_flag = 'Y'  "+
                 "     and nobukbay <> nobukkom"+ komisi +"  and proses3 is null and kodeko = '00'"+
                 " ORDER BY NOMOR",
                HashDTO.class);
        ...
        for (int i = 0; i < listBayar.size(); i++) { 
            ...
            ARInvoiceView inv = getARInvoiceUsingPolID(invoice_id,dto.getFieldValueByFieldNameBD("utang_komisi"+ komisi));
            
            if(inv==null){
                //continue;
                BigDecimal jmlBayar =  dto.getFieldValueByFieldNameBD("bayarkom"+ komisi);
                DTOList cariHutkom = null;
                cariHutkom = ListUtil.getDTOListFromQuery(
                        "   select ar_invoice_id,round(amount,0) as amount "+
                        "   from ( "+
                        "   select b.ar_invoice_id,sum(b.amount) as amount "+
                        "   from ar_invoice a "+
                        "   inner join ar_invoice_details b on a.ar_invoice_id = b.ar_invoice_id "+
                        "   where ar_trx_type_id = 11 and substr(a.refid2,0,4) = 'POL' and attr_pol_id = ? "+
                        "   group by b.ar_invoice_id) x "+
                       "    where ( (round(amount,0)-1) = ? or (round(amount,0)+1) = ? or round(amount,0)=?)",
                        new Object [] {invoice_id, jmlBayar, jmlBayar, jmlBayar},
                        ARInvoiceView.class);

                if(cariHutkom.size() == 0) continue;

                ARInvoiceView inv2 = (ARInvoiceView) cariHutkom.get(0);

                inv = getRemoteAccountReceivable().getARInvoice(inv2.getStARInvoiceID());
            }
            ...
            */
            ...
            for(int k=0; k < det.size(); k++){
              ...
              //form.getReceipt().generateReceiptNo();
              form.getReceipt().setStFoxproReceiptNo(dto.getFieldValueByFieldNameST("nobukkom"+ komisi));
              
              getRemoteAccountReceivable().savePembayaranKomisi(form.getReceipt());
            

            PreparedStatement PS = S.setQuery("update data_bayar_aba_2012 set proses3 ='KOM_C' where pol_id = ? and kodeko = '00'");
    ...
    public ARInvoiceView getHutangKoas(String attrpolid) throws Exception {
        final ARInvoiceView iv = (ARInvoiceView) ListUtil.getDTOListFromQuery(
                "select * from ar_invoice where attr_pol_id = ? and ar_trx_type_id = 10 and amount_settled is null",
        ...
        if(iv!=null) {
            iv.setDetails(
                    ListUtil.getDTOListFromQuery(
                    "select a.* from ar_invoice_details a,ar_invoice b where a.ar_invoice_id = b.ar_invoice_id and b.attr_pol_id = ? and b.ar_trx_type_id = 10",
                    new Object [] {attrpolid},
                    ARInvoiceDetailView.class
                    )
    ...
    public void clickCreatePembayaranDataFoxproKoas() throws Exception {
        ...
        //try {
                ...
                listBayar = ListUtil.getDTOListFromQuery(
                        " select * "+
                            " from data_bayar_aba "+
                            " where KODEKO <> '00' and nopol in "+
                            " (select SUBSTR(attr_pol_no,0,17) "+
                            "         from ar_invoice A "+
                            "         left join ent_master b on a.ent_id = b.ent_id "+
                            "         where ar_trx_type_id in (10) "+
                            "         AND AMOUNT_SETTLED IS NULL "+
                            "         AND SUBSTR(attr_pol_no,0,17) || b.ref_ent_id IN "+
                           "          ( "+
                           "                  select NOPOL || KODEKO "+
                           "                  from data_bayar_aba "+
                          "                   where length(nopol) = 16 "+
                           "                  AND KODEKO <> '00' AND PROSES_FLAG = 'Y' "+
                           "          ) "+
                           "          AND INVOICE_DATE <= '2012-12-31 00:00:00') order by nomor",
                        HashDTO.class);
                ...
                for (int i = 0; i < listBayar.size(); i++) {
                    ...
                    form.createNewTes("13");

                    form.getReceipt().setStReceiptClassID("4");
                    if(dto.getFieldValueByFieldNameST("ent_id_gl")==null) continue;
                    form.getReceipt().setStAccountEntityID(dto.getFieldValueByFieldNameST("ent_id_gl"));
                    if(form.getReceipt().getStAccountEntityID().equalsIgnoreCase("TIDAK ADA"))
                        form.getReceipt().setStAccountEntityID(dto.getFieldValueByFieldNameBD("entity_id").toString());
                    ...
                    form.getReceipt().setStCostCenterCode(dto.getFieldValueByFieldNameST("nopol").substring(4,6));
                    //form.getReceipt().setStCostCenterCode("00");
                    form.getReceipt().setStCurrencyCode("IDR");
                    form.getReceipt().setDtReceiptDate(dto.getFieldValueByFieldNameDT("tglbay"));
                    logger.logDebug("nopol : "+ dto.getFieldValueByFieldNameST("nopol"));
                    logger.logDebug("cost center : "+ dto.getFieldValueByFieldNameST("nopol").substring(4,6));
                    ARInvoiceView inv = getHutangKoas(invoice_id);
                  ...
                  form.getReceipt().generateReceiptNo();
                  form.getReceipt().setStFoxproReceiptNo(dto.getFieldValueByFieldNameST("nobukbay"));
                  form.getReceipt().setStJournalType("NON_OFFSET");

                  getRemoteAccountReceivable().savePembayaranCoas(form.getReceipt());


                  String sql = "update data_bayar_aba set proses_flag ='Z'";
                  sql = sql + " where pol_id = ? and kodeko <> '00'";
   ...
   public void jurnalUlang() throws Exception {

        final SQLUtil S = new SQLUtil();

        DTOList listReceipt = null;
        listReceipt = ListUtil.getDTOListFromQuery(
                "select * "+
                " from realisasi_salah "+
                "",
                ARReceiptView.class);

        for (int i = 0; i < listReceipt.size(); i++) {
           ARReceiptView rcp = (ARReceiptView) listReceipt.get(i);

           ARReceiptView receipt = getRemoteAccountReceivable().getARReceipt(rcp.getStARReceiptID());
          ...
          for (int j = 0; j < details.size(); j++) {
           ...
           if(receipt.getStARSettlementID().equalsIgnoreCase("25"))
                   getRemoteAccountReceivable().savePembayaranPremi(receipt);
           else if(receipt.getStARSettlementID().equalsIgnoreCase("33"))
                   getRemoteAccountReceivable().savePembayaranKomisi(receipt);

           logger.logWarning("+++++++++++++++++++++++++ PROSES NO BUKTI : "+ receipt.getStReceiptNo());
   ...
   public void clickCreatePembayaranBrokerfeeFoxpro2() throws Exception {
        ...
        DTOList listBayar = null;
        listBayar = ListUtil.getDTOListFromQuery(
                " select pol_id,nopol,nobukbay,nobukfee,b_fee,bayarb_fee,tglb_fee,pajb_fee, "+
                "  b_fee-pajb_fee as utangb_fee,*  "+
                "   from data_bayar_aba_2012 "+
                "   where proses_flag = 'Y' "+
                "   and nobukfee is not null and proses5 is null and kodeko = '00' "+
                "   ORDER BY NOMOR",
                HashDTO.class);
        ...
        for (int i = 0; i < listBayar.size(); i++) {
            ...
            ARInvoiceView inv = getARInvoiceUsingPolID(invoice_id,dto.getFieldValueByFieldNameBD("utangb_fee"));

            if(inv==null){
                continue;
                /*
                BigDecimal jmlBayar =  dto.getFieldValueByFieldNameBD("bayarb_fee");
                DTOList cariHutkom = null;
                cariHutkom = ListUtil.getDTOListFromQuery(
                        "   select ar_invoice_id,round(amount,0) as amount "+
                        "   from ( "+
            ...
            for(int k=0; k < det.size(); k++){
              ...
              form.getReceipt().generateReceiptNo();
              form.getReceipt().setStFoxproReceiptNo(dto.getFieldValueByFieldNameST("nobukfee"));

              getRemoteAccountReceivable().savePembayaranKomisi(form.getReceipt());


            PreparedStatement PS = S.setQuery("update data_bayar_aba_2012 set proses5 ='BFEE' where pol_id = ? and kodeko = '00'");
    ...
    public void clickValidateClaim() throws Exception {
        final ReceiptPembayaranKlaimForm form = (ReceiptPembayaranKlaimForm) newForm("vldform_clm", this);
    ...
    public DTOList getListReadOnly() throws Exception {
      final SQLAssembler sqa = new SQLAssembler();

      if (listReadOnly==null) {
            listReadOnly=new DTOList();
            final Filter filter = new Filter();
            //filter.orderKey="root_id desc, create_date desc";
            listReadOnly.setFilter(filter.activate());
        }
...
//      }
      ...
      sqa.addFilter(listReadOnly.getFilter());

      sqa.addOrder("a.create_date desc");

      sqa.setLimit(200);

      listReadOnly = sqa.getList(ARReceiptView.class);

      return listReadOnly;
   }

    /**
     * @return the listReadOnly
     */
   

    /**
     * @param listReadOnly the listReadOnly to set
     */
    public void setListReadOnly(DTOList listReadOnly) {
        this.listReadOnly = listReadOnly;
    }

    public void clickCreateRCP() throws Exception {
        final ReceiptForm form = (ReceiptForm) newForm("policy_tag", this);

        form.createNewRCP(stSettlementID);
