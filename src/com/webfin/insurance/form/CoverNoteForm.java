...
public class CoverNoteForm extends Form {
   ...
   public CoverNoteView getPolicy() {
      return policy;
   }

   public void setPolicy(CoverNoteView policy) {
      this.policy = policy;
   }

   public void createNew() throws Exception {
      policy = new CoverNoteView();
		
	  /*	
      policy.setDetails(new DTOList());
      policy.setObjects(new DTOList());
      policy.setClausules(new DTOList());
      ...
      policy.setLgInstallmentPeriods(new Long(1));
      */
   ...
   public LookUpUtil getLOV_KreasiRe() throws Exception {
      return ListUtil.getLookUpFromQuery(
              ...
              "   where " +
              "      ent_id in (\n" +
              "         select distinct refid1 from ins_rates_big4 where rate_class='PAKREASI'\n" +
              "      )" +
              "   order by ent_name"
      );
   }
   ...
   public void switchSelectedObject() {
      if (stSelectedObject==null){selectedObject=null;} 
      else{
      ...
      for (int i = 0; i < objects.size(); i++) {
         ...
         for (int j = 0; j < treatyDetails.size(); j++) {
			...
			final DTOList shares = trdi.getShares();
             ...
             //trdi.getCoverage().markAllUpdate();
 ...
 */  
   ...
   public void view(String policyID) throws Exception {
      ...
      policy = getRemoteInsurance().getInsurancePolicy2(policyID);
...
//      o.setPolicy(policy);
      ...
      for (int i = 0; i < mainCovers.size(); i++) {
         ...
         mainCover.markNew();
         //mainCover.setStInsuranceCoverID(policy.getStConditionID());
  ...
  private DTOList getMainCoverReins() throws Exception {
   ...
   public void btnSave() throws Exception {
      ...
      //getRemoteInsurance().save2(policy,policy.getStNextStatus());
   ...
   public void editCreateClaimPLA(String policyID) throws Exception {
    ...
    public void editCreateClaimDLA(String policyID) throws Exception {
      ...
      if(!policy.isLunas()){
      	 throw new RuntimeException("Pembayaran Tagihan Belum Lunas");
      }
...
//			      }
               ...
               PS.setString(n++,obj.getStReference5());
   ...
   public void btnApprove() throws Exception {
      ...
      if (policy.isStatusPolicy() || policy.isStatusSPPA()) {
         ...
         if(policy.getStPolicyTypeID().equalsIgnoreCase("21")){
         	BigDecimal kreasiComissionLimit = BDUtil.getRateFromPct(getKreasiComissionLimit());
         	overKreasiLimit = Tools.compare(comissionRatio, kreasiComissionLimit)>0;
         }
         ...
         if(policy.getStPolicyTypeID().equalsIgnoreCase("21")){
         	if (overKreasiLimit) throw new RuntimeException("Kreasi Comission amount is too high");
         }else{
