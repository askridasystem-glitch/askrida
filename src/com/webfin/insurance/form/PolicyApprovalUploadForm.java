...
public class PolicyApprovalUploadForm extends Form {
    ...
    private PolicyApprovalHeaderView titipan;
    ...
    public PolicyApprovalHeaderView getTitipan() {
        return titipan;
    }

    public void setReceipt(PolicyApprovalHeaderView titipan) {
    ...
    public PolicyApprovalUploadForm() {
    }
    ...
    public void masterEdit(String titipanID) throws Exception {
        ...
        titipan = new PolicyApprovalHeaderView();
...
//        titipan.setStAccountID(titip.getLgAccountID().toString());
    ...
    public void view(String titipanID) throws Exception {
        ...
        final DTOList details = getRemoteInsurance().getUploadPolicyApprovalDetail(titipanID);

        titipan = (PolicyApprovalHeaderView) DTOPool.getInstance().getDTO(PolicyApprovalHeaderView.class, titipanID);
    ...
    public void doSave() throws Exception {
        ...
        getRemoteInsurance().savePolicyApprovalUpload(titipan, titipan.getDetails());
    ...
    public void createNewUpload() throws Exception {
        ...
        setTitle("Upload Persetujuan Polis");
    ...
    public void uploadExcel() throws Exception {
        ...
        HSSFSheet sheet = wb.getSheet("UPLOAD_PERSETUJUAN");

        SimpleDateFormat df2 = new SimpleDateFormat("yyyyMMdd");

        if(sheet==null)
            throw new RuntimeException("Sheet UPLOAD_PERSETUJUAN tidak Ditemukan, download format excel pada menu Dokumen");
        ...
        for (int r = 4; r < rows; r++) {
            ...
            HSSFCell cellKeterangan = row.getCell(2);
            HSSFCell cellPremi = row.getCell(3);

            HSSFCell cellSTNC = row.getCell(4);
            
            String noPolis = cellNoPolis.getStringCellValue().trim();
            //04593333
            //0123456789
            String polTypeId  = noPolis.substring(2, 4);

            //cari polis nya
            DTOList listPolicy = null;

            listPolicy = ListUtil.getDTOListFromQuery(
                " SELECT * FROM ins_policy where status in ('POLICY','ENDORSE','RENEWAL') and pol_no = '"+ noPolis +"'"+
                " order by POL_ID",
            ...
            if(!polis.getStPolicyTypeID().equalsIgnoreCase("59")){
                if(polis.isEffective())
                    throw new RuntimeException("No polis "+ noPolis +" sudah disetujui sebelumnya");
            }

            //jika askred & sudah disetujui maka skip
            if(polis.getStPolicyTypeID().equalsIgnoreCase("59")){
                if(polis.isEffective())
                    continue;
            }
            ...
            //Jika CIS CIT maka harus sudah off risk
            if(polis.getStPolicyTypeID().equalsIgnoreCase("5") || polis.getStPolicyTypeID().equalsIgnoreCase("14")){
            ...
            if(polis.getStPolicyTypeID().equalsIgnoreCase("59")){
                if(polis.isStatusPolicy()){
                    int jumlahHariKalender = DateUtil.getDaysAmount(polis.getDtPeriodStart(), polis.getDtPolicyDate());

                    if(jumlahHariKalender <= 60)
                        throw new RuntimeException("No polis "+ noPolis +" Aging < 60 hari -> Aging : "+ jumlahHariKalender);
                }
            }


            //Jika cab bandung, hanya boleh askred & endorse saja
            boolean allowInputBandung = false;

            if(titipan.getStCostCenterCode().equalsIgnoreCase("21")){

                if(polis.getStPolicyTypeID().equalsIgnoreCase("59") || polis.getStPolicyTypeID().equalsIgnoreCase("21")){
                    if(polis.isStatusEndorse())
                        allowInputBandung = true;
                }

                if(polis.getStPolicyTypeID().equalsIgnoreCase("87") || polis.getStPolicyTypeID().equalsIgnoreCase("88")){
                    if(polis.isStatusPolicy())
                        allowInputBandung = true;
                }

                if(!allowInputBandung){
                    throw new RuntimeException("No polis "+ noPolis +" Cabang Bandung hanya boleh upload Polis Serbaguna/Macet only & endorsemen Askred & PA Kreasi");
                }

                //if(!BDUtil.isEqual(polis.getDbPremiTotal(),new BigDecimal(cellPremi.getNumericCellValue()),2)){
                //    throw new RuntimeException("No polis "+ noPolis +" premi upload "+ NumberUtil.getFormattedValue(cellPremi.getNumericCellValue()) +" tidak sama total premi nya "+ NumberUtil.getFormattedValue(polis.getDbPremiTotal()));
                //}

            }


            //Jika cab jkt, hanya boleh...
            boolean allowInputJKT = false;

            if(titipan.getStCostCenterCode().equalsIgnoreCase("20")){

                //polis kebakaran, boleh
                if(polis.getStPolicyTypeID().equalsIgnoreCase("1")){
                        allowInputJKT = true;
                }

                ////polis cit cis konsorsium pasar, boleh
                if(polis.getStPolicyTypeID().equalsIgnoreCase("5") || polis.getStPolicyTypeID().equalsIgnoreCase("14") || polis.getStPolicyTypeID().equalsIgnoreCase("2")){
                        allowInputJKT = true;
                }

                //endorse cob askred, boleh
                if(polis.getStPolicyTypeID().equalsIgnoreCase("59") || polis.getStPolicyTypeID().equalsIgnoreCase("21")){
                    if(polis.isStatusEndorse())
                        allowInputJKT = true;
                }

                ////polis surety & KBG, boleh
                if(polis.getStPolicyTypeGroupID().equalsIgnoreCase("7") || polis.getStPolicyTypeGroupID().equalsIgnoreCase("8")){
                        allowInputJKT = true;
                }

                if(!allowInputJKT){
                    throw new RuntimeException("No polis "+ noPolis +" Cabang Jakarta tidak dibolehkan approval multi polis (Yang boleh Fire, CIT, CIS, Konsorsium Pasar, Surety, KBG, & Endorse Askred)");
                }

            }

            PolicyApprovalUploadDetailView data = new PolicyApprovalUploadDetailView();
            data.markNew();

            data.setStPolicyNo(cellNoPolis.getStringCellValue());

            if(cellKeterangan!=null)
                data.setStDescription(cellKeterangan.getStringCellValue());

            if(cellPremi!=null)
                data.setDbPremiTotal(new BigDecimal(cellPremi.getNumericCellValue()));

            if(polis.getStPolicyTypeID().equalsIgnoreCase("59")){
                if(polis.isStatusPolicy()){
                   if(cellSTNC!=null){
                       data.setDtTanggalStnc(cellSTNC.getDateCellValue());
                   }
                }
            }
    ...
    public void cekData() throws Exception{
        ...
        if(UserManager.getInstance().getUser().getStBranch()!=null)
            if(UserManager.getInstance().getUser().getStBranch().equalsIgnoreCase("20"))
                if(details.size()>10)
                    throw new RuntimeException("Jumlah data maksimal 10 baris");
