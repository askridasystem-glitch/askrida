...
import com.webfin.ar.forms.ReceiptPembayaranRealisasiTitipanForm;
...
public class PolicyForm extends Form {
    ...
    private String titipanIndex;

    private boolean editReas = false;
    private String stClausulesSearch;

    public String getStClausulesSearch() {
        return stClausulesSearch;
    }

    public void setStClausulesSearch(String stClausulesSearch) {
        this.stClausulesSearch = stClausulesSearch;
    }
   ...
   public void setDrillMode(boolean drillMode) {
    ...
    public void changeTreatyPerObject() {
        ...
        if(policy.getStPolicyTypeID().equalsIgnoreCase("41")){
            ...
            InsurancePolicyObjDefaultView obj = (InsurancePolicyObjDefaultView) selectedObject;
    ...
    public void superEdit(String policyID) throws Exception {
        ...
        policy.getTitipan().markAllUpdate();
    ...
    private void initTabs() {
        ...
        tabs.add(new FormTab.TabBean("TAB_TITIPAN","{L-ENGTITIPAN-L}{L-INATITIPAN-L}",false));
        ...
        boolean statusEndorseClaimInward = FinCodec.PolicyStatus.ENDORSECLAIMINWARD.equalsIgnoreCase(getStStatus());
        ...
        if (statusEndorse||statusClaimEndorse||statusEndorseRI||statusEndorseClaimInward) {
            tabs.enable("TAB_ENDORSE",true);
            tabs.setActiveTab("TAB_ENDORSE");
        }
 
        if (statusClaim || statusClaimEndorse || statusClaimInward || statusEndorseClaimInward) {
        ...
        if (statusEndorse||statusRenewal){
            tabs.enable("TAB_TITIPAN",true);
        }
        
        rdtabs = new FormTab();
        
        rdtabs.add(new FormTab.TabBean("TAB_DETAIL","{L-ENGDETAILS-L}{L-INADETIL-L}",true));
        rdtabs.add(new FormTab.TabBean("TAB_DETAIL_POLICY","SIMULASI",true));
        ...
        rdtabs.add(new FormTab.TabBean("TAB_SUBROGASI", "SUBROGASI", true));
        ...
        if (!(reasMode || statusPolicy || statusRenewal || statusEndorse || statusClaim || statusEndorseRI || statusInward || statusClaimEndorse || statusClaimInward || statusEndorseClaimInward)) {
        ...
        if(statusClaimEndorse||statusEndorseClaimInward){
            tabs.setActiveTab("TAB_ENDORSE");
        }
        
        if(statusDLA||statusClaimEndorse||statusEndorseClaimInward){
    ...
    public void doNewObject() throws Exception {
        ...
        InsurancePolicyObjDefaultView selectedObjectX = (InsurancePolicyObjDefaultView) selectedObject;
    ...
    public void onChgCurrency() throws Exception {
        policy.setDbCurrencyRate(
                CurrencyManager.getInstance().getLatestRate(
...
//        }
    ...
    public void onNewDetail() throws Exception {
        ...
        if(!getPolicy().isStatusEndorse() && !getPolicy().isStatusClaim() && !getPolicy().isStatusClaimEndorse()){
            if(itemCat.isNotActive())
                throw new RuntimeException("Item komisi tidak dapat digunakan");
        }
    ...
    public void onChangeRegion() throws Exception {
        ...
        //get user cabang penerbit, jika ada set sesuai data xxxx

        UserSession us = SessionManager.getInstance().getSession();

        UserSessionView user = getUser(us.getStUserID());

        if(us!=null){
            if(user.getStCostCenterCodeSource()!=null)
                policy.setStCostCenterCodeSource(user.getStCostCenterCodeSource());
            else
                policy.setStCostCenterCodeSource(policy.getStCostCenterCode());

            if(user.getStRegionIDSource()!=null)
                policy.setStRegionIDSource(user.getStRegionIDSource());
            else
                policy.setStRegionIDSource(policy.getStRegionID());
        }
    }
    ...
    private void defineTreaty() throws Exception{
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            if(polTypeID.equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("80") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")){
                setCreditTreaty(obj);
            }

            //jika uw year 2022 pakai treaty khusus kredit 2022
            if(policy.getStPolicyTypeID().equalsIgnoreCase("80")){
            ...
            if(polTypeID.equalsIgnoreCase("1") || polTypeID.equalsIgnoreCase("19") || polTypeID.equalsIgnoreCase("81")){
                ...
                if(periodeAwal.isAfter(bulanDesember2023)){
                    int lamaTahun = 0;
                    int lamaBulan = 0;
                    Date tglAwal = policy.getDtPeriodStart();
                    ...
                    if(polTypeID.equalsIgnoreCase("1") || polTypeID.equalsIgnoreCase("81")){
                     ...
                     if(polTypeID.equalsIgnoreCase("19")){
                         if(policy.getDtPeriodStart()!=null && policy.getDtPeriodEnd()!=null){
                             lamaTahun = DateUtil.getYearsBetweenHUTB(policy.getDtPeriodStart(), policy.getDtPeriodEnd());
                             lamaBulan = DateUtil.getMonthsBetween(policy.getDtPeriodStart(), policy.getDtPeriodEnd());
                         }
                    ...
                    if(lamaBulan > 15){
                        ...
                        obj.setStInsuranceTreatyID(policy.getInsTreatyIDMultiyears(tglAwal));
                    }
            ...
            //jika period start 2025 set treaty ke only OR aja
            //String uwYear = DateUtil.getYear(policy.getDtPeriodStart());

            //if(uwYear.equalsIgnoreCase("2025"))
    ...
    private void defineObjectTreaty() throws Exception{
        ...
        InsurancePolicyObjDefaultView objx = (InsurancePolicyObjDefaultView) selectedObject;
    ...
    public void selectTreaty() throws Exception{
        ...
        policy.recalculateChangeTreaty();
        policy.recalculateTreatyInitialize();
...
//        }
    ...
    public void btnSave() throws Exception {
        if(FinCodec.PolicyStatus.RENEWAL.equalsIgnoreCase(policy.getStNextStatus()) || FinCodec.PolicyStatus.POLICY.equalsIgnoreCase(policy.getStNextStatus())){
            defineTreaty();
        }
        ...
        if(!isInputPaymentDateMode() && !isEditKeteranganMode() && !editReasMode && !isEditClaimNoteMode()){
            ...
            if(policy.isStatusPolicy()||policy.isStatusClaim()||policy.isStatusEndorse()||policy.isStatusRenewal()||policy.isStatusEndorseRI()||policy.isStatusTemporaryPolicy()||policy.isStatusTemporaryEndorsemen()){
                ...
                if(policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88") || policy.getStPolicyTypeID().equalsIgnoreCase("80")){
                    policy.recalculateTreaty();
                }
        ...
        if(policy.isStatusSPPA() && kirimNotifikasi())
            sendApprovalLetters();
        
        if(isCreateEndorseAndPolicyMode()){
            getRemoteInsurance().saveEndorseCreateNewPOlicy(policy);
        }

        if(!isInputPaymentDateMode() && !isCreateEndorseAndPolicyMode()){

            //Pengajuan polis ke asuransi Jiwa untuk PAB
            getRemoteInsurance().pengajuanPolisToPAJ(policy, approvalMode);
            
            getRemoteInsurance().save(policy, policy.getStNextStatus(), approvalMode);
            
            //AUTO BENTUK RENEWAL POLIS JIKA KLAIM LKP DISETUJUI
            createRenewalSerbaguna(policy, approvalMode);

            //Simpan nota ke sistem care via API
            //getRemoteInsurance().saveNotaToCare(policy, approvalMode);
    ...
    public void editCreateRenewal(String policyID) throws Exception {
        ...
        if(policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88"))
            throw new RuntimeException("Kredit Macet dan Kredit Serbaguna tidak bisa dibuat renewal menggunakan fitur ini");
        ...
        if(policy.getStCostCenterCode().equalsIgnoreCase("80"))
            throw new RuntimeException("Polis Non AKS tidak bisa dibuat Perpanjangan menggunakan cabang Non AKS");
        ...
        //if(policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88"))
            //policy.generateRenewalPolicyNoPOJK();
...
//                obj.setStReference2(null);
        ...
        if(blockPremiNotPaid)
            if(!policy.isBypassValidation())
                if(!policy.isPremiPaidInSystem())
                    if(!policy.isBypassValidasiKlaim())
    ...
    public void editCreateClaimEndorse(String policyID) throws Exception {
      ...
      /*if (!FinCodec.PolicyStatus.POLICY.equalsIgnoreCase(policy.getStStatus())) {
        ...
        if (FinCodec.PolicyStatus.CLAIMINWARD.equalsIgnoreCase(policy.getStStatus())) {
            throw new RuntimeException("Endorse Claim Inward hanya bisa dibuat dari Claim Inward");
        }
        ...
        //policy.generateClaimEndorseNo();

        policy.checkDLANoBefore(policy.getStDLANo());
    ...
    public void btnApprove() throws Exception {
        ...
        if (policy.isStatusPolicy() || policy.isStatusRenewal()) {
            ...
            boolean sudahUploadDocPersetujuanDivisi = false;
            boolean sudahUploadSKP = false;
            ...
            if(true){
                ...
                for (int i = 0; i < policyDocuments.size(); i++) {
                    ...
                    if(doc.getStInsuranceDocumentTypeID().equalsIgnoreCase("154") || doc.getStInsuranceDocumentTypeID().equalsIgnoreCase("155"))
                        if(doc.isMarked()&& doc.getStFilePhysic()!=null)
                            sudahUploadDocPersetujuanDivisi = true;

                    //SKP
                    if(doc.getStInsuranceDocumentTypeID().equalsIgnoreCase("156")){
                        if(doc.isMarked()&& doc.getStFilePhysic()!=null)
                            if(doc.getDtDocumentDate()!=null && doc.getStDocumentNumber()!=null && policy.getStCacCbcFlag()!=null)
                                sudahUploadSKP = true;
                    }

                    //cek mandatory dokumen XXX
                }
            }
            
            BigDecimal transactionLimit = getTransactionLimit("ACCEPT", policy.getStRiskClass(), SessionManager.getInstance().getUserID());

            //Jika sudah upload doc persetujuan divisi, TSI limit set unlimited/setara kadiv-uw
            if(sudahUploadDocPersetujuanDivisi || sudahUploadSKP){
            ...
            else policy.validateTSILimit(transactionLimit, sudahUploadDocPersetujuanDivisi);
            ...
            //Jika sudah upload doc persetujuan divisi, limit komisi set unlimited/setara kadiv-uw
            if(sudahUploadDocPersetujuanDivisi || sudahUploadSKP){
                comissionLimit = BDUtil.getRateFromPct(getComissionLimitNew("kadiv-uw"));
            }
    ...
    public void selectClaimObject() throws Exception {
        ...
        //if(o.isInActive())
            //throw new RuntimeException("Objek klaim sudah tidak aktif (Hasil Rekon), tidak bisa dilakukan pembuatan klaim");
        ...
        doDeleteAllObjectExceptClaimObject(stClaimObject);
    ...
    public void uploadExcel()throws Exception{
        try {
            ...
            if("87".equalsIgnoreCase(policy.getStPolicyTypeID()))
                uploadKreditMacetOnlyHdr(fis,treatyID);

            if("88".equalsIgnoreCase(policy.getStPolicyTypeID()))
                uploadKreditSerbagunaHdr(fis,treatyID);
   ...
   private void uploadPAR(FileInputStream fis, String treatyID) throws Exception {
        ...
        for (int r = 4; r < rows + 1; r++) {
            ...
            HSSFCell cellPeriodStart = row.getCell(71);//periode awal
            HSSFCell cellPeriodEnd = row.getCell(72);//periode akhir
            HSSFCell cell4 = row.getCell(75);//penerangan

            HSSFCell cellPeriodRate = row.getCell(17);//period rate
            HSSFCell cellPeriodBaseID = row.getCell(103);//period base
            HSSFCell cellPremiumFactor = row.getCell(104);//premium factor
            ...
            //tsi 1
            HSSFCell cell_Ins_Tcpt_ID1 = row.getCell(77);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID1 = row.getCell(78);//ins_tsi_cat_id
            ...
            //tsi 2
            HSSFCell cell_Ins_Tcpt_ID2 = row.getCell(79);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID2 = row.getCell(80);//ins_tsi_cat_id
            ...
            //tsi 3
            HSSFCell cell_Ins_Tcpt_ID3 = row.getCell(81);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID3 = row.getCell(82);//ins_tsi_cat_id
            ...
            //tsi 4
            HSSFCell cell_Ins_Tcpt_ID4 = row.getCell(83);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID4 = row.getCell(84);//ins_tsi_cat_id
            HSSFCell cell_TSI_DESC4 = row.getCell(33);//tsi desc
            HSSFCell cell_TSI4 = row.getCell(34);//tsi
            HSSFCell cell_TSI4CoMem = row.getCell(35);//tsi100

            String tsiDesc4 = cell_TSI_DESC4 != null ? cell_TSI_DESC4.getStringCellValue() : null;


            if (cell_Ins_Tcpt_ID4.getCellType() == cell_Ins_Tcpt_ID4.CELL_TYPE_STRING) {
                if (!cell_Ins_Tcpt_ID4.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranSumInsured3(cell_Ins_Tcpt_ID4.getStringCellValue(), new BigDecimal(cell_TSI4.getNumericCellValue()), new BigDecimal(cell_TSI4CoMem.getNumericCellValue()), tsiDesc4);
                }
            } else if (cell_Ins_Tcpt_ID4.getCellType() == cell_Ins_Tcpt_ID4.CELL_TYPE_NUMERIC) {
                doAddLampiranSumInsured3(new BigDecimal(cell_Ins_Tcpt_ID4.getNumericCellValue()).toString(), new BigDecimal(cell_TSI4.getNumericCellValue()), new BigDecimal(cell_TSI4CoMem.getNumericCellValue()), tsiDesc4);
            }

            //tsi 5
            HSSFCell cell_Ins_Tcpt_ID5 = row.getCell(85);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID5 = row.getCell(86);//ins_tsi_cat_id
            HSSFCell cell_TSI_DESC5 = row.getCell(37);//tsi desc
            HSSFCell cell_TSI5 = row.getCell(38);//tsi
            HSSFCell cell_TSI5CoMem = row.getCell(39);//tsi100

            String tsiDesc5 = cell_TSI_DESC5 != null ? cell_TSI_DESC5.getStringCellValue() : null;


            if (cell_Ins_Tcpt_ID5.getCellType() == cell_Ins_Tcpt_ID5.CELL_TYPE_STRING) {
                if (!cell_Ins_Tcpt_ID5.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranSumInsured3(cell_Ins_Tcpt_ID5.getStringCellValue(), new BigDecimal(cell_TSI5.getNumericCellValue()), new BigDecimal(cell_TSI5CoMem.getNumericCellValue()), tsiDesc5);
                }
            } else if (cell_Ins_Tcpt_ID5.getCellType() == cell_Ins_Tcpt_ID5.CELL_TYPE_NUMERIC) {
                doAddLampiranSumInsured3(new BigDecimal(cell_Ins_Tcpt_ID5.getNumericCellValue()).toString(), new BigDecimal(cell_TSI5.getNumericCellValue()), new BigDecimal(cell_TSI5CoMem.getNumericCellValue()), tsiDesc5);
            }
            ...
            //cover 1
            HSSFCell cell_Ins_Cvpt_ID1 = row.getCell(87);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID1 = row.getCell(88);//RATE
            HSSFCell cell_Rate1 = row.getCell(89);//premi
            HSSFCell cell_Premi1 = row.getCell(90);//tsi
            //HSSFCell cell_TSI1  = row.getCell(23);//ins_cvpt_id
            ...
            //cover 2
            HSSFCell cell_Ins_Cvpt_ID2 = row.getCell(91);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID2 = row.getCell(92);//RATE
            HSSFCell cell_Rate2 = row.getCell(93);//premi
            HSSFCell cell_Premi2 = row.getCell(94);//tsi
            //HSSFCell cell_TSI2  = row.getCell(28);//ins_cvpt_id
            ...
            //cover 3
            HSSFCell cell_Ins_Cvpt_ID3 = row.getCell(95);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID3 = row.getCell(96);//RATE
            HSSFCell cell_Rate3 = row.getCell(97);//premi
            HSSFCell cell_Premi3 = row.getCell(98);//tsi
            ...
            //cover 4
            HSSFCell cell_Ins_Cvpt_ID4 = row.getCell(99);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID4 = row.getCell(100);//RATE
            HSSFCell cell_Rate4 = row.getCell(101);//premi
            HSSFCell cell_Premi4 = row.getCell(102);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id
            ...
            //cover 5
            HSSFCell cell_Ins_Cvpt_ID5 = row.getCell(105);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID5 = row.getCell(106);//RATE
            HSSFCell cell_Rate5 = row.getCell(107);//premi
            HSSFCell cell_Premi5 = row.getCell(108);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id
            ...
            BigDecimal premi5 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi5.getNumericCellValue())) ? null : new BigDecimal(cell_Premi5.getNumericCellValue());

            if (premi5 != null) {
                premi5 = premi5.setScale(2, BigDecimal.ROUND_HALF_UP);
            }

            if (rate5 != null) {
                rate5 = rate5.setScale(6, BigDecimal.ROUND_HALF_UP);
            }
            ...
            //cover 6
            HSSFCell cell_Ins_Cvpt_ID6 = row.getCell(109);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID6 = row.getCell(110);//RATE
            HSSFCell cell_Rate6 = row.getCell(111);//premi
            HSSFCell cell_Premi6 = row.getCell(112);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id

            BigDecimal rate6 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate6.getNumericCellValue())) ? null : new BigDecimal(cell_Rate6.getNumericCellValue());
            BigDecimal premi6 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi6.getNumericCellValue())) ? null : new BigDecimal(cell_Premi6.getNumericCellValue());

            if (premi6 != null) {
                premi6 = premi6.setScale(2, BigDecimal.ROUND_HALF_UP);
            }

            if (rate6 != null) {
                rate6 = rate6.setScale(6, BigDecimal.ROUND_HALF_UP);
            }

            if (cell_Ins_Cvpt_ID6.getCellType() == cell_Ins_Cvpt_ID6.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID6.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID6.getStringCellValue(), rate6, premi6, null);
                }
            } else if (cell_Ins_Cvpt_ID6.getCellType() == cell_Ins_Cvpt_ID6.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID6.getNumericCellValue()).toString(), rate6, premi6, null);
            }

            //cover 7
            HSSFCell cell_Ins_Cvpt_ID7 = row.getCell(113);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID7 = row.getCell(114);//RATE
            HSSFCell cell_Rate7 = row.getCell(115);//premi
            HSSFCell cell_Premi7 = row.getCell(116);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id

            BigDecimal rate7 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate7.getNumericCellValue())) ? null : new BigDecimal(cell_Rate7.getNumericCellValue());
            BigDecimal premi7 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi7.getNumericCellValue())) ? null : new BigDecimal(cell_Premi7.getNumericCellValue());

            if (premi7 != null) {
                premi7 = premi7.setScale(2, BigDecimal.ROUND_HALF_UP);
            }

            if (rate7 != null) {
                rate7 = rate7.setScale(7, BigDecimal.ROUND_HALF_UP);
            }

            if (cell_Ins_Cvpt_ID7.getCellType() == cell_Ins_Cvpt_ID7.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID7.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID7.getStringCellValue(), rate7, premi7, null);
                }
            } else if (cell_Ins_Cvpt_ID7.getCellType() == cell_Ins_Cvpt_ID7.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID7.getNumericCellValue()).toString(), rate7, premi7, null);
            }

            //cover 8
            HSSFCell cell_Ins_Cvpt_ID8 = row.getCell(117);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID8 = row.getCell(118);//RATE
            HSSFCell cell_Rate8 = row.getCell(119);//premi
            HSSFCell cell_Premi8 = row.getCell(120);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id

            BigDecimal rate8 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate8.getNumericCellValue())) ? null : new BigDecimal(cell_Rate8.getNumericCellValue());
            BigDecimal premi8 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi8.getNumericCellValue())) ? null : new BigDecimal(cell_Premi8.getNumericCellValue());

            if (premi8 != null) {
                premi8 = premi8.setScale(2, BigDecimal.ROUND_HALF_UP);
            }

            if (rate8 != null) {
                rate8 = rate8.setScale(8, BigDecimal.ROUND_HALF_UP);
            }

            if (cell_Ins_Cvpt_ID8.getCellType() == cell_Ins_Cvpt_ID8.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID8.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID8.getStringCellValue(), rate8, premi8, null);
                }
            } else if (cell_Ins_Cvpt_ID8.getCellType() == cell_Ins_Cvpt_ID8.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID8.getNumericCellValue()).toString(), rate8, premi8, null);
            }

            //cover 9
            HSSFCell cell_Ins_Cvpt_ID9 = row.getCell(121);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID9 = row.getCell(122);//RATE
            HSSFCell cell_Rate9 = row.getCell(123);//premi
            HSSFCell cell_Premi9 = row.getCell(124);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id

            BigDecimal rate9 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate9.getNumericCellValue())) ? null : new BigDecimal(cell_Rate9.getNumericCellValue());
            BigDecimal premi9 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi9.getNumericCellValue())) ? null : new BigDecimal(cell_Premi9.getNumericCellValue());

            if (premi9 != null) {
                premi9 = premi9.setScale(2, BigDecimal.ROUND_HALF_UP);
            }

            if (rate9 != null) {
                rate9 = rate9.setScale(9, BigDecimal.ROUND_HALF_UP);
            }

            if (cell_Ins_Cvpt_ID9.getCellType() == cell_Ins_Cvpt_ID9.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID9.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID9.getStringCellValue(), rate9, premi9, null);
                }
            } else if (cell_Ins_Cvpt_ID9.getCellType() == cell_Ins_Cvpt_ID9.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID9.getNumericCellValue()).toString(), rate9, premi9, null);
            }

            //cover 10
            HSSFCell cell_Ins_Cvpt_ID10 = row.getCell(125);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID10 = row.getCell(126);//RATE
            HSSFCell cell_Rate10 = row.getCell(127);//premi
            HSSFCell cell_Premi10 = row.getCell(128);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id

            BigDecimal rate10 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate10.getNumericCellValue())) ? null : new BigDecimal(cell_Rate10.getNumericCellValue());
            BigDecimal premi10 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi10.getNumericCellValue())) ? null : new BigDecimal(cell_Premi10.getNumericCellValue());

            if (premi10 != null) {
                premi10 = premi10.setScale(2, BigDecimal.ROUND_HALF_UP);
            }

            if (rate10 != null) {
                rate10 = rate10.setScale(10, BigDecimal.ROUND_HALF_UP);
            }

            if (cell_Ins_Cvpt_ID10.getCellType() == cell_Ins_Cvpt_ID10.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID10.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID10.getStringCellValue(), rate10, premi10, null);
                }
            } else if (cell_Ins_Cvpt_ID10.getCellType() == cell_Ins_Cvpt_ID10.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID10.getNumericCellValue()).toString(), rate10, premi10, null);
            }
    ...
    public void approveEndorseKreasiByExcel()throws Exception{
     ...
     private void uploadEarthquake(FileInputStream fis, String treatyID) throws Exception {
        ...
        for (int r = 4; r < rows + 1; r++) {
            ...
            HSSFCell cellKodePos = row.getCell(10); //Alamat Risiko
            ...
            String kodePos = policy.getStKodePos(cellKodePos.getCellType() == cellKodePos.CELL_TYPE_STRING ? cellKodePos.getStringCellValue() : new BigDecimal(cellKodePos.getNumericCellValue()).toString());
            if (kodePos != null) {
                getSelectedDefaultObject().setStReference9(kodePos);
            }

            String kodePosTest = policy.getKodePos(cellKodePos.getCellType() == cellKodePos.CELL_TYPE_STRING ? cellKodePos.getStringCellValue() : new BigDecimal(cellKodePos.getNumericCellValue()).toString());
            if (kodePosTest != null) {
                getSelectedDefaultObject().setStReference9(kodePosTest);
            }
    ...
    private void uploadMV(FileInputStream fis, String treatyID) throws Exception {
        ...
        for (int r = 4; r < rows + 1; r++) {
            ...
            HSSFCell cell1 = row.getCell(1);//no polisi
            HSSFCell cell2 = row.getCell(87);//merk/jenis
            HSSFCell cell3 = row.getCell(3);//Tahun pembuatan
            HSSFCell cell4 = row.getCell(4);//No rangka
            HSSFCell cell5 = row.getCell(5);//No Mesin
            HSSFCell cell6 = row.getCell(6);//Tempat duduk
            HSSFCell cell7 = row.getCell(88);//Digunakan
            HSSFCell cell8 = row.getCell(8);//Tipe
            HSSFCell cell9 = row.getCell(9);//nama
            HSSFCell cell10 = row.getCell(89);//Kode Resiko
            HSSFCell cellPeriodStart = row.getCell(48);
            HSSFCell cellPeriodEnd = row.getCell(49);

            HSSFCell cellPeriodRate = row.getCell(45);//period rate period factor
            HSSFCell cellPeriodBaseID = row.getCell(112);//period base of period
            HSSFCell cellPremiumFactor = row.getCell(113);//premium factor rate factor
            ...
            //tsi kendaraan
            HSSFCell cell11 = row.getCell(90);//ins_tcpt_id
            HSSFCell cell12 = row.getCell(91);//ins_tsi_cat_id
            HSSFCell cell13 = row.getCell(92);//tsi
            //tsi peralatan
            HSSFCell cell14 = row.getCell(93);///ins_tcpt_id
            HSSFCell cell15 = row.getCell(94);//ins_tsi_cat_id
            HSSFCell cell16 = row.getCell(95);//tsi
...
//            HSSFCell cell31 = row.getCell(31);//insured_amount
            ...
            //cover 1
            HSSFCell cell_Ins_Cover_ID1 = row.getCell(96);//ins_cover_id
            HSSFCell cell_Rate1 = row.getCell(97);//rate
            HSSFCell cell_Premi1 = row.getCell(98);//premi
            HSSFCell cell_Ins_Cvpt_ID1 = row.getCell(99);//ins_cvpt_id
            ...
            //cover 2
            HSSFCell cell_Ins_Cover_ID2 = row.getCell(100);//ins_cover_id
            HSSFCell cell_Rate2 = row.getCell(101);//rate
            HSSFCell cell_Premi2 = row.getCell(102);//premi
            HSSFCell cell_Ins_Cvpt_ID2 = row.getCell(103);//ins_cvpt_id
            ...
            //cover 3
            HSSFCell cell_Ins_Cover_ID3 = row.getCell(104);//ins_cover_id
            HSSFCell cell_Rate3 = row.getCell(105);//rate
            HSSFCell cell_Premi3 = row.getCell(106);//premi
            HSSFCell cell_Ins_Cvpt_ID3 = row.getCell(107);//ins_cvpt_id
            ...
            //cover 4
            HSSFCell cell_Ins_Cover_ID4 = row.getCell(108);//ins_cover_id
            HSSFCell cell_Rate4 = row.getCell(109);//rate
            HSSFCell cell_Premi4 = row.getCell(110);//premi
            HSSFCell cell_Ins_Cvpt_ID4 = row.getCell(111);//ins_cvpt_id
            ...
            //cover 5
            HSSFCell cell_Ins_Cover_ID5 = row.getCell(128);//ins_cover_id
            HSSFCell cell_Rate5 = row.getCell(129);//rate
            HSSFCell cell_Premi5 = row.getCell(130);//premi
            HSSFCell cell_Ins_Cvpt_ID5 = row.getCell(131);//ins_cvpt_id
            HSSFCell cell_TSI5 = row.getCell(30);//tsi

            BigDecimal rate5 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate5.getNumericCellValue())) ? null : new BigDecimal(cell_Rate5.getNumericCellValue());
            BigDecimal premi5 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi5.getNumericCellValue())) ? null : new BigDecimal(cell_Premi5.getNumericCellValue());
            BigDecimal tsi5 = BDUtil.isZeroOrNull(new BigDecimal(cell_TSI5.getNumericCellValue())) ? null : new BigDecimal(cell_TSI5.getNumericCellValue());
            ...
            //cover 6
            HSSFCell cell_Ins_Cover_ID6 = row.getCell(132);//ins_cover_id
            HSSFCell cell_Rate6 = row.getCell(133);//rate
            HSSFCell cell_Premi6 = row.getCell(134);//premi
            HSSFCell cell_Ins_Cvpt_ID6 = row.getCell(135);//ins_cvpt_id
            HSSFCell cell_TSI6 = row.getCell(34);//tsi

            BigDecimal rate6 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate6.getNumericCellValue())) ? null : new BigDecimal(cell_Rate6.getNumericCellValue());
            BigDecimal premi6 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi6.getNumericCellValue())) ? null : new BigDecimal(cell_Premi6.getNumericCellValue());
            BigDecimal tsi6 = BDUtil.isZeroOrNull(new BigDecimal(cell_TSI6.getNumericCellValue())) ? null : new BigDecimal(cell_TSI6.getNumericCellValue());
            ...
            if (cell_Ins_Cvpt_ID6.getCellType() == cell_Ins_Cvpt_ID6.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID6.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID6.getStringCellValue(), rate6, premi6, tsi6);
                }
            } else if (cell_Ins_Cvpt_ID6.getCellType() == cell_Ins_Cvpt_ID6.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID6.getNumericCellValue()).toString(), rate6, premi6, tsi6);
            }
            //END NEW

            //cover 7
            HSSFCell cell_Ins_Cover_ID7 = row.getCell(136);//ins_cover_id
            HSSFCell cell_Rate7 = row.getCell(137);//rate
            HSSFCell cell_Premi7 = row.getCell(138);//premi
            HSSFCell cell_Ins_Cvpt_ID7 = row.getCell(139);//ins_cvpt_id
            HSSFCell cell_TSI7 = row.getCell(38);//tsi

            BigDecimal rate7 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate7.getNumericCellValue())) ? null : new BigDecimal(cell_Rate7.getNumericCellValue());
            BigDecimal premi7 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi7.getNumericCellValue())) ? null : new BigDecimal(cell_Premi7.getNumericCellValue());
            BigDecimal tsi7 = BDUtil.isZeroOrNull(new BigDecimal(cell_TSI7.getNumericCellValue())) ? null : new BigDecimal(cell_TSI7.getNumericCellValue());
            ...
            if (cell_Ins_Cvpt_ID7.getCellType() == cell_Ins_Cvpt_ID7.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID7.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID7.getStringCellValue(), rate7, premi7, tsi7);
                }
            } else if (cell_Ins_Cvpt_ID7.getCellType() == cell_Ins_Cvpt_ID7.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID7.getNumericCellValue()).toString(), rate7, premi7, tsi7);
            }
            //END NEW

            //cover 8
            HSSFCell cell_Ins_Cover_ID8 = row.getCell(140);//ins_cover_id
            HSSFCell cell_Rate8 = row.getCell(141);//rate
            HSSFCell cell_Premi8 = row.getCell(142);//premi
            HSSFCell cell_Ins_Cvpt_ID8 = row.getCell(143);//ins_cvpt_id
            HSSFCell cell_TSI8 = row.getCell(42);//tsi

            BigDecimal rate8 = BDUtil.isZeroOrNull(new BigDecimal(cell_Rate8.getNumericCellValue())) ? null : new BigDecimal(cell_Rate8.getNumericCellValue());
            BigDecimal premi8 = BDUtil.isZeroOrNull(new BigDecimal(cell_Premi8.getNumericCellValue())) ? null : new BigDecimal(cell_Premi8.getNumericCellValue());
            BigDecimal tsi8 = BDUtil.isZeroOrNull(new BigDecimal(cell_TSI8.getNumericCellValue())) ? null : new BigDecimal(cell_TSI8.getNumericCellValue());
            ...
            if (cell_Ins_Cvpt_ID8.getCellType() == cell_Ins_Cvpt_ID8.CELL_TYPE_STRING) {
                if (!cell_Ins_Cvpt_ID8.getStringCellValue().equalsIgnoreCase("NULL")) {
                    doAddLampiranCoverPA(cell_Ins_Cvpt_ID8.getStringCellValue(), rate8, premi8, tsi8);
                }
            } else if (cell_Ins_Cvpt_ID8.getCellType() == cell_Ins_Cvpt_ID8.CELL_TYPE_NUMERIC) {
                doAddLampiranCoverPA(new BigDecimal(cell_Ins_Cvpt_ID8.getNumericCellValue()).toString(), rate8, premi8, tsi8);
            }
            //END NEW

            HSSFCell cell_Ins_Clm_CauseID1 = row.getCell(114);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID1 = row.getCell(115);//deduct type
            HSSFCell cell_Ded_PCT1 = row.getCell(52);//deduct pct
            HSSFCell cell_Ded_Min1 = row.getCell(54);//deduct min
            HSSFCell cell_Ded_Max1 = row.getCell(55);//deduct max
            ...
            HSSFCell cell_Ins_Clm_CauseID2 = row.getCell(116);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID2 = row.getCell(117);//deduct type
            HSSFCell cell_Ded_PCT2 = row.getCell(57);//deduct pct
            HSSFCell cell_Ded_Min2 = row.getCell(59);//deduct min
            HSSFCell cell_Ded_Max2 = row.getCell(60);//deduct max
            ...
            HSSFCell cell_Ins_Clm_CauseID3 = row.getCell(118);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID3 = row.getCell(119);//deduct type
            HSSFCell cell_Ded_PCT3 = row.getCell(62);//deduct pct
            HSSFCell cell_Ded_Min3 = row.getCell(64);//deduct min
            HSSFCell cell_Ded_Max3 = row.getCell(65);//deduct max
            ...
            HSSFCell cell_Ins_Clm_CauseID4 = row.getCell(120);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID4 = row.getCell(121);//deduct type
            HSSFCell cell_Ded_PCT4 = row.getCell(67);//deduct pct
            HSSFCell cell_Ded_Min4 = row.getCell(69);//deduct min
            HSSFCell cell_Ded_Max4 = row.getCell(70);//deduct max
            ...
            HSSFCell cell_Ins_Clm_CauseID5 = row.getCell(122);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID5 = row.getCell(123);//deduct type
            HSSFCell cell_Ded_PCT5 = row.getCell(72);//deduct pct
            HSSFCell cell_Ded_Min5 = row.getCell(74);//deduct min
            HSSFCell cell_Ded_Max5 = row.getCell(75);//deduct max
            ...
            HSSFCell cell_Ins_Clm_CauseID6 = row.getCell(124);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID6 = row.getCell(125);//deduct type
            HSSFCell cell_Ded_PCT6 = row.getCell(77);//deduct pct
            HSSFCell cell_Ded_Min6 = row.getCell(79);//deduct min
            HSSFCell cell_Ded_Max6 = row.getCell(80);//deduct max
            ...
            HSSFCell cell_Ins_Clm_CauseID7 = row.getCell(126);//ins_clm_cause_id
            HSSFCell cell_Ins_Ded_typeID7 = row.getCell(127);//deduct type
            HSSFCell cell_Ded_PCT7 = row.getCell(82);//deduct pct
            HSSFCell cell_Ded_Min7 = row.getCell(84);//deduct min
            HSSFCell cell_Ded_Max7 = row.getCell(85);//deduct max
    ...
    private void uploadBurglary(FileInputStream fis, String treatyID)throws Exception{
        ...
        for (int r = 4; r < rows+1; r++){
            ...
            **/
            ...
            HSSFCell cell4  = row.getCell(36);//penerangan
            HSSFCell cell5  = row.getCell(6);//alamat
            HSSFCell cell6  = row.getCell(7);//kode pos
            HSSFCell cellProvinsi = row.getCell(13);//kode pos
            ...
            HSSFCell cellPeriodStart  = row.getCell(32);//periode awal
            HSSFCell cellPeriodEnd = row.getCell(33);//periode akhir

            HSSFCell cellRiskCat = row.getCell(37);//kode resiko 1
            ...
            HSSFCell cellPeriodRate = row.getCell(14);//period rate
            HSSFCell cellPeriodBaseID = row.getCell(56);//period base
            HSSFCell cellPremiumFactor = row.getCell(57);//premium factor
            ...
            //tsi 1
            HSSFCell cell_Ins_Tcpt_ID1  = row.getCell(38);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID1  = row.getCell(39);//ins_tsi_cat_id
            ...
            //tsi 2
            HSSFCell cell_Ins_Tcpt_ID2  = row.getCell(40);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID2  = row.getCell(41);//ins_tsi_cat_id
            HSSFCell cell_TSI2  = row.getCell(20);//tsi
            ...
            //tsi 3
            HSSFCell cell_Ins_Tcpt_ID3  = row.getCell(42);//ins_tcpt_id
            HSSFCell cell_Ins_Tsi_Cat_ID3  = row.getCell(43);//ins_tsi_cat_id
            HSSFCell cell_TSI3  = row.getCell(22);//tsi
            ...
            //cover 1
            HSSFCell cell_Ins_Cvpt_ID1  = row.getCell(44);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID1  = row.getCell(45);//RATE
            HSSFCell cell_Rate1  = row.getCell(46);//premi
            HSSFCell cell_Premi1  = row.getCell(47);//tsi
            //HSSFCell cell_TSI1  = row.getCell(23);//ins_cvpt_id
            ...
            //cover 2
            HSSFCell cell_Ins_Cvpt_ID2  = row.getCell(48);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID2  = row.getCell(49);//RATE
            HSSFCell cell_Rate2  = row.getCell(50);//premi
            HSSFCell cell_Premi2  = row.getCell(51);//tsi
            //HSSFCell cell_TSI2  = row.getCell(28);//ins_cvpt_id
            ...
            //cover 3
            HSSFCell cell_Ins_Cvpt_ID3  = row.getCell(52);//ins_cover_id
            HSSFCell cell_Ins_Cover_ID3  = row.getCell(53);//RATE
            HSSFCell cell_Rate3 = row.getCell(54);//premi
            HSSFCell cell_Premi3  = row.getCell(55);//tsi
            //HSSFCell cell_TSI3  = row.getCell(33);//ins_cvpt_id
    ...
    public void applyCoverageToAllObjects() throws Exception{
        ...
        for (int  i= 0; i < object.size(); i++) {
            ...
            for (int  j= 0; j < covSize ; j++) {
                ...
                cv.setStDescription(cov.getStDescription());
    ...
    public void btnApproveENDORSE() throws Exception {
        ...
        if (policy.isStatusPolicy() || policy.isStatusRenewal() || policy.isStatusEndorseRI()) {
          ...
          */
            ...
            if(!BDUtil.isZeroOrNull(comissionCompanyLimit)){
                ...
                overLimit = Tools.compare(comissionRatio, comissionLimit)>0;
                if(overLimit) errorMessage = "Jumlah Komisi/Diskon Melebihi Kewenangan Anda ("+ BDUtil.getPctFromRate(comissionRatio) +" > "+ BDUtil.getPctFromRate(comissionLimit) +")";
            }
        ...
        policy.validateNotManual(true);
        //policy.setStPostedFlag("Y");

        //approvalMode = true;
        
        if(!isInputPaymentDateMode() && !isEditKeteranganMode()){
            policy.recalculate();
            ...
            if(policy.isStatusPolicy()||policy.isStatusClaim()||policy.isStatusEndorse()||policy.isStatusRenewal()||policy.isStatusEndorseRI()||policy.isStatusTemporaryPolicy()||policy.isStatusTemporaryEndorsemen()){
                if(!approvalMode){
                    policy.recalculateTreaty();
                    policy.validateExclusionRisk(false);
                    policy.validateTreatyNotManual(false);
                }
            }
            policy.validateData();
            policy.validate(false);
        ...
        if(!isInputPaymentDateMode() && !isCreateEndorseAndPolicyMode())
            getRemoteInsurance().saveAutoEndorse(policy,policy.getStNextStatus(),approvalMode);
    ...
    public void setTaxCode()throws Exception{
        ...
        //if(item.getEntity().getStTaxFile()==null)
            //throw new RuntimeException("Penerima outgo harus mempunyai Nomor NPWP");
    ...
    private void uploadKredit(POIFSFileSystem fis, String treatyID) throws Exception {
        ...
        for (int r = 4; r < rows + 1; r++) {
            ...
            HSSFCell cellStatus = row.getCell(33);//kategori
            HSSFCell cellKategori = row.getCell(32);//kategori debitur
            HSSFCell cellCoverage = row.getCell(34);//kategori debitur
            HSSFCell cellRekPinjaman = row.getCell(17);//rek pinjaman
            HSSFCell cellJenisKredit = row.getCell(18);//Jenis Kredit
            HSSFCell cellKodeUnik = row.getCell(19);//Kode Unik
            HSSFCell cellPembayaran = row.getCell(35);//Kode Unik
            ...
            //add coverkreasi
            HSSFCell cellCoverPA = row.getCell(25);
            if (cellCoverPA.getCellType() != cellCoverPA.CELL_TYPE_NUMERIC) {
                throw new RuntimeException("Kolom CvptID Harus Numeric");
            }
            ...
            HSSFCell cellRatePA = row.getCell(27);
            if (cellRatePA.getCellType() != cellRatePA.CELL_TYPE_NUMERIC) {
                throw new RuntimeException("Kolom Rate Harus Numeric");
            }
            ...
            HSSFCell cellPremiPA = row.getCell(28);
            if (cellPremiPA.getCellType() != cellPremiPA.CELL_TYPE_NUMERIC) {
                throw new RuntimeException("Kolom Rate Harus Numeric");
            }
            ...
            //add coverkreasi
            HSSFCell cell_Ins_Cvpt_ID2 = row.getCell(30);//ins_cover_id
            HSSFCell cell_Premi2 = row.getCell(31);//tsi
...
//                else if(tax_code.equalsIgnoreCase("PPH23")) item.setStTaxCode("10");
    ...
    public void superEditReverseReinsurance(String policyID) throws Exception {
     ...
     public void btnReverseReinsurance() throws Exception {
        ...
        if(isPostedReinsurance()){
            //throw new RuntimeException("Reasuransi bulan "+ DateUtil.getMonth2Digit(policy.getDtPolicyDate()) +" tahun "+ DateUtil.getYear(policy.getDtPolicyDate()) +" sudah closing");
        }
    ...
    public void btnApproveReinsuranceOnly() throws Exception {
        ...
        getRemoteInsurance().saveReinsuranceAfterJurnalBalik(policy);
    ...
    public void validateJenisKredit() throws Exception{
        if(!policy.getStPolicyTypeID().equalsIgnoreCase("21") && !policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88"))
            return;
        ...
        for (int i = 0; i < object.size(); i++) {
            ...
            if(policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")){
                if(tglAwal.isAfter(bulanIzinKredit)){
                    if(isPlafonAwal){
                        policy.setStKreasiTypeID(null);
    ...
    public void setCreditTreaty(InsurancePolicyObjDefaultView obj)throws Exception{
        ...
        String jenisKredit = policy.getStKreasiTypeID();
    ...
    public void btnReverseJurnalBalikOnly() throws Exception {
        ...
        if(!policy.isStatusClaim() && !policy.isStatusClaimEndorse() && !policy.isStatusClaimInward() && !policy.isStatusEndorseClaimInward())
            if(!withinCurrentMonth) canReverse = false;
...
//                     canReverse = false;
    ...
    public void selectClaimObjectUseOrder() throws Exception {
        ...
        doDeleteAllObjectExceptClaimObject(stSelectedObject);
    ...
    public void btnSaveReinsurance() throws Exception {
        ...
        if(policy.isStatusInward())
            ...
            getRemoteInsurance().saveReinsuranceOnly(policy,policy.getStNextStatus(),approvalMode);
    ...
    public void setujuiPusatAllObject() throws Exception{
        ...
        for (int i = 0; i < object.size(); i++) {
            ...
            else if(policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")) obj.setStReference10("Y");
    ...
    public void defaultPeriodObject(){
        ...
        InsurancePolicyObjDefaultView objx = (InsurancePolicyObjDefaultView) getSelectedObject();
...
//                getSelectedDefaultObject().setStReference15(cellNOKPAK.getStringCellValue());
    ...
    public void validateClaimCause()throws Exception{
        ...
        if(cause!=null){
            if(cause.isSubrogationDefault())
                policy.setStReference13("Y");
            else
                policy.setStReference13(null);
        }
    ...
    public void applyDateHeader() throws Exception{
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            if(i==0){
                        ...
                        if(policy.getStPolicyTypeID().equalsIgnoreCase("21") || policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")|| policy.getStPolicyTypeID().equalsIgnoreCase("64")
    ...
    public void applyPeriodToAll() throws Exception{
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            if(policy.getStPolicyTypeID().equalsIgnoreCase("21") || policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")|| policy.getStPolicyTypeID().equalsIgnoreCase("64")
                                || policy.getStPolicyTypeID().equalsIgnoreCase("4")|| policy.getStPolicyTypeID().equalsIgnoreCase("87")|| policy.getStPolicyTypeID().equalsIgnoreCase("88")){
                obj.setDtReference2(policy.getDtPeriodStart());
                obj.setDtReference3(policy.getDtPeriodEnd());
            }

             if(policy.getStPolicyTypeID().equalsIgnoreCase("3") || policy.getStPolicyTypeID().equalsIgnoreCase("1") || policy.getStPolicyTypeID().equalsIgnoreCase("81")|| policy.getStPolicyTypeID().equalsIgnoreCase("5")){
                ...
                if(objx.getStOrderNo().trim().equalsIgnoreCase(objectUploadDet.getStOrderNo().trim())
                   && objx.getStReference1().trim().toUpperCase().equalsIgnoreCase(objectUploadDet.getStNama().trim().toUpperCase())){
                    ...
                    if(objectUploadDet.getDtRestitutionDate()!=null){
                        ...
                        //set jika ada EM, nol aja jgn ada premi nya
                        if(policy.getStPolicyTypeID().equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88"))
                            if(objx.getDbReference6()!=null)
                                objx.setDbReference6(BigDecimal.ZERO);
                    }
...
//            }
    ...
    private void uploadKreditKonsumtif(FileInputStream fis, String treatyID) throws Exception{
        ...
        if (sheet2 != null){
            uploadKredit(fsCek, treatyID); //existing
    ...
    public void btnSaveEndorsePPN() throws Exception{

        policy.recalculate();
        policy.recalculateTreaty();

        getRemoteInsurance().saveEndorsePPN(policy,policy.getStNextStatus(), true);
...
//        headerProposal.setDetails(details);
        ...
        for (int r = 5; r < rows; r++) {
            ...
            for (int i = 0; i < to.length; i++) {
                logger.logDebug("@@@@@@@@@@@@@@@@@@@@@@ " + to[i]);
                if (!tglLahir.equalsIgnoreCase("NULL")) {
                    logger.logDebug("@@@@@@@@@@@@@@@@@@@@@@1 " + tglLahir);
//                    tglLahir = tglLahir.substring(0, 2) + "-" + tglLahir.substring(3, 5) + "-" + tglLahir.substring(6, 10);
                    String tglLahirFix = tglLahir.substring(8, 10) + "/" + tglLahir.substring(5, 7) + "/" + tglLahir.substring(0, 4);
                    logger.logDebug("@@@@@@@@@@@@@@@@@@@@@@ " + cellNoPol.getStringCellValue() + " " + cellTglLahir.getStringCellValue());
                    logger.logDebug("@@@@@@@@@@@@@@@@@@@@@@2 " + tglLahirFix);
                    Date dateStart = DateUtil.getDate(tglLahirFix);
...
//                entityList = getEntityViewbyEntName(cellNoPol.getStringCellValue(), null);
    ...
    public void sendStatusDocumentToInterkoneksi() throws Exception{
        getRemoteInsurance().updateStatusDocument(policy);
    }
    ...
    public void editCreateEndorseRIProses(String policyID, String treatyID) throws Exception {
        superEdit(policyID);
      ...
      /*if (!FinCodec.PolicyStatus.POLICY.equalsIgnoreCase(policy.getStStatus())) {
        ...
        policy.setStReinsuranceApprovedWho("12061991");
        policy.setDtReinsuranceApprovedDate(new Date());
    ...
    public void editCreateEndorseBentukKomisi(InsurancePolicyView polis, InsurancePolicyView endorse) throws Exception {
        ...
        policy.setStReinsuranceApprovedWho(null);
        policy.setStApprovedReinsName(null);
        policy.setStAdminNotes("KOMISI");
        policy.setStEndorseNotes("ENDORSEMEN KOMISI");

        policy.setStEffectiveFlag("Y");
        policy.setStPostedFlag("Y");
        policy.setStApprovedWho("00000002");
        policy.setDtApprovedDate(DateUtil.getDate("04/10/2021"));
        policy.setDtPolicyDate(DateUtil.getDate("04/10/2021"));
        policy.setDtEndorseDate(DateUtil.getDate("04/10/2021"));
     ...
     public void editCreateEndorseSelisihKomisi(InsurancePolicyView polis, InsurancePolicyView endorse) throws Exception {
        superEdit(polis.getStPolicyID());
        ...
        policy.setStReinsuranceApprovedWho(null);
        policy.setStApprovedReinsName(null);
        policy.setStAdminNotes("KOMISI4");
        policy.setStEndorseNotes("Endorsemen Pajak");

        policy.setStEffectiveFlag("Y");
        policy.setStPostedFlag("Y");
        policy.setStApprovedWho("00000002");
        policy.setDtApprovedDate(DateUtil.getDate("19/12/2023"));
        policy.setDtPolicyDate(DateUtil.getDate("19/12/2023"));
        policy.setDtEndorseDate(DateUtil.getDate("19/12/2023"));
        ...
        boolean taxSudah = false;
        
        for (int l = 0; l < details.size(); l++) {
            InsurancePolicyItemsView items = (InsurancePolicyItemsView) details.get(l);

            //if(items.isStampFee() || items.isPolicyCost())
                items.setDbAmount(BDUtil.zero);

             //set komisi & pajak
             if (items.isKomisi() && !items.isFeeBase()){
                items.setDbAmount(endorse.getDbNDComm1());
                items.setStFlagEntryByRate("N");

                //set pajak
                items.setDbTaxAmount(endorse.getDbNDTaxComm1());
                ...
                /*
                 if(!taxSudah){
                     ...
                     items.setDbTaxAmount(BDUtil.zero);
                 }*/
            ...
            //set fee base
             if (items.isKomisi() && items.isFeeBase()){
                //items.setDbAmount(endorse.getDbNDFeeBase1());
                //items.setStFlagEntryByRate("N");
            }

            //set ppn
             if (items.isPPNFeeBase()){
                //items.setDbAmount(endorse.getDbNDPPN());
                //items.setStFlagEntryByRate("N");
            }
    ...
    public void btnSaveProsesEndorseRI() throws Exception {
        if(FinCodec.PolicyStatus.RENEWAL.equalsIgnoreCase(policy.getStNextStatus()) || FinCodec.PolicyStatus.POLICY.equalsIgnoreCase(policy.getStNextStatus())){
            defineTreaty();
        }
...
public void editCreateEndorseSelisihPremi(InsurancePolicyView polis, InsurancePolicyView endorse) throws Exception {
        superEdit(polis.getStPolicyID());
        ...
        policy.setStReinsuranceApprovedWho(null);
        policy.setStApprovedReinsName(null);
        policy.setStAdminNotes("PREMI");
        policy.setStEndorseNotes("ENDORSEMEN SELISIH PREMI");

        policy.setStEffectiveFlag("Y");
        policy.setStPostedFlag("Y");
        policy.setStApprovedWho("00000002");
        policy.setDtApprovedDate(DateUtil.getDate("27/10/2021"));
        policy.setDtPolicyDate(DateUtil.getDate("27/10/2021"));
        policy.setDtEndorseDate(DateUtil.getDate("27/10/2021"));
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            for (int j = 0; j < coverage.size(); j++) {
                ...
                cov.setDbPremi(endorse.getDbPremiTotal());
                cov.setDbPremiNew(endorse.getDbPremiTotal());
                cov.setStEntryPremiFlag("Y");
...
public void generateNoPolis()throws Exception{
    policy.generatePolicyNo();
}

public void editCreateClaimInwardEndorse(String policyID) throws Exception {
        superEdit(policyID);
      ...
      /*if (!FinCodec.PolicyStatus.POLICY.equalsIgnoreCase(policy.getStStatus())) {
        ...
        if (!FinCodec.PolicyStatus.CLAIMINWARD.equalsIgnoreCase(policy.getStStatus())&& !FinCodec.PolicyStatus.ENDORSECLAIMINWARD.equalsIgnoreCase(policy.getStStatus())) {
            throw new RuntimeException("Endorse Claim Inward hanya bisa dibuat dari Claim Inward/Endorse Claim Inward");
        }
        ...
        policy.setStNextStatus(FinCodec.PolicyStatus.ENDORSECLAIMINWARD);

        initTabs();
        //
    }

public void cekRatioKomisi() throws Exception{

    BigDecimal comissionRatio = BDUtil.div(BDUtil.sub(policy.getDbPremiTotal(), policy.getDbPremiNetto()), policy.getDbPremiTotal(),4);

    comissionRatio = BDUtil.div(BDUtil.sub(policy.getDbPremiTotal(), policy.getPremiNetto()), policy.getDbPremiTotal(),4);

    logger.logWarning("############ ratio komisi = "+ comissionRatio);
}

public void editCreateUploadEndorsePolisFire(String policyID, UploadEndorseFireHeaderView header, DTOList object) throws Exception {
        superEdit(policyID);
        ...
        uploadEndorseFireDetailView objectUpload = (uploadEndorseFireDetailView) object.get(0);
        ...
        if(hitungDetilPremi){ 
            ...
            for (int j = 0; j < item.size(); j++) {
                ...
                if(items.isFeeBase()){
                    //jika amount > 0
                    if(objectUpload.getDbFeeBase()!=null && !BDUtil.isZero(objectUpload.getDbFeeBase())){
                        items.setStFlagEntryByRate("N");
                        items.setDbAmount(objectUpload.getDbFeeBase());
                    }

                    //jika pct > 0
                    if(objectUpload.getDbFeeBasePct()!=null && !BDUtil.isZero(objectUpload.getDbFeeBasePct())){
                        items.setStFlagEntryByRate("Y");
                        items.setDbRate(objectUpload.getDbFeeBasePct());
                    }
                }
                ...
                if(items.isKomisi2()){
                    //jika amount > 0
                    if(objectUpload.getDbKomisi()!=null && !BDUtil.isZero(objectUpload.getDbKomisi())){
                        items.setStFlagEntryByRate("N");
                        items.setDbAmount(objectUpload.getDbKomisi());
                    }

                    //jika pct > 0
                    if(objectUpload.getDbKomisiPct()!=null && !BDUtil.isZero(objectUpload.getDbKomisiPct())){
                        items.setStFlagEntryByRate("Y");
                        items.setDbRate(objectUpload.getDbKomisiPct());
                    }
                }

                //jika brokerfee
                if(items.isBrokerFee()){
                    //jika amount > 0
                    if(objectUpload.getDbBrokerFee()!=null && !BDUtil.isZero(objectUpload.getDbBrokerFee())){
                        items.setStFlagEntryByRate("N");
                        items.setDbAmount(objectUpload.getDbBrokerFee());
                    }

                    //jika pct > 0
                    if(objectUpload.getDbBrokerFeePct()!=null && !BDUtil.isZero(objectUpload.getDbBrokerFeePct())){
                        items.setStFlagEntryByRate("Y");
                        items.setDbRate(objectUpload.getDbBrokerFeePct());
                    }
                }

                if(items.isPolicyCost() && objectUpload.getDbBiayaPolis()!=null){
                    if(!BDUtil.isZero(objectUpload.getDbBiayaPolis())){
                        items.setStFlagEntryByRate("N");
                        items.setDbAmount(objectUpload.getDbBiayaPolis());
                    }
                ...
                if(items.isStampFee() && objectUpload.getDbMaterai()!=null){
                    if(!BDUtil.isZero(objectUpload.getDbMaterai())){
                        items.setStFlagEntryByRate("N");
                        items.setDbAmount(objectUpload.getDbMaterai());
                    }
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            for (int j = 0; j < object.size(); j++) {
                uploadEndorseFireDetailView objectUploadDet = (uploadEndorseFireDetailView) object.get(j);

                if(objx.getStOrderNo().trim().equalsIgnoreCase(objectUploadDet.getStOrderNo().trim())){

                    //set period factor objek endorse, ambil dari induk nya
                    objx.setDbPeriodRateBefore(objx.getDbPeriodRate());
                    objx.setStPeriodBaseBeforeID(objx.getStPeriodBaseID());
                    ...
                    if(objectUploadDet.getStAlamatRisiko()!=null){
                        //EEI
                        if(policy.getStPolicyTypeID().equalsIgnoreCase("10"))
                               objx.setStReference3(objectUploadDet.getStAlamatRisiko().trim());
                        //EQ
                        if(policy.getStPolicyTypeID().equalsIgnoreCase("19"))
                               objx.setStReference8(objectUploadDet.getStAlamatRisiko().trim());
                    }
            ...
            for (int k = 0; k < coverage.size(); k++) {
                ...
                for (int l = 0; l < object.size(); l++) {
                    uploadEndorseFireDetailView objectUploadDet = (uploadEndorseFireDetailView) object.get(l);

                    if(objx.getStOrderNo().trim().equalsIgnoreCase(objectUploadDet.getStOrderNo().trim())){

                        if(objectUploadDet.getDbPremi()!=null) {

                            if(!BDUtil.isZeroOrNull(objectUploadDet.getDbPremi())){
                                cov.setDbPremi(objectUploadDet.getDbPremi());
                                cov.setDbPremiNew(objectUploadDet.getDbPremi());
            ...
            for (int m = 0; m < suminsureds.size(); m++) {
                ...
                for (int n = 0; n < object.size(); n++) {
                    ...
                    uploadEndorseFireDetailView objectUploadDet = (uploadEndorseFireDetailView) object.get(n);

                    if(objx.getStOrderNo().trim().equalsIgnoreCase(objectUploadDet.getStOrderNo().trim())){

                        logger.logDebug("##################  objek urutan ke "+objectUploadDet.getStOrderNo() +" = "+ objectUploadDet.getDbTSI());
...
public void doDeleteAllObjectExceptClaimObject(String claimObject) throws Exception {

        //objek klaim yg dipilih
        final int n = Integer.parseInt(claimObject);
        
        logger.logDebug("################## klaim objek urutan ke "+n +" #########################");

        final DTOList objects = policy.getObjects();

        if(objects.size()>500){

            //tampung object yg dipilih
            final InsurancePolicyObjectView obj = (InsurancePolicyObjectView)objects.get(n);

            //hapus semua object
            doDeleteAllObjects();

            //tambahin object yg dipilih
            policy.getObjects().add(obj);
...
public void updateTreaty() throws Exception{
        ...
        //hapus dlu spreading asli
        InsurancePolicyObjDefaultView obj = (InsurancePolicyObjDefaultView) policy.getClaimObject();
        obj.getTreaties().deleteAll();
        obj.getTreatyDetails().deleteAll();
        obj.setStInsuranceTreatyID(selectedObject.getStInsuranceTreatyID());
        ritabs=null;

        //baca spreading endorse
        final DTOList treaty = obj.getTreatiesEndorseRI(policy.getStPolicyNo(), obj.getStOrderNo());
        InsurancePolicyTreatyView treatyEndorseRI = (InsurancePolicyTreatyView) treaty.get(0);

        final DTOList treatyDetail = treatyEndorseRI.getDetails();

        obj.setTreaties(treaty);

//        for (int i = 0; i < treatyDetail.size(); i++) {
//              Object object = treatyDetail.get(i);
//
//        }
//
//
...
//                }
    ...
    private boolean bypassValidationMode;

    public boolean isBypassValidationMode() {
        return bypassValidationMode;
    }

    public void setBypassValidationMode(boolean bypassValidationMode) {
        this.bypassValidationMode = bypassValidationMode;
    }

    public void btnSaveBypassValidation() throws Exception {
        ...
        getRemoteInsurance().saveBypassValidation(policy, approvalMode);

        close();
    }

    public void editCreateRenewalNonAKS(String policyID) throws Exception {
        ...
        if(policy.getStCostCenterCode().equalsIgnoreCase("80"))
             throw new RuntimeException("Menu perpanjangan ini khusus untuk renewal polis AKS ke Non AKS");
        ...
        policy.setStCostCenterCodeSource(policy.getStCostCenterCode());
        policy.setStRegionIDSource(policy.getStRegionID());
        policy.setStCostCenterCode("80");
        policy.setStRegionID("85");
...
//                obj.setStReference2(null);
    ...
    public DTOList getTitipan() throws Exception {
        return policy.getTitipan();
    }

    public void selectRealisasiTitipanPremi() throws Exception{

        doDeleteAllTitipan();

        final DTOList details = getRealisasiTitipan(policy.getStReceiptNoTitipan());

        BigDecimal totalAmount = BDUtil.zero;

        for (int i = 0; i < details.size(); i++) {
            TitipanPremiView titipanRealisasi = (TitipanPremiView) details.get(i);

            InsurancePolicyTitipanPremiView detil = new InsurancePolicyTitipanPremiView();

            detil.setStTransactionID(titipanRealisasi.getStTransactionID());
            detil.setStTransactionNo(titipanRealisasi.getStTransactionNo());
            detil.setStCounter(titipanRealisasi.getStCounter());
            detil.setStDescription(titipanRealisasi.getStDescription());
            detil.setDbRealisasiAmount(titipanRealisasi.getDbRealisasiUsed());

            detil.markNew();

            getTitipan().add(detil);

            totalAmount = BDUtil.add(totalAmount, detil.getDbRealisasiAmount());
        }
        ...
        BigDecimal totalAmount4x = BDUtil.mul(totalAmount, new BigDecimal(4), 2);
        ...
        for (int i = 0; i < covers.size(); i++) {
            ...
            cov.setDbPremi(BDUtil.negate(totalAmount));
            cov.setDbPremiNew(BDUtil.negate(totalAmount));
        ...
        for (int i = 0; i < suminsured.size(); i++) {
            ...
            if (tsi.getStInsurancePolicyTSIRefID()!=null){
                final InsurancePolicyTSIView refTSI = tsi.getRefTSI();

                final BigDecimal insAmountTo = BDUtil.sub(refTSI.getDbInsuredAmount(), totalAmount4x);

                tsi.setDbInsuredAmount(insAmountTo);
            }
    ...
    private DTOList realisasiTitipan;

    private DTOList loadDetailsTitipan(String trxNo, String receiptLineType) throws Exception {
        if (trxNo!=null){
            return ListUtil.getDTOListFromQuery(
                    "select c.trx_id,c.trx_no,c.counter, b.description, b.titipan_premi_used_amount as realisasi_used "+
                    " from ar_receipt a "+
                    " inner join ar_receipt_lines b on a.ar_receipt_id = b.receipt_id "+
                    " left join ar_titipan_premi_extracomptable c on b.titipan_premi_id = c.trx_id "+
                    " where line_type = ? and a.status = 'POST' and a.receipt_no = ? "+
                    " order by ar_rcl_id",
                    new Object [] {receiptLineType, trxNo},
    ...
    public DTOList getRealisasiTitipan(String trxNo) throws Exception {
        
        DTOList realisasiTitipan = loadDetailsTitipan(trxNo, FinCodec.ARReceiptLineType.TITIPAN);

        return realisasiTitipan;
    }

    public void setRealisasiTitipan(DTOList realisasiTitipan) {
        this.realisasiTitipan = realisasiTitipan;
    }

    public String getTitipanIndex() {
        return titipanIndex;
    }

    public void setTitipanIndex(String titipanIndex) {
        this.titipanIndex = titipanIndex;
    }

    public void onDeleteTitipanPolisKhusus() throws Exception{
        getTitipan().delete(Integer.parseInt(titipanIndex));

        String n = String.valueOf(getTitipan().size());

        policy.recalculateTitipanPremi();
    }

     private void defineTreatyNonAKS() throws Exception{
        final boolean inward = policy.getCoverSource().isInward();
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            //jika kredit konsumtif
            if(polTypeID.equalsIgnoreCase("59") || policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")){
                setCreditTreaty(obj);
            }
     ...
     public void doDeleteAllTitipan() throws Exception{
        getTitipan().deleteAll();

        if(realisasiTitipan!=null)
            realisasiTitipan.deleteAll();
    }

     public void validateTreatyAgain() throws Exception{

     }

     private void defineTreatyAgain() throws Exception{
     ...
     public void btnRecalculateH2H()throws Exception {

        policy.recalculateH2H();

    }

     public void changeClaimSubCause() throws Exception{
         policy.setPotensiSubrogasi();
     }

     public void btnCekStatusBayarCare() throws Exception{
         policy.getStatusBayarCaretech();
     }
     ...
     public boolean isPostedReinsurance() throws Exception {
            ...
            try {
                String cek = "select count(*) as jumlah_closing"+
                             ...
                             "   and reins_closing_status = 'Y' "+
                             "   and date_trunc('month',policy_date_end) = ?";

                S.setQuery(cek);

                S.setParam(1, DateUtil.getFirstDate(policy.getDtPolicyDate()));

                final ResultSet RS = S.executeQuery();

                if (RS.next()) {
                    int jumlahClosing = RS.getInt("jumlah_closing");
     ...
     public void applyRIInstallmentToALLMember() throws Exception{
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            for (int j = 0; j < treatyDetails.size(); j++) {
                ...
                DTOList inst1 = new DTOList();

                for (int k = 0; k < share.size(); k++) {
                    InsurancePolicyReinsView member = (InsurancePolicyReinsView) share.get(k);

                    if(k==0) inst1 = member.getInstallment();

                    if(k>0){

                        DTOList instDNew = new DTOList();

                        for (int l = 0; l < inst1.size(); l++) {
                            InsurancePolicyReinsInstallmentView instCopy = (InsurancePolicyReinsInstallmentView) inst1.get(l);

                            InsurancePolicyReinsInstallmentView instNew = new InsurancePolicyReinsInstallmentView();
                            instNew.markNew();

                            instNew.setStManualFlag(instCopy.getStManualFlag());
                            instNew.setDtDueDate(instCopy.getDtDueDate());

                            instDNew.add(instNew);

                        }

                        member.setInstallment(instDNew);
                    }
     ...
     public void btnSavePolicy() throws Exception {
        if(FinCodec.PolicyStatus.RENEWAL.equalsIgnoreCase(policy.getStNextStatus()) || FinCodec.PolicyStatus.POLICY.equalsIgnoreCase(policy.getStNextStatus())){
            defineTreaty();
        }
     ...
     public void btnApproveFlagReinsurance() throws Exception {
        policy.setExcludeReasMode(isExcludeReasMode());

        policy.setStReinsuranceApprovedWho(UserManager.getInstance().getUser().getStUserID());
        policy.setStRIFinishFlag("Y");
        policy.setStRIPostedFlag("Y");

    }

     public void editCreateEndorseTambahFeebaseExclude(InsurancePolicyView polis, InsurancePolicyView endorse) throws Exception {
        superEdit(polis.getStPolicyID());
        ...
        policy.setStReinsuranceApprovedWho(null);
        policy.setStApprovedReinsName(null);
        policy.setStAdminNotes("FEEBASE12");
        policy.setStEndorseNotes("Endorsemen Feebase");

        policy.setStEffectiveFlag("Y");
        policy.setStPostedFlag("Y");
        policy.setStApprovedWho("00000002");
        policy.setDtApprovedDate(DateUtil.getDate("13/12/2024"));
        policy.setDtPolicyDate(DateUtil.getDate("13/12/2024"));
        policy.setDtEndorseDate(DateUtil.getDate("13/12/2024"));

        policy.checkEndorseNoBefore(policy.getStPolicyNo());

        if(true){
            //add feebase
            onNewDetailAutomaticProses("92","N", policy.getStEntityID(),null, null, endorse.getDbNDComm1(), new BigDecimal(11));
     ...
     public void onNewDetailAutomaticProses(String insItemID, String stReadOnly, String entityID, BigDecimal pct, String taxCode, BigDecimal amount, BigDecimal ppn) throws Exception {
        ...
        logger.logDebug("################## amount feebase = "+ amount);
        ...
        if(itemCat.getStInsuranceItemChildID()!=null)
            onNewDetailByChildIDProses(itemCat.getStInsuranceItemChildID(), ppn);

    }

     public void onNewDetailByChildIDProses(String itemID, BigDecimal ppnRate) throws Exception {
     ...
     private void uploadKreditSerbaguna(POIFSFileSystem fis, String treatyID) throws Exception {
        ...
        for (int r = 4; r < rows + 1; r++) {
            ...
            //KREDIT SERBAGUNA FIELD

             //tgl awal 100%
            if (cell3.getCellType() == cell3.CELL_TYPE_STRING) {
                getSelectedDefaultObject().setDtReference7(DateUtil.getDate(cell3.getStringCellValue())); //awal
            } else {
                getSelectedDefaultObject().setDtReference7(cell3.getDateCellValue());
            }

            //tgl akhir 100%
            if (cell4.getCellType() == cell4.CELL_TYPE_STRING) {
                getSelectedDefaultObject().setDtReference8(DateUtil.getDate(cell4.getStringCellValue())); //awal
            } else {
                getSelectedDefaultObject().setDtReference8(cell4.getDateCellValue());
            }

            HSSFCell cellPremi100 = row.getCell(28);

            if (cellPremi100.getCellType() != cellPremi100.CELL_TYPE_NUMERIC) {
                throw new RuntimeException("Kolom Premi Harus Numeric");
            }

            BigDecimal premi100 = null;
            if (cellPremi100 != null) {
                double premiPA = cellPremi100.getNumericCellValue();
                premi100 = new BigDecimal(premiPA);
            }

            //PREMI 100%
            getSelectedDefaultObject().setDbReference11(premi100);
     ...
     public void editCreateEndorseTambahBrokerfeeExclude(InsurancePolicyView polis, InsurancePolicyView endorse) throws Exception {
        superEdit(polis.getStPolicyID());
        ...
        policy.setStReinsuranceApprovedWho(null);
        policy.setStApprovedReinsName(null);
        policy.setStAdminNotes("BRKER12");
        policy.setStEndorseNotes("Endorsemen Brokerfee");

        policy.setStEffectiveFlag("Y");
        policy.setStPostedFlag("Y");
        policy.setStApprovedWho("00000002");
        //policy.setDtApprovedDate(DateUtil.getDate("18/12/2024"));
        //policy.setDtPolicyDate(DateUtil.getDate("18/12/2024"));
        //policy.setDtEndorseDate(DateUtil.getDate("18/12/2024"));

        Date tglPolis = new Date();

        policy.setDtApprovedDate(tglPolis);
        policy.setDtPolicyDate(tglPolis);
        policy.setDtEndorseDate(tglPolis);
        ...
        if(true){
            //add brokerfee pakar
            onNewDetailAutomaticProses("12","N","1044424",null, "5", endorse.getDbNDBrok1(), new BigDecimal(2.2));
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            if(policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")){
                objx.setDbReference11(BDUtil.zero);
            }
     ...
     private void uploadKreditSerbagunaHdr(FileInputStream fis, String treatyID) throws Exception{
        POIFSFileSystem fsCek = new POIFSFileSystem(fis);
        HSSFWorkbook wbCek = new HSSFWorkbook(fsCek);
     ...
     private void uploadKreditMacetOnlyHdr(FileInputStream fis, String treatyID) throws Exception{
        POIFSFileSystem fsCek = new POIFSFileSystem(fis);
        HSSFWorkbook wbCek = new HSSFWorkbook(fsCek);
     ...
     public void editCreateRenewalPOJK20(String policyID, int ctrPolis) throws Exception {
        ...
        policy.setStPolicyNo(policy.getStPolicyNo().substring(0, 16)+ ctrPolis + "00");
        ...
        policy.setStEffectiveFlag("Y");
        policy.setStRIFinishFlag("Y");
        policy.setStAdminNotes("RENEWAL");
        policy.setStApprovedWho("00000002");
        policy.setDtApprovedDate(new Date());
        ...
        for (int j = 0; j < object.size(); j++) {
            ...
            //set tgl awal polis
            int counter = policy.getCounterPolisCurrent();

            //jika renewal 1
            if(counter==2){


                policy.setDtPeriodStart(obj.getDtReference11());
                policy.setDtPeriodEnd(obj.getDtReference12());
                obj.setDtReference2(obj.getDtReference11());
                obj.setDtReference3(obj.getDtReference12());

                final DTOList cover = obj.getCoverage();

                for (int i = 0; i < cover.size(); i++) {
                    InsurancePolicyCoverView cov = (InsurancePolicyCoverView) cover.get(i);

                    cov.setDbPremi(obj.getDbReference15());
                    cov.setDbPremiNew(obj.getDbReference15());
            ...
            //jika renewal 2
            if(counter==3){
                policy.setDtPeriodStart(obj.getDtReference13());
                policy.setDtPeriodEnd(obj.getDtReference14());
                obj.setDtReference2(obj.getDtReference13());
                obj.setDtReference3(obj.getDtReference14());
                ...
                for (int i = 0; i < cover.size(); i++) {
                    ...
                    cov.setDbPremi(obj.getDbReference17());
                    cov.setDbPremiNew(obj.getDbReference17());
            ...
            //jika renewal 3
            if(counter==4){
                policy.setDtPeriodStart(obj.getDtReference15());
                policy.setDtPeriodEnd(obj.getDtReference16());
                obj.setDtReference2(obj.getDtReference15());
                obj.setDtReference3(obj.getDtReference16());
                ...
                for (int i = 0; i < cover.size(); i++) {
                    ...
                    cov.setDbPremi(obj.getDbReference19());
                    cov.setDbPremiNew(obj.getDbReference19());
            ...
            //jika renewal 4
            if(counter==5){
                policy.setDtPeriodStart(obj.getDtReference17());
                policy.setDtPeriodEnd(obj.getDtReference18());
                obj.setDtReference2(obj.getDtReference17());
                obj.setDtReference3(obj.getDtReference18());
                ...
                for (int i = 0; i < cover.size(); i++) {
                    ...
                    cov.setDbPremi(obj.getDbReference21());
                    cov.setDbPremiNew(obj.getDbReference21());
            ...
            //jika renewal 5
            if(counter==6){
                policy.setDtPeriodStart(obj.getDtReference19());
                policy.setDtPeriodEnd(obj.getDtReference20());
                obj.setDtReference2(obj.getDtReference19());
                obj.setDtReference3(obj.getDtReference20());
                ...
                for (int i = 0; i < cover.size(); i++) {
                    ...
                    cov.setDbPremi(obj.getDbReference23());
                    cov.setDbPremiNew(obj.getDbReference23());
     ...
     public String btnSaveRenewalPOJK() throws Exception{

        policy.recalculate();
        policy.recalculateTreaty();
        policy.setStRIFinishFlag("Y");

        String policyID = getRemoteInsurance().saveEndorsePPN(policy,policy.getStNextStatus(), true);

        InsurancePolicyView pol = getRemoteInsurance().getInsurancePolicy(policyID);

         //Simpan nota ke sistem care via API
        //getRemoteInsurance().saveNotaToCare(pol, true);
     ...
     public void createRenewalSerbaguna(InsurancePolicyView klaim, boolean approvalMode) throws Exception{

         if(klaim.getStPolicyTypeID().equalsIgnoreCase("87") || klaim.getStPolicyTypeID().equalsIgnoreCase("88")){

             if (Tools.isYes(klaim.getStPostedFlag())){

                 if(klaim.isStatusClaimDLA()){

                     if(approvalMode){

                        //dapetin polis nya
                        InsurancePolicyView pol = getInsurancePolicyWithPolNo(klaim.getStPolicyNo(), klaim.getStPolicyTypeID()); //getRemoteInsurance().getInsurancePolicy(policyID);
                        ...
                        if(pol!=null){
                            ...
                            if(!pol.isPremiPaidInSystem())
                                throw new RuntimeException("Polis belum dibayar premi, tidak bisa renewal (harus input pembayaran premi terlebih dulu)");

                            if(!pol.getStPolicyTypeID().equalsIgnoreCase("87") && !pol.getStPolicyTypeID().equalsIgnoreCase("88"))
                                throw new RuntimeException("Fitur ini hanya untuk perpanjangan polis Asuransi Kredit Macet (87) & Serbaguna (88)");
                            ...
                            int counterPolisCurrent = Integer.parseInt(pol.getStPolicyNo().substring(16, 17)); //1

                            int loop = jmlPolis.intValue() - counterPolisCurrent;

                            List<String> policyList = new ArrayList<String>();

                            //looping buat renewal
                            for (int i = 0; i < loop; i++) {
                                //Object object = objects.get(i);

                                counterPolisCurrent++;

                                editCreateRenewalPOJK20(pol.getStPolicyID(), counterPolisCurrent);
                                String policyID = btnSaveRenewalPOJK();
                             ...
                             if(policyList.size()>0){
                                    ...
                                    form.createAutoRealisasiTitipanPremi(pol, policyList);
                            }
     ...
     public InsurancePolicyView getInsurancePolicyWithPolNo(String stPolicyNo, String polTypeID) throws Exception {

        final InsurancePolicyView pol = (InsurancePolicyView) ListUtil.getDTOListFromQuery("select a.*"+
                                        " from ins_policy a "+
                                        " where a.status in ('POLICY') and effective_flag = 'Y' and pol_type_id = ? "+
                                        " and substr(a.pol_no, 0, 18) = ? ",
                new Object[]{polTypeID, stPolicyNo.substring(0,17) },
                InsurancePolicyView.class).getDTO();

        return pol;
    }

     public void cekDeductible() throws Exception{
         InsurancePolicyObjectView obj = (InsurancePolicyObjectView) policy.getObjects().get(0);
                        ...
                        for (int i = 0; i < deductibles.size(); i++) {
                            ...
                            deductibleText = ded.getStDescriptionForPolicy();
                        }

                        throw new RuntimeException(deductibleText);
     }

     public void editCreateRenewalToCabang(String policyID) throws Exception {
        ...
        if(!policy.getStCostCenterCode().equalsIgnoreCase("80"))
             throw new RuntimeException("Menu perpanjangan ini khusus untuk renewal polis Non AKS ke Cabang");
        ...
        //policy.setStCostCenterCodeSource(policy.getStCostCenterCode());
        //policy.setStRegionIDSource(policy.getStRegionID());
        policy.setStCostCenterCode(policy.getStCostCenterCodeSource());
        policy.setStRegionID(policy.getStRegionIDSource());
...
//                obj.setStReference2(null);
     ...
     public void editReActivateClaim(String policyID) throws Exception {
        superEditHistory(policyID);
