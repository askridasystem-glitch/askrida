...
public class DocumentUploadForm extends Form {
    ...
    private PolicyDocumentUploadHeaderView titipan;
    ...
    public PolicyDocumentUploadHeaderView getTitipan() {
        return titipan;
    }

    public void setReceipt(PolicyDocumentUploadHeaderView titipan) {
    ...
    public void masterEdit(String titipanID) throws Exception {
        ...
        titipan = new PolicyDocumentUploadHeaderView();
...
//        titipan.setStAccountID(titip.getLgAccountID().toString());
    ...
    public void view(String titipanID) throws Exception {
        ...
        final DTOList details = getRemoteInsurance().getUploadPolicyDocDetail(titipanID);

        titipan = (PolicyDocumentUploadHeaderView) DTOPool.getInstance().getDTO(PolicyDocumentUploadHeaderView.class, titipanID);
    ...
    public void doSave() throws Exception {
        ...
        getRemoteInsurance().saveUploadDocumentPolis(titipan, titipan.getDetails());
    ...
    public void uploadExcel() throws Exception {
        ...
        HSSFSheet sheet = wb.getSheet("UPLOAD_DOKUMEN");
        ...
        for (int r = 4; r < rows; r++) {
            ...
            PolicyDocumentUploadDetailView data = new PolicyDocumentUploadDetailView();
            data.markNew();
    ...
    public void cekData() throws Exception{
        ...
        for (int i = 0; i < details.size(); i++) {
            PolicyDocumentUploadDetailView det = (PolicyDocumentUploadDetailView) details.get(i);

            //if(det.isBypassValidasi()) continue;
            det.setStDescription(null);

            //CARI POLIS ADA / TIDAK
                ...
                " and pol_no = ? "+
                " limit 1",
                new Object[]{det.getStPolicyNo().trim()},
                InsurancePolicyView.class);

                InsurancePolicyView pol = (InsurancePolicyView) listPolis.get(0);
                ...
                if(pol==null){
                    dataIsValid = false;
                    cek = false;
                    msg = msg + " [Polis Tidak Ditemukan]";
                    
                    det.setStBypassValidasiFlag("N");
                    det.setStDescription("Polis Tidak Ditemukan");

                }

                if(pol!=null){

                    det.setStBypassValidasiFlag("Y");
                    //det.setStDescription("OK");

                    if(pol.isEffective()){
                        dataIsValid = false;
                        cek = false;
                        msg = msg + " [Polis Sudah Disetujui]";

                        det.setStBypassValidasiFlag("N");
                        det.setStDescription("Polis Sudah Disetujui");
                    }
                }

                if(!cek){
                    pesan =  pesan+ "- "+ det.getStPolicyNo()+ " Data tidak valid : " + msg;
                }
