...
import com.webfin.gl.model.ARTitipanPremiView;
...
public class PolicyListForm extends Form {
    
    private final static transient LogManager logger = LogManager.getInstance(PolicyListForm.class);
    ...
    private boolean enableInputBypassValidasi;

    private String stBranchSource = SessionManager.getInstance().getSession().getStCostCenterCodeSource();

    private String stPolicyNoRenewal;

    private boolean enableRenewalNonAKS;

    public boolean isEnableRenewalNonAKS() {
        return enableRenewalNonAKS;
    }

    public void setEnableRenewalNonAKS(boolean enableRenewalNonAKS) {
        this.enableRenewalNonAKS = enableRenewalNonAKS;
    }

    public String getStPolicyNoRenewal() {
        return stPolicyNoRenewal;
    }

    public void setStPolicyNoRenewal(String stPolicyNoRenewal) {
        this.stPolicyNoRenewal = stPolicyNoRenewal;
    }
    ...
    public boolean isEnableInputBypassValidasi() {
        return enableInputBypassValidasi;
    }

    public void setEnableInputBypassValidasi(boolean enableInputBypassValidasi) {
        this.enableInputBypassValidasi = enableInputBypassValidasi;
    }
    ...
    public DTOList getList() throws Exception {
        ...
        */
        ...
        if(getStNoRekeningPinjaman()!=null){
         	Sql = Sql +
         		" inner join ins_pol_obj c on c.pol_id = a.pol_id"+
         		" where upper(c.ref16) = ?";
            sqa.addPar(getStNoRekeningPinjaman());
         }
        ...
        select = select + " *,"+
                " (select user_name"+
                " from s_users"+
                " where user_id = a.create_who) as create_name,"+
                " (select user_name"+
                " from s_users"+
                " where user_id = a.approved_who) as approved_name,"+
                " (select user_name"+
                " from s_users"+
                " where user_id = a.ri_approved_who) as approved_reins_name ";

        if(printingLOV.equalsIgnoreCase("CLAIM"))
                select = select + " ,(select cek_bayar_klaim(a.pol_id)) as claim_payment_date2 ";
    ...
    public void clickReverseUW() throws Exception {
        ...
        form.getPolicy().setStMigrasiFlag(null);
    ...
    public void underwrit() {
        ...
        enableRenewalNonAKS = hasResource("POL_UWRIT_RENEWAL_NON_AKS");
...
//                enableView = false;
    ...
    public void claim() {
        ...
        statusFilter = array2list(new String [] {
            ...
            FinCodec.PolicyStatus.ENDORSECLAIMINWARD,
        });
        enableClaim=true;
        enableEdit = canEdit;
        listEffective = true;
        ...
        editFilter.add(FinCodec.PolicyStatus.ENDORSECLAIMINWARD);
        ...
        if (stPolicy == null && stName == null && dtBirthDate == null && dtLiquidityFrom == null && dtEndOfCreditFrom == null
            ...
            if (stKreasi.equalsIgnoreCase("1")) {
                ...
                sqa.addSelect(" a.pol_no, b.ref1 as nama, b.ref2 as umur, b.refd1 as tgl_lhr, "
                        ...
                        + " b.refd7 as tgl_awal_pk,b.refd8 as tgl_akhir_pk, "
                        + " getpremi(a.pol_type_id = 21,getpremi(a.status = 'ENDORSE',b.insured_amount,b.refn4),b.insured_amount) as insured_amount,"
                        + " case when a.pol_type_id = 21 then b.refn6 "
                        + " when a.pol_type_id in (87,88) then b.refn11 "
                        + " else b.premi_total end as premi,"
                ...
                sqa.addClause("a.pol_type_id in (21,59,80,87,88)");
                sqa.addClause("a.active_flag='Y'");
                ...
                if (dtLiquidityFrom != null) {
                    sqa.addClause("date_trunc('day',a.tgl_cair) >= ?");
                    sqa.addPar(dtLiquidityFrom);
                }
                ...
                if (dtEndOfCreditFrom != null) {
                    sqa.addClause("date_trunc('day',a.tgl_akhir) >= ?");
                    sqa.addPar(dtEndOfCreditFrom);
                }
...
//                sqa.addPar(stPolicyLevel);
    ...
    public void clickCreateEndorseByMode() throws Exception {
        ...
        //JENIS ENDORSE 2025
        /*
        if(getStEndorseMode().contains("TOTAL")){

            form.editCreateEndorseBatalTotalMode(policyID);
            form.setTotalEndorseMode(true);

        }else if(getStEndorseMode().contains("RESTITUTION")){
        ...
        form.getPolicy().setStEndorseMode(getStEndorseMode());
...
//            pol.setStReinsuranceApprovedWho("14068310");
    ...
    public void clickJurnalUlangPerBulan() throws Exception {
        ...
        */
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
            ...
            logger.logInfo("+++++++ UPDATE STATUS POLIS : "+ policy.getStPolicyNo() +" ++++++++++++++++++");
            PreparedStatement PS = S.setQuery("update proses_jurnal_ulang set admin_notes = 'Y' where pol_id = ?");
    ...
    public void EXCEL_PAKREASI() throws Exception {
        ...
        if (stKreasi.equalsIgnoreCase("1")) {
            ...
            sqa.addSelect(" a.status,a.cc_code,';'||a.pol_no as pol_no,a.cust_name,b.order_no,trim(b.ref1) as nama, b.ref2 as umur,"
                    + "b.refd1 as tgl_lhr,';'||trim(b.ref3) as noktp,';'||trim(b.ref16) as norekpinj,';'||trim(b.ref4) as nopk,b.refd2 as tgl_cair,b.refd3 as tgl_akhir,"
                    + " getpremi(a.pol_type_id = 21,getpremi(a.status = 'ENDORSE',b.insured_amount,b.refn4),b.insured_amount) as insured_amount,"
                    + " getpremi(a.pol_type_id = 21,b.refn6,b.premi_total) as premi,"
                    + " b.refn5 as rate_premi,a.pol_id " + sqls + "");
...
//                    sqa.addPar(stPekerjaanDeb);
            ...
            if (dtLiquidityFrom != null) {
                sqa.addClause("date_trunc('day',a.tgl_cair) >=  '" + dtLiquidityFrom + "'");
//                sqa.addPar(dtLiquidityFrom);
            }

            if (dtLiquidityTo != null) {
                sqa.addClause("date_trunc('day',a.tgl_cair) <=  '" + dtLiquidityTo + "'");
//                sqa.addPar(dtLiquidityTo);
            }

            if (dtEndOfCreditFrom != null) {
                sqa.addClause("date_trunc('day',a.tgl_akhir) >=  '" + dtEndOfCreditFrom + "'");
//                sqa.addPar(dtEndOfCreditFrom);
            }

            if (dtEndOfCreditTo != null) {
                sqa.addClause("date_trunc('day',a.tgl_akhir) <=  '" + dtEndOfCreditTo + "'");
//                sqa.addPar(dtEndOfCreditTo);
            }
    ...
    public void clickChangeKlaimTreatyByDate() throws Exception {
        ...
        //dtFilterPolicyDateFrom

        if(getDtFilterPolicyDateFrom()==null)
            throw new RuntimeException("Tanggal awal belum diisi");

        Date tanggalAwal = getDtFilterPolicyDateFrom();
        Date tanggalAkhir = getDtFilterPolicyDateTo();
        String mmyy = DateUtil.getMonth2Digit(tanggalAwal) + DateUtil.getYear2Digit(tanggalAwal);
        
        listPolicy = ListUtil.getDTOListFromQuery(
                " select * "+ 
                "   from ins_policy "+
                " where status in ('CLAIM','CLAIM ENDORSE') and claim_status = 'DLA' and active_flag='Y' AND effective_flag = 'Y' AND coalesce(admin_notes,'') <> ?   "+
                " and pol_type_id in (21,59,80,73,74,87,88) and date_trunc('day',dla_date) >= ? and date_trunc('day',dla_date) <= ?  "+
                 " order by pol_id ", new Object [] {mmyy, tanggalAwal, tanggalAkhir}
                ,InsurancePolicyView.class);
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
            ...
            getRemoteInsurance().saveReinsuranceOnly(pol, pol.getStNextStatus(), false);
            logger.logInfo("+++++++ UPDATE STATUS POLIS : "+ pol.getStPolicyNo() +" ++++++++++++++++++");
            PreparedStatement PS = S.setQuery("update ins_policy set admin_notes = '"+ mmyy +"' where pol_id = ?");
    ...
    public void clickChangePassword() throws Exception {
        ...
        listPolicy = ListUtil.getDTOListFromQuery(
                " select * "+
                " from s_users "+
                " where branch = '80' and password is null order by user_id",
               UserSessionView.class);

        for (int i = 0; i < listPolicy.size(); i++) {
            UserSessionView policy = (UserSessionView) listPolicy.get(i);

            String password = Digest.computeDigest(policy.getStUserID(), "askrida00");
 ...
 public void cekpolicy() {
    ...
    public boolean verifikasiCaptcha() throws Exception{
     ...
     public void clickApproveKumpulan() throws Exception {
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
            ...
            PreparedStatement PS = S.setQuery("update ins_policy set admin_notes = 'SKKOMISI' where pol_id = ?");
    ...
    public void createEndorsemenRIByDATA() throws Exception{
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
                ...
                form.editCreateEndorseRIProses(polis.getStPolicyID(),pol2.getStInsuranceTreatyID());
    ...
    public void createEndorsemenKomisiByData() throws Exception{
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
                ...
                //form.editCreateEndorseSelisihKomisi(polis, endorse);

                logger.logDebug("################## amount brokerfee 1 = "+ endorse.getDbNDComm1());

                form.editCreateEndorseTambahBrokerfeeExclude(polis, endorse);

                form.btnSaveEndorsePPN();

                PreparedStatement PS = S.setQuery("update proses_endorse_h2h set flag = 'Y' where pol_no = ?");
    ...
    public void getDataDetailPolisToExcel() throws Exception{
        ...
        HSSFSheet sheet = wbCek.getSheet("KONVERSI");
        ...
        //CREATE SHEET INDUK
        HSSFSheet sheetOutput = wb.createSheet("OUTPUT");

        HSSFRow row1 = sheetOutput.createRow(0);

        row1.createCell(0).setCellValue("SEARCH BY");
        row1.createCell(1).setCellValue("KEY PENCARIAN");
        row1.createCell(2).setCellValue("NAMA");
        row1.createCell(3).setCellValue("NO IDENTITAS");
        row1.createCell(4).setCellValue("NO PERJANJIAN KREDIT");
        row1.createCell(5).setCellValue("NO REKENING PINJAMAN");
        row1.createCell(6).setCellValue("NOMOR POLIS");
        row1.createCell(7).setCellValue("KODE CABANG");
        row1.createCell(8).setCellValue("DATA DITEMUKAN");
        ...
        for (int r = 5; r <= rows; r++) {
            ...
            HSSFCell cellNoRekeningPinjaman = row.getCell(3);//nomor rekening
            HSSFCell cellSearchBy = row.getCell(4);//key filter

            String key = "";

            if(cellSearchBy.getStringCellValue().equalsIgnoreCase("NO REKENING PINJAMAN")){
                key = "upper(b.ref16)";
            }else if(cellSearchBy.getStringCellValue().equalsIgnoreCase("NO PERJANJIAN KREDIT")){
                key = "upper(b.ref4)";
            }else if(cellSearchBy.getStringCellValue().equalsIgnoreCase("NO POLIS")){
                key = "a.pol_no";
            }else{
                throw new RuntimeException("Kolom search by belum dipilih");
            }

            String value = "";

            if (cellNoRekeningPinjaman.getCellType() == cellNoRekeningPinjaman.CELL_TYPE_STRING) {
                value = cellNoRekeningPinjaman.getStringCellValue();
            } else {
                value = String.valueOf(cellNoRekeningPinjaman.getNumericCellValue());
            }

            DTOList listPolis = null;

                listPolis = ListUtil.getDTOListFromQuery(
                " select a.pol_id,a.status,a.pol_no,b.ref1 as nama,b.ref16,a.cc_code,b.ref4,b.ref3 "+
                "     from ins_policy a "+
                "     inner join ins_pol_obj b on a.pol_id = b.pol_id "+
                "     where status in ('POLICY','RENEWAL') and active_flag = 'Y' and effective_flag = 'Y' and pol_type_id = 59 "+
                "     and "+ key +" = ? ",
                new Object[]{value.toUpperCase().trim()},
                InsurancePolicyView.class);

                InsurancePolicyView polis = new InsurancePolicyView();

                if(listPolis.size()>0)
                    polis = (InsurancePolicyView) listPolis.get(0);

            String noPolis = "TIDAK DITEMUKAN";
            String noPK = "TIDAK DITEMUKAN";
            String noRekPinjaman = "TIDAK DITEMUKAN";
            String nama = "TIDAK DITEMUKAN";
            String noKTP = "TIDAK DITEMUKAN";
            String cabang = "TIDAK DITEMUKAN";
            String dataFound = "N";

            if(listPolis.size()>0){

                if(cellSearchBy.getStringCellValue().equalsIgnoreCase("NO POLIS")){
                    noPK = polis.getStReference4();
                    noRekPinjaman = polis.getStReference16();
                    noPolis =  polis.getStPolicyNo();
                }else{
                    noPK = polis.getStReference4();
                    noRekPinjaman = polis.getStReference16();
                    noPolis = polis.getStPolicyNo();
                ...
                nama = polis.getStNama();
                noKTP = polis.getStReference3();
                cabang = polis.getStCostCenterCode();
                dataFound = "Y";
            }

            HSSFRow data = sheetOutput.createRow(rowData);

            data.createCell(0).setCellValue(cellSearchBy.getStringCellValue());
            data.createCell(1).setCellValue(value.toUpperCase().trim());
            data.createCell(2).setCellValue(nama);
            data.createCell(3).setCellValue(noKTP);
            data.createCell(4).setCellValue(noPK);
            data.createCell(5).setCellValue(noRekPinjaman);
            data.createCell(6).setCellValue(noPolis);
            data.createCell(7).setCellValue(cabang);
            data.createCell(8).setCellValue(dataFound);
        ...
        SessionManager.getInstance().getResponse().setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        SessionManager.getInstance().getResponse().setHeader("Content-Disposition","attachment; filename=OUTPUT"+ System.currentTimeMillis() +".xls;");
        SessionManager.getInstance().getResponse().setHeader("Pragma", "token");
        ServletOutputStream sosStream = SessionManager.getInstance().getResponse().getOutputStream();
    ...
    public void clickUpdateKursInwardBulanan() throws Exception {

        if(getDtFilterPolicyDateFrom()==null) throw new RuntimeException("Tanggal Polis awal tidak boleh kosong");

        if(getDtFilterPolicyDateTo()==null) throw new RuntimeException("Tanggal Polis akhir tidak boleh kosong");
        ...
        listPolicy = ListUtil.getDTOListFromQuery(
                " select * "+
                " from ins_policy a "+
                " where a.status = 'INWARD' " +
                " and date_trunc('day',a.policy_date) >= ? " +
                " and date_trunc('day',a.policy_date) <= ? "+
                " and coalesce(a.effective_flag,'') <> 'Y' and a.active_flag = 'Y' and a.ccy <> 'IDR' "+
                " order by pol_id",
                new Object[]{getDtFilterPolicyDateFrom(), getDtFilterPolicyDateTo()},
                InsurancePolicyView.class);
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
            ...
            pol.setDbCurrencyRateTreaty(CurrencyManager.getInstance().getRate(pol.getStCurrencyCode(), pol.getDtPolicyDate()));
            pol.setDbCurrencyRate(CurrencyManager.getInstance().getRate(pol.getStCurrencyCode(), pol.getDtPolicyDate()));
            ...
            //simpan spreading reas & kurs di polis
            getRemoteInsurance().saveReinsuranceOnly(pol, pol.getStNextStatus(), false);

            getRemoteInsurance().savePolicyOnly(pol);

            logger.logInfo("+++++++ UPDATE STATUS POLIS : "+ pol.getStPolicyNo() +" ++++++++++++++++++");
            PreparedStatement PS = S.setQuery("update ins_policy set admin_notes = 'CCY_UPD' where pol_id = ?");
    ...
    public void createEndorsemenPremiByData() throws Exception{
        final SQLUtil S = new SQLUtil();
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
                ...
                form.editCreateEndorseSelisihPremi(polis, endorse);
    ...
    public void clickEditEndorseKumpulan() throws Exception {
        ...
        listPolicy = ListUtil.getDTOListFromQuery(
                ...
                " where pol_id in ( "+
                " select pol_id "+
                " from endorse_mantap_h2h "+
                " ) and coalesce(admin_notes,'') <> 'PRM2' "+
                " order by pol_id ",
                InsurancePolicyView.class);
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
            ...
            logger.logInfo("+++++++ approve POLIS : "+ policy.getStPolicyNo() +" ++++++++++++++++++");
            PreparedStatement PS = S.setQuery("update ins_policy set admin_notes = 'PRM2' where pol_id = ?");
    ...
    public void createEndorsemenRIByData2() throws Exception{
        final SQLUtil S = new SQLUtil();
    ...
    public void clickCreateEndorseClaimInward() throws Exception {
        validatePolicyID();
        ...
        form.editCreateClaimInwardEndorse(policyID);

        form.show();
    }

    public void clickAddDeductibleH2HFire() throws Exception {

        final SQLUtil S = new SQLUtil();

        DTOList listPolicyH2H = null;


        listPolicyH2H = ListUtil.getDTOListFromQuery(
                "select * "+
                " from proses_deductible_h2h   "+
                " where proses_flag is null and kode_risiko <> '2976' "+
                " order by data_id", PengajuanPolisFireView.class);

        for (int h = 0; h < listPolicyH2H.size(); h++) {
            PengajuanPolisFireView fire = (PengajuanPolisFireView) listPolicyH2H.get(h);
            ...
            listPolicy = ListUtil.getDTOListFromQuery(
                    " select * "+
                         " from ins_policy  "+
                         " where status = 'POLICY' and pol_no = ? ",new Object[]{fire.getStPolicyNo()},
                    InsurancePolicyView.class);
            ...
            for (int i = 0; i < listPolicy.size(); i++) {
                ...
                for (int j = 0; j < objects.size(); j++) {
                    ...
                    InsurancePolicyDeductibleView ded = new InsurancePolicyDeductibleView();

                    ded.markNew();

                    //kelas 1
                    if(fire.getStKelasKonstruksi().equalsIgnoreCase("1")){
                        ded.setStInsuranceClaimCauseID("3910");
                        ded.setStCurrencyCode("IDR");
                        ded.setDbPct(new BigDecimal(10));
                        ded.setStDeductType("DED_PCT_CLAIM");
                    }

                    //kelas 2
                    if(fire.getStKelasKonstruksi().equalsIgnoreCase("2")){
                        ded.setStInsuranceClaimCauseID("3910");
                        ded.setStCurrencyCode("IDR");
                        ded.setDbPct(new BigDecimal(10));
                        ded.setStDeductType("DED_PCT_CLAIM");
                        ded.setDbAmountMin(new BigDecimal(25000000));
                    }

                    //kelas 3
                    if(fire.getStKelasKonstruksi().equalsIgnoreCase("3")){
                        ded.setStInsuranceClaimCauseID("3910");
                        ded.setStCurrencyCode("IDR");
                        ded.setDbPct(new BigDecimal(5));
                        ded.setStDeductType("DED_PCT_CLAIM");
                        ded.setDbAmountMin(new BigDecimal(50000000));
                    }
                ...
                PreparedStatement PS = S.setQuery("update proses_deductible_h2h set proses_flag = 'Y' where pol_no = ?");
    ...
    public void clickEditBypassValidasiKlaim() throws Exception {
        validatePolicyID();

        String form_name = "policy_bypass_edit";
        ...
        form.setBypassValidationMode(true);

        form.editPayment(policyID);

        form.setReadOnly(true);

        form.getTabs().setActiveTab("TAB_INST");

        form.show();
    }

    public void clickViewBypassValidasi() throws Exception {
        String form_name = "policy_bypass_edit";
    ...
    public void bypassvalidasi() {
        setTitle("Bypass Validasi Klaim");
        printingLOV = "UWRIT";

        enableInputBypassValidasi = hasResource("BYPASS_CLAIM_EDIT");
...
//                enableView = false;
    ...
    public void clickCreateRenewalNonAKS() throws Exception {
        if(stPolicyNoRenewal==null){
            throw new RuntimeException("Nomor Polis Sebelumnya Harus diisi lengkap (18 digit)");
        }

        if(stPolicyNoRenewal.length()!=18){
            throw new RuntimeException("Nomor Polis Sebelumnya Harus diisi lengkap (18 digit)");
        }

        InsurancePolicyView polisInduk = getRemoteInsurance().getInsurancePolicyByPolicyNo(stPolicyNoRenewal);
        ...
        if(polisInduk!=null){
            ...
            form.editCreateRenewalNonAKS(polisInduk.getStPolicyID());

            form.show();
        }else{
            throw new RuntimeException("Data Polis tidak ditemukan");
        }

    }

    public void clickCreatePolisButton() throws Exception {
    ...
    public void clickEditReinsurance2() throws Exception {
        validatePolicyID();
        ...
        form.setEditReas(true);
        form.getPolicy().setEditReas(true);

        form.show();
    }


    public void clickCreateRenewalPOJK20() throws Exception {
        validatePolicyID();
    ...
    public void createRenewalSerbaguna() throws Exception{
        ...
        for (int i = 0; i < loop; i++) {
            ...
            PolicyForm form = new PolicyForm();
            form.editCreateRenewalPOJK20(policyID, counterPolisCurrent);
            String policyID = form.btnSaveRenewalPOJK();
    ...
    public void createEndorseKomisiMantapBrokerPakarData() throws Exception{
        final SQLUtil S = new SQLUtil();
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
                ...
                " FROM INS_POLICY "+
                " WHERE STATUS IN ('POLICY','RENEWAL','ENDORSE') "+
                " AND SUBSTR(POL_NO,0,18) = ? "+
                " ORDER BY POL_ID DESC LIMIT 1",
                new Object[]{endorse.getStPolicyNo().substring(0, 17)},
                InsurancePolicyView.class);
    ...
    public void clickCreateRenewalToCabang() throws Exception {
        if(stPolicyNoRenewal==null){
            throw new RuntimeException("Nomor Polis Sebelumnya Harus diisi lengkap (18 digit)");
        }
        ...
        if(polisInduk!=null){
            ...
            form.editCreateRenewalToCabang(polisInduk.getStPolicyID());
    ...
    public void clickReActivateClaim() throws Exception {
        validatePolicyID();
    ...
    public void changePolisToAKS() throws Exception{

        //dapetin polis nya
        InsurancePolicyView pol = getRemoteInsurance().getInsurancePolicy(policyID);

        if(pol.isStatusPolicy() || pol.isStatusRenewal() || pol.isStatusEndorse())
            if(pol.isEffective())
                throw new RuntimeException("Polis tidak bisa dipindah, sudah disetujui");

        if(!pol.getStCostCenterCode().equalsIgnoreCase("80"))
            throw new RuntimeException("Fitur ini hanya untuk memindahkan polis Non AKS (yang belum disetujui) ke cabang");

        pol.markUpdate();
        pol.setStCostCenterCode(pol.getStCostCenterCodeSource());
        pol.setStRegionID(pol.getStRegionIDSource());
    ...
    public void clickReApproveKumpulan() throws Exception {
        ...
        listPolicy = ListUtil.getDTOListFromQuery(
                " select pol_id,pol_no,approved_date, "+
                " (select receipt_no "+
                " from ar_invoice x where x.ar_trx_type_id in (5,6,7) and x.attr_pol_id = a.pol_id) as bayar "+
                " from ins_policy a "+
                " where pol_no in ( "+
                "        select pol_no "+
                "        from proses_pajak "+
                "        where proses_flag is null "+
                " ) and policy_date >= '2025-08-01 00:00:00' "+
                " and (select receipt_no "+
                " from ar_invoice x where x.ar_trx_type_id in (5,6,7) and x.attr_pol_id = a.pol_id) is null "+
        ...
        for (int i = 0; i < listPolicy.size(); i++) {
            ...
            logger.logInfo("+++++++ approve POLIS : "+ policy.getStPolicyNo() +" ++++++++++++++++++");
            PreparedStatement PS = S.setQuery("update proses_pajak set proses_flag = 'Y',proses_date = 'now' where pol_no = ?");
