...
public class InsurancePolicyPreEndorseView extends DTO implements RecordAudit, RecordBackup {
    /*
 ...
 ALTER TABLE ins_policy ADD COLUMN period_rate_before numeric;
    ...
    public static String tableName = "ins_policy_endorse";
    ...
    private InsurancePolicyObjectPreEndorseView claimObject;
...
//   ALTER TABLE ins_policy ADD COLUMN inst_periods int8;
    ...
    private void recalculateClaim() throws Exception {
        ...
        if (isStatusClaim()||isStatusClaimEndorse()){
            ...
            if (claimItems != null)
                ...
                final boolean custAmount = claimg || deduct || jasabengkel || sparepart;

                if(panjarKlaim) continue;
                ...
                if (claimg) it.setDbAmount(claimAmount);
                ...
                if(adjusterFee) adjusterAmount = it.getDbAmount();
                ...
                if (claimObject != null)
                    if(claimg)
                        if(BDUtil.biggerThan(it.getDbAmount(), claimObject.getDbObjectInsuredAmount()))
                            throw new RuntimeException("Jumlah Klaim "+ it.getDbAmount() +" Melebihi TSI "+ claimObject.getDbObjectInsuredAmount());
                }
            ...
            /*
                for (int i = 0; i < claimItems.size(); i++) {
                    ...
                    InsurancePolicyItemsView itTax = (InsurancePolicyItemsView) claimItems.get(i);

                    final boolean jasabengkel2 = Tools.isEqual(itTax.getInsuranceItem().getStItemType(), "BENGKELFEE");

                    if(itTax.getStTaxCode()!=null){
                        boolean hasNPWP = itTax.getStNPWP()!=null?true:false;
                        BigDecimal ratePajak = getPPHRate(itTax.getStTaxCode(),hasNPWP,getDtPeriodStart());

                        if(itTax.isAutoTaxRate())
                                itTax.setDbTaxRate(ratePajak);

                        itTax.setDbTaxAmount(BDUtil.mul(itTax.getDbTaxRate(), itTax.getDbAmount(), scale));

                        if(jasabengkel2 && BDUtil.isZeroOrNull(itTax.getDbAmount()))
                            itTax.setDbTaxAmount(BDUtil.mul(itTax.getDbTaxRate(), dbClaimCustAmount, scale));
                    }
                }*/
                        
            //if (claimCause.getStExcClaimFlag() != null)
                //if (Tools.isYes(claimCause.getStExcClaimFlag()))
                    //dbClaimREAmount = BDUtil.zero;
...
//                    if(BDUtil.isZeroOrNull(trd.getDbTSIAmount())){
//                        if(!BDUtil.isZeroOrNull(trd.getDbPremiAmount()))
//                            trd.setDbClaimAmount(BDUtil.div(BDUtil.mul(trd.getDbPremiAmount(), claimTot, scale), totalPremi));
//                    }
                    
                    if(scale==0) trd.setDbClaimAmount(trd.getDbClaimAmount().setScale(scale, BigDecimal.ROUND_HALF_DOWN));
                    ...
                    for (int j = 0; j < shares.size(); j++) {
                        ...
                        if(BDUtil.isZeroOrNull(ri.getDbSharePct())){
                            double proRate = ri.getDbTSIAmount().doubleValue() / trd.getDbTSIAmount().doubleValue();
                            ri.setDbClaimAmount(BDUtil.mulRound(new BigDecimal(proRate), trd.getDbClaimAmount(),0));
                        }
                    }
                }
                    ...
                    for (int i = 0; i < coins.size(); i++) {
                        ...
                        if (stPolicyTypeID.equalsIgnoreCase("21")) {
                            ...
                            if(co.isManualClaim()){
                                ...
                                logger.logDebug("+++++++++++++++++ MANUAL NIH ???????");
                                if (!co.isHoldingCompany()){
                                    logger.logDebug(">>>>>>>>>>>>>>>>>>>>>>>> MANUAL klaim koas");
                                    logger.logDebug(">>>>>>>>>>>>>>>>>>>>>>>> MANUAL klaim koas : "+ claimLeader);
                                    co.setStManualClaimFlag("Y");
                                ...
                                if (claimCause.getStExcClaimFlag() != null) {
                                    if (claimCause.getStExcClaimFlag().equalsIgnoreCase("Y")) {
                                        if(!co.isAutoClaimAmount()&&!manualClaim){
                                            if (co.isHoldingCompany()) {
                                                ...
                                                if(coins.size()==1) co.setDbClaimAmount(BDUtil.mul(dbClaimAmount, BDUtil.getRateFromPct(askridaSharePct)));
                                            } else {
                                                if (isStatusClaimPLA())
                                            ...
                                            if(!co.isHoldingCompany()) co.setDbClaimAmount(BDUtil.negate(claimLeader));
                                            if(co.isHoldingCompany()){
                                                manualClaim = true;
                                                claimLeader = co.getDbClaimAmount();
                                            }
                                    ...
                                    if(tahun<2008) beforeApril2008 = true;
                                    if(tahun==2008 && bulan <4) beforeApril2008 = true;
    ...
    private void recalculateClaimBackup() {
        ...
        if (claimObject != null) {
            ...
            //tes
            {
                        ...
                        if (claimCause.getStExcClaimFlag() != null) {
                            if (claimCause.getStExcClaimFlag().equalsIgnoreCase("Y")) {
                                if (co.isHoldingCompany()) {
                                    ...
                                    if (isStatusClaimPLA())
                                        co.setDbClaimAmount(BDUtil.mul(getDbClaimAmountEstimate(), BDUtil.getRateFromPct(askridaSharePct)));
                                    else if (isStatusClaimDLA())
                                        //co.setDbClaimAmount(getDbClaimAmountApproved());
                                        co.setDbClaimAmount(BDUtil.mul(getDbClaimAmountApproved(), BDUtil.getRateFromPct(askridaSharePct)));
                                }
                            }
                ...
                for (int i = 0; i < coins2.size(); i++) {
                    ...
                    if (isStatusClaimPLA())
                        claimAmt = BDUtil.mul(getDbClaimAmountEstimate(), BDUtil.getRateFromPct(co2.getDbSharePct()), scale);
                    else if (isStatusClaimDLA())
                        claimAmt = BDUtil.mul(getDbClaimAmountApproved(), BDUtil.getRateFromPct(co2.getDbSharePct()), scale);
        ...
        try {
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                ...
                if (stPolicyTypeGroupID.equalsIgnoreCase(FinCodec.InsurancePolicyTypeGroup.EARTHQUAKE)){
                    ...
                    tre.adjustRatioEarthquake(null, tlrMaipark, tlr, dbCurrencyRateTreaty, getStCurrencyCode());
        ...
        try {
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                ...
                if ((stPolicyTypeGroupID.equalsIgnoreCase(stEarthquakeTypeId))){
                    tre.adjustRatioEarthquake(null,tlrMaipark, tlr, dbCurrencyRateTreaty, currencyCode);
    ...
    public void recalculateTreatyKBGEndorse() throws Exception {
        if (getStCoverTypeCode() == null) return;
        if (getStPolicyTypeID() == null) return;
        ...
        try {
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                ...
                final BigDecimal insuredAmountEndorse = ipo.getDbObjectInsuredAmountShare();
                
                final BigDecimal insuredAmount = getDbInsuredAmount();
 ...
 */
    ...
    public void recalculateBasicBackup() throws Exception {
        ...
        if (isStatusEndorse() || isStatusEndorseIntern()) recalculateEndorsement();
    }
    ...
    public void recalculateBasic() throws Exception {
        ...
        for (int j = 0; j < objects.size(); j++) {
            ...
            DateTime bulanOJK = new DateTime(DateUtil.getDate("31/01/2014"));
            DateTime bulanProduksi = new DateTime(getDtPeriodStart());

            if(bulanProduksi.isAfter(bulanOJK)){
                final boolean blockKetentuanOJK = Parameter.readBoolean("BLOCKING_OJK");

                if(blockKetentuanOJK){
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if(!BDUtil.isZeroOrNull(co.getDbCommissionAmount())) item.setDbAmount(co.getDbCommissionAmount());
            item.calculateRateAmount(totalAfterDiscount, scale);
        ...
        
        {
            ...
            for (int i = 0; i < coins.size(); i++) {
                ...
                if(!BDUtil.isZeroOrNull(coin.getDbCommissionRate())) coin.setDbCommissionAmount(BDUtil.mul(premiAfterDisc, BDUtil.getRateFromPct(coin.getDbCommissionRate()), scale));
                if(!BDUtil.isZeroOrNull(coin.getDbBrokerageRate())) coin.setDbBrokerageAmount(BDUtil.mul(premiAfterDisc, BDUtil.getRateFromPct(coin.getDbBrokerageRate()), scale));
                if(!BDUtil.isZeroOrNull(coin.getDbHandlingFeeRate())) coin.setDbHandlingFeeAmount(BDUtil.mul(premiAfterDisc, BDUtil.getRateFromPct(coin.getDbHandlingFeeRate()), scale));
        ...
        if (isStatusEndorse()||isStatusEndorseRI()|| isStatusEndorseIntern() || isStatusTemporaryEndorsemen()) recalculateEndorsement();
    ...
    private void recalculateFee(){
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if (item.isHandlingFee() || item.isStampFee() || item.isPolicyCost() || item.isPPN()) {
                if (item.isHandlingFee()) {
                    dbNDHFee = item.getDbAmount();
                    dbNDHFeePct = item.getDbRatePct();
  ...
  125.000.000 x 35% = 43.750.000
    ...
    private void calculatePPH21And23() throws Exception{
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if (item.isComission() || item.isJasaBengkel()) ;
            else continue;
            ...
            if(item.getStNPWP()!=null)
                if(item.getStNPWP().equalsIgnoreCase("")) hasNPWP = false;
...
//            throw new RuntimeException("Total Premi Cicilan Tidak Sama Dengan Tagihan Netto");
    ...
    public void loadObjects() {
        try {
            ...
            if (objects == null) {
                ...
                for (int i = 0; i < objects.size(); i++) {
                    InsurancePolicyObjectPreEndorseView obj = (InsurancePolicyObjectPreEndorseView) objects.get(i);
    ...
    private void loadARInvoices() {
        try {
            ...
            arinvoices = ListUtil.getDTOListFromQuery(
                    ...
                    "         ar_invoice   a" +
                    "      where " +
                    "         posted_flag = 'Y' and ar_trx_type_id = ? and attr_pol_id = ? order by a.ar_invoice_id",
    ...
    public void generatePolicyNo() throws Exception {
       ...
       String orderNo = StringTools.leftPad(String.valueOf(IDFactory.createNumericID(policyType2Digit + month2Digit + year2Digit + ccCode, 1)), '0', 4);
        ...
        //  12345678901234567890
        //  01234567890123456789
        if(isStatusTemporaryPolicy()) coasCode = "9";
    ...
    public void generateEndorseNo() {
        ...
        stPreEndorseNo = new String(policyno);
    ...
    public void generateClaimEndorseDLA() {
        ...
        if (stDLANo.endsWith("A"))
            stDLANo = stDLANo.replaceAll("A","B");
        else if (stDLANo.endsWith("B"))
            stDLANo = stDLANo.replaceAll("B","C");
        else if (stDLANo.endsWith("C"))
            stDLANo = stDLANo.replaceAll("C","D");
        else
            stDLANo = stDLANo + "A";
    ...
    public InsurancePolicyObjectPreEndorseView getClaimObject() {
    ...
    public void validate(boolean isApproving) throws Exception {
        ...
        if (isApproving) {
            if (isStatusClaim() || isStatusClaimEndorse()) {
                if (isStatusClaimPLA()) {
                    if (getStPLANo() == null) throw new RuntimeException("No PLA/LKS Belum Diisi");
                    
                    setStClaimStatus(FinCodec.ClaimStatus.PLA);
                    setStActiveFlag("Y");
                } else if (isStatusClaimDLA()) {
                    if (getStDLANo() == null) throw new RuntimeException("No DLA/LKP Belum Diisi");
                ...
                if(isStatusClaimDLA() || isStatusClaimEndorse()){
                    if (dtDLADate != null) {
                        ...
                        if(isAdmin){
                            ...
                            if(!withinCurrentMonth)
                                if(getDtConfirmDate()==null) throw new RuntimeException("Tanggal LKP Tidak Valid (Sudah Tutup Produksi), Ubah Tanggal LKP menjadi Tanggal Sekarang (Bulan Produksi)");
                        }*/
                        /*
                        if (blockValidPolicyDate && !withinCurrentMonth && !isAdmin)
                            if(getDtConfirmDate()==null) throw new RuntimeException("Tanggal LKP Tidak Valid (Sudah Tutup Produksi), Ubah Tanggal LKP menjadi Tanggal Sekarang (Bulan Produksi)");
                        
                        if (blockValidPolicyDateAdmin && !withinCurrentMonth && isAdmin)
                            if(getDtConfirmDate()==null) throw new RuntimeException("Tanggal LKP Tidak Valid (Sudah Tutup Produksi), Ubah Tanggal LKP menjadi Tanggal Sekarang (Bulan Produksi)");
                        */
                    }
                }
                                ...
                                if(!isUserSuperEdit){
                                    if(isAdmin){
                                        if(!isEditReasOnlyMode())
                                            if(!withinCurrentMonth) throw new RuntimeException("Tanggal Polis / Endorsemen Tidak Valid (Sudah Tutup Produksi)");
                                    }else{
                                         if(!withinCurrentMonth) dtPolicyDate = new Date();
                                    }
                                }
                            }
                    }
        ...
        if (isStatusClaim()) {
            if (isStatusClaimPLA()) {
                ...
                if(tgl == 1) validPeriod = false;
                //if(tgl == 0) desc = "Periode Awal / Periode Akhir Tidak Boleh Sama";
                if(tgl == 1) desc = "Periode Akhir Tidak Boleh < Periode Awal";
                if (!validPeriod) throw new RuntimeException(desc);
                ...
                if (getStDLANo() == null) throw new RuntimeException("NO LKP Belum Diisi");
                
                if(getDbClaimAmountApproved()==null)
                    throw new RuntimeException("Jumlah Klaim Disetujui Belum Diisi");
                
                if(getDtDLADate()==null) throw new RuntimeException("Tanggal LKP Belum Diisi");
            ...
            if(stPolicyTypeGroupID.equalsIgnoreCase("1")){
                if (raiseErrors) {
                    if(stReinsuranceApprovedWho==null)
                        throw new RuntimeException("Endorsemen fire harus melalui persetujuan bagian reasuransi kantor pusat terlebih dahulu");
                } else {
                    return; 
                }
            }
        
        if (!(isStatusEndorse() || isStatusPolicy() || isStatusRenewal() || isStatusEndorseRI()|| isStatusEndorseIntern() || isStatusTemporaryPolicy() || isStatusTemporaryEndorsemen() || isStatusInward())) return;
    ...
    public boolean canClaimAgain() throws Exception {
        ...
        try {
            ...
            if(getClaimObject().getStVoidFlag()==null) canClaim = true;
...
//   }
        ...
        try {
            ...
            if (objectsClaim == null) {
                ...
                for (int i = 0; i < objectsClaim.size(); i++) {
                    InsurancePolicyObjectPreEndorseView obj = (InsurancePolicyObjectPreEndorseView) objectsClaim.get(i);
    ...
    public void recalculateEndorsePAKreasi() throws Exception {
        if (!stPolicyTypeID.equalsIgnoreCase("21") && !stPolicyTypeID.equalsIgnoreCase("59")) return;
        ...
        if (isStatusEndorse()||isStatusEndorseRI()|| isStatusEndorseIntern()) {
            ...
            for (int i = 0; i < objects.size(); i++) {
                ...
                if(objx.getDtReference5()!=null) continue;
                
                if(objx.getStPolicyObjectRefID()==null) continue;
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if(!isStatusClaim())
                if(items.getInsItem().getStItemCategory().equalsIgnoreCase("PCOST") || items.getInsItem().getStItemCategory().equalsIgnoreCase("SFEE"))
                    if(items.getDbAmount()==null) throw new RuntimeException("Biaya Polis/Materai Harus Diisi / Dihapus Jika Tidak Ada");

            if (!items.isComission() || items.getInsItem().isNotUseTax()) continue;
    ...
    public void validateExcludedRiskTSI(boolean isApproving) throws Exception {
            ...
            for (int j = 0; j < treaty.size(); j++) {
                ...
                for (int k = 0; k < treatyDetails.size(); k++) {
                    ...
                    for (int l = 0; l < shares.size(); l++) {
                        ...
                        /*
                        if (isFireEndorseNeedManualReinsurance()) {
                            if (isApproving)
                                throw new RuntimeException("Reas harus di spread manual");
                            else
                                ri.setStApprovedFlag(null);   
    ...
    public void validateTSILimit(BigDecimal transactionLimit) throws Exception{
        ...
        for(int i=0;i<object.size();i++){
            ...
            final BigDecimal transactionLimitPerRiskCode = getTransactionLimitPerRiskCode("ACCEPT", getStRiskClass(),SessionManager.getInstance().getUserID(), obj.getStRiskCategoryID());
            
            boolean enoughLimit = false;
            
            if(!BDUtil.isZeroOrNull(transactionLimitPerRiskCode))
                enoughLimit = Tools.compare(transactionLimitPerRiskCode, BDUtil.mul(obj.getDbObjectInsuredAmount(),getDbCurrencyRate()))>=0;
            else
                enoughLimit = Tools.compare(transactionLimit, BDUtil.mul(obj.getDbObjectInsuredAmount(),getDbCurrencyRate()))>=0;
  
            if(!enoughLimit){
                stUnderwritingFinishFlag = "N";
                stEffectiveFlag = "N";
                //getRemoteInsurance().saveInputPaymentDate(this, approvalMode);
                throw new RuntimeException("Nilai TSI Objek No ("+(i+1)+") "+ object2.getStReference1()+"  Melebihi Limit Kewenangan Anda");
            }
        }
    }
    ...
    public BigDecimal getPPHRate(String taxCode, boolean hasNPWP, Date dtPeriodStart) throws Exception {
        ...
        if(hasNPWP) rateSelect = "rate1_npwp";
        else rateSelect = "rate1";
    ...
    public String getPolicyNoOldFormat(String nopolis) throws Exception{
        ...
        if(enos.equalsIgnoreCase("01")) nopol = nopolis.substring(0,16)+"A";
        else if(enos.equalsIgnoreCase("02")) nopol = nopolis.substring(0,16)+"B";
        else if(enos.equalsIgnoreCase("03")) nopol = nopolis.substring(0,16)+"C";
        else if(enos.equalsIgnoreCase("04")) nopol = nopolis.substring(0,16)+"D";
        else if(enos.equalsIgnoreCase("05")) nopol = nopolis.substring(0,16)+"E";
        else if(enos.equalsIgnoreCase("06")) nopol = nopolis.substring(0,16)+"F";
        else if(enos.equalsIgnoreCase("07")) nopol = nopolis.substring(0,16)+"G";
        else if(enos.equalsIgnoreCase("08")) nopol = nopolis.substring(0,16)+"H";
        else if(enos.equalsIgnoreCase("09")) nopol = nopolis.substring(0,16)+"I";
        else if(enos.equalsIgnoreCase("10")) nopol = nopolis.substring(0,16)+"J";
        else if(enos.equalsIgnoreCase("11")) nopol = nopolis.substring(0,16)+"K";
        else if(enos.equalsIgnoreCase("12")) nopol = nopolis.substring(0,16)+"L";
        else if(enos.equalsIgnoreCase("13")) nopol = nopolis.substring(0,16)+"M";
        else if(enos.equalsIgnoreCase("14")) nopol = nopolis.substring(0,16)+"N";
        else if(enos.equalsIgnoreCase("15")) nopol = nopolis.substring(0,16)+"O";
        else if(enos.equalsIgnoreCase("16")) nopol = nopolis.substring(0,16)+"P";
        else if(enos.equalsIgnoreCase("17")) nopol = nopolis.substring(0,16)+"Q";
        else if(enos.equalsIgnoreCase("18")) nopol = nopolis.substring(0,16)+"R";
        else if(enos.equalsIgnoreCase("19")) nopol = nopolis.substring(0,16)+"S";
        else if(enos.equalsIgnoreCase("20")) nopol = nopolis.substring(0,16)+"T";
        else if(enos.equalsIgnoreCase("21")) nopol = nopolis.substring(0,16)+"U";
        else if(enos.equalsIgnoreCase("22")) nopol = nopolis.substring(0,16)+"V";
        else if(enos.equalsIgnoreCase("23")) nopol = nopolis.substring(0,16)+"W";
        else if(enos.equalsIgnoreCase("24")) nopol = nopolis.substring(0,16)+"X";
        else if(enos.equalsIgnoreCase("25")) nopol = nopolis.substring(0,16)+"Y";
        else if(enos.equalsIgnoreCase("26")) nopol = nopolis.substring(0,16)+"Z";
        else if(enos.equalsIgnoreCase("27")) nopol = nopolis.substring(0,16)+"a";
        else if(enos.equalsIgnoreCase("28")) nopol = nopolis.substring(0,16)+"b";
        else if(enos.equalsIgnoreCase("29")) nopol = nopolis.substring(0,16)+"c";
        else if(enos.equalsIgnoreCase("30")) nopol = nopolis.substring(0,16)+"d";
        else if(enos.equalsIgnoreCase("31")) nopol = nopolis.substring(0,16)+"e";
        else if(enos.equalsIgnoreCase("32")) nopol = nopolis.substring(0,16)+"f";
        else if(enos.equalsIgnoreCase("33")) nopol = nopolis.substring(0,16)+"g";
        else if(enos.equalsIgnoreCase("34")) nopol = nopolis.substring(0,16)+"h";
        else if(enos.equalsIgnoreCase("35")) nopol = nopolis.substring(0,16)+"i";
        else if(enos.equalsIgnoreCase("36")) nopol = nopolis.substring(0,16)+"j";
        else if(enos.equalsIgnoreCase("37")) nopol = nopolis.substring(0,16)+"k";
        else if(enos.equalsIgnoreCase("38")) nopol = nopolis.substring(0,16)+"l";
        else if(enos.equalsIgnoreCase("39")) nopol = nopolis.substring(0,16)+"m";
        else if(enos.equalsIgnoreCase("40")) nopol = nopolis.substring(0,16)+"n";
        else if(enos.equalsIgnoreCase("41")) nopol = nopolis.substring(0,16)+"o";
        else if(enos.equalsIgnoreCase("42")) nopol = nopolis.substring(0,16)+"p";
        else if(enos.equalsIgnoreCase("43")) nopol = nopolis.substring(0,16)+"q";
        else if(enos.equalsIgnoreCase("44")) nopol = nopolis.substring(0,16)+"r";
        else if(enos.equalsIgnoreCase("45")) nopol = nopolis.substring(0,16)+"s";
        else if(enos.equalsIgnoreCase("46")) nopol = nopolis.substring(0,16)+"t";
        else if(enos.equalsIgnoreCase("47")) nopol = nopolis.substring(0,16)+"u";
        else if(enos.equalsIgnoreCase("48")) nopol = nopolis.substring(0,16)+"v";
        else if(enos.equalsIgnoreCase("49")) nopol = nopolis.substring(0,16)+"w";
        else if(enos.equalsIgnoreCase("50")) nopol = nopolis.substring(0,16)+"x";
        else if(enos.equalsIgnoreCase("51")) nopol = nopolis.substring(0,16)+"y";
        else if(enos.equalsIgnoreCase("52")) nopol = nopolis.substring(0,16)+"z";
        else nopol = nopolis;
    ...
    public String convertOldPolicyNoToNewFormat(String nopolis) throws Exception{
        ...
        if(stLastEndorsemen!=null){
            ...
            if(enos!=null){
                if(enos.equals("A")) nopol = nopolis.substring(0,16)+"01";
                else if(enos.equals("B")) nopol = nopolis.substring(0,16)+"02";
                else if(enos.equals("C")) nopol = nopolis.substring(0,16)+"03";
                else if(enos.equals("D")) nopol = nopolis.substring(0,16)+"04";
                else if(enos.equals("E")) nopol = nopolis.substring(0,16)+"05";
                else if(enos.equals("F")) nopol = nopolis.substring(0,16)+"06";
                else if(enos.equals("G")) nopol = nopolis.substring(0,16)+"07";
                else if(enos.equals("H")) nopol = nopolis.substring(0,16)+"08";
                else if(enos.equals("I")) nopol = nopolis.substring(0,16)+"09";
                else if(enos.equals("J")) nopol = nopolis.substring(0,16)+"10";
                else if(enos.equals("K")) nopol = nopolis.substring(0,16)+"11";
                else if(enos.equals("L")) nopol = nopolis.substring(0,16)+"12";
                else if(enos.equals("M")) nopol = nopolis.substring(0,16)+"13";
                else if(enos.equals("N")) nopol = nopolis.substring(0,16)+"14";
                else if(enos.equals("O")) nopol = nopolis.substring(0,16)+"15";
                else if(enos.equals("P")) nopol = nopolis.substring(0,16)+"16";
                else if(enos.equals("Q")) nopol = nopolis.substring(0,16)+"17";
                else if(enos.equals("R")) nopol = nopolis.substring(0,16)+"18";
                else if(enos.equals("S")) nopol = nopolis.substring(0,16)+"19";
                else if(enos.equals("T")) nopol = nopolis.substring(0,16)+"20";
                else if(enos.equals("U")) nopol = nopolis.substring(0,16)+"21";
                else if(enos.equals("V")) nopol = nopolis.substring(0,16)+"22";
                else if(enos.equals("W")) nopol = nopolis.substring(0,16)+"23";
                else if(enos.equals("X")) nopol = nopolis.substring(0,16)+"24";
                else if(enos.equals("Y")) nopol = nopolis.substring(0,16)+"25";
                else if(enos.equals("Z")) nopol = nopolis.substring(0,16)+"26";
                else if(enos.equals("a")) nopol = nopolis.substring(0,16)+"27";
                else if(enos.equals("b")) nopol = nopolis.substring(0,16)+"28";
                else if(enos.equals("c")) nopol = nopolis.substring(0,16)+"29";
                else if(enos.equals("d")) nopol = nopolis.substring(0,16)+"30";
                else if(enos.equals("e")) nopol = nopolis.substring(0,16)+"31";
                else if(enos.equals("f")) nopol = nopolis.substring(0,16)+"32";
                else if(enos.equals("g")) nopol = nopolis.substring(0,16)+"33";
                else if(enos.equals("h")) nopol = nopolis.substring(0,16)+"34";
                else if(enos.equals("i")) nopol = nopolis.substring(0,16)+"35";
                else if(enos.equals("j")) nopol = nopolis.substring(0,16)+"36";
                else if(enos.equals("k")) nopol = nopolis.substring(0,16)+"37";
                else if(enos.equals("l")) nopol = nopolis.substring(0,16)+"38";
                else if(enos.equals("m")) nopol = nopolis.substring(0,16)+"39";
                else if(enos.equals("n")) nopol = nopolis.substring(0,16)+"40";
                else if(enos.equals("o")) nopol = nopolis.substring(0,16)+"41";
                else if(enos.equals("p")) nopol = nopolis.substring(0,16)+"42";
                else if(enos.equals("q")) nopol = nopolis.substring(0,16)+"43";
                else if(enos.equals("r")) nopol = nopolis.substring(0,16)+"44";
                else if(enos.equals("s")) nopol = nopolis.substring(0,16)+"45";
                else if(enos.equals("t")) nopol = nopolis.substring(0,16)+"46";
                else if(enos.equals("u")) nopol = nopolis.substring(0,16)+"47";
                else if(enos.equals("v")) nopol = nopolis.substring(0,16)+"48";
                else if(enos.equals("w")) nopol = nopolis.substring(0,16)+"49";
                else if(enos.equals("x")) nopol = nopolis.substring(0,16)+"50";
                else if(enos.equals("y")) nopol = nopolis.substring(0,16)+"51";
                else if(enos.equals("z")) nopol = nopolis.substring(0,16)+"52";
                else nopol = nopolis;
            }
        }else{
            nopol = nopol;
    ...
    public DTOList getCoinsCoverage() {
        if (coinsCoverage == null) loadCoinsCoverage();
    ...
    public String getPolicyNoOldFormat2(String nopolis) throws Exception{
        ...
        else if(enos.equalsIgnoreCase("00")) nopol = nopolis.substring(0,16);
        else nopol = nopolis;
    ...
    public DTOList getCoverageReinsurance() {
        if (coverageReins == null) loadCoverageReinsurance();
    ...
    public void recalculateCoverageReinsurance() throws Exception{
        ...
        if(!getStPolicyTypeID().equals("21")) return;
        ...
        for (int j = 0; j < objects.size(); j++) {
            ...
            for(int k = 0; k < covers.size(); k++){
                ...
                if(cov.getStInsuranceCoverID().equalsIgnoreCase("78")) premiKreasi = BDUtil.add(premiKreasi, cov.getDbPremi());
                
                if(cov.getStInsuranceCoverID().equalsIgnoreCase("124") || cov.getStInsuranceCoverID().equalsIgnoreCase("125")) premiPHK = BDUtil.add(premiPHK, cov.getDbPremi());
            }
        }
        ...
        try {
            ...
            if (objects_year == null) {
                ...
                for (int i = 0; i < objects_year.size(); i++) {
                    InsurancePolicyObjectPreEndorseView obj = (InsurancePolicyObjectPreEndorseView) objects_year.get(i);
    ...
    public void recalculateCoinsCoverage2() throws Exception{
    ...
    public void validateLimit(boolean raiseErrors) {
        ...
        if (!(isStatusEndorse() || isStatusPolicy() || isStatusRenewal() || isStatusEndorseRI()|| isStatusEndorseIntern())) return;
        ...
        try {
            ...
            if (objects_koas == null) {
                ...
                for (int i = 0; i < objects_koas.size(); i++) {
                    InsurancePolicyObjectPreEndorseView obj = (InsurancePolicyObjectPreEndorseView) objects_koas.get(i);
        ...
        try {
            if (claimItemsFee == null)
                claimItemsFee = ListUtil.getDTOListFromQuery(
                        "select * from ins_pol_items where pol_id = ? and item_class='CLM' and ins_item_id = 50 order by ins_pol_item_id",
                        new Object[]{stPolicyID},
                        InsurancePolicyItemsView.class
                        );
        } catch (Exception e) {
        ...
        try {
            if (rootclaim == null)
                rootclaim = ListUtil.getDTOListFromQuery(
                        "select pol_no "+
                        " from ins_policy "+
                        " where pol_no like ? and active_flag='Y' and effective_flag = 'Y' order by pol_no desc limit 1",
    ...
    public void checkLastPolicyNo(String stPolicyNo) throws Exception{
        ...
        for (int i = 0; i < claim.size(); i++) {
            InsurancePolicyView pol = (InsurancePolicyView) claim.get(i);
    ...
    public boolean isBatalTotalEndorseMode() {
        return "BATAL".equalsIgnoreCase(getStEndorseMode());
    }
    ...
    public void recalculateBasicMandiri() throws Exception {
        ...
        if(year<=1) setDbPeriodRate(BDUtil.one);
        else if(year>1) setDbPeriodRate(new BigDecimal(year));
        //end
        ...
        if ((objects != null) && (objects.size() > 0)) {
                ...
                //penentuan coas kreasi ke BAJ /JS
                   ...
                   if(getStPolicyTypeID().equalsIgnoreCase("21")){
                        ...
                        if(isStatusHistory() || isStatusEndorse()){
                            if(getStCoinsID()!=null)
                                if(objx.getStReference8()==null) objx.setStReference8(getStCoinsID());
                        }
            ...
            if(getStPolicyTypeID().equalsIgnoreCase("21")){
                if(isStatusClaim() && getClaimObject()!=null){
                    setCoasAJS(false);
                    setCoasJS(false);
                    InsurancePolicyObjDefaultPreEndorseView obje = (InsurancePolicyObjDefaultPreEndorseView) getClaimObject();
    ...
    public boolean validateClaimObject(InsurancePolicyObjectView claimObj) throws Exception{
        ...
        if(BDUtil.isZeroOrNull(totalTSI)) canNotClaim = true;
...
//        return view.getStUserName();
    ...
    public void validateDoubleObjectKreasi()throws Exception{
        if(getStPolicyTypeID().equalsIgnoreCase("21")){
            if(isStatusDraft() || isStatusSPPA() || isStatusPolicy()){
                ...
                for (int i = 0; i < object.size(); i++) {
                    ...
                    String param = objx.getStReference1()+ DateUtil.getDateStr7(objx.getDtReference1()) + DateUtil.getDateStr7(objx.getDtReference2());
                    ...
                    try {
                        ...
                        final PreparedStatement PS = S.setQuery("select checking, pol_no "+
                                                    " from ins_pol_kreasi_obj "+
                                                    " where checking = ? "+
                                                    " limit 1");
                        ...
                        if (RS.next()) {
                            desc = desc + "Debitur "+objx.getStReference1()+" sudah pernah di input pada nopol :"+ RS.getString("pol_no") +"<br>";
                            isDouble = true;
                            /*logger.logDebug("++++++++++++++ CHECKING DOUBLE FOUND ++++++++++++");
                            logger.logDebug("params : "+ param);
                            logger.logDebug("++++++++++++++ CHECKING DOUBLE FOUND ++++++++++++");*/
                        }
    ...
    public boolean isFireEndorseNeedManualReinsurance(){
        ...
        if(!stPolicyTypeGroupID.equalsIgnoreCase("1")) return false;
        if(!isStatusEndorse()) return false;
        
        boolean isLongPolice = false;
        if(length>1) isLongPolice = true;
        return isLongPolice;
    }
    ...
    public void generatePolicyNoFromPersetujuanPrinsip() throws Exception {
        ...
        if(stReference1 == null) throw new Exception("Nomor persetujuan prinsip masih kosong");
    ...
    private void invokeCustomHandlersPolicy(boolean validate) {
        if(objects.size()>0){
    ...
    public String getUserIDSign(){
        ...
        String NIPPimpinan = Parameter.readString("BRANCH_NAME_"+ getStCostCenterCode());

        if(stApprovedWho==null) stApprovedWho = NIPPimpinan;
    ...
    public String getParaf(){
        ...
        String NIPPimpinan = Parameter.readString("BRANCH_NAME_"+ getStCostCenterCode()); //04740297
    ...
    public EntityView getProducer() {
        ...
        if (entity != null)
            if (!Tools.isEqual(entity.getStEntityID(), stProducerID)) entity = null;
    ...
    public void cekPeriodeAwal(){
        ...
        if(!isStatusClaim() && !isStatusEndorse() && !isStatusHistory() && !isStatusClaimEndorse()){
            if((polDate.getYear() - startDate.getYear()) > 7)
                throw new RuntimeException("Cek periode awal polis apakah sudah betul");
            ...
            if (detailsInduk == null)
                if(getStPolicyNo()!=null){
                    detailsInduk = ListUtil.getDTOListFromQuery(
                        ...
                        new Object[]{getStPolicyNo().substring(0, 16)},
                        InsurancePolicyItemsView.class
                        );
                }else{
                    detailsInduk = ListUtil.getDTOListFromQuery(
        ...
        if (isModified()) {
            if (isStatusClaimDLA() || isStatusClaimEndorse()) {
                ...
                if (dtPolicyDate != null) {
                    ...
                    if(currentDate.isAfter(maximumBackDate))
                          if(canEntryBackDate) throw new RuntimeException("Batas waktu 3 hari approval setelah akhir bulan sudah terlewati");
    ...
    public boolean isReverseReinsurance() {
        boolean cek = false;
        if(stRIPostedFlag==null) cek =  false;

        if(stRIPostedFlag!=null){
            if(stRIPostedFlag.equalsIgnoreCase("N")) cek =  true;
            if(stRIPostedFlag.equalsIgnoreCase("Y")) cek =  false;
        }
    ...
    public DTOList getClaimDocuments2() {
        if (claimDocuments == null && stPolicyID != null && stPolicyTypeID != null)
            claimDocuments = loadDocuments2(stPolicyTypeID, stPolicyID, "CLAIM");
        return claimDocuments;
    }

    private static DTOList loadDocuments2(String poltype, String stPolicyID, String documentClass) {
        try {
            ...
            final DTOList l = ListUtil.getDTOListFromQuery(
                    ...
                    "      ins_documents a" +
                    "      inner join ins_document_type c on c.ins_document_type_id = a.ins_document_type_id" +
                    "      inner join ins_pol_documents b on  b.document_class=a.document_class and b.policy_id=? and b.ins_document_type_id=a.ins_document_type_id" +
                    "   where " +
                    "      a.pol_type_id=? " +
                    "      and a.document_class=? ",
                    new Object[]{stPolicyID, poltype, documentClass},
    ...
    private DTOList claimFiles;

    public DTOList getClaimFiles() {
        if (claimFiles == null && stPolicyID != null && stPolicyTypeID != null)
            claimFiles = loadFiles(stPolicyID);
        return claimFiles;
    }

    private static DTOList loadFiles(String stPolicyID) {
        try {

            final DTOList l = ListUtil.getDTOListFromQuery(
                    "  select b.* "+
                    " from ins_pol_documents a "+
                    " inner join s_files b on a.file_physic = b.file_id "+
                    " where a.policy_id = ? ",
                    new Object[]{stPolicyID},
                    FileView.class
                    );
    ...
    public void generatePrePLANo() throws Exception {
        ...
        orderNo = StringTools.leftPad(String.valueOf(IDFactory.createNumericID("preclaim" + policyType2Digit + month2Digit + year2Digit + ccCode, 1)), '0', 4);
        ...
        stPLANo = "PRECLAIM/" + getDigit(policyType2Digit, 2) + "/" +
                ccCode + "/" + month2Digit + year2Digit + "/" +
                orderNo;
    ...
    /**
     * @return the stPreEndorseNo
     */
    public String getStPreEndorseNo() {
        return stPreEndorseNo;
    }

    /**
     * @param stPreEndorseNo the stPreEndorseNo to set
     */
    public void setStPreEndorseNo(String stPreEndorseNo) {
        this.stPreEndorseNo = stPreEndorseNo;
    }
