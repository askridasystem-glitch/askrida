...
public class InsuranceCashCallView extends DTO implements RecordAudit {
   /*
...
ALTER TABLE ins_treaty ADD COLUMN active_flag varchar(1);
   ...
   */
   ...
   public static String tableName = "ins_cashcall";

   public static String fieldMap[][] = {
      {"stInsuranceTreatyID","ins_cashcall_id*pk"},
      {"stTreatyName","cashcall_name"},
      ...
      try {
         if (details == null) {
            details = ListUtil.getDTOListFromQuery(
                    ...
                    "      a.* " +
                    "   from " +
                    "      ins_cashcall_detail a" +
                    "   where " +
                    "      a.ins_cashcall_id = ? and a.policy_type_id = ? and coalesce(a.delete_flag,'N') <> 'Y'" +
                    "   order by" +
                    "      a.ins_cashcall_detail_id",
                    new Object [] {stInsuranceTreatyID, stPolicyTypeID},
                    InsuranceCashCallDetailView.class
            );
 

            HashMap mapByID = details.getMapOf("ins_cashcall_detail_id");
            ...
            for (int i = 0; i < details.size(); i++) {
               ...
               if (d.getStParentID()!=null) {
                  InsuranceCashCallDetailView parent = (InsuranceCashCallDetailView) mapByID.get(d.getStParentID());
