...
import com.webfin.care.model.StatusBayarCareView;
...
public class InsurancePolicyView extends DTO implements RecordAudit, RecordBackup {
    /*
    ...
    public static String fieldMap[][] = {
        ...
        {"stPaymentCompanyID", "payment_company_id"},
        {"stReceiptNoTitipan", "receipt_no_titipan"},
        {"dbRealisasiAmountTotal", "realisasi_amount_total"},
        {"stCacCbcFlag", "cac_cbc_flag"},
        {"stClaimCashCallFlag", "claim_cash_call_f"},
        {"stClaimRestructFlag", "claim_restruct_f"},
        {"stBusinessSourceID", "business_source_id"},
        {"dbTSI100", "tsi_100"},
        {"dbClaim100", "claim_100"},
        {"stMigrasiFlag", "migrasi_f"},
...
//    private DTOList list1;
    ...
    private String stPaymentCompanyID;
    private String stReceiptNoTitipan;
    private BigDecimal dbRealisasiAmountTotal;
    private String stCacCbcFlag;
    private String stClaimCashCallFlag;

    private boolean editReas = false;
    private boolean isProgressifTaxComm = true;

    private String stClaimRestructFlag;
    private String stBusinessSourceID;
    private BigDecimal dbTSI100;
    private BigDecimal dbClaim100;
    private String stMigrasiFlag;

    public String getStMigrasiFlag() {
        return stMigrasiFlag;
    }

    public void setStMigrasiFlag(String stMigrasiFlag) {
        this.stMigrasiFlag = stMigrasiFlag;
    }

    public BigDecimal getDbClaim100() {
        return dbClaim100;
    }

    public void setDbClaim100(BigDecimal dbClaim100) {
        this.dbClaim100 = dbClaim100;
    }

    public BigDecimal getDbTSI100() {
        return dbTSI100;
    }

    public void setDbTSI100(BigDecimal dbTSI100) {
        this.dbTSI100 = dbTSI100;
    }

    public String getStBusinessSourceID() {
        return stBusinessSourceID;
    }

    public void setStBusinessSourceID(String stBusinessSourceID) {
        this.stBusinessSourceID = stBusinessSourceID;
    }

    public String getStClaimRestructFlag() {
        return stClaimRestructFlag;
    }

    public void setStClaimRestructFlag(String stClaimRestructFlag) {
        this.stClaimRestructFlag = stClaimRestructFlag;
    }

    public String getStClaimCashCallFlag() {
        return stClaimCashCallFlag;
    }

    public void setStClaimCashCallFlag(String stClaimCashCallFlag) {
        this.stClaimCashCallFlag = stClaimCashCallFlag;
    }

    public boolean isEditReas() {
        return editReas;
    }

    public void setEditReas(boolean editReas) {
        this.editReas = editReas;
    }
    

    public String getStCacCbcFlag() {
        return stCacCbcFlag;
    }

    public void setStCacCbcFlag(String stCacCbcFlag) {
        this.stCacCbcFlag = stCacCbcFlag;
    }

    public BigDecimal getDbRealisasiAmountTotal() {
        return dbRealisasiAmountTotal;
    }

    public void setDbRealisasiAmountTotal(BigDecimal dbRealisasiAmountTotal) {
        this.dbRealisasiAmountTotal = dbRealisasiAmountTotal;
    }

    public String getStReceiptNoTitipan() {
        return stReceiptNoTitipan;
    }

    public void setStReceiptNoTitipan(String stReceiptNoTitipan) {
        this.stReceiptNoTitipan = stReceiptNoTitipan;
    }

    public String getStPaymentCompanyID() {
        return stPaymentCompanyID;
    }

    public void setStPaymentCompanyID(String stPaymentCompanyID) {
        this.stPaymentCompanyID = stPaymentCompanyID;
    }
    ...
    private final int maksUsiaKendaraanComprehensive = 5;
    private final int maksUsiaKendaraanTLO = 12;
...
//   ALTER TABLE ins_policy ADD COLUMN inst_periods int8;
    ...
    private DTOList installmentsNotPaid;
    ...
    private static DTOList loadPolicyDocuments(String poltype, String stPolicyID, String documentClass) {
        try {
            ...
            final DTOList l = ListUtil.getDTOListFromQuery(
                    ...
                    + "   where "
                    + "      a.pol_type_id=? "
                    + "      and a.document_class=? order by a.ins_document_type_id",
    ...
    public void recalculate() throws Exception {
        ...
        if (isStatusPolicy() || isStatusClaim() || isStatusEndorse() || isStatusRenewal() || isStatusEndorseRI() || isStatusEndorseIntern() || isStatusTemporaryPolicy() || isStatusTemporaryEndorsemen() || isStatusInward() || isStatusClaimInward() || isStatusEndorseClaimInward()) {
    ...
    public void recalculateInterkoneksi() throws Exception {
        ...
        if (isStatusPolicy() || isStatusClaim() || isStatusEndorse() || isStatusRenewal() || isStatusEndorseRI() || isStatusEndorseIntern() || isStatusTemporaryPolicy() || isStatusTemporaryEndorsemen() || isStatusInward()) {
            validateTreatyInterkoneksi(false);
        }
    ...
    public void recalculateClaim() throws Exception {
        ...
        if (isStatusClaim() || isStatusClaimEndorse() || isStatusClaimInward() || isStatusEndorseClaimInward()) {
            ...
            if (claimItems != null) {
                for (int i = 0; i < claimItems.size(); i++) {
                    ...
                    final boolean insentifSubrogasi = Tools.isEqual(it.getInsuranceItem().getStItemType(), "INSENTIFSUBRO");
                    ...
                    } else if (isStatusClaimEndorse() || isStatusEndorseClaimInward()) {
...
//                            setDbClaimAmountEstimate(getDbClaimAmountApproved());
                        ...
                        //jika insentif subrogasi, hitung dari % x nilai subro
                        if(insentifSubrogasi){

                            if(it.getStReferencePolicyID()==null)
                                throw new RuntimeException("No LKP Subrogasi belum diinput");

                            if(it.getStReferencePolicyID()!=null){
                                InsurancePolicyView dlaSubrogasi = it.getReferencePolicy();

                                final DTOList dlaItems = dlaSubrogasi.getClaimItems();

                                BigDecimal nilaiSubrogasi = BDUtil.zero;
                                BigDecimal nilaiFee = BDUtil.zero;

                                for (int j = 0; j < dlaItems.size(); j++) {
                                    InsurancePolicyItemsView det = (InsurancePolicyItemsView) dlaItems.get(j);

                                    if(det.isSubrogasi())
                                        nilaiSubrogasi = det.getDbAmount();

                                    if(det.isFeeRecovery())
                                        nilaiFee = det.getDbAmount();

                                }

                                if(it.isOfGross())
                                    claimAmount = nilaiSubrogasi;
                                else
                                    claimAmount = BDUtil.sub(nilaiSubrogasi, nilaiFee);

                                //Cek tanggal pengajuan to tanggal approval subrogasi
                                int jumlahHariPengajuan = DateUtil.getDaysAmount(dlaSubrogasi.getDtApprovedClaimDate(), getDtDLADate());

                                if(jumlahHariPengajuan > 90)
                                    throw new RuntimeException("Tanggal LKP Insentif Subrogasi > 90 hari sejak LKP Subrogasi disetujui ("+ dlaSubrogasi.getStDLANo() +" | "+ DateUtil.getDateStr(dlaSubrogasi.getDtApprovedClaimDate())+")");
...
//                        totalTSI2 = totalTSITreaty;
    ...
    public void recalculateBasic() throws Exception { 
        ...
        if ((objects != null) && (objects.size() > 0)){
            if(size > 3000){
                InsurancePolicyObjectView obj1 = (InsurancePolicyObjectView) objects.get(0);
        ...
        for (int j = 0; j < objects.size(); j++) {
            ...
            if (getPolicyType().getStControlFlags() != null) {
                if (bulanProduksi.isAfter(bulanOJK)) {
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if (item.isPPN() || item.isPPNFeeBase() || item.isPPNComission()) {
                continue;
            }
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if(usePMKBaru){
                if (item.isPPNInclude()) {
                    ...
                    if(isBankMantap())
                        afterDiscount = BDUtil.mul(BDUtil.div(new BigDecimal(100), new BigDecimal(111), 15), afterDiscount, scale);
                    else
        ...
        if(isProgressifTaxComm)
            calculatePPH21Progresif2025Method2();
        ...
        recalculateTitipanPremi();

    }

    public void recalculateBasicSpreading() throws Exception {
        if (stPolicyTypeID == null) {
            return;
        }
    ...
    private void recalculateFee() throws Exception {
        ...
        BigDecimal dbTotalKomisi = null;
        BigDecimal dbTotalKomisiWithPpn = null;
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if (item.isComission2()) {
                dbTotalKomisi = BDUtil.add(dbTotalKomisi, item.getDbAmount());
            }

             if (item.isComission2() && item.isIncludePPN()) {
                dbTotalKomisiWithPpn = BDUtil.add(dbTotalKomisiWithPpn, item.getDbAmount());
            }
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if(!isStatusHistory() && !isStatusEndorse() && !isStatusClaim() && !isStatusClaimEndorse()){
                if(item.isPPN() || item.isPPNFeeBase() || item.isPPNComission()){
                    BigDecimal ratePPN = getPPNRate(item.getStInsItemID(), getDtPeriodStart());

                    if(ratePPN!=null){
                        item.setStFlagEntryByRate("Y");
                        item.setDbRate(ratePPN);
                    }
                }
            }
            

            if (item.isHandlingFee() || item.isStampFee() || item.isPolicyCost() || item.isPPN() || item.isPPNFeeBase() || item.isPPNComission()) {
                if (item.isHandlingFee()) {
                    ...
                    dbNDPPN = BDUtil.add(dbNDPPN,item.getDbAmount());
                } else if (item.isPPNFeeBase()) {
                    item.calculateRateAmount(dbTotalFeeBase, scale);
                    dbNDPPN = BDUtil.add(dbNDPPN,item.getDbAmount());
                }else if (item.isPPNComission()) {
                    item.calculateRateAmount(dbTotalKomisiWithPpn, scale);
                    dbNDPPN = BDUtil.add(dbNDPPN,item.getDbAmount());
                }
            } else {
                continue;
    ...
    private void calculatePPH21Progresif2025() throws Exception {

        if(!isStatusClaim() && !isStatusClaimEndorse() && !isStatusClaimInward() && !isStatusInward() && !isStatusEndorseRI()){

            DateTime tglAktifProgresif = new DateTime(DateUtil.getDate("31/07/2025"));
            DateTime tglPolis = new DateTime(getDtPolicyDate());

            //JIKA SETELAH TGL 01/08/2025
            if(tglPolis.isAfter(tglAktifProgresif)){

                //cek polis induk
                boolean isUseTarifLama = false;

                if(isStatusEndorse()){
                    String queryCek = "select pol_id,pol_no,policy_date "+
                                  "   from ins_policy  "+
                                  "  where active_flag = 'Y' and effective_flag = 'Y' and status in ('POLICY','RENEWAL')  "+
                                  "  and date_trunc('day',policy_date) >= '2025-01-01 00:00:00' and date_trunc('day',policy_date) <= '2025-07-31 00:00:00'";

                    if(getStPolicyNo().length()==18)
                        queryCek = queryCek + " and substr(pol_no,0,17) = '"+ getStPolicyNo().substring(0,16)+"'";
                    else
                        queryCek = queryCek + " and substr(pol_no,0,18) = '"+ getStPolicyNo().substring(0,17)+"'";


                    InsurancePolicyView polisInduk2025 = (InsurancePolicyView)
                            ListUtil.getDTOListFromQuery(queryCek, new Object[]{},InsurancePolicyView.class).getDTO();

                    if(polisInduk2025!=null) isUseTarifLama = true;

                     logger.logDebug("#################### CEK ENDORSE TARIF LAMA =  "+ isUseTarifLama);
                }



                logger.logDebug("#################### MASUK PERHITUNGAN PPH21 PROGRESSIF 2025 ###################");
                ...
                for (int i = 0; i < details.size(); i++) {
                    ...
                    if(isStatusEndorse() && isUseTarifLama) continue;

                    if(!item.isAutoTaxAmount()) continue;

                    if(!item.isComission2()) continue;

                    //jika bukan pph 21 maka continue
                    if (!"1".equalsIgnoreCase(item.getStTaxCode())&& !"4".equalsIgnoreCase(item.getStTaxCode()))
                        continue;
                    ...
                    //PERHITUNGAN PAJAK KOMISI AGEN PROGRESIF

                    if(item.isKomisi()){

                        //GET AKUMULASI KOMISI PER BULAN AGEN TSB
                        BigDecimal akumulasiKomisi = getAkumulasiKomisiBulanan(item.getStEntityID(), getDtPolicyDate());
                        BigDecimal dpp = BDUtil.mul(item.getDbAmount(), new BigDecimal(0.5), 0);
                        BigDecimal dppSaatIni = BDUtil.add(BDUtil.mul(akumulasiKomisi, new BigDecimal(0.5), 0),dpp);

                        //BigDecimal dppAkumulasi = getAkumulasiPPhBulanan(item.getStEntityID(), getDtPolicyDate());

                        logger.logDebug("############### DPP = "+ NumberUtil.getMoneyStr(dpp,2));
                        logger.logDebug("############### DPP SAMPAI SAAT INI = "+ NumberUtil.getMoneyStr(dppSaatIni,2));

                        logger.logDebug("############### REVERSE SIZE = "+ getReverse().size());

                        boolean isPernahReverse = false;

                        if(getReverse()!=null){
                            if(getReverse().size()>0){
                                isPernahReverse = true;
                            }
                        }


                            if(isPernahReverse){
                                dppSaatIni = getDPPSdPolisIni(item.getStEntityID(), getDtPolicyDate(),item.getDbAmount());

                                logger.logDebug("############### POLIS PERNAH REVERSE, DPP SAMPAI SAAT POLIS INI = "+ NumberUtil.getMoneyStr(dppSaatIni,2));
                            }
                        


                        BigDecimal rateProgresif = getPPHProgressiveRate(dppSaatIni, getDtPolicyDate());

                        if (item.isAutoTaxRate()) {
                            item.setDbTaxRate(rateProgresif);
                        }

                        logger.logDebug("############### AKUMULASI KOMISI = "+ NumberUtil.getMoneyStr(akumulasiKomisi,2));
                       
                        logger.logDebug("############### DPP SAAT INI = "+ NumberUtil.getMoneyStr(dppSaatIni,2));


                        //hitung progresif
                        String[][] taxTab = new String[][]{
                            {"60000000", "0.05", "60000000"},
                            {"250000000", "0.15","190000000"},
                            {"500000000", "0.25","250000000"},
                            {"5000000000", "0.30","4500000000"},
                            {"-1", "0.35"},};

                        BigDecimal taxAmt = null;

                        BigDecimal dbPPH21totS = dppSaatIni;
                        ...
                        for (int j = 0; j < taxTab.length; j++) {
                            ...
                            //BigDecimal lim = new BigDecimal(t[0]);
                            BigDecimal lim = new BigDecimal(t[2]);

                            if (Tools.compare(lim, BDUtil.zero) < 0) {
                                lim = dbPPH21totS;
                            }

                            if (Tools.compare(dbPPH21totS, lim) < 0) {
                                amt = dbPPH21totS;
                            } else {
                                amt = lim;
                            }
                            ...
                            //logger.logDebug("############### amount layer "+ (j+1) +" = "+ NumberUtil.getMoneyStr(amt,2));

                            taxAmt = BDUtil.add(taxAmt, BDUtil.mul(amt, new BigDecimal(t[1]), scale));

                            logger.logDebug("############### tax amount layer "+ (j+1) +" = "+ NumberUtil.getMoneyStr(taxAmt,2));


                            dbPPH21totS = BDUtil.sub(dbPPH21totS, amt);

                            if (Tools.compare(dbPPH21totS, BDUtil.zero) <= 0) {
                                break;
                            }
                        }

                        taxAmount = BDUtil.add(taxAmount, BDUtil.mul(dppSaatIni, item.getDbTaxRate()));

                        //taxAmount = taxAmt;

                        item.setDbPph21SampaiSaatIni(taxAmt);

                        logger.logDebug("############### PPH21 sampai SAAT INI = "+ NumberUtil.getMoneyStr(taxAmt,2));

                        if(isPernahReverse){
                             //dppAkumulasi = getAkumulasiPPhBulananAfterReverse(item.getStEntityID(), getDtPolicyDate(), taxAmt);
                        }

                        BigDecimal dppAkumulasi = getAkumulasiPPhBulananAfterReverse(item.getStEntityID(), getDtPolicyDate(), taxAmt);
                        

                        logger.logDebug("############### PPH21 AKUMULASI SD POLIS SEBELUM = "+ NumberUtil.getMoneyStr(dppAkumulasi,2));

                        taxAmount = BDUtil.sub(taxAmt, dppAkumulasi);


                        logger.logDebug("############### PPH21 POLIS TERKAIT = "+ NumberUtil.getMoneyStr(taxAmount,2));

                        
                        if(BDUtil.biggerThanZero(item.getDbAmount()))
                            if(BDUtil.lesserThanZero(taxAmount))
                                throw new RuntimeException("Komisi Plus Namun Pph Minus, cek ulang data");
                        

                        if(BDUtil.biggerThan(taxAmount, item.getDbAmount())){
                            //throw new RuntimeException("Pph21 Lebih Besar dari Komisi, cek ulang data");
                        }
    ...
    public BigDecimal getDPPSdPolisIni(String stEntityID, Date dtTanggalPolis, BigDecimal komisi)throws Exception{

        DateTime tanggalPalingAwal = new DateTime(dtTanggalPolis);
        tanggalPalingAwal = tanggalPalingAwal.withDayOfMonth(1);

        DateTime tanggalPalingAkhir = new DateTime(dtTanggalPolis);
        tanggalPalingAkhir = tanggalPalingAkhir.dayOfMonth().withMaximumValue();

        DTOList outgo = (DTOList) ListUtil.getDTOListFromQuery(" select b.* "+
                                                               "     from ins_policy a   "+
                                                               "      left join ins_pol_items b on a.pol_id = b.pol_id  "+
                                                               "      left join ent_master c on b.ent_id = c.ent_id "+
                                                               "      where status in ('POLICY','ENDORSE','RENEWAL') and a.active_flag = 'Y' and effective_flag = 'Y'  "+
                                                               "      and ins_item_id in (11,18,25,32,112,116) and b.tax_code in (1,4)  "+
                                                               "      and b.ent_id = ?"+
                                                               "      and date_trunc('day',policy_date) >= ? and date_trunc('day',policy_date) <= ? "+
                                                               "      order by b.pph21_sd_saat_ini",
                    new Object[]{stEntityID, tanggalPalingAwal.toDate(), tanggalPalingAkhir.toDate()},InsurancePolicyItemsView.class);

        BigDecimal dppSdSaatIni = null;
        BigDecimal totalDPP = null;

        for (int i = 0; i < outgo.size(); i++) {
            InsurancePolicyItemsView it = (InsurancePolicyItemsView) outgo.get(i);

            if(it.getStPolicyID().equalsIgnoreCase(getStPolicyID())) break;

            totalDPP = BDUtil.add(totalDPP, it.getDbAmount());
        }

        totalDPP = BDUtil.add(totalDPP, komisi);

        dppSdSaatIni = BDUtil.mul(totalDPP, new BigDecimal(0.5), scale);

        //logger.logDebug("############# DPP SD POLIS INI = "+ dppSdSaatIni);
    ...
    public void reCalculateInstallment() throws Exception {
        ...
        //logger.logWarning("############# total = "+ total);
        //logger.logWarning("############# dbTotalDue = "+ dbTotalDue);
        ...
        try {
            if (clausules == null && stPolicyTypeID != null) {
                clausules = ListUtil.getDTOListFromQuery(
                        ...
                        + "   and coalesce(b.active_flag,'Y') <> 'N' "
        ...
        try {
            if (details == null) {
                details = ListUtil.getDTOListFromQuery(
                        "select * from ins_pol_items where pol_id = ? and item_class='PRM' order by ins_pol_item_id",
                        new Object[]{stPolicyID},
                        InsurancePolicyItemsView.class);
            }
        } catch (Exception e) {
    ...
    public void generatePolicyNo() throws Exception {
        ...
        if(policyType2Digit.equalsIgnoreCase("87") || policyType2Digit.equalsIgnoreCase("88")){
    ...
    public void generatePLANo() throws Exception {
        ...
        //orderNo = StringTools.leftPad(String.valueOf(IDFactory.createNumericID("claim" + policyType2Digit + month2Digit + year2Digit + ccCode, 1)), '0', 6);
    ...
    public void generateEndorseNo() {
        ...
        if(stPolicyNo.length()==18)
            enos = stPolicyNo.substring(16, 18);
        else
            enos = stPolicyNo.substring(17, 19);
    ...
    public void validate(boolean isApproving) throws Exception {
        ...
        if (isModified()) {
            if (isStatusClaimDLA() || isStatusClaimEndorse()) {
                        ...
                        if (!canEntryBackDate) {
                            if (!isUserSuperEdit) {
                                if (isAdmin) {
                                    ...
                                    //jika polis khusus TP, set policy date to 29/12/2023
                                    /*
                                    if(getRegion().getStRegionCode().equalsIgnoreCase("99")){
                                            DateTime tglPolisKhususJoda = new DateTime(DateUtil.getDate("29/12/2023"));

                                            Date tglPolisKhusus = tglPolisKhususJoda.toDate();

                                            dtPolicyDate = tglPolisKhusus;
                                        }*/
                                }
                            }
                        }
                ...
                //cek prod AKS/NON AKS
                //jika NON AKS & periode >= 2024, block
                if(!isStatusEndorse() && !isStatusClaim() && !isStatusClaimEndorse() && !isStatusInward() && !isStatusClaimInward() && !isStatusEndorseRI()){
                    
                    if(getPolicyType().getStBusinessTypeID().equalsIgnoreCase("1")){

                        DateTime bulanNonAKS = new DateTime(DateUtil.getDate("31/12/2023"));
                        DateTime tglAwal = new DateTime(getDtPeriodStart());

                        DateTime bulanNonAKS2 = new DateTime(DateUtil.getDate("01/01/2024"));

                        DateTime tglNonAktifNonAKS = new DateTime(DateUtil.getDate("01/08/2025"));
                        DateTime tglNonAktifNonAKS2 = new DateTime(DateUtil.getDate("31/07/2025"));
                        DateTime tglPolis = new DateTime(getDtPolicyDate());

                        //JIKA sebelum TGL 01/08/2025, maka masih bisa ke non aks
                        if(tglPolis.isBefore(tglNonAktifNonAKS)){
                            if(tglAwal.isAfter(bulanNonAKS)){
                                   if(!getStCostCenterCode().equalsIgnoreCase("80"))
                                         throw new RuntimeException("Asuransi COB Non AKS periode awal 2024 dapat di input menggunakan User Non AKS");
                            }

                            if(tglAwal.isBefore(bulanNonAKS2)){
                                   if(getStCostCenterCode().equalsIgnoreCase("80"))
                                         throw new RuntimeException("Asuransi COB Non AKS periode awal < 2024 tidak dapat di input menggunakan kode Non AKS");
                            }
                        }

                        //JIKA SETELAH TGL 1/8/2025 MASIH ADA YG INPUT POLIS BARU KE NON AKS, PROTECT
                        if(tglPolis.isAfter(tglNonAktifNonAKS2)){
                            if(getStCostCenterCode().equalsIgnoreCase("80"))
                                    throw new RuntimeException("Asuransi COB Non AKS >= Agustus 2025 sudah tidak dapat di input menggunakan kode Non AKS, silakan input menggunakan user cabang");
                        }
                ...
                //blocking persetujuan polis multiyears jika bukan user pusat
                if(isApproving){
                    if (isStatusPolicy() || isStatusEndorse() || isStatusRenewal()) {
                        if(getPolicyType().isMultiyearsBlock()){

                            int jumlahTahun = getJumlahTahunPertanggungan();
                            ...
                            if(jumlahTahun>1){
                                ...
                                if(!headOfficeUser)
                                    throw new RuntimeException("Polis COB "+ getPolicyType().getStDescription() +" dengan jangka waktu multiyears (lebih dari 1 tahun) hanya bisa disetujui Kantor Pusat");
                            }
                        }
                    }
                }
        ...
        if (isStatusPolicy()) {
            if (getStPolicyNo() != null) {
                if (getStPolicyNo().length() < 18) {
                    throw new RuntimeException("Format No Polis Harus Minimal 18 Digit");
                }
            }
        }
    ...
    public void validateTreaty(boolean raiseErrors)  throws Exception {
        ...
        String message = "";
        BigDecimal totalPremiMember = BDUtil.zero;
        ...
        if (!(isStatusEndorse() || isStatusPolicy() || isStatusRenewal() || isStatusEndorseRI() || isStatusEndorseIntern() || isStatusTemporaryPolicy() || isStatusTemporaryEndorsemen() || isStatusInward() || isStatusClaimInward() || isStatusEndorseClaimInward())) {
            return;
        }
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            for (int j = 0; j < treatyDetails.size(); j++) {
                ...
                if (!isStatusEndorse()) {
                    if (getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")) {
                ...
                for (int k = 0; k < shares.size(); k++) {
                    ...
                    totalPremiMember = BDUtil.add(totalPremiMember, ri.getDbPremiAmount());
                    ...
                    //cek total premi cicilan member
                    /*
                    BigDecimal totalPremiCicilan = BDUtil.zero;

                    DTOList riInstallment = ri.getInstallment();

                    for (int l = 0; l < riInstallment.size(); l++) {
                        InsurancePolicyReinsInstallmentView inst = (InsurancePolicyReinsInstallmentView) riInstallment.get(l);

                        totalPremiCicilan = BDUtil.add(totalPremiCicilan, inst.getDbPremiAmount());
                    }

                    if(ri.getInstallment().size()>0){
                        if(!BDUtil.isEqual(BDUtil.round(ri.getDbPremiAmount(), 0), totalPremiCicilan,2)){

                            BigDecimal selisih = BDUtil.sub(totalPremiCicilan, ri.getDbPremiAmount());

                            logger.logDebug("################# selisih cicilan reas = "+ selisih);

                            if(BDUtil.biggerThan(selisih, BDUtil.thousand) || BDUtil.lesserThan(selisih, BDUtil.negate(BDUtil.thousand))){
                                //if (stReinsuranceApprovedWho == null)
                                {
                                    validasiReasuransi = true;
                                    message = " - Total Premi Member "+ JSPUtil.print(ri.getDbPremiAmount(), 2) +" Tidak Sama Dengan Total Premi Installment Reasuransi "+ JSPUtil.print(BDUtil.round(totalPremiCicilan, 0), 2);
                                }
                            }
        ...
        //JIKA TOTAL PREMI BRUTO POLIS <> TOTAL PREMI BRUTO REAS & deviasi > 1000, MAKA TIDAK BS APPROVE
        logger.logDebug("################# total premi bruto = "+ getDbPremiTotal());
        logger.logDebug("################# total premi reas = "+ totalPremiMember);


        if(!BDUtil.isEqual(BDUtil.round(totalPremiMember, 0), getDbPremiTotal(),2)){

            BigDecimal selisih = BDUtil.sub(getDbPremiTotal(), totalPremiMember);
            
            logger.logDebug("################# selisih direct = "+ selisih);

            if(isCoinsurance()){
                InsurancePolicyCoinsView coins = getHoldingCoin();

                if(coins.getDbSharePct()!=null){
                    selisih = BDUtil.sub(BDUtil.mul(getDbPremiTotal(), BDUtil.getRateFromPct(coins.getDbSharePct())), totalPremiMember);
                }

                logger.logDebug("################# selisih coins = "+ selisih);
            }

            if(BDUtil.biggerThan(selisih, BDUtil.thousand) || BDUtil.lesserThan(selisih, BDUtil.negate(BDUtil.thousand))){
                if (stReinsuranceApprovedWho == null){
                    validasiReasuransi = true;
                    message = " - Total Premi Bruto "+ JSPUtil.print(getDbPremiTotal(), 2) +" Tidak Sama Dengan Total Premi Reasuransi "+ JSPUtil.print(BDUtil.round(totalPremiMember, 0), 2);
                }
            }
    ...
    public boolean isPremiPaidInSystem() throws Exception {
        boolean canClaim = false;

        //cek status pembayaran input manual tgl bayar
        ...
        //cek pembayaran polis induk nya
        final DTOList pembayaran = getArinvoices();

        //jika bukan cicilan
        if (pembayaran.size() == 1) {
            ...
            //jika endorse keterangan aja
            if(inv.getDbAmount()== null){
                if(BDUtil.isZero(getDbPremiTotal())){
                    canClaim = true;
                }
            }
        }

        //jika jumlah cicilan > 1
        if (pembayaran.size() > 1) {

            final DTOList installmentNotPaid = getInstallmentsNotPaid();

            canClaim = true;

            if(installmentNotPaid.size()>0){
                canClaim = false;

                if(!isBypassValidasiKlaim())
                    throw new RuntimeException("Tidak bisa buat klaim, premi cicilan belum lunas ");
            }
    ...
    public boolean canClaimAgain() throws Exception {
        ...
        try {
            ...
            */
            ...
            final PreparedStatement PS = S.setQuery("select claim_loss_id,a.status, a.claim_status,a.pla_no,a.effective_flag, a.active_flag, "+
                                                    " (select x.reject_flag from ins_policy x where x.dla_no = replace(a.pla_no,'LKS','LKP')) as dla_reject_f "+
                                                    " FROM ins_policy a"+
                                                    " left join ins_pol_obj b on a.pol_id = b.pol_id "+
                                                    " WHERE ins_pol_obj_ref_id = ? limit 1");
            ...
            if (RS.next()) {
                ...
                //JIKA SUDAH DI REJECT, BISA DI AJUKAN ULANG KLAIM
                final String dlaRejectFlag = RS.getString("dla_reject_f");

                if(dlaRejectFlag!=null){
                    if (dlaRejectFlag.equalsIgnoreCase("Y")) {
                        canClaim = true;
                    }
                }
            }
    ...
    public void cekSudahInputLKS() throws Exception {
        ...
        String filterBy = "substr(pol_no, 0, 17)";
        String filterValue = getStPolicyNo().substring(0, 16);

        if(getStPolicyNo().length()==19){
            filterBy = "substr(pol_no, 0, 18)";
            filterValue = getStPolicyNo().substring(0, 17);
        }
        ...
        if(objectKlaim.getStOrderNo()!=null){
                ...
                try {
                    ...
                    final PreparedStatement PS = S.setQuery("select a.pol_id,a.pla_no, a.dla_no ,b.claim_loss_id "+
                                                            ...
                                                            "inner join ins_poL_obj b on a.pol_id = b.pol_id "+
                                                            "where a.status in ('CLAIM','CLAIM ENDORSE') and active_flag = 'Y' "+
                                                            "and "+ filterBy +" = ? and btrim(upper(b.ref1)) = ?  "+
                                                            "and a.pol_id <> ?"+
                                                            " order by a.pol_id desc limit 1");
...
//   }
        ...
        try {
            ...
            if (objectsClaim == null) {
                ...
                for (int i = 0; i < objectsClaim.size(); i++) {
                    InsurancePolicyObjectView obj = (InsurancePolicyObjectView) objectsClaim.get(i);
    ...
    public void recalculateEndorsePAKreasi() throws Exception {
        if (!stPolicyTypeID.equalsIgnoreCase("21") && !stPolicyTypeID.equalsIgnoreCase("59") && !stPolicyTypeID.equalsIgnoreCase("80")) {
            return;
        }
    ...
    public void validateTaxCode() throws Exception {
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            if (!isStatusClaim() && !isStatusEndorse() && !isStatusEndorseRI() && !isStatusClaimEndorse()) {
                
                if (items.getInsItem().isNotActive()) {
                        throw new RuntimeException("Item "+items.getInsItem().getStDescription()+ " tidak boleh digunakan");
                }
            }
            ...
            if(items.isComission() && !items.getInsItem().isNotUseTax()){
                ...
                logger.logDebug("############### CEK NPWP = "+ items.getEntity());
                logger.logDebug("###############  NPWP = "+ items.getEntity().getStTaxFile());

                //if(items.getEntity().getStTaxFile()==null)
                    //throw new RuntimeException("Penerima outgo "+ items.getInsItem().getStDescription() +" harus mempunyai Nomor NPWP");
            }
...
//        }
    ...
    public void validateObjects() throws Exception {
        if (stPolicyTypeID != null) {
            ...
            Date tglAwalHeader = getDtPeriodStart();
            Date tglAkhirHeader = getDtPeriodEnd();
            boolean cekDetailTgl = false;

            boolean tglTidakSama = false;
            ...
            if (objectMap != null) {
                for (int i = 0; i < objects.size(); i++) {
                    ...
                    if (!isStatusClaim()) {
                        ...
                        for (int p = 0; p < details.size(); p++) {
                            ...
                            if (getStPolicyTypeID().equalsIgnoreCase("3")) {
                                ...
                                //cek proteksi usia kendaraan dengan coverage
                                if (isStatusPolicy() || isStatusRenewal() || isStatusEndorse()){
                                    if (obj.getCoverage().size() > 0 && isEffective()) {
                                        ...
                                        for (int j = 0; j < coverage.size(); j++) {
...

                                            {
                                                ...
                                                DTOList policyDocuments = getPolicyDocuments();

                                                boolean check = false;

                                                if(getPolicyDocuments().size()>0){
                                                    for (int k = 0; k < policyDocuments.size(); k++) {
                                                    InsurancePolicyDocumentView doc = (InsurancePolicyDocumentView) policyDocuments.get(k);

                                                    if(doc.getStInsuranceDocumentTypeID().equalsIgnoreCase("95") || doc.getStInsuranceDocumentTypeID().equalsIgnoreCase("91"))
                                                        if(doc.isMarked()&& doc.getStFilePhysic()!=null)
                                                            check = true;
                                                }

                                                //cek usia comprehensive
                                                if(cover.getStInsuranceCoverID().equalsIgnoreCase("280"))
                                                    if(!check && usiaKendaraan > maksUsiaKendaraanComprehensive)
                                                        throw new RuntimeException("Objek No " + (i + 1) + " - Coverage comprehensive - Usia Kendaraan > "+ maksUsiaKendaraanComprehensive +" tahun, File Catatan Persetujuan Divisi Harus Dilampirkan");

                                                 //cek usia TLO
                                                if(cover.getStInsuranceCoverID().equalsIgnoreCase("281"))
                                                    if(!check && usiaKendaraan > maksUsiaKendaraanTLO)
                                                        throw new RuntimeException("Objek No " + (i + 1) + " - Coverage TLO - Usia Kendaraan > "+ maksUsiaKendaraanTLO +" tahun, File Catatan Persetujuan Divisi Harus Dilampirkan");
                                                }
                    ...
                    if (isStatusClaim() || isStatusClaimEndorse()) {
                        DateTime tglKejadianKlaim = new DateTime(getDtClaimDate());
                        DateTime tglAwal = new DateTime(getDtPeriodStart());
                        DateTime tglAkhir = new DateTime(getDtPeriodEnd());

                        if (getStPolicyTypeID().equalsIgnoreCase("3") || getStPolicyTypeID().equalsIgnoreCase("1") || getStPolicyTypeID().equalsIgnoreCase("81")) {
                            InsurancePolicyObjDefaultView objc = (InsurancePolicyObjDefaultView) getClaimObject();
                            if (objc.getDtReference1() != null) {
                                tglAwal = new DateTime(objc.getDtReference1());
                                tglAkhir = new DateTime(objc.getDtReference2());
                            }
                        }

                        if (getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")) {
                            InsurancePolicyObjDefaultView objc = (InsurancePolicyObjDefaultView) getClaimObject();
                            if (objc.getDtReference2() != null) {
                                tglAwal = new DateTime(objc.getDtReference2());
                                tglAkhir = new DateTime(objc.getDtReference3());
                            }

                            if (!isBypassValidation()){
                                if (objc.getDtReference6() != null) {
                                    DateTime tglSTNC = new DateTime(objc.getDtReference6());

                                    if (tglKejadianKlaim.isBefore(tglSTNC)) {
                                        throw new RuntimeException("Tanggal klaim tidak boleh < tanggal STNC");
                                    }
                                }
                            }
                        ...
                        if(!isApprovedRisk()){
                            if (tglKejadianKlaim.isBefore(tglAwal)) {
                                throw new RuntimeException("Tanggal klaim tidak boleh < tanggal awal");
                            }
                        }
                        

                        boolean cekTglKejadian = true;
                        if (getStPolicyTypeGroupID().equalsIgnoreCase("10")) {
                            cekTglKejadian = false;
                        }

                        if (getStPolicyTypeGroupID().equalsIgnoreCase("8") && getEntity().isBankJatim()) {
                            cekTglKejadian = false;
                        }

                        if (getStPolicyTypeID().equalsIgnoreCase("6") || getStPolicyTypeID().equalsIgnoreCase("7")
                                || getStPolicyTypeID().equalsIgnoreCase("43") || getStPolicyTypeID().equalsIgnoreCase("44")) {
                            InsurancePolicyObjDefaultView objc = (InsurancePolicyObjDefaultView) getClaimObject();

                            if (objc.getDtReference1() != null) {
                                DateTime tglMaintenancePeriod = new DateTime(objc.getDtReference1());

                                if (tglKejadianKlaim.isAfter(tglAkhir)) {
                                    if (tglKejadianKlaim.isBefore(tglMaintenancePeriod)) {
                                        cekTglKejadian = false;
                                    }
                                }
                            }
                            ...
                            if (objc.getDtReference2() != null) {
                                DateTime tglSparetimeObjek = new DateTime(objc.getDtReference2());

                                if (tglKejadianKlaim.isAfter(tglAkhir)) {
                                    if (tglKejadianKlaim.isBefore(tglSparetimeObjek)) {
                                        cekTglKejadian = false;
                                    }
                                }
                            }
                        ...
                        //cek jika penyebab klaim ada sub nya, wajib isi
                        if(getClaimCauses()!=null){
                            if(getClaimCauses().isHaveSubCause()){
                                if(getStClaimSubCauseID()==null)
                                    throw new RuntimeException("Penyebab Klaim Spesifik wajib di input");
                            }

                        }

                        //cek penyebab klaim spesifik
                        if(getStClaimSubCauseID()!=null){
                            if(getStClaimSubCauseID().equalsIgnoreCase("32") || getStClaimSubCauseID().equalsIgnoreCase("42")
                                    || getStClaimSubCauseID().equalsIgnoreCase("55"))
                                if(getStClaimSubCauseOther()==null)
                                    throw new RuntimeException("Penyebab klaim spesifik LAINNYA harus mengisi keterangan lainnya");
                        }

                        InsurancePolicyObjDefaultView objc = (InsurancePolicyObjDefaultView) getClaimObject();
                        
                        if(objc.getClaimLoss()==null)
                                throw new RuntimeException("Status kerugian belum di input");

                        //cek pekerjaan klaim lainnya
                        if (getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")){

                            if (getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")){
                                if(getStReference14()==null)
                                    throw new RuntimeException("Kode pekerjaan wajib di input");
                            }

                            if(getStReference14()!=null)
                                if(getStReference14().equalsIgnoreCase("999"))
                                    if(getStReference15()==null)
                                        throw new RuntimeException("Kode pekerjaan LAINNYA harus mengisi keterangan pekerjaan lainnya");
                        }

                        boolean isSubrogasi = false;
                        if (claimItems != null) {
                            for (int j = 0; j < claimItems.size(); j++) {

                                InsurancePolicyItemsView it = (InsurancePolicyItemsView) claimItems.get(j);

                                if (it.getInsuranceItem().isSubrogasi()) {
                                    cekTglKejadian = false;
                                    isSubrogasi = true;
                                }
                            }
                        ...
                        if (cekTglKejadian) {
                            if (tglKejadianKlaim.isAfter(tglAkhir)) {
                                throw new RuntimeException("Tanggal klaim tidak boleh > tanggal akhir");
                            }
                        }

                        if(isSubrogasi){
                            if(Tools.isNo(getStReference13()))
                                throw new RuntimeException("Potensi subrogasi harus dipilih Ya untuk klaim subrogasi");
                        }
                    ...
                    if (!isBypassValidation()) {
                        if (isStatusDraft() || isStatusSPPA() || isStatusPolicy() || isStatusRenewal()) {
                            ...
                            //cek deductible
                            if(getPolicyType().isValidateDeductible()){
                                if (obj.getDeductibles().size() < 1) {
                                    throw new RuntimeException("Resiko Sendiri/Deductible Objek No " + (i + 1) + " Belum Diisi");
                                }
                            }
                        }
                    }

                    //validasi reas SUDAH SESUAI / BELUM
                    if (isStatusPolicy() || isStatusRenewal() || isStatusEndorse()){
                        if(!isEditReas())
                            validateReinsurance(obj);
                    }

                    //validasi klaim reas SUDAH SESUAI / BELUM
                    if (isStatusClaim() || isStatusClaimEndorse()){
                        if(!isEditReas())
                            validateReinsuranceClaim(obj);
                    }
                    

                    if (isStatusDraft() || isStatusSPPA() || isStatusPolicy() || isStatusRenewal() || isStatusEndorse()) {

                        //VALIDASI TGL HEADER POLIS TO OBJEK
                        if(getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")|| getStPolicyTypeID().equalsIgnoreCase("64")
                            || getStPolicyTypeID().equalsIgnoreCase("4") || getStPolicyTypeID().equalsIgnoreCase("3") || getStPolicyTypeID().equalsIgnoreCase("1")
                            || getStPolicyTypeID().equalsIgnoreCase("80") || getStPolicyTypeID().equalsIgnoreCase("81")){
                            validatePeriodeHeader(obj, i);
                        }    
                    }
                ...
                if(objects.size()>0){
                    ...
                    if (isStatusDraft() || isStatusSPPA() || isStatusPolicy() || isStatusRenewal() || isStatusEndorse()){
                        ...
                        if(getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")|| getStPolicyTypeID().equalsIgnoreCase("64")
                                ...
                                || getStPolicyTypeID().equalsIgnoreCase("80") || getStPolicyTypeID().equalsIgnoreCase("81"))
                        {
                                logger.logDebug("###################### tgl awal header = "+ DateUtil.getDateStr(getDtPeriodStart()));
                                logger.logDebug("###################### tgl awal objek terawal = "+ DateUtil.getDateStr(tglAwalTerawal.toDate()));

                                logger.logDebug("###################### tgl akhir header = "+ DateUtil.getDateStr(getDtPeriodEnd()));
                                logger.logDebug("###################### tgl akhir objek terakhir = "+ DateUtil.getDateStr(tglAkhirTerakhir.toDate()));

                                if(!DateUtil.getDateStr(getDtPeriodStart()).equalsIgnoreCase(DateUtil.getDateStr(tglAwalTerawal.toDate())))
                                    throw new RuntimeException("Tanggal awal header polis "+ DateUtil.getDateStr(getDtPeriodStart()) +" tidak sesuai dengan tanggal awal objek paling awal "+ DateUtil.getDateStr(tglAwalTerawal.toDate()));

                                if(!DateUtil.getDateStr(getDtPeriodEnd()).equalsIgnoreCase(DateUtil.getDateStr(tglAkhirTerakhir.toDate())))
                                    throw new RuntimeException("Tanggal akhir header polis "+ DateUtil.getDateStr(getDtPeriodEnd()) +" tidak sesuai dengan tanggal akhir objek paling akhir "+ DateUtil.getDateStr(tglAkhirTerakhir.toDate()));
                        }
    ...
    public void validatePeriodeHeader(InsurancePolicyObjectView obj, int i) throws Exception{
        ...
        InsurancePolicyObjDefaultView objx = (InsurancePolicyObjDefaultView) obj;
            ...
            if(i==0){
                    ...
                    if(getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")|| getStPolicyTypeID().equalsIgnoreCase("64")
                            || getStPolicyTypeID().equalsIgnoreCase("4") || getStPolicyTypeID().equalsIgnoreCase("80")){
                        tglAwalTerawal = new DateTime(objx.getDtReference2());
                        tglAkhirTerakhir = new DateTime(objx.getDtReference3());
                    }


                    if(getStPolicyTypeID().equalsIgnoreCase("3") || getStPolicyTypeID().equalsIgnoreCase("1") || getStPolicyTypeID().equalsIgnoreCase("81")){
                        if(objx.getDtReference1()!=null){
                            tglAwalTerawal = new DateTime(objx.getDtReference1());
                            tglAkhirTerakhir = new DateTime(objx.getDtReference2());
                        }
                    }
            }

            DateTime tglAwal2 = new DateTime(objx.getDtReference2());
            DateTime tglAkhir2 = new DateTime(objx.getDtReference3());

            if(getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")|| getStPolicyTypeID().equalsIgnoreCase("64")|| getStPolicyTypeID().equalsIgnoreCase("4")  || getStPolicyTypeID().equalsIgnoreCase("80")){
                    tglAwal2 = new DateTime(objx.getDtReference2());
                    tglAkhir2 = new DateTime(objx.getDtReference3());
            }


            if(getStPolicyTypeID().equalsIgnoreCase("3") || getStPolicyTypeID().equalsIgnoreCase("1") || getStPolicyTypeID().equalsIgnoreCase("81")){
                {
                    tglAwal2 = new DateTime(objx.getDtReference1());
                    tglAkhir2 = new DateTime(objx.getDtReference2());
                }
            }
    ...
    public void validateReinsurance(InsurancePolicyObjectView objx) throws Exception{

        logger.logDebug("####################### VALIDATE REINSURANCE NIH ####################");
        ...
        final String polTypeID = getStPolicyTypeID();
        ...
        if(polTypeID.equalsIgnoreCase("3") || polTypeID.equalsIgnoreCase("1") || polTypeID.equalsIgnoreCase("81")){
                ...
                for (int i = 0; i < cov.size(); i++) {
                    ...
                    if(cover.getStInsuranceCoverID().equalsIgnoreCase("139")||cover.getStInsuranceCoverID().equalsIgnoreCase("304")){
                        if(getDtPeriodStart()!=null)
                            ...
                            treatyID = getInsuranceTreatyIDCM(getDtPeriodStart(), "139");
                    ...
                    if(cover.getStInsuranceCoverID().equalsIgnoreCase("143")||cover.getStInsuranceCoverID().equalsIgnoreCase("319")
                        ...
                        if(getDtPeriodStart()!=null)
                            ...
                            treatyID = getInsuranceTreatyIDCM(getDtPeriodStart(), "143");
                    }
                }
            ...
            if(polTypeID.equalsIgnoreCase("19")){
                ...
                if(getDtPeriodStart()!=null){
                    ...
                    DateTime dt = new DateTime(getDtPeriodStart());
                    // get the year
                    int year = dt.getYear();

                    if(year>=2020)
                        treatyID = getEarthquakeTreatyID(getDtPeriodStart(), zonaMaipark);
                }
            }

            //jika kredit konsumtif
            if(polTypeID.equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")){

                final EntityView entity = getEntity();
                String treatyIDByCompGroup = getInsuranceTreatyID(getDtPeriodStart(), entity.getStRef2());
                ...
                if(getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")){
                    ...
                    if(usiaKredit >= 17 && usiaPlusLamaKredit <= 75) orOnly = false;
                }
                ...
                if(getStPolicyTypeID().equalsIgnoreCase("21")){
                    ...
                    if(usiaKreasi >= 17 && usiaPlusLamaKreasi <= 75 && isORKreasi) orOnly = false;

                }

                //jika uw year 2022 pakai treaty khusus kredit 2022
                if(getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")  || getStPolicyTypeID().equalsIgnoreCase("80")){

                    String uwYear = DateUtil.getYear(obj.getDtReference2());

                    if(uwYear.equalsIgnoreCase("2022"))
                        treatyID = "209";
                }

                //jika periode awal >= juli-des 2025, pakai treaty kredit juli-des 2025 (id 252)
                if(getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88") || getStPolicyTypeID().equalsIgnoreCase("80")){

                    DateTime tglTreatyJuliDes25 = new DateTime(DateUtil.getDate("30/06/2025"));
                    DateTime tglAkhirTreatyJuliDes25 = new DateTime(DateUtil.getDate("01/01/2026"));
                    DateTime tglAwal= new DateTime(obj.getDtReference2());

                    if(tglAwal.isAfter(tglTreatyJuliDes25) && tglAwal.isBefore(tglAkhirTreatyJuliDes25))
                        treatyID = "252";
                }

                String jenisKredit = getStKreasiTypeID();

                if(getStPolicyTypeID().equalsIgnoreCase("59")){
                    //BNI macet j kredit 628 -> treaty tib atm 220
                    if(jenisKredit.equalsIgnoreCase("628"))
                        treatyID = "220";

                    //BNI pensiunan j kredit 627 -> treaty tib pensiunan 219
                    if(jenisKredit.equalsIgnoreCase("627"))
                        treatyID = "219";
                }
                

                if(getStRegionID().equalsIgnoreCase("39")) orOnly = true;
            ...
            //polis multiyears
            if(polTypeID.equalsIgnoreCase("1") || polTypeID.equalsIgnoreCase("19") || polTypeID.equalsIgnoreCase("81")){

                DateTime bulanDesember2023 = new DateTime(DateUtil.getDate("31/12/2023"));
                DateTime periodeAwal = new DateTime(getDtPeriodStart());

                if(periodeAwal.isAfter(bulanDesember2023)){
                    int lamaTahun = 0;
                    int lamaBulan = 0;
                    Date tglAwal = getDtPeriodStart();

                    if(polTypeID.equalsIgnoreCase("1") || polTypeID.equalsIgnoreCase("81")){
                        if(obj.getDtReference1()!=null && obj.getDtReference2()!=null){
                            lamaTahun = DateUtil.getYearsBetweenHUTB(obj.getDtReference1(), obj.getDtReference2());
                            lamaBulan = DateUtil.getMonthsBetween(obj.getDtReference1(), obj.getDtReference2());
                            tglAwal = obj.getDtReference1();
                        }

                    }

                     if(polTypeID.equalsIgnoreCase("19")){
                         lamaTahun = DateUtil.getYearsBetweenHUTB(getDtPeriodStart(), getDtPeriodEnd());
                         lamaBulan = DateUtil.getMonthsBetween(getDtPeriodStart(), getDtPeriodEnd());
                     }

                    if(lamaBulan > 15){
                        obj.setStInsuranceTreatyID(getInsTreatyIDMultiyears(tglAwal));
                        treatyID = getInsTreatyIDMultiyears(tglAwal);
                    }
            ...
            if(polTypeID.equalsIgnoreCase("21")){
                ...
                DateTime bulanProduksi = new DateTime(getDtPolicyDate());
            ...
            //cek jika tidak sesuai

            DateTime periodStart = new DateTime(getDtPeriodStart());

            InsuranceTreatyView treaty = obj.getTreaty();

            logger.logDebug("################# ############################## treaty objek = "+ obj.getStObjectDescriptionWithoutCounter());
            logger.logDebug("################# treaty objek = "+ treaty.getStInsuranceTreatyID());
            logger.logDebug("################# treaty seharusnya = "+ treatyID);
             logger.logDebug("################# treaty details size = "+ obj.getTreatyDetails().size());

            if(!isStatusEndorse()){
                if(!treaty.getStInsuranceTreatyID().equalsIgnoreCase(treatyID)){
                    if(getStReinsuranceApprovedWho()==null){

                        logger.logDebug("################# RESET treaty objek ");
                        ...
                        recalculateChangeTreaty();

                        recalculateTreatyInitialize();

                    }
                }
            }

             //jika endorse & ganti ke treaty detail
             if(isStatusEndorse()){
                {
                    if(getStReinsuranceApprovedWho()==null){

                        boolean changeTreaty = false;

                        logger.logDebug("################# GANTI treaty ke treaty DETAIL ");

                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("189")){
                            treatyID = "229";
                            changeTreaty = true;
                        } //2022
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("156")){
                            treatyID = "230";
                            changeTreaty = true;
                        } //2021
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("140")){
                            treatyID = "231";
                            changeTreaty = true;
                        } //2020
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("96")){
                            treatyID = "232";
                            changeTreaty = true;
                        } //2019
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("94")){
                            treatyID = "233";
                            changeTreaty = true;
                        } //2018
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("93")){
                            treatyID = "234";
                            changeTreaty = true;
                        } //2017
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("92")){
                            treatyID = "235";
                            changeTreaty = true;
                        } //2016
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("84")){
                            treatyID = "236";
                            changeTreaty = true;
                        } //2015
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("38")){
                            treatyID = "237";
                            changeTreaty = true;
                        } //2014
                        if(treaty.getStInsuranceTreatyID().equalsIgnoreCase("31")){
                            treatyID = "238";
                            changeTreaty = true;
                        } //2013
            ...
            //cek jika tidak ada treaty details, define ulang treaty
             if(obj.getTreatyDetails().size() == 0)
            {
                logger.logDebug("################# RESET treaty objek ");
    ...
    public void validateReinsuranceClaim(InsurancePolicyObjectView objx) throws Exception{

        logger.logDebug("####################### VALIDATE REINSURANCE CLAIM NIH ####################");
        ...
        String treatyID = getDtPeriodStart()!=null?getInsuranceTreatyID(getDtPeriodStart()):Parameter.readString(inward?"UWRIT_DEF_ITREATY":"UWRIT_DEF_OTREATY");
            ...
            InsuranceTreatyView treaty = obj.getTreaty();
             ...
             //jika klaim & ganti ke treaty detail
             if(isStatusClaim() || isStatusClaimEndorse()){
    ...
    public void validateTSILimit(BigDecimal transactionLimit, boolean uploadDocPersetujuan) throws Exception {
        ...
        for (int i = 0; i < object.size(); i++) {
            ...
            BigDecimal transactionLimitPerRiskCode = getTransactionLimitPerRiskCode("ACCEPT", getStRiskClass(), SessionManager.getInstance().getUserID(), obj.getStRiskCategoryCode());
            ...
            if(uploadDocPersetujuan){
                    transactionLimitPerRiskCode = transactionLimit;
            }
            ...
            if(isCoinsurance()){
                InsurancePolicyCoinsView leader = getHoldingCoin();

                if(!BDUtil.isZeroOrNull(leader.getDbSharePct())){
                    tsiObject = BDUtil.mul(tsiObject, BDUtil.getRateFromPct(leader.getDbSharePct()));
                }
    ...
    public void checkEndorseNoBefore(String stPolicyNo) throws Exception {
        ...
        for (int i = 0; i < root.size(); i++) {
            InsurancePolicyView pol = (InsurancePolicyView) root.get(i);
    ...
    public void checkHistoryPolicyNo() throws Exception {
        ...
        for (int i = 0; i < history.size(); i++) {
            InsurancePolicyView pol = (InsurancePolicyView) history.get(i);
        ...
        try {
            ...
            if (objects_year == null) {
                ...
                for (int i = 0; i < objects_year.size(); i++) {
                    InsurancePolicyObjectView obj = (InsurancePolicyObjectView) objects_year.get(i);
    ...
    public InsurancePolicyView getStParentPolicy() {
        return (InsurancePolicyView) DTOPool.getInstance().getDTO(InsurancePolicyView.class, stReference2);
    }
    ...
    public void checkPolicyNoBefore(String stPolicyNo) throws Exception {
        ...
        for (int i = 0; i < root2.size(); i++) {
            InsurancePolicyView pol = (InsurancePolicyView) root2.get(i);
        ...
        try {
            ...
            if (objects_koas == null) {
                ...
                for (int i = 0; i < objects_koas.size(); i++) {
                    InsurancePolicyObjectView obj = (InsurancePolicyObjectView) objects_koas.get(i);
    ...
    public InsurancePolicyView getTemporaryPolicy() {
        return (InsurancePolicyView) DTOPool.getInstance().getDTO(InsurancePolicyView.class, stReference6);
    }
        ...
        try {
            if (claimItemsFee == null) {
                claimItemsFee = ListUtil.getDTOListFromQuery(
                        "select * from ins_pol_items where pol_id = ? and item_class='CLM' and ins_item_id in (50,76,78,81,82,83,84,109,110) order by ins_pol_item_id",
                        new Object[]{stPolicyID},
                        InsurancePolicyItemsView.class);
            }
        } catch (Exception e) {
    ...
    public void loadRootClaim() {
        try {
            ...
            String paramValue = stPolicyNo.substring(0, 16);
            String param = "substr(pol_no,0,17) = ?";
            String searchLength = "and length(pol_no) = 18";

            if(stPolicyNo.length() > 18){
                paramValue = stPolicyNo.substring(0, 17);
                param = "substr(pol_no,0,18) = ?";
                searchLength = "";
            }
            ...
            if (rootclaim == null) {
                rootclaim = ListUtil.getDTOListFromQuery(
                        "select pol_no "
                        + " from ins_policy "
                        + " where "+ param +" and active_flag='Y' and effective_flag = 'Y' "+ searchLength +" order by pol_no desc",
    ...
    public void checkLastPolicyNo(String stPolicyNo) throws Exception {
        ...
        for (int i = 0; i < claim.size(); i++) {
            InsurancePolicyView pol = (InsurancePolicyView) claim.get(0);
...
//        return view.getStUserName();
    ...
    public void validateDoubleObject() throws Exception {
        if (getStPolicyTypeID().equalsIgnoreCase("21") || getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88") || getStPolicyTypeID().equalsIgnoreCase("80")) {
    ...
    public boolean isRecalculateTreaty() {
        if (isStatusPolicy() || isStatusClaim() || isStatusEndorse() || isStatusRenewal()
                ...
                || isStatusClaimInward() || isStatusEndorseClaimInward()) {
            return true;
        }
    ...
    public void setCustomStringToPolicy(InsurancePolicyObjDefaultView objc) {
        if (getStPolicyTypeGroupID().equalsIgnoreCase(FinCodec.InsurancePolicyTypeGroup.SURETYBOND) || getStPolicyTypeGroupID().equalsIgnoreCase(FinCodec.InsurancePolicyTypeGroup.BANKGUARANTEE)) {
            if (!getStPolicyTypeID().equalsIgnoreCase("59")) {
                if (objc.getStReference1() != null) {
                    setStReference5(objc.getStReference1()); //nama principal
   ...
   public String getUserIDSignYusas() {

        String sub_cccode = getCostCenter(getStCostCenterCode()).getStSubCostCenterCode();//80
        String cccode_source = getStCostCenterCodeSource();//52

        if (sub_cccode.equalsIgnoreCase("80")) {
            if (cccode_source.equalsIgnoreCase("52")) {
                sub_cccode = cccode_source;
            } else {
                sub_cccode = sub_cccode;
            }
        } else {
            sub_cccode = sub_cccode;
        }
        ...
        System.out.print("#################1 " + sub_cccode);
        System.out.print("#################2 " + cccode_source);
        System.out.print("#################3 " + NIPPimpinan);
//        String NIPPimpinan = getStApprovedWho();

//        String NIPPimpinan = getUserApprovedbySK();
...
//        String NIPPimpinan = getUserApprovedbySK();
    ...
    public String getUserIDSignNameYusas() {
...
//        String NIPPimpinan = getStApprovedWho();
        ...
        if (stPolicyTypeID != null) {
            ...
            if (objectMap != null) {
                for (int i = 0; i < objects.size(); i++) {
                    ...
                    if (!isStatusClaim()&&!isStatusEndorse()) {
                        DTOList details = objectMap.getDetails();
    ...
    public void defineTreaty() throws Exception {
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            //jika kredit konsumtif
            if(polTypeID.equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("80") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")){
                setCreditTreaty(obj);
            }
            ...
            if(polTypeID.equalsIgnoreCase("1") || polTypeID.equalsIgnoreCase("19") || polTypeID.equalsIgnoreCase("81")){
                ...
                if(periodeAwal.isAfter(bulanDesember2023)){
                    ...
                    if(lamaBulan > 15){
                        //treatyID = Parameter.readString("UWRIT_DEF_TREATY_MULTIYEAR");
                        //obj.setStInsuranceTreatyID(Parameter.readString("UWRIT_DEF_TREATY_MULTIYEAR"));
                        obj.setStInsuranceTreatyID(getInsTreatyIDMultiyears(tglAwal));
                    }
                }
            }
    ...
    public void setCreditTreaty(InsurancePolicyObjDefaultView obj) throws Exception {
        ...
        if(getStPolicyTypeID().equalsIgnoreCase("59") || getStPolicyTypeID().equalsIgnoreCase("87") || getStPolicyTypeID().equalsIgnoreCase("88")){
            ...
            //if(usiaKredit >= 17 && usiaKredit <=64 && usiaPlusLamaKredit <= 75) orOnly = false;
            if(usiaKredit >= 17 && usiaPlusLamaKredit <= 75) orOnly = false;
        }
        ...
        if(getStPolicyTypeID().equalsIgnoreCase("21")){
            ...
            //if(usiaKreasi >= 17 && usiaKreasi <=64 && usiaPlusLamaKreasi <= 75 && isORKreasi) orOnly = false;
            if(usiaKreasi >= 17 && usiaPlusLamaKreasi <= 75 && isORKreasi) orOnly = false;
    ...
    public void recalculateInterkoneksi2() throws Exception {

        recalculateBasicInterkoneksi();
        recalculateClaim();
        invokeCustomHandlers(true);
        recalculateBasicInterkoneksi();
        invokeCustomHandlers(false);
    ...
    public void recalculateBasicInterkoneksi() throws Exception {
        if (stPolicyTypeID == null) {
            return;
        }
    ...
    public void recalculateInterkoneksiOnly() throws Exception {
    ...
    private void loadARInvoicesPolisInduk() {
        try {
            ...
            String nopol = getStPolicyNo().substring(0, 16) + "00";

            if(getStPolicyNo().length()>18)
                nopol = getStPolicyNo().substring(0, 17) + "00";
            ...
            arinvoices = ListUtil.getDTOListFromQuery(
                    ...
                    + "         ) as payment_date,"
                    + "         ( "
                    + "         select max(b.receipt_no) from ar_receipt_lines c, ar_receipt b "
                    + "         where c.ar_invoice_id=a.ar_invoice_id and c.receipt_id=b.ar_receipt_id "
                    + "         ) as receipt_no"
                    + "      from "
                    + "         ar_invoice   a"
                    ...
                    + "  union "
                    + " select premi_total as amount_settled,payment_date,payment_note "
                    + " from ins_policy "
                    + " where status = 'HISTORY' and effective_flag = 'Y' and active_flag = 'Y' "
                    + " and substr(pol_no,0,17) = ?",
                    new Object[]{trx_type_id, nopol, getStPolicyNo().substring(0, 16)},
                    ARInvoiceView.class);
    ...
    public void validateJenisKredit(InsurancePolicyObjectView obj) throws Exception{
    ...
    public void setPeriods(String type, int jk_waktu) {
        setDbPeriodRate(new BigDecimal(100));
        setStPeriodBaseID("2");
        setStPremiumFactorID("1");
        ...
        if(type.equalsIgnoreCase("YEAR")){
            if(jk_waktu>1){
                BigDecimal factorPengkali = BDUtil.mul(new BigDecimal(jk_waktu), BDUtil.hundred);
        ...
        if(type.equalsIgnoreCase("MONTH")){
            if(jk_waktu>1){
                ...
                setDbPeriodRate(new BigDecimal(jk_waktu));
                setStPeriodBaseID("1");
            }
        }
    ...
    private void loadReinsInvoices() {
        try {
            ...
            reinsInvoices = ListUtil.getDTOListFromQuery(
                    ...
                    + "            select max(b.receipt_date) from ar_receipt_lines c, ar_receipt b\n"
                    + "            where c.ar_invoice_id=a.ar_invoice_id and c.receipt_id=b.ar_receipt_id"
                    + "         ), a.receipt_date) as payment_date, b.ent_name as reins_name"
                    + "      from "
    ...
    public boolean isPremiPaidForClaim() throws Exception {
        boolean canClaim = false;
        ...
        if (pembayaran.size() > 0) {
            ...
            if (inv.getDbAmountSettled() != null) {
                if (inv.getDtPaymentDate() != null) {
                    canClaim = true;
                    setDtPaymentDate(inv.getDtPaymentDate());
                    //setStPaymentNotes(inv.getStReceiptNo());
                }
            }
        }
    ...
    public DTOList getObjectsByOrderNo(String orderNo) {
        loadObjectsByOrderNo(orderNo);
        return objects;
    }

    public void loadObjectsByOrderNo(String orderNo) {
        try {
            getClObjectClass();
            if (objects == null) {
                objects = ListUtil.getDTOListFromQuery(
                        "select * from " + DTOCache.getTableName(clObjectClass) + " where pol_id  = ? and order_no = ? order by ins_pol_obj_id asc",
                        new Object[]{stPolicyID, orderNo},
                        clObjectClass);
    ...
    public BigDecimal getPPNRate(String insItemID, Date dtPeriodStart) throws Exception {
        final SQLUtil S = new SQLUtil();
    ...
    public boolean isPremiPaidInSystemDLA() throws Exception {
        boolean canClaim = false;
    ...
    public DTOList getInstallmentsNotPaid() {
        loadInstallmentsNotPaid();
        return installmentsNotPaid;
    }

    public void setInstallments(DTOList installmentsNotPaid) {
        this.installmentsNotPaid = installmentsNotPaid;
    }

    private void loadInstallmentsNotPaid() {
        try {
            ...
            installmentsNotPaid = ListUtil.getDTOListFromQuery(
                    "      select "
                    + "         a.*,"
                    + "         ("
                    + "            select max(b.receipt_date) from ar_receipt_lines c, ar_receipt b\n"
                    ...
                    + "         ar_invoice   a"
                    + "      where "
                    + "         coalesce(a.cancel_flag,'N') <> 'Y' and posted_flag = 'Y' and ar_trx_type_id = ? and attr_pol_id = ? and amount_settled is null and receipt_date is null and amount is not null order by a.ar_invoice_id",
                    new Object[]{trx_type_id, getStPolicyID()},
                    ARInvoiceView.class);

        } catch (NullPointerException e) {

            installmentsNotPaid = new DTOList();
    ...
    public void generateClaimEndorseNo(){

        int panjangNoLKS = stPLANo.length();

        //sebelum penerapan baru, masih pakai kodifikasi lama
        if(panjangNoLKS == 19){
            generateClaimEndorseDLA();
        }else{
            generateClaimEndorseNoNew();
        }
    }

    public void generateClaimEndorseNoNew() {
        ...
        //LKP/59/11/1122/00000100
        //01234567890123456789012

        final char[] dlano = stDLANo.toCharArray();

        final String enos = stDLANo.substring(21, 23);
    ...
    public void recalculateTreatyUploadSpreading() throws Exception {
        if (getStCoverTypeCode() == null) {
            return;
        }
        if (getStPolicyTypeID() == null) {
        ...
        try {
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                ...
                invokeTreatyCustomHandlers(ipo);
                tre.setStRateMethod(getStRateMethod());
                tre.calculateUploadSpreading(ipo, insuredAmount, ipo.getDbObjectPremiTotalAmount(), stPolicyTypeGroupID, dbCurrencyRate, dbInsuranceCoinsScale);
    ...
    public DTOList getObjectsNoUrut() {
        loadObjectsNoUrut();
        return objects;
    }

    public void loadObjectsNoUrut() {
        try {
            getClObjectClass();
            if (objects == null) {
                objects = ListUtil.getDTOListFromQuery(
                        "select * from " + DTOCache.getTableName(clObjectClass) + " where pol_id  = ? and order_no = 5 order by ins_pol_obj_id asc",
                        new Object[]{stPolicyID},
                        clObjectClass);
    ...
    public void validateRestitusiSudahKlaim(InsurancePolicyObjectView obj) throws Exception{
        if(!getStPolicyTypeID().equalsIgnoreCase("21") && !getStPolicyTypeID().equalsIgnoreCase("59"))
            return;
    ...
    public void cekSudahPernahKlaim(InsurancePolicyObjDefaultView objectRestitusi) throws Exception {

        if(!isStatusEndorse())
            return;

        String paramValue = getStPolicyNo().substring(0, 16);
        String param = "substr(pol_no,0,17) = ?";
        String searchLength = "and length(pol_no) = 18";

        if(stPolicyNo.length() > 18){
            paramValue = getStPolicyNo().substring(0, 17);
            param = "substr(pol_no,0,18) = ?";
            searchLength = "";
        }

        if(objectRestitusi.getStOrderNo()!=null){
                ...
                try {
                    ...
                    final PreparedStatement PS = S.setQuery("select a.pol_id,a.pla_no, a.dla_no ,b.claim_loss_id "+
                                                            "from ins_policy a "+
                                                            "inner join ins_poL_obj b on a.pol_id = b.pol_id "+
                                                            "where a.status in ('CLAIM','CLAIM ENDORSE') and active_flag = 'Y' and claim_status = 'DLA' and effective_flag = 'Y' "+
                                                            "and "+ param +" and b.order_no = ? "+
                                                            "and a.pol_id <> ? "+ searchLength +
                                                            " order by a.pol_id desc limit 1");

                    PS.setString(1, paramValue);
                    PS.setString(2, objectRestitusi.getStOrderNo());
                    //PS.setString(3, objectKlaim.getStReference1().toUpperCase().trim());
                    PS.setString(3, getStPolicyID());
                    ...
                    if (RS.next()) {
                        ...
                        if (plaNo != null) {
                            ...
                            if (cekClaimRepeat.equalsIgnoreCase("N"))
                                throw new RuntimeException("Objek no urut "+ objectRestitusi.getStOrderNo() +" " + objectRestitusi.getStReference1() +" sudah pernah klaim pada nomor "+ dlaNo);
                        }
                    }
         ...
         }else if(objectRestitusi.getStOrderNo()==null){
                ...
                try {
                    ...
                    final PreparedStatement PS = S.setQuery("select a.pol_id,a.pla_no, a.dla_no ,b.claim_loss_id "+
                                                            ...
                                                            "inner join ins_poL_obj b on a.pol_id = b.pol_id "+
                                                            "where a.status in ('CLAIM','CLAIM ENDORSE') and active_flag = 'Y' and claim_status = 'DLA' and effective_flag = 'Y' "+
                                                            "and "+ param +" and btrim(upper(b.ref1)) = ?  "+
                                                            "and a.pol_id <> ? "+ searchLength +
                                                            " order by a.pol_id desc limit 1");

                    PS.setString(1, paramValue);
                    PS.setString(2, objectRestitusi.getStReference1().toUpperCase().trim());
                    PS.setString(3, getStPolicyID());
    ...
    private DTOList rootpolicy3;

    public DTOList getRootPolicy3() {
        loadRootPolicy3();
        return rootpolicy3;
    }

    public void loadRootPolicy3() {
        try {
            if (rootpolicy3 == null) {
                rootpolicy3 = ListUtil.getDTOListFromQuery(
                        "select * "
                        + " from ins_policy "
                        + " where dla_no = ? and active_flag = 'Y' limit 1",
                        new Object[]{stDLANo},
                        InsurancePolicyView.class);
            }
        } catch (Exception e) {
    ...
    public void checkDLANoBefore(String stDLANo) throws Exception {
        final DTOList root2 = getRootPolicy3();

        for (int i = 0; i < root2.size(); i++) {
            InsurancePolicyView pol = (InsurancePolicyView) root2.get(i);

            if (pol.getStDLANo().equalsIgnoreCase(stDLANo)) {
                throw new RuntimeException("No DLA " + stDLANo + " Sudah Pernah Dibuat");
            }

        }
    }

    public String getKreditType(String stVsCode) throws Exception {
        final SQLUtil S = new SQLUtil();
    ...
    public void recalculateInterkoneksiKonstruksi() throws Exception {

        recalculateBasicKonstruksi();
        recalculateClaim();
        invokeCustomHandlers(true);
        recalculateBasicKonstruksi();
        invokeCustomHandlers(false);
    ...
    public void recalculateBasicKonstruksi() throws Exception {
        if (stPolicyTypeID == null) {
            return;
        }
    ...
    public GLCostCenterView3 getCostCenter3(String stCostCenterCode) {
    ...
    public void loadTitipan() throws Exception {
        try {
            if (titipan == null) {
                titipan = ListUtil.getDTOListFromQuery(
                        ...
                        + "      *"
                        + "   from"
                        + "      ins_pol_titipan_premi"
                        + "   where"
                        + "      policy_id = ? order by ins_pol_titipan_premi_id",
                        new Object[]{getStPolicyID()},
                        InsurancePolicyTitipanPremiView.class);
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public void recalculateTitipanPremi() throws Exception{

        //Jika entity/customer nya polis khusus
        if(getStEntityID()!=null){
            if(getStEntityID().equalsIgnoreCase("1042112") || getStEntityID().equalsIgnoreCase("1042113")){

                final DTOList titipan = getTitipan();

                if(titipan.size()==0)
                    throw new RuntimeException("Endorsemen polis khusus harus menginput rincian realisasi titipan premi polis khusus");

                BigDecimal total = BDUtil.zero;

                for (int i = 0; i < titipan.size(); i++) {
                    InsurancePolicyTitipanPremiView titip = (InsurancePolicyTitipanPremiView) titipan.get(i);

                    total = BDUtil.add(total, titip.getDbRealisasiAmount());
                }

                setDbRealisasiAmountTotal(total);

                BigDecimal dbTitipanDisetujui = BDUtil.negate(getDbRealisasiAmountTotal());

                if(BDUtil.biggerThanZero(getDbRealisasiAmountTotal())){
                    if(BDUtil.lesserThan(getDbPremiNetto(), dbTitipanDisetujui))
                        throw new RuntimeException("Jumlah premi netto tidak boleh > Realisasi titipan premi disetujui");
                }

                if(BDUtil.lesserThanZero(getDbRealisasiAmountTotal())){
                    if(BDUtil.lesserThan(getDbPremiNetto(), getDbRealisasiAmountTotal()))
                        throw new RuntimeException("Jumlah premi netto tidak boleh > Realisasi titipan premi disetujui");
                }
    ...
    public boolean isPolisKhusus() {
        boolean polisKhusus = false;
    ...
    public void recalculateChangeTreaty() throws Exception {
        validateEntity();
    ...
    public int getJumlahTahunPertanggungan(){
        ...
        if(getStPolicyTypeID().equalsIgnoreCase("1") || getStPolicyTypeID().equalsIgnoreCase("81")){
            ...
            for (int i = 0; i < object.size(); i++) {
                ...
                InsurancePolicyObjDefaultView objd = (InsurancePolicyObjDefaultView) obj;

                if(getStPolicyTypeID().equalsIgnoreCase("1") || getStPolicyTypeID().equalsIgnoreCase("81"))
                    lamaTahun = DateUtil.getYearsBetweenHUTB(objd.getDtReference1(), objd.getDtReference2());
            }
        }    
    ...
    public String getStCostCenterSourceDesc() {

        final GLCostCenterView gcc = getCostCenterSource();
    ...
    private GLCostCenterView getCostCenterSource() {

        final GLCostCenterView gcc = (GLCostCenterView) DTOPool.getInstance().getDTO(GLCostCenterView.class, stCostCenterCodeSource);

        return gcc;

    }

    public String getStCostCenterSourceAddress() {
    ...
    public boolean isBypassValidasiKlaim() throws Exception {
        final SQLUtil S = new SQLUtil();
    ...
    public boolean isDataSPPAOnline() {
        return "5".equalsIgnoreCase(stDataSourceID);
    }

    public String getRiskCategoryIDByCode(String stPolicyTypeId, Date per_start, String code) throws Exception {
        final SQLUtil S = new SQLUtil();
        ...
        try {
            S.setQuery(
                    ...
                    + "      period_start <= ? "
                    + "   and period_end >= ? "
                    + "   and poltype_id = ?  and ins_risk_cat_code = ?");
    ...
    public String getStPolicyNoInduk() {

        String noPolis = getStPolicyNo();

        String noPolisInduk = "";

        if(noPolis.length()==18){
           noPolisInduk = noPolis.substring(0,16)+ "00";
       }

       if(noPolis.length()==19){
           noPolisInduk = noPolis.substring(0,17)+ "00";
    ...
    public boolean isRejected(){
        return Tools.isYes(stRejectFlag);
    }

    public BigDecimal getAkumulasiKomisiBulanan(String stEntityID, Date dtTanggalPolis) throws Exception {
        ...
        //System.out.println("####################### tanggalPalingAwal = "+ tanggalPalingAwal.toDate());
        //System.out.println("####################### tanggalPalingAkhir = "+ tanggalPalingAkhir.toDate());

        final SQLUtil S = new SQLUtil();

        String query =  " select coalesce(sum(b.amount),0) as total_komisi "+
                    " from ins_policy a  "+
                    " left join ins_pol_items b on a.pol_id = b.pol_id "+
                    " where status in ('POLICY','ENDORSE','RENEWAL') and active_flag = 'Y'  "+
                    " and a.effective_flag = 'Y' "+
                    " and ins_item_id in (11,18,25,32,112,116) and b.tax_code in (1,4) and b.ent_id = ? "+
                    " and pph21_sd_saat_ini is not null "+
                    " and premi_total <> 0"+
                    " and date_trunc('day',policy_date) >= ? and date_trunc('day',policy_date) <= ? "+
                    " and date_trunc('day',policy_date) >= '2025-08-01 00:00:00'";

        try {
            S.setQuery(query);

            S.setParam(1, stEntityID);
            S.setParam(2, tanggalPalingAwal.toDate());
            S.setParam(3, tanggalPalingAkhir.toDate());
    ...
    public BigDecimal getAkumulasiPPhBulananAfterReverse(String stEntityID, Date dtTanggalPolis, BigDecimal dppNow) throws Exception {
        ...
        //logger.logDebug("####################### tanggalPalingAwal = "+ tanggalPalingAwal.toDate());
        //logger.logDebug("####################### tanggalPalingAkhir = "+ tanggalPalingAkhir.toDate());

        final SQLUtil S = new SQLUtil();

        try {
            S.setQuery(
                    "   select coalesce(pph21_sd_saat_ini,0) as pph21_sd_saat_ini "+
                    " from ins_policy a  "+
                    " left join ins_pol_items b on a.pol_id = b.pol_id "+
                    " where status in ('POLICY','ENDORSE','RENEWAL') and active_flag = 'Y' and effective_flag = 'Y'  "+
                    " and ins_item_id in (11,18,25,32,112,116) and b.tax_code in (1,4) and b.ent_id = ? "+
                    " and date_trunc('day',policy_date) >= ? and date_trunc('day',policy_date) <= ?"+
                    " and date_trunc('day',policy_date) >= '2025-08-01 00:00:00' "+
                    " and pph21_sd_saat_ini < ? "+
                    " order by pph21_sd_saat_ini desc");
            ...
            if (RS.next()) {
                return RS.getBigDecimal("pph21_sd_saat_ini");
            }
    ...
    public BigDecimal getAkumulasiPPhBulanan(String stEntityID, Date dtTanggalPolis) throws Exception {
    ...
    public BigDecimal getAkumulasiKomisiBulananAlter(String stEntityID, Date dtTanggalPolis) throws Exception {
        ...
        try {
            S.setQuery(
                    " select coalesce(sum(b.amount),0) as total_komisi "+
                    " from ins_policy a  "+
                    " left join ins_pol_items b on a.pol_id = b.pol_id "+
                    " where status in ('POLICY','ENDORSE','RENEWAL') and active_flag = 'Y'  "+
                    " and a.effective_flag = 'Y' "+
    ...
    public BigDecimal getPPHProgressiveRate(BigDecimal akumulasi, Date dtPeriodStart) throws Exception {
        final SQLUtil S = new SQLUtil();
        ...
        try {
            S.setQuery(
                    ...
                    + " and period_start <= ? "
                    + "   and period_end >= ? "
                    + " and batas_bawah <= ? and batas_atas >= ?");

            S.setParam(1, dtPeriodStart);
            S.setParam(2, dtPeriodStart);
            S.setParam(3, akumulasi);
            S.setParam(4, akumulasi);
    ...
    public String generateVoucherNo(String type, Date date) throws Exception {
        String coasCode = (String) FinCodec.InsuranceCoverType.getLookUpCoasCode().getValue(getStCoverTypeCode());
        ...
        if(date!=null){
            year2Digit = DateUtil.getYear2Digit(date);
            month2Digit = DateUtil.getMonth2Digit(date);
        }
    ...
    public void setPotensiSubrogasi() throws Exception{
        if(stClaimSubCauseID!=null){

            InsuranceClaimSubCauseView subCause = getClaimSubCauses();

            if(subCause.isDefault()){
                setStReference13("Y");
            }else{
                setStReference13("N");
            }

            //khusus bank mandiri & bni otomatis isi ya utk potensi subrogasi kecuali meninggal
            EntityView ent = getEntity();

            if(ent.isBankMandiri() || ent.isBankBNI()){
                InsuranceClaimCauseView cause = getClaimCauses();

                if(!cause.getStInsuranceClaimCauseID().equalsIgnoreCase("3875") && !cause.getStInsuranceClaimCauseID().equalsIgnoreCase("3876") && !cause.getStInsuranceClaimCauseID().equalsIgnoreCase("3877"))
                {
                    setStReference13("Y");
                }
            }
    ...
    public InsuranceClaimSubCauseView getClaimSubCauses() {
        return (InsuranceClaimSubCauseView) DTOPool.getInstance().getDTO(InsuranceClaimSubCauseView.class, stClaimSubCauseID);
    }

    DTOList statusBayarCareList;

    public DTOList getStatusBayarCareList() throws Exception{

        if(statusBayarCareList==null) {
            getStatusBayarCaretech();
        }
        
        return statusBayarCareList;
    }

    public void setStatusBayarCareList(DTOList statusBayarCareList) {
        this.statusBayarCareList = statusBayarCareList;
    }

    public void getStatusBayarCaretech() throws Exception{

        statusBayarCareList = new DTOList();

        if(isStatusPolicy() || isStatusRenewal() || isStatusEndorse()){
            if(isEffective()){
                getStatusBayarPremiCaretech();
                getStatusBayarKomisiAgenCaretech();
                getStatusBayarFeebaseCaretech();
                getStatusBayarBrokerFeeCaretech();
            }
        }
        
    }

    public void getStatusBayarPremiCaretech() throws Exception{
        try {

                    logger.logDebug("############### HIT API STATUS PEMBAYARAN DI CARE ####################### ");

                    String careAPIURLPaymentStatus = Parameter.readString("SYS_CARE_NOTES_URL");
                    String careAPIKey = Parameter.readString("SYS_CARE_API_KEY");

                    URL url = new URL(careAPIURLPaymentStatus);
                    ...
                    conn.setRequestProperty("x-api-key", careAPIKey);
                    ...
                    // For POST only - START
                        ...
                        if (responseCode == HttpURLConnection.HTTP_OK) { //success
                                ...
                                while (i.hasNext()) {
                                    ...
                                    StatusBayarCareView statusBayar = new StatusBayarCareView();
                                    statusBayar.markNew();
                                    ...
                                    String Payment_Ref = (String) slide.get("Payment_Reference");
                                    String net = (String) slide.get("Net");
                                    String voucherNo = (String) slide.get("VoucherNo");
                                    ...
                                    statusBayar.setType("DI - Premi");
                                    statusBayar.setNet(new BigDecimal(net));
                                    statusBayar.setOutstanding(new BigDecimal(osPremi));
                                    statusBayar.setPayment(new BigDecimal(Payment));
                                    statusBayar.setPaymentDate(Payment_Date);
                                    statusBayar.setPaymentReference(Payment_Ref);
                                    statusBayar.setVoucherNo(voucherNo);

                                    statusBayarCareList.add(statusBayar);
                                }
    ...
    public void getStatusBayarKomisiAgenCaretech() throws Exception{
        try {
                    ...
                    // For POST only - START
                        ...
                        if (responseCode == HttpURLConnection.HTTP_OK) { //success
                                ...
                                while (i.hasNext()) {
                                    ...
                                    statusBayar.setType("BA - Komisi Agen");
                                    statusBayar.setNet(new BigDecimal(net));
                                    statusBayar.setOutstanding(new BigDecimal(osPremi));
                                    statusBayar.setPayment(new BigDecimal(Payment));
                                    statusBayar.setPaymentDate(Payment_Date);
    ...
    public void getStatusBayarFeebaseCaretech() throws Exception{
        try {
                    ...
                    // For POST only - START
                        ...
                        if (responseCode == HttpURLConnection.HTTP_OK) { //success
                                ...
                                while (i.hasNext()) {
                                    ...
                                    statusBayar.setType("BD - Fee Base");
                                    statusBayar.setNet(new BigDecimal(net));
                                    statusBayar.setOutstanding(new BigDecimal(osPremi));
                                    statusBayar.setPayment(new BigDecimal(Payment));
                                    statusBayar.setPaymentDate(Payment_Date);
    ...
    public void getStatusBayarBrokerFeeCaretech() throws Exception{
        try {
                    ...
                    // For POST only - START
                        ...
                        if (responseCode == HttpURLConnection.HTTP_OK) { //success
                                ...
                                while (i.hasNext()) {
                                    ...
                                    statusBayar.setType("BB - Komisi Brokerfee");
                                    statusBayar.setNet(new BigDecimal(net));
                                    statusBayar.setOutstanding(new BigDecimal(osPremi));
                                    statusBayar.setPayment(new BigDecimal(Payment));
                                    statusBayar.setPaymentDate(Payment_Date);
    ...
    DTOList statusBayarKlaimCareList;

    public DTOList getStatusBayarKlaimCareList() throws Exception{

        if(statusBayarKlaimCareList==null) {
            getStatusBayarKlaimCaretech();
        }

        return statusBayarKlaimCareList;
    }

    public void setStatusBayarKlaimCareList(DTOList statusBayarKlaimCareList) {
        this.statusBayarKlaimCareList = statusBayarKlaimCareList;
    }

    public void getStatusBayarKlaimCaretech() throws Exception{

        statusBayarKlaimCareList = new DTOList();

        if(isStatusClaimDLA() || isStatusClaimEndorse()){
            if(isEffective()){
                getStatusBayarKlaimCare();
            }
    ...
    public void getStatusBayarKlaimCare() throws Exception{
        try {

                    logger.logDebug("############### HIT API STATUS PEMBAYARAN KLAIM DI CARE ####################### ");
                    ...
                    // For POST only - START
                        ...
                        if (responseCode == HttpURLConnection.HTTP_OK) { //success
                                ...
                                while (i.hasNext()) {
                                    ...
                                    statusBayarKlaimCareList.add(statusBayar);
                                }
    ...
    public void validateTreatyInterkoneksi(boolean raiseErrors)  throws Exception {
    ...
    public void loadAdmLinkPremi() {
        try {
            if (admLinkPremi == null) {
                admLinkPremi = ListUtil.getDTOListFromQuery(
                        "select * from care_admlink_premi where type in ('DI','BD','BA','BB','CF') and pol_id = ? order by data_id",
                        new Object[]{stPolicyID},
                        AdmLinkPremi.class);
            }
        } catch (Exception e) {
    ...
    public void loadAdmLinkClaim() {
        try {
            if (admLinkClaim == null) {
                admLinkClaim = ListUtil.getDTOListFromQuery(
                        "select * from care_admlink_claim where type in ('DI','OP','CF') and pol_id = ? order by data_id",
                        new Object[]{stPolicyID},
                        AdmLinkClaim.class);
            }
        } catch (Exception e) {
    ...
    public void defineClaimCashCall() throws Exception{
        BigDecimal limitCashCall = getInsuranceCashCallLimit(getDtPeriodStart());

        if(!isBypassValidation()){
            if(limitCashCall!=null){
                if(BDUtil.biggerThan(getDbClaimAmountEstimate(), limitCashCall)){
                    setStClaimCashCallFlag("Y");
                }else{
                    setStClaimCashCallFlag(null);
                }
            }
        }
    ...
    public BigDecimal getInsuranceCashCallLimit(Date per_start) throws Exception {
        final SQLUtil S = new SQLUtil();

        try {
            S.setQuery(
                    " select b.treaty_limit "+
                    " from ins_cashcall a "+
                    " inner join ins_cashcall_detail b on a.ins_cashcall_id = b.ins_cashcall_id "+
                    " where policy_type_id = ? and treaty_period_start <= ? and treaty_period_end >= ? ");

            S.setParam(1, getStPolicyTypeID());
            S.setParam(2, per_start);
            S.setParam(3, per_start);
    ...
    public boolean isKreditSerbaguna() throws Exception {

        boolean isSerbaguna = false;

        if(getStKreasiTypeDesc()!=null){
            if(getStKreasiTypeDesc().trim().toUpperCase().equalsIgnoreCase("KREDIT SERBAGUNA")){
                isSerbaguna = true;
            }
        }else{
            String namaJenisKredit = getKreditType(getStKreasiTypeID());

            if(namaJenisKredit.trim().toUpperCase().equalsIgnoreCase("KREDIT SERBAGUNA"))
                isSerbaguna = true;
        }
    ...
    public void generatePolicyNoSerbaguna() throws Exception {
        String coasCode = (String) FinCodec.InsuranceCoverType.getLookUpCoasCode().getValue(getStCoverTypeCode());
        ...
        //  ABCCDDEEFFGGHHHHII
        //	  010320200210010400
        //  0488 6161 1224 0003 1 00
        //  0488 6161 1224 0003 2 00
        //  0488 6161 1224 0003 3 00
        //  12345678901234567890
        //  01234567890123456789
        if (isStatusTemporaryPolicy()) {
            coasCode = "9";
    ...
    public void generateRenewalPolicyNoPOJK() {
        ...
        enos = stPolicyNo.substring(16, 17);
        ...
        //final String z = StringTools.leftPad(String.valueOf(eNo), '0', 2);
        final String z = String.valueOf(eNo);
    ...
    public int getCounterPolisCurrent(){
        return Integer.parseInt(getStPolicyNo().substring(16, 17));
    }

    public boolean isKreditFLPP() throws Exception {

        boolean isFLPP = false;

        if(getStKreasiTypeDesc()!=null){
            if(getStKreasiTypeDesc().trim().toUpperCase().contains("FLPP")){
                isFLPP = true;
            }
        }else{
            String namaJenisKredit = getKreditType(getStKreasiTypeID());

            if(namaJenisKredit.trim().toUpperCase().contains("FLPP"))
                isFLPP = true;
        }
    ...
    public boolean isKreditTapera() throws Exception {

        boolean isTapera = false;

        if(getStKreasiTypeDesc()!=null){
            if(getStKreasiTypeDesc().trim().toUpperCase().contains("TAPERA")){
                isTapera = true;
            }
        }else{
            String namaJenisKredit = getKreditType(getStKreasiTypeID());

            if(namaJenisKredit.trim().toUpperCase().contains("TAPERA"))
                isTapera = true;
        }
    ...
    public String getInsTreatyIDMultiyears(Date per_start) throws Exception {
        final SQLUtil S = new SQLUtil();
    ...
    public boolean isPremiPaidInSystem2() throws Exception {
        ...
        if (pembayaran.size() > 1) {
            ...
            if(installmentNotPaid.size()>0){
                ...
                throw new RuntimeException("Tidak bisa bayar, premi cicilan polis induk "+ getStPolicyNo() +" belum lunas ");
            }
    ...
    public void recalculateUploadApproval() throws Exception {
        validateEntity();
    ...
    public boolean isPolicyKSG(){
        ...
        if(stPolicyTypeID.equalsIgnoreCase("87") || stPolicyTypeID.equalsIgnoreCase("88"))
            isKSG = true;

        return isKSG;
    }

    private void calculatePPH21Progresif2025Method2() throws Exception {
        ...
        if(!isStatusClaim() && !isStatusClaimEndorse() && !isStatusClaimInward() && !isStatusInward() && !isStatusEndorseRI()){
            ...
            if(tglPolis.isAfter(tglAktifProgresif)){
                ...
                for (int i = 0; i < details.size(); i++) {
                    ...
                    //get polis sebelumnya
                    DTOList detailsSebelumnya = getListPolisBefore(item.getStEntityID());

                    BigDecimal dppSampaiSaatIni = BDUtil.zero;
                    BigDecimal pphSampaiSaatIni = BDUtil.zero;
                    ...
                    for (int j = 0; j < detailsSebelumnya.size(); j++) {
                        InsurancePolicyItemsView itemB = (InsurancePolicyItemsView) detailsSebelumnya.get(j);

                        if(isPernahReverse){

                            logger.logDebug("#################### POLIS PERNAH REVERSE "+ itemB.getStPolicyNo());
                            logger.logDebug("#################### POL ID POLIS INI "+ stPolicyID);
                            logger.logDebug("#################### POL ID POLIS SEBELUM "+ itemB.getStPolicyID());

                            if(itemB.getStPolicyID().equalsIgnoreCase(stPolicyID))
                                break;
                        }

                        BigDecimal dpp = BDUtil.mul(itemB.getDbAmount(), new BigDecimal(0.5), scale);

                        dppSampaiSaatIni = BDUtil.add(dppSampaiSaatIni, dpp);

                        pphSampaiSaatIni = calculatePPH21SampaiSaatIni(dppSampaiSaatIni);

                        logger.logDebug("#########################################################");
                        logger.logDebug("############ PERHITUNGAN NEW PROGRESIF PPH POLIS ke "+ (j+1)+ " "+ itemB.getStPolicyNo());
                        logger.logDebug("############ DPP = "+ NumberUtil.getMoneyStr(dpp,2));
                        logger.logDebug("############ DPP SAMPAI SAAT INI = "+ NumberUtil.getMoneyStr(dppSampaiSaatIni,2));
                        logger.logDebug("############ PPH SAMPAI SAAT INI = "+ NumberUtil.getMoneyStr(pphSampaiSaatIni,2));

                        BigDecimal pphSebelumnya = BDUtil.zero;

                        BigDecimal dppSampaiSaatIniS = BDUtil.zero;
                        for (int k = 0; k < detailsSebelumnya.size(); k++) {
                            InsurancePolicyItemsView itemS = (InsurancePolicyItemsView) detailsSebelumnya.get(k);

                            if(k==j) break;

                            BigDecimal dppS = BDUtil.mul(itemS.getDbAmount(), new BigDecimal(0.5), scale);

                            dppSampaiSaatIniS = BDUtil.add(dppSampaiSaatIniS, dppS);

                            BigDecimal pphSampaiSaatIniS = calculatePPH21SampaiSaatIni(dppSampaiSaatIniS);

                            pphSebelumnya = pphSampaiSaatIniS;
                            
                        }

                        BigDecimal pphPolisTerkait = BDUtil.sub(pphSampaiSaatIni, pphSebelumnya);

                        logger.logDebug("############ PPH SEBELUMNYA = "+ NumberUtil.getMoneyStr(pphSebelumnya,2));
                        logger.logDebug("############ PPH POLIS TERKAIT = "+ NumberUtil.getMoneyStr(pphPolisTerkait,2));
                        logger.logDebug("#########################################################");
                    ...
                    if(item.isKomisi()){
                        ...
                        BigDecimal dppPolis = BDUtil.mul(item.getDbAmount(), new BigDecimal(0.5), scale);

                        BigDecimal dppSampaiSaatIniPolis = BDUtil.add(dppSampaiSaatIni, dppPolis);

                        BigDecimal pphSampaiSaatIniPolis = calculatePPH21SampaiSaatIni(dppSampaiSaatIniPolis);

                        BigDecimal pphPolisIni = BDUtil.sub(pphSampaiSaatIniPolis, pphSampaiSaatIni);

                        BigDecimal rateProgresif = getPPHProgressiveRate(dppSampaiSaatIniPolis, getDtPolicyDate());

                        if (item.isAutoTaxRate()) {
                            item.setDbTaxRate(rateProgresif);
                        }

                        item.setDbPph21SampaiSaatIni(pphSampaiSaatIni);
                        item.setDbTaxAmount(pphPolisIni);

                        logger.logDebug("############ HITUNG PAJAK POLIS INI ############## ");
                        logger.logDebug("############ PPH21 SAMPAI SAAT POLIS INI = "+ NumberUtil.getMoneyStr(pphSampaiSaatIniPolis,2));
                        logger.logDebug("############ PPH SAMPAI SAAT POLIS SEBELUMNYA = "+ NumberUtil.getMoneyStr(pphSampaiSaatIni,2));
                        logger.logDebug("############ PPH POLIS INI = "+ NumberUtil.getMoneyStr(pphPolisIni,2));


                        if(BDUtil.biggerThanZero(item.getDbAmount()))
                            if(BDUtil.lesserThanZero(pphPolisIni))
                                throw new RuntimeException("Komisi Plus Namun Pph Minus, cek ulang data");

                        //jika komisi >0 & pajak > komisi
                        if(BDUtil.biggerThanZero(item.getDbAmount())){
                            if(BDUtil.biggerThan(pphPolisIni, item.getDbAmount())){
                                throw new RuntimeException("Pph21 Lebih Besar dari Komisi, cek ulang data");
                            }
                        }
                        ...
                        //jika komisi <0 & pajak > komisi
                        if(BDUtil.lesserThanZero(item.getDbAmount())){
                            if(BDUtil.lesserThan(pphPolisIni, item.getDbAmount())){
                                throw new RuntimeException("Pph21 Lebih Besar dari Komisi, cek ulang data");
                            }
                        }
    ...
    public DTOList getListPolisApproveBefore(String stEntityID) throws Exception{
        
        //get polis sebelumnya
        DateTime tanggalPalingAwal = new DateTime(getDtPolicyDate());
        tanggalPalingAwal = tanggalPalingAwal.withDayOfMonth(1);

        DateTime tanggalPalingAkhir = new DateTime(getDtPolicyDate());
        tanggalPalingAkhir = tanggalPalingAkhir.dayOfMonth().withMaximumValue();

        final DTOList detailsSebelumnya = ListUtil.getDTOListFromQuery(
                "select c.ent_name,pol_no,policy_date,premi_total,b.amount,1.1 as rate_ppn,b.pph21_sd_saat_ini,b.tax_amount,approved_date,a.pol_id,b.ins_pol_item_id,b.ent_id,"+
                " (select count(*) from ins_pol_reverse_history  x where x.pol_id = a.pol_id) as reverse_c"+
                " from ins_policy a  "+
                " left join ins_pol_items b on a.pol_id = b.pol_id "+
                " left join ent_master c on b.ent_id = c.ent_id"+
                " where status in ('POLICY','ENDORSE','RENEWAL') and a.active_flag = 'Y'  "+
                " and a.effective_flag = 'Y' "+
                " and ins_item_id in (11,18,25,32,112,116,117) and b.tax_code in (1,4) "+
                " and b.ent_id = ?"+
                " and date_trunc('day',policy_date) >= ? and date_trunc('day',policy_date) <= ?"+
                " and date_trunc('day',policy_date) >= '2025-08-01 00:00:00'"+
                " and pph21_sd_saat_ini is not null and premi_total <> 0"+
                " order by a.approved_date",
                new Object[]{stEntityID, tanggalPalingAwal.toDate(), tanggalPalingAkhir.toDate()},
                InsurancePolicyItemsView.class);

        return detailsSebelumnya;
    }

    public DTOList getListPolisBefore(String stEntityID) throws Exception{
        ...
        String policyID = stPolicyID!=null?stPolicyID:"1";

        final DTOList detailsSebelumnya = ListUtil.getDTOListFromQuery(
                "select * "+
                " from ("+
                " select c.ent_name,pol_no,policy_date,premi_total,b.amount,1.1 as rate_ppn,b.pph21_sd_saat_ini,b.tax_amount,effective_flag, approved_date,a.pol_id,b.ins_pol_item_id,b.ent_id,"+
                " (select count(*) from ins_pol_reverse_history  x where x.pol_id = a.pol_id) as reverse_count"+
                " from ins_policy a  "+
                " left join ins_pol_items b on a.pol_id = b.pol_id "+
                " left join ent_master c on b.ent_id = c.ent_id"+
                " where status in ('POLICY','ENDORSE','RENEWAL') and a.active_flag = 'Y'  "+
                ...
                " and pph21_sd_saat_ini is not null and premi_total <> 0"+
                " union"+
                " select c.ent_name,pol_no,policy_date,premi_total,b.amount as komisi,1.1 as rate_ppn,b.pph21_sd_saat_ini,b.tax_amount,effective_flag,"+
                " (select create_date "+
                "        from ar_invoice x where ar_trx_type_id in (5,6,7) and x.attr_pol_id = a.pol_id "
                + "      and (case when premi_total > 0 then x.amount > 0 else x.amount < 0  end) limit 1) as approved_date,"+
                " a.pol_id,b.ins_pol_item_id,b.ent_id,"+
                "        (select count(*) from ins_pol_reverse_history  x where x.pol_id = a.pol_id) as reverse_count"+
                " from ins_policy a  "+
                " left join ins_pol_items b on a.pol_id = b.pol_id "+
                " left join ent_master c on b.ent_id = c.ent_id"+
                ...
                " and date_trunc('day',policy_date) >= '2025-08-01 00:00:00'"+
                " and pph21_sd_saat_ini is not null and premi_total <> 0"+
                " and (select count(*) from ins_pol_reverse_history  x where x.pol_id = a.pol_id) > 0"+
                ")z order by pph21_sd_saat_ini",
                new Object[]{stEntityID, tanggalPalingAwal.toDate(), tanggalPalingAkhir.toDate(),
                             stEntityID, tanggalPalingAwal.toDate(), tanggalPalingAkhir.toDate()},
                InsurancePolicyItemsView.class);

        return detailsSebelumnya;
    }

    public BigDecimal calculatePPH21SampaiSaatIni(BigDecimal dppSaatIni) throws Exception{
    ...
    public String getKodePos(String kodePos) throws Exception {
        ...
        try {
            ...
            PreparedStatement PS = S.setQuery("select region_map_id "+
                        " from s_region_map_maipark "+
                        " where postal_code = ? and active_flag = 'Y' "
                    + " limit 1");
            PS.setString(1, kodePos);
