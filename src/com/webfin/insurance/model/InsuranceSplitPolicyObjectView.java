...
public abstract class InsuranceSplitPolicyObjectView extends DTO implements RecordAudit {
    ...
    private final static transient LogManager logger = LogManager.getInstance(InsuranceSplitPolicyObjectView.class);
    ...
    public InsuranceSplitPolicyView getPolicy() throws Exception {
        return policy;
    }
        ...
        try {
            if (deductibles==null) {
                deductibles = ListUtil.getDTOListFromQuery(
                        "select * from ins_split_pol_deduct where ins_pol_obj_id=? order by ins_pol_deduct_id",
 ...
 */
    ...
    public void reCalculateNOW() throws Exception{
      ...
      if (clausules!=null)
        ...
        if (suminsureds!=null)
            for (int i = 0; i < suminsureds.size(); i++) {
                InsuranceSplitPolicyTSIView itsi = (InsuranceSplitPolicyTSIView) suminsureds.get(i);
        ...
        if (coverage!=null)
            for (int i = 0; i < coverage.size(); i++) {
            InsuranceSplitPolicyCoverView icv = (InsuranceSplitPolicyCoverView) coverage.get(i);
            ...
            if(isDepreciationMethod()){
                    ...
                    if(i>0){
                        InsuranceSplitPolicyCoverView icvBefore = (InsuranceSplitPolicyCoverView) coverage.get(i-1);
            ...
            if (policy.isStatusEndorse()||policy.isStatusEndorseRI()) {
             //{
                ...
                if (icv.getStInsurancePolicyCoverRefID()!=null){
                    final InsuranceSplitPolicyCoverView refCover = icv.getRefCover();
    ...
    private void loadSumInsureds() throws Exception {
        if (suminsureds==null) {
            suminsureds = ListUtil.getDTOListFromQuery(
                    ...
                    new Object [] {getStPolicyObjectID()},
                    InsuranceSplitPolicyTSIView.class
                    );
            
            for (int i = 0; i < suminsureds.size(); i++) {
                InsuranceSplitPolicyTSIView tsi = (InsuranceSplitPolicyTSIView) suminsureds.get(i);
        ...
        if (coverage==null) {
         /*coverage = ListUtil.getDTOListFromQuery(
            ...
            coverage = ListUtil.getDTOListFromQuery(
                    ...
                    "      a.*" +
                    "   from  " +
                    "      ins_split_pol_cover a" +
                    "   where" +
                    "      a.ins_pol_obj_id=? order by ins_pol_cover_id" ,
                    new Object [] {getStPolicyObjectID()},
                    InsuranceSplitPolicyCoverView.class
                    );
        }
    }
    ...
    public void validateRateOJK() throws Exception{
        ...
        InsuranceSplitPolicyObjDefaultView objx = (InsuranceSplitPolicyObjDefaultView) this;
...
//            System.out.println("First number is lowest.");
            ...
            if(rc==null)
                throw new RuntimeException("Kode resiko "+ objx.getStRiskCategoryCode1() +" tidak ditemukan pada sistem");
...
//            System.out.println("Second number is lowest.");
            ...
            if(rc==null)
                throw new RuntimeException("Kode resiko "+ objx.getStRiskCategoryCode2() +" tidak ditemukan pada sistem");
...
//            System.out.println("2 & 3 sama, kurang dari 1");
    ...
    public void validateBolehKomisi(InsuranceSplitPolicyObjDefaultView obj) throws Exception {
...
//                System.out.println("#################################################################");
    ...
    public void validateHighestRateOJK() throws Exception{
        ...
        if(policy.isStatusEndorse() || policy.isStatusClaim() || policy.isStatusClaimEndorse() || policy.isStatusEndorseRI() || policy.isStatusHistory() || policy.isStatusInward() || policy.isStatusClaimInward())
            return;
        ...
        //if(polType.getStPolicyTypeID().equalsIgnoreCase("3")){
            ...
            for (int i = 0; i < suminsured.size(); i++) {
                InsuranceSplitPolicyTSIView tsi = (InsuranceSplitPolicyTSIView) suminsured.get(i);
...
//                System.out.println("########### MASUK TSI #################");
        ...
        for (int i = 0; i < coverage.size(); i++) {
                InsuranceSplitPolicyCoverView cover = (InsuranceSplitPolicyCoverView) coverage.get(i);
                ...
                sqa.addClause("period_start <= ?");
                sqa.addPar(policy.getDtPolicyDate());
...
//                        System.out.println("**************  batas atas pro rata : "+ batasAtas);
    ...
    public void validateSplitPolis(InsuranceSplitPolicyView policy) throws Exception{

        if(policy.getStPolicyTypeID().equalsIgnoreCase("87") || policy.getStPolicyTypeID().equalsIgnoreCase("88")){

            InsuranceSplitPolicyObjDefaultView objx = (InsuranceSplitPolicyObjDefaultView) this;

            int jangkaWaktuTahun = DateUtil.getYearsBetweenHUTB(objx.getDtReference2(), objx.getDtReference3());

            if(jangkaWaktuTahun > 5)
                  throw new RuntimeException("Jangka waktu pertanggungan asuransi kredit Debitur "+ objx.getStReference1()+" maksimal 5 tahun");

            Date birth = objx.getDtReference1();
            Date perStart = objx.getDtReference2();
            Date perEnd = objx.getDtReference3();
            ...
            if(startDate.isAfter(policyDateLastDay))
                throw new RuntimeException("Tanggal mulai kredit Debitur "+objx.getStReference1()+" tidak boleh > tanggal polis");

            if(endDate.isBefore(startDate))
                    throw new RuntimeException("Tanggal akhir kredit Debitur "+objx.getStReference1()+" tidak boleh kurang dari tanggal mulai");

                if(startDate.isEqual(birthDate))
                    throw new RuntimeException("Tanggal mulai kredit Debitur "+objx.getStReference1()+" tidak boleh sama dengan tanggal lahir");

            if(startDate.isEqual(endDate))
                throw new RuntimeException("Tanggal mulai kredit Debitur "+objx.getStReference1()+" tidak boleh sama dengan tanggal akhir");

            if(Integer.parseInt(objx.getStReference2()) < 17)
                throw new RuntimeException("Umur Debitur "+objx.getStReference1()+" Tidak Boleh Dibawah 17 Tahun");

            if(Integer.parseInt(objx.getStReference2()) > 75)
                throw new RuntimeException("Umur Debitur "+objx.getStReference1()+" Tidak Boleh Melebihi 75 Tahun");

            if(age > 75 || age < 17)
                throw new RuntimeException("Umur Debitur "+objx.getStReference1()+" Salah, Cek Tanggal Lahir");
        }
