...
public abstract class InsurancePolicyObjectView extends DTO implements RecordAudit {
    ...
    public abstract BigDecimal getDbLimitOfLiability();

    public abstract BigDecimal getDbObjectInsuredAmountEndorse();
    public abstract void setDbObjectInsuredAmountEndorse(BigDecimal dbObjectInsuredAmountEndorse);
    ...
    public abstract String getStDebiturActive();
    public abstract void setStDebiturActive(String stDebiturActive);
    ...
    public InsurancePolicyView getPolicy() throws Exception {
        return policy;
    }
 ...
 */
    ...
    public void reCalculateNOW() throws Exception{
      ...
      if (clausules!=null)
        ...
        setDbObjectInsuredAmount(null);
        setDbObjectInsuredAmountEndorse(null);
        
        if (suminsureds!=null)
            for (int i = 0; i < suminsureds.size(); i++) {
                InsurancePolicyTSIView itsi = (InsurancePolicyTSIView) suminsureds.get(i);

                if(itsi.getDbInsuredAmountFull()!=null){
                    itsi.setDbInsuredAmount(BDUtil.mul(itsi.getDbInsuredAmountFull(), BDUtil.getRateFromPct(policy.getDbSharePct()), scale));
                }

                setDbObjectInsuredAmount(BDUtil.add(getDbObjectInsuredAmount(), itsi.getDbInsuredAmount()));

                if (itsi.getStInsurancePolicyTSIRefID()!=null){
                    final InsurancePolicyTSIView refTSI = itsi.getRefTSI();

                    final BigDecimal insAmountDiff = BDUtil.sub(itsi.getDbInsuredAmount(),refTSI.getDbInsuredAmount());

                    setDbObjectInsuredAmountEndorse(BDUtil.add(getDbObjectInsuredAmountEndorse(), insAmountDiff));
                }
        ...
        if (coverage!=null)
            ...
            if(isDepreciationMethod()){

                    BigDecimal tsiBefore = getDbObjectInsuredAmount();

                    if(i>0){
                        InsurancePolicyCoverView icvBefore = (InsurancePolicyCoverView) coverage.get(i-1);
                        tsiBefore = icvBefore.getDbInsuredAmount();
                    }

                    if(icv.getStDepreciationYear()!=null)
                        if(icv.getStDepreciationYear().equalsIgnoreCase("1"))
                            tsiBefore = getDbObjectInsuredAmount();

                    if(!icv.isEntryInsuredAmount())
                        icv.setDbInsuredAmount(BDUtil.mul(tsiBefore, BDUtil.getRateFromPct(icv.getDbDepreciationPct()),2));
            }
            ...
            if (icv.isEntryRate()) {
                ...
                //JIKA ADA DISKON
                icv.setDbDiscountAmount(null);

                if(!BDUtil.isZeroOrNull(icv.getDbDiscountPct())){

                    BigDecimal premiBruto = icv.getDbPremiNew();

                    icv.setDbDiscountAmount(BDUtil.mul(premiBruto, BDUtil.getRateFromPct(icv.getDbDiscountPct()), scale));

                    icv.setDbPremiNew(BDUtil.sub(premiBruto, BDUtil.mul(premiBruto, BDUtil.getRateFromPct(icv.getDbDiscountPct()), scale)));
                ...
                if (icv.getDbPremi().longValue()!=0) {
                    ...
                    if(!BDUtil.isZeroOrNull(icv.getDbDiscountPct()))
                        szc.append(" - "+ p(icv.getDbDiscountPct())+"%");
             ...
             //{
                ...
                if (icv.getStInsurancePolicyCoverRefID()!=null){
                    ...
                    if (insAmountDiff.longValue()!=0) {
                        ...
                        m = BDUtil.mul(m, policy.getDbPremiumFactorValue(),scale);
                        ...
                        if (m.longValue()!=0) {
                            ...
                            if (!isOne(policy.getDbPremiumFactorValue()))
                                szCalc.append(" x "+policy.getStPremiumFactorDesc());
                        ...
                        if(!BDUtil.isZeroOrNull(icv.getDbPremi()))
                            setStDebiturActive("1");
                    }
                   }
    ...
    public String getStInActiveFlag() {
        return "N";
    }

    public void setStInActiveFlag(String stInActiveFlag) {

    }

    public boolean isInActive(){
        return Tools.isYes(getStInActiveFlag());
    }
    
    public String getStClaimFlag() {
        return "N";
    }

    public boolean isAlreadyInputLKS() {
        return Tools.isYes(getStClaimFlag());
    }
    ...
    private void loadCoverageVehicle() throws Exception {
        if (coverageVec==null) {
            ...
            coverageVec = ListUtil.getDTOListFromQuery(
                    ...
                    "   sum(checkreas(a.ins_cover_id in (286,287),a.insured_amount)) as premi,"+
                    "   sum(checkreas(a.ins_cover_id in (286,287),a.premi)) as premi_new,"+
                    "   sum(checkreas(a.cover_category='EXT' and a.ins_cover_id not in (286,287),a.premi)) as premi_not,"+
    ...
    private void loadSumCoverage() throws Exception {
        if (SumCoverage == null) {
            SumCoverage = ListUtil.getDTOListFromQuery(
                    "   select a.ins_cover_id,a.rate,sum(a.insured_amount) as insured_amount,sum(a.premi) as premi,sum(a.premi_new) as premi_new "
                    + "   from ins_pol_cover a"
                    + "   where a.pol_id = ? group by a.ins_cover_id,a.rate order by a.ins_cover_id ",
    ...
    private void loadSumTSIAmount() throws Exception {
        if (sumTSIAmount == null) {
            sumTSIAmount = ListUtil.getDTOListFromQuery(
                    ...
                    " select a.ins_tsi_cat_id,sum(a.insured_amount) as insured_amount "
                    + "from ins_pol_tsi a "
                    + "where a.pol_id = ? group by a.ins_tsi_cat_id order by a.ins_tsi_cat_id ",
                    new Object[]{getStPolicyID()},
                    InsurancePolicyTSIView.class);
        }
    }
    ...
    public void validateRateOJK() throws Exception{
        ...
        InsurancePolicyObjDefaultView objx = (InsurancePolicyObjDefaultView) this;
...
//                System.out.println("#################################################################");
    ...
    public void validateDeductibleOJK() throws Exception{
        ...
        if(policy.isStatusEndorse() || policy.isStatusClaim() || policy.isStatusEndorseRI() || policy.isStatusHistory() || policy.isStatusInward() || policy.isStatusClaimInward())
            return;
        ...
        for (int i = 0; i < deductible.size(); i++) {
                ...
                */
                ...
                sqa.addPar(policy.getDtPeriodStart());
...
//                        System.out.println("**************  batas bawah  pct : "+ batasBawahPCT);
    ...
    public InsuranceRiskCategoryView getRiskCategoryMaiparkByDate(Date periodStart) throws Exception {
        ...
        final InsuranceRiskCategoryView cat = (InsuranceRiskCategoryView)ListUtil.getDTOListFromQuery(
                " select * "+
                " from ins_risk_cat "+
                " where poltype_id = 99 and ins_risk_cat_id = ? ",
                new Object [] {getStReference2()},
                InsuranceRiskCategoryView.class
                ).getDTO();
...
//        logger.logWarning("########## KODE RESIKO NIH limit 3 : "+rc.getDbTreatyLimit3());
    ...
    public void applyHighestRiskCode() throws Exception {
        ...
        if(policy.isStatusClaim() || policy.isStatusHistory()|| policy.isStatusClaimEndorse())
            return;
...
//                System.out.println("#################################################################");
    ...
    public void validateHighestRateOJK() throws Exception{
        ...
        if(policy.isStatusEndorse() || policy.isStatusClaim() || policy.isStatusClaimEndorse() || policy.isStatusEndorseRI() || policy.isStatusHistory() || policy.isStatusInward() || policy.isStatusClaimInward() || policy.isStatusEndorseClaimInward())
            return;
        ...
        if(polType.getStPolicyTypeID().equalsIgnoreCase("3")){
            ...
            if(objx.getDtReference1()==null)
                throw new RuntimeException("Periode Awal objek "+ objx.getStReference1() +" belum diisi");

            int tahunSekarang = Integer.parseInt(DateUtil.getYear(objx.getDtReference1()));
            int tahunPembuatan = Integer.parseInt(objx.getStReference3());
            int usiaKendaraan = tahunSekarang - tahunPembuatan;
            boolean isKendaraanKomersial = objx.getStReference7().equalsIgnoreCase("11")?true:false;
            boolean isKendaraanKhusus = objx.getStReference7().equalsIgnoreCase("12")?true:false;

            if( usiaKendaraan > 5) cekBatasAtas = false;

            if(isKendaraanKomersial || isKendaraanKhusus) cekBatasAtas = false;
...
//                System.out.println("########### MASUK TSI #################");
        ...
        for (int i = 0; i < coverage.size(); i++) {
                ...
                if(polType.getStPolicyTypeID().equalsIgnoreCase("3")){
                    ...
                    if(isDepreciationMethod())
                        tsiObject = cover.getDbInsuredAmount();
...
//                System.out.println("********** premium factor : "+getPolicy().getDbPremiumFactorValue());
                ...
                if(useKodeOkupasi)
                    sql = sqa.getSQL() + " order by rate_top"+ rcl+" desc limit 1 ";
...
//                        System.out.println("**************  batas atas pro rata : "+ batasAtas);
                        ...
                        BigDecimal rateCover = BDUtil.zero;
                        if((BDUtil.isZeroOrNull(cover.getDbRate()) && !BDUtil.isZeroOrNull(cover.getDbPremiNew()))
                            || (cover.isEntryPremi() && !BDUtil.isZeroOrNull(cover.getDbPremiNew()))){
                            rateCoverage = BDUtil.div(cover.getDbPremiNew(), cover.getDbInsuredAmount(), 6);
                        ...
                        rateCover = BDUtil.roundUp(rateCover,6);
...
//                        System.out.println("**************  rate Cover pro rata: "+ rateCover);
                        ...
                        if(limitByRate){
                            ...
                            if(BDUtil.isZeroOrNull(rateCover))
                                throw new RuntimeException("Objek No. "+ objx.getStOrderNo()+ " Salah.<br>Rate "+cover.getInsuranceCoverage().getStDescription()+ " tidak boleh kosong");
...
//                System.out.println("#################################################################");
    ...
    public FlexFieldHeaderView getAnalysisFF() {
        final String pt = policy.getStPolicyTypeID();
        if (pt==null) return null;
        return (FlexFieldHeaderView) DTOPool.getInstance().getDTO(FlexFieldHeaderView.class, "ANALYSIS_"+pt);
    }

    private DTOList spreading;

    public void setspreading(DTOList spreading) {
        this.spreading = spreading;
    }

    public DTOList getSpreading(String insUploadID) {
        loadSpreading(insUploadID);
        return spreading;
    }

    public void loadSpreading(String insUploadID) {
        try {
            if (spreading==null) {
                spreading = ListUtil.getDTOListFromQuery(
                        "select * from  ins_upload_reins where ins_pol_obj_id = ? and ins_upload_id = ? and status is null",
                        new Object [] {getStPolicyObjectID(), insUploadID},
    ...
    public boolean isDepreciationMethod() throws Exception {

        boolean useDepreciation = false;

        if(policy.getPolicyType().getStControlFlags()!=null)
               useDepreciation = policy.getPolicyType().checkProperty("DEPRECIATION","Y");

        return Tools.isYes(getStReference11()) && useDepreciation;
        
    }

    private DTOList coverageRes;

    public DTOList getCoverRestitusi() throws Exception {
        loadCoverageRestitusi();
        return coverageRes;
    }

    private void loadCoverageRestitusi() throws Exception {
        if (coverageRes==null) {

            coverageRes = ListUtil.getDTOListFromQuery(
                    "   select" +
                    "   sum(checkreas(a.cover_category='MAIN',a.premi)) as premi,"+
                    "   sum(checkreas(a.cover_category='EXT',a.premi)) as premi_new "+
                    "   from  " +
                    "      ins_pol_cover a" +
                    "   where" +
                    "      a.ins_pol_obj_id=?" ,
    ...
    public void setCoverRestitusi(DTOList coverageRes) {
        this.coverageRes = coverageRes;
    }

    private DTOList detailDocuments;

    /**
     * @return the detailDocuments
     */
    public DTOList getDetailDocuments() {
        if (detailDocuments == null && getStPolicyObjectID() != null && policy.getStPolicyTypeID() != null)
            detailDocuments = loadDetailDocuments(policy.getStPolicyTypeID(), getStPolicyObjectID(), "OBJECT");
        return detailDocuments;
    }

    private static DTOList loadDetailDocuments(String poltype, String stPolicyObjectID, String documentClass) {
        try {
            ...
            final DTOList l = ListUtil.getDTOListFromQuery(
                    ...
                    "      left join ins_pol_documents b on  b.document_class=a.document_class and b.ins_pol_obj_id=? and b.ins_document_type_id=a.ins_document_type_id" +
                    "   where " +
                    "      a.pol_type_id=? " +
                    "      and a.document_class=? ",
                    new Object[]{stPolicyObjectID, poltype, documentClass},
            ...
            for (int i = 0; i < l.size(); i++) {
                ...
                docs.setStInsurancePolicyObjectID(stPolicyObjectID);
    ...
    /**
     * @param detailDocuments the detailDocuments to set
     */
    public void setDetailDocuments(DTOList detailDocuments) {
        this.detailDocuments = detailDocuments;
    }

    public void getLowestRateOJK() throws Exception{

        if(policy.isStatusEndorse() || policy.isStatusClaim() || policy.isStatusEndorseRI() || policy.isStatusHistory() || policy.isStatusInward())
            return;
...
//                        System.out.println("**************  batas atas pro rata : "+ batasAtas);
        ...
        try {
            if (subrogasi == null) {
                subrogasi = ListUtil.getDTOListFromQuery(
                        "select * from ins_pol_subrogasi where ins_pol_obj_id=? order by ins_pol_subrogasi_id",
                        new Object[]{getStPolicyObjectID()},
    ...
    public abstract String getStSubrogasiID();

    public DTOList getSuminsuredsNotNull() throws Exception {
        loadSumInsuredsNotNull();
        return suminsureds;
    }

    private void loadSumInsuredsNotNull() throws Exception {
        if (suminsureds==null) {
            suminsureds = ListUtil.getDTOListFromQueryDS(
                    "   select" +
                    "      a.*" +
                    "   from  " +
                    "      ins_pol_tsi a" +
                    "   where" +
                    "      a.ins_pol_obj_id=? and a.insured_amount <> 0 order by a.ins_tsi_cat_id asc" ,
                    new Object [] {getStPolicyObjectID()},
                    InsurancePolicyTSIView.class,"GATEWAY"
                    );
    ...
    public DTOList getCoverageNotNull() throws Exception {
        loadCoverageNotNull();
        return coverage;
    }

    private void loadCoverageNotNull() throws Exception {
        if (coverage==null) {
         /*coverage = ListUtil.getDTOListFromQuery(
                 "   select" +
                 "      b.ins_cover_id, b.description as ins_cover_desc, b.cover_category" +
            ...
            coverage = ListUtil.getDTOListFromQueryDS(
                    "   select" +
                    "      a.*" +
                    "   from  " +
                    "      ins_pol_cover a" +
                    "   where" +
                    "      a.ins_pol_obj_id=? and a.premi <> 0 order by ins_pol_cover_id" ,
                    new Object [] {getStPolicyObjectID()},
                    InsurancePolicyCoverView.class,"GATEWAY"
                    );
        }
    }

    public DTOList getDeductiblesInterkoneksi() {
        loadDeductiblesInterkoneksi();
        return deductibles;
    }

    private void loadDeductiblesInterkoneksi() {
        try {
            if (deductibles==null) {
                deductibles = ListUtil.getDTOListFromQueryDS(
                        "select * from ins_pol_deduct where ins_pol_obj_id=? order by ins_pol_deduct_id",
                        new Object [] {getStPolicyObjectID()},
                        InsurancePolicyDeductibleView.class,"GATEWAY"
                        );
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    public DTOList getTreatiesInterkoneksi() {
        loadTreatiesInterkoneksi();
        return treaties;
    }

    private void loadTreatiesInterkoneksi() {
        try {
            if (treaties==null) {
                treaties = ListUtil.getDTOListFromQueryDS(
                        "select * from ins_pol_treaty where ins_pol_obj_id=?",
                        new Object [] {getStPolicyObjectID()},
                        InsurancePolicyTreatyView.class,"GATEWAY"
                        );
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    public DTOList getTreatyDetailsInterkoneksi() {

        DTOList l = new DTOList();

        if (getTreatiesInterkoneksi()!=null && getTreatiesInterkoneksi().size()>0) {
            final InsurancePolicyTreatyView tty = (InsurancePolicyTreatyView)getTreaties().get(0);

            l = tty.getDetailsInterkoneksi();
        }

        return l;
    }

    public String getEQZoneCode() throws Exception
    {
        final SQLUtil S = new SQLUtil();
        ...
        try
        {
            S.setQuery(
                    ...
                    "         s_valueset " +
                    "   where" +
                    "      vs_group = 'KABUPATEN' and vs_code = ?");

            S.setParam(1,getStReference11());
    ...
    public DTOList getTreatiesEndorseRI(String polNo, String orderNo) {
        loadTreatiesEndorseRI(polNo, orderNo);
        return treaties;
    }

    private void loadTreatiesEndorseRI(String polNo, String orderNo) {
        try {
            if (treaties==null) {
                treaties = ListUtil.getDTOListFromQuery(
                        "select c.* "+
                        ...
                        "      where a.status in ('ENDORSE RI')  and a.active_flag = 'Y' and a.effective_flag = 'Y' "+
                        "      and substr(a.pol_no, 0, 17) = ? and b.order_no = ?  "+
                        "      order by a.pol_id desc limit 1",
                        new Object [] {polNo.length()>16?polNo.substring(0,16):polNo, orderNo},
                        InsurancePolicyTreatyView.class
                        );
            }
        } catch (Exception e) {
    ...
    public String getKreditCoverage(String stVsCode) throws Exception {
        final SQLUtil S = new SQLUtil();
        ...
        try {
            S.setQuery(
                    ...
                    + "         s_valueset "
                    + "   where"
                    + "      vs_group = 'INSOBJ_CREDIT_COVER_PACKAGE' and vs_code = ?");
    ...
    public InsurancePolicyObjDefaultView getObjectDefault() throws Exception{
    ...
    public void applyAPUPPTCriteria() throws Exception{

        if(policy.isStatusClaim() || policy.isStatusClaimEndorse() || policy.isStatusEndorseRI() || policy.isStatusHistory() || policy.isStatusInward() || policy.isStatusClaimInward() || policy.isStatusEndorseClaimInward())
            return;

        InsurancePolicyObjDefaultView objx = (InsurancePolicyObjDefaultView) this;

        final EntityView cust = policy.getEntity();

        // set NPWP jika ada NPWP di data cust
        if(cust.getStTaxFile()!=null){
            objx.setStReference76("2");
            objx.setStReference77(null);
            objx.setStReference78(cust.getStTaxFile());
        }
            

        // set NIK jika ada NIK di data cust
        if(cust.getStIdentificationNumber()!=null){
            objx.setStReference76("2");
            objx.setStReference77(cust.getStIdentificationNumber());
            objx.setStReference78(null);
        }

        if(objx.getStReference77()!=null){
            String nik = objx.getStReference77();

            /*
            if(!StringTools.onlyDigits(nik))
                throw new RuntimeException("NIK No Urut "+ objx.getStOrderNo() +" tidak sesuai, harus numerik");

            if(nik.length() != 16)
                throw new RuntimeException("NIK No Urut "+ objx.getStOrderNo() +" tidak sesuai, panjangnya harus 16 digit");
            */
        }
    ...
    public FlexFieldHeaderView getPolicyDetailFF() {
        final String pt = policy.getStPolicyTypeID();

        if (pt==null) return null;

        return (FlexFieldHeaderView) DTOPool.getInstance().getDTO(FlexFieldHeaderView.class, "POLDETAIL_"+pt);
    }
